...
- **Closed, extensible effect union.** The Rust `ImpactEffect` enum (`crates/postretro/src/impact_effects.rs`) carries 7 arms — `Despawn`, `SetHealth`, `GrantHealth`, `GrantAmmo`, `PlayAnimation`, `Present`, `SetOwnerSlot`; `BoundEffect` (`impact_policy.rs`) adds `Write`, for 8. The author-facing builder surface — the TS `ImpactEffectWire` union in `sdk/lib/data_script.ts` — enumerates 9 wire members, and `impact_policy_surface_uses_author_ids_and_closed_effect_union` pins that surface at 9 (`): Effect;`) / 7 (Luau receiver methods). The death and economy specs already added arms — the "add an arm" path is proven.
...
A projectile-stage or ready-hit-ingest impact effect writes **before** the same tick's AI read borrow is taken — so a projectile backstab, or a co-op remote-authorized hitscan hit ingested that tick, can flip hostility on the same tick. Impact effects at the later stages (AI `apply_outcomes`, local-weapon, mover-blocking) write **after** the read and land for the next tick, like frame-end reaction writes. Every write phase is disjoint from the AI read phase, so the RefCell borrows never overlap; no overlay borrow spans an `on_impact` call.
