...
**Pose consumers — one clock, two triggers.** The renderer samples poses for **visible instances only**. A game-side consumer needing poses independent of visibility — skeletal hit zones (forthcoming, M10), later AI/perception — samples the same CPU pose functions directly, off the same game-layer clock, so its poses never desync from what is drawn. Such a consumer owns its **own** CPU model copy (skeleton + clips), independent of the renderer's: per-model-type data is O(model types), not O(instances), so the duplication is negligible and the subsystem boundary stays clean. The crossfade snapshot store is renderer-owned; a game-side sampler cannot read it and degrades a snapshot fade to its fallback clip.
