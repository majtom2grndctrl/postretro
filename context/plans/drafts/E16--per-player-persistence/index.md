...
- **The join seed is sent during the parity stage.** The host buffers the seed per-connection. When `SlotEvent::Participating` fires, the host applies the buffered seed in the `Participating` handler between seat lookup and pawn spawn — after the seat is known but before the player materializes. If the seed has not arrived when `Participating` fires (parity and seed travel separate channels; delivery order is not guaranteed), the host spawns with defaults and applies the seed when it arrives on a subsequent poll — `set_per_seat_value` works regardless of pawn state.
...
3. If the seed has not arrived when `Participating` fires (parity and seed travel separate channels; delivery order is not guaranteed), spawn with defaults. When the seed arrives on a subsequent poll, apply it to the existing seat — `set_per_seat_value` works regardless of pawn state, and the next owner-private replication cycle delivers the values to the client.
4. If the seat was **reclaimed** within the hold window, discard the seed with a log note — live values take precedence.
5. After apply, the next owner-private replication cycle delivers the values to the client's HUD.
