...
fn define_store_emits_flattened_handle_and_converged_refs() {
    ...
    assert!(
        ts.contains("accumulate: (t: TickParams) => RuntimeValue")
            ...
            && ts.contains("network?: \"ownerPrivate\"; perOwner: true; persist?: never; accumulate?: never")
    ...
    assert!(
        luau.contains("accumulate: (TickParams) -> RuntimeValue")
            ...
            && luau.contains("network: \"ownerPrivate\"?, perOwner: true, persist: nil?, accumulate: nil?")
