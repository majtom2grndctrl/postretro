...
## Epic 1: BSP Loading and Wireframe ✓
...
- [x] Minimal free-fly camera (raw winit keyboard/mouse, enough to navigate — replaced by action-mapped input in Epic 2)
...
## Epic 1.5: PRL Compiler and Voxel-Based Visibility ✓
...
**Status note:** superseded by the BVH + portal pipeline in Epic 4. Voxel code remains in repo as reference.

---

## Epic 2: Input and Frame Timing ✓
...
## Epic 3.5: Rendering Foundation Extension ✓
...
- [x] **Check-in gate** — visual parity with Epic 3.5 confirmed.

**Testable outcome:** ✓ identical visual output to Epic 3.5, rendered through a global BVH. Epic 5 unblocked.
...
- [x] **SH irradiance volume baker** — prl-build stage; ray-casts through the Epic 4 BVH; SH L2 projection; validity mask.
...
## Epic 6: Scripting + Entity Foundation ✓
...
- [x] **Reference behaviors (script)** — ~~`RotatorDriver` and `DamageSource` written as scripts~~. **✂ Cut (2026-06):** the per-tick behavior scripts went with the live-VM removal (`plans/done/remove-live-vm/`); only their data-archetype descriptors survive (`sdk/behaviors/reference/entities.{ts,luau}`). The damage-source role is superseded by the `applyDamage` reaction primitive (E10 entity health + damage surface).
...
**Prerequisite:** Epic 6 (entity model + scripting) ✓
...
## Epic 8: Material Optimization ✓

Texture and material pipeline polish. Move mip generation offline, establish Post Retro (hardware aniso + in-shader texel-grid reconstruction) as the foundational default look, and shrink normals on disk and in VRAM. Post Retro is now the sole texture-filtering path. Independent of Epic 7 — ships in either order.
...
## Epic 9: Diffuse GI Upgrade (depth-aware probes + fog) ✓

Kill light-leak-through-walls by adding per-probe visibility data to the Epic 5 SH irradiance volume, then extend the fog system. The depth-aware interpolant replaces the plain trilinear SH sample entirely — one runtime path. Probe streaming is deferred: this epic keeps probes resident and produces the VRAM-fit measurement that decides whether streaming ever becomes its own epic.

**Assumes the shipped Epic 5 lighting foundation** — SH irradiance volume + baker, runtime probe sampling (SH as a 3D texture), lightmaps, and CSM sun shadows are all in place. Epic 9 is a pure upgrade layer on top; it builds nothing E5 already delivers. Independent of Epics 7–8.

**Pre-epic fix — already satisfied:** the fog pass (`src/render/fog_pass.rs`) is imported, owned by `Renderer`, and runs in every frame's render stage. It was wired as part of the portal-fog-culling work, not as a standalone E9 prerequisite. See `rendering_pipeline.md` §7.5 and `plans/done/perf-portal-fog-culling/`. Directional fog (below) builds directly on the live pass.
...
- [x] **Probe depth/visibility atlas (bake)** — prl-build stage baking per-probe depth moments alongside the existing SH bands, ray-cast through the Epic 4 BVH. Format kept chunk-friendly so a later brick split needs no interpolant rewrite (deferred-streaming insurance). New/extended PRL section.
...
- [x] **Memory-budget checkpoint + coarse open-area spacing** — VRAM budget readout plus coarser probe spacing in open volumes; produces the empirical resident-fit number that gates any future streaming epic.
