...
| Kinematic mover | direct-SH base | id 45 composed delta (promoted: + pool self-shadow, not built) | runtime direct loop |
| Skinned mesh | direct-SH base | id 45 composed delta (promoted: + pool self-shadow, not built) | runtime direct loop |
...
**Promoted animated lights (not yet built).** A script-animated baked light promotes into the same shadow pool as a static light, on the same runtime gates — a shadow-relevant receiver in its influence, portal-reachable — sharing the one ranker and the 8-spot/2-cube budget. It mirrors static promotion on the *additive* side: a mover or mesh receives `(1 − w) × section-45 animated delta + w × runtime shadowed term`, the runtime cone frozen at the light's authored rest direction so the runtime term encodes the same illumination the delta does and no receiver sums the light twice. Only moving receivers gain the runtime term; world surfaces and billboards keep the baked path. The fixed position and frozen cone let the world depth cache exactly as a static promoted light's. Eligibility is runtime-only — every animated baked light already carries a section-45 delta, so there is no compile-time selection list — and gates on the light's brightness over the crossfade horizon (a forward-lookahead max of its curve), not its instantaneous strobe value, so a strobe neither thrashes the pool nor holds a slot through a long dark trough.
...
**Collection-id keying.** A sprite *collection* is keyed by a **collection id** — the pair `(asset reference, draw contract)` — not by the asset reference alone. Distinct draw contracts over one asset (differing loop period, emissive, or specular) become distinct collections rather than colliding at registration. Each collection id keeps its own `SpriteDrawParams` uniform, group-1 bind group, region, and draw call — the one-draw-call-per-collection rule and the per-region layout above hold unchanged. Each producer derives the render-collect key locally from its matching raw contract: projectile bodies agree across peers because their shared descriptor supplies that contract, while emitters and fixed impacts use local fixed contracts. The key is never replicated or baked. GPU texture upload is temporarily per collection id; the asset-reference dedup seam is owned by the next registration step.
