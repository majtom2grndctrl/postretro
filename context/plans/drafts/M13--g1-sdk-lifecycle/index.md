...
- [ ] The static SDK block declares `ui.localState` (an SDK-lib function, drift-checked — not auto-emitted from the registry) and the generated typedefs include the manifest UI fields; the parity guard passes; `gen-script-types` reports no drift (regeneration incremental on G1a's committed output).
