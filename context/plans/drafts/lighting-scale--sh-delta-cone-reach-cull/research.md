...
- 338 promotable spots (`_bake_only 0`, `_shadow_type static_light_map`, 48° outer cone,
  `_falloff_range 1024` ≈ 26 m); 0 animated lights → the OOM is purely id-41.
- Pre-cull dense: 659,904 `(affinity-cell, light)` CSR entries × 18,432 B/entry ≈ 12 GB
  (matches the `lighting-scale--compile-peak-ram` decomposition).
- Cone-vs-cube solid-angle estimate ≈ 10× fewer entries — **unmeasured**. The first slice
  measures the post-cull projection with `--sh-delta-working-set-max-size 0`.
