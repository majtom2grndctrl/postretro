...
pub(crate) fn register_shared_types(registry: &mut PrimitiveRegistry) {
    ...
    registry
        ...
        .field("bloomMaxDegrees?", "f32", "Maximum hitscan-only sustained-fire bloom above base spread, in degrees. Range: 0..=45; defaults to 0.")
        .field("bloomDecayDegreesPerSecond?", "f32", "Rate at which hitscan-only sustained-fire bloom decays after its delay, in degrees per second. Must be finite and >= 0; defaults to 0.")
        .field("bloomDecayDelayMs?", "f32", "Idle delay before hitscan-only sustained-fire bloom begins to decay, in milliseconds. Must be finite and >= 0; defaults to 0.")
...
pub fn register_all(registry: &mut PrimitiveRegistry, ctx: ScriptCtx) {
