...
binding + target-alive + range, and **drops world-LOS for projectiles** (see below —
it defends only wallhacking, out of scope for this co-op engine).

**Consequence — no wire-format change.** The authority-critical path reuses the shipped
combat wire contract verbatim (no new message, no new field, no layout change). One net-new
authority mechanism, host-side and off the wire: **open-shot lifetime** (a projectile shot stays
open until its declaration arrives or a host timeout retires it). Projectiles run **no world-LOS
...
| Fixed tick | `frame_timing.rs::TICK_DURATION = 16_667µs` (**60 Hz**); `sim/mod.rs::simulate_tick` → `simulate_tick_with_presentation_aim` | 11 ordered steps. Weapon fire = step 9 (`weapon_stage::run_local_weapon_command`), death sweep = step 10. **Projectile advance inserts after step 9, before step 10**, reusing the threaded `on_impact` closure. |
...
(shot_id binding → target alive → range×tolerance; **projectiles skip world-LOS**) and applies.
On expiry-without-hit the client sends an **empty** declaration so the host retires the
shot promptly; a host **timeout** is the backstop for a dropped client.
...
fire-and-take-cover hit. Validating from a snapshotted fire-time origin would fix the geometry, but
world-LOS only ever defends the **wallhack** case, a PvP concern out of scope for this co-op engine.
So projectiles run **no world-LOS** (validated by `shot_id` binding + target-alive + range only),
which is both simpler (no fire-origin snapshot; open-shot lifetime is the sole net-new mechanism) and
strictly more client-honoring (no geometric false-reject surface). Hitscan keeps its cheap same-tick
world-LOS. See the Co-op fairness note in `index.md`.
