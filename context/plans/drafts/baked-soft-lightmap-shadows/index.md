...
- [ ] Forward-pass cost is unchanged aside from one added sampler binding (no new per-fragment loop); holds an acceptable framerate on the NVIDIA GTX 16-series perf floor (e.g. GTX 1660 Super). The AMD Radeon Pro 5500M compatibility floor (2020 16-inch MacBook Pro) must run but is not framerate-gated. Lightmap-bake wall-time increase is measured and documented.

## Tasks

### Task 1: Light size authoring
Add `_light_size` (Point/Spot, world-unit radius) and `_angular_diameter` (Directional, degrees) to the `light` / `light_spot` / `light_sun` FGD entity definitions, and corresponding fields on `MapLight` (`map_data.rs`). Parse and validate where `MapLight` is built from FGD KVPs (clamp to non-negative; documented defaults applied when the KVP is absent). The new fields must flow into the bake stages' `StageInputs` so they enter the cache `input_hash`; without this, changing `_light_size` on a map won't invalidate the cache and the "softens on recompile" behavior fails.

In the same authoring pass, hard-rename the existing falloff-distance KVP `_fade` → `_falloff_range` (clearer name; no alias — per the pre-stable convention, update every call site in one change): the FGD `Light` and `DynamicLight` base classes, the parser key and its `MissingProperty("_fade")` error variant in `format/quake_map.rs`, and the ~40 `_fade` test fixtures there. The Rust field stays `MapLight.falloff_range` and the inches→meters translation is unchanged; only the authored key string moves. Breaking change: existing `.map` files using `_fade` must be re-authored to `_falloff_range`.
...
In the per-texel loop, replace the hard `if !shadow_visible(...) { continue; }` gate with `let v = soft_visibility(...); if v <= 0 { continue; } irr += contribution * v;` and weight the dominant-direction accumulation by `v` (this biases the dominant direction toward partially-visible lights in penumbras — direction-channel encoding/format unchanged, values shift; reconciled with the magnification AC above). Output stays a single Rgba16 irradiance atlas + `Rgba8Unorm` direction atlas, `LightmapMode::Shadowed`. The dilation/chart-seam handling (`dilate_edges`, `CHART_PADDING_TEXELS`) is unchanged — soft visibility is a scalar that dilates identically to today's irradiance. Bump `lightmap_bake::STAGE_VERSION` (currently 4): the per-texel output computation changes, and the cache-bump contract (enforced by tests) requires it — without the bump, recompiles return stale hard-shadow output from the build cache.
...
Emit a per-light bake warning when the emitter subtends a sub-texel penumbra. Coarse heuristic: compute the emitter's angular size from `_light_size` over the light's `_falloff_range` reach (Point/Spot) or directly from `_angular_diameter` (Directional), project it against the atlas texel world-size at the receiver, and warn when the estimate is < ~1 texel. Use only `_light_size`/`_angular_diameter`, `_falloff_range`, and atlas density — no distance-to-occluder term (consistent with Task 2's no-occluder-distance design). This is an author hint, not an exact penumbra width. Expose the area-sample count as a bake knob in `StageConfig` (folded into `input_hash`); the adaptive-escalation thresholds must be fixed constants — both requirements keep bake deterministic and cache-correct. Add or extend a test map with a single key light, a box-on-floor contact case, and a many-overlapping-light room. Measure forward-pass cost on the perf floor and lightmap-bake wall-time delta; record both.
...
- Reuse primitive: `segment_clear(bvh, primitives, geometry, origin, target) -> bool` (`lightmap_bake.rs`). Soft visibility = fraction of `M` area samples whose segment is clear. Direction atlas format is `Rgba8Unorm` (octahedral).
