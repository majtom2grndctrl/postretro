...
4. **Task A:** Runtime rendering of any `--pvs`-compiled map is bit-identical to the same map compiled with `--pvs-fast`. Verification: spot-check `content/base/maps/test.prl` and `content/base/maps/occlusion-test.map` outputs in both textured and wireframe modes; no geometry disappears.
