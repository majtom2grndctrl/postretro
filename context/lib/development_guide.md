...
**Never hand-maintain a crate roster here.** Query it. `crate-graph.md` is the generated
snapshot — every workspace crate, its layer, and its dependent count — kept fresh by the
`crate-graph --check` preflight gate. For anything it doesn't answer:

```bash
cargo run -p xtask -- crate-graph --rdeps <crate>   # blast radius (what breaks if I change this)
cargo run -p xtask -- crate-graph --deps <crate>    # what this pulls in
cargo run -p xtask -- crate-graph --mermaid         # full edge diagram, on demand
```

A crate's own `Cargo.toml` carries the rationale for each external dependency, annotated
at the workspace root.

Four properties the graph cannot show, because they are rules rather than edges:

- **`foundation` and `entities` are VM-free**; `scripting-core` is the VM-coupled layer.
  Anything touching rquickjs or mlua belongs above the floor, not in the substrate.
- **`net` is glam-free and postretro-free.** The dependency arrow points engine → net and
  never back.
- **Two crates ship binaries under a different name** than the package: `level-compiler`
  builds `prl-build`, `script-compiler` builds `scripts-build`.
- **Dev-dependency edges are invisible to the graph** — `crate-graph` reduces to normal
  edges only. A test-only edge does not enter the layering, so it cannot violate it.
...
**Collision and fixed-tick gameplay live in `postretro-sim`.** It owns collision, movement, kinematic movers, triggers, AI, combat resolution, and the scripting runtime/handlers that drive the headless simulation seam. The binary supplies data at that boundary; sim never depends upward on binary input, UI, renderer, or application state.

**Target shape.** `postretro-sim` and `postretro-netcode` are built crates. `postretro-combat-model` holds combat domain data — stat resolution, resources, augments, damage taxonomy — off the registry, plus shot-authority and carried-loadout data. `postretro-sim` owns fixed-tick gameplay and collision; `postretro-netcode` depends on sim and has sole ownership of registry replication and authoritative network gameplay (`networking.md`). Normal dependencies point downward: netcode → sim → lower crates; the binary orchestrates above both through their public seams. Neither sim nor netcode depends on the binary, renderer, UI, or input. Enemy AI still lives in `postretro-sim`, but it is not fused to the scripting VM runtime — it holds no script-store or reaction-dispatch state (`scripting-core` owns both). M3 (planned, not built) carves it into `postretro-ai` **above** sim, inverting the injected `simulate_tick` → AI edge behind a synchronous `AiHost` effects trait so sim never depends on ai, with two graph-query helpers and two state constants sunk to `foundation` so `postretro-netcode` stops reaching into AI; the same milestone extracts a `postretro-physics` leaf — collision, movement, kinematic movers — below sim. The render stack reads entity state from `entities` directly, making sim a sibling to renderer; the binary orchestrates input → game logic → audio → render → present.
