...
**Billboard direct scatter.** Valid `BillboardDirectScatterVolume` (PRL section 47) replaces a billboard's directional direct-SH sample with normal-free RGB scatter. The billboard vertex shader samples group 3 binding 17 only; `FrameUniforms.has_scatter` at byte 112 selects it. Animated maps compose section 48's dense deltas using the section-45 descriptor mapping, shared animation time, and the direct-term mask before the billboard draw. The scatter path has neither the direct-SH nor static-specular branch, and its runtime direct loop stops at `light_count`, dropping promoted-static records instead of subtracting a direct-SH share. Missing or invalid section 47 (or an invalid required 45/48 pair) selects the exact legacy path: binding 15 direct SH, static specular, and `total_light_count` including the promoted tail.
...
Camera-facing quads driven by the particle system. Alpha-blended additive pass; depth write disabled, depth test enabled. Quads are expanded in the vertex shader using the view-space right and up vectors — no geometry shader. Every billboard receives baked indirect and dynamic direct. With valid direct scatter it also samples normal-free baked direct RGB at group 3 binding 17, has no static-specular or direct-SH branch, and excludes promoted-static records from the runtime loop. This binding is VERTEX-only, and `has_scatter` in `FrameUniforms` byte 112 selects it. Missing or invalid scatter uses the exact legacy vertex path: binding 15 `sample_sh_direct`, static specular via the chunk light list, and the promoted tail. The camera-facing `N = V` convention applies only to that legacy direct-SH/specular path; scatter itself is normal-free. Terms are evaluated at the sprite center in `vs_main` and interpolated rather than re-evaluated per fragment.
