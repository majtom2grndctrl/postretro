...
**Variable base-probe density (planned; not yet in the shipped renderer).** The composed total and composed direct atlases stop being dense over the grid. Each 4×4×4 probe brick is stored at one of three densities — 64, 8, or 1 tiles — chosen at bake by the composed-receiver-error classifier; both atlases and the at-rest id-34/id-35 payloads carry exactly that stored set under one shared slot geometry. Reconstruction moves out of the compose and into the sampler: a shared resolve over the stored geometry replaces the dense per-probe tap, reading an L0 probe's own tile, an L2 brick's single mean tile, or an L1 brick's eight corner slots trilinearly (`sh_reconstruct` is the one definition the sampler and dev-tools decoder share). A per-probe indirection word — level, validity, and stored slot — derived at load and never serialized, drives the compose (stored-slot writes) and every sampler; it rides the depth-moments texture, which becomes `Rgba16Uint` with RG the f16 moments unchanged and B/A the word. Storage is never coarser than a present delta entry (ids 27/41/45): id 41 pins base storage to its own per-cell level, while ids 27 and 45 — which stay uniform L0 — pin base storage to L0 wherever an animated light reaches. Format contracts bump (id 34 → v10, id 35 → v3; see `build_pipeline.md` §PRL section IDs). The binding low floor for this work is laptop shared-memory iGPU smoothness, below §10's desktop perf floor.
