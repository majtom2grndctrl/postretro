...
- [ ] The relocated `scripting-core` code names `postretro-foundation` / `postretro-entities` directly for floor types and does **not** route through `postretro`'s `crate::scripting::*` compatibility barrels (which would make `scripting-core` depend on the `postretro` binary — a circular/backwards edge, since `scripting-core` sits below `postretro`). The barrels remain in `postretro` for the un-moved `scripting/systems/*` bridges and `Session`. `cargo build --workspace` proves no `scripting-core` → `postretro` edge.
...
- `sequence.rs` — `SequencedPrimitiveRegistry` + `SequenceError`
...
All four registrars stay in `scripting-core`. `SystemCommandQueue` itself lives in `postretro-entities` (the floor); the **drain** runs through `reaction_dispatch.rs` (in `scripting-core`) via the `ScriptCtx::system_commands` field. Several families register in **more than one** registry — **fog** registers in both `ReactionPrimitiveRegistry` and `SequencedPrimitiveRegistry`; **light** registers in both `PrimitiveRegistry` (via `register_all`) and `SequencedPrimitiveRegistry`. Move every site for a family together.
