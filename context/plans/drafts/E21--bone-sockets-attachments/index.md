...
Add a per-node `extras` key `socket` (string) parsed beside `hitZone` in the module Task 1 created. Surface it on `LoadedModel` as a `sockets` table (socket name → `SocketBinding`, an explicit two-variant enum): **`SkinnedJoint(topo index)`** — built inside `build_skeleton` through the same `skin_joint_to_topo` remap the `joint_zones`/`pose_masks` reads use (a raw skin-joint-order read reindexed to topo order); **`RigidRest(Mat4)`** — for a model with no skin, the tagged node's model-space rest transform, computed by composing the node's TRS through the glTF node hierarchy (rigid models load with one synthetic identity joint and no skin joints, so tags must resolve on plain nodes or rigid weapon/prop models could never carry sockets). The two variants are the whole reason resolution is not a bare joint index — the rigid case carries a matrix, not an index. Rigid resolution composes the node TRS in the **same authored model space the loader consumes vertex `POSITION` from** (no node-transform bake into vertices; `resource_management.md` §7's one-mesh-node-per-model, authored-at-final-meters convention), so socket and geometry share a space; a rigid model with a non-identity transform above the mesh/socket node is out of contract and out of scope. Degradation contract matches the existing extras family, and applies to both variants: malformed value warns and skips; a duplicate socket name warns and keeps the first winner in a deterministic traversal order; on a *skinned* model a socket on a non-joint node is ignored with a warning. Retain the table game-side on the per-model store in `crates/postretro/src/scripting/systems/hit_zones.rs` (`ModelHitZones`), keyed by `ModelHandle` like `joint_zones`/`legs`, so the Task 4 resolver reads it without reloading the glTF. Unit tests cover the remap (a socket tagged on a node whose skin index ≠ topo index resolves to the right joint), the rigid-node rest-transform composition (a socket on a child node inherits its parent chain's TRS), duplicate handling on both variants, and malformed degradation.
...
- **Oversized-file watch:** `gltf_loader.rs` 4,826 (split: Task 1), `hit_zones.rs` 3,296 and `mesh_render.rs` 1,576 (new logic goes to the new `attachments` module; the in-file deltas are a socket-table field on `ModelHitZones` and a resolver parameter threaded through `collect_inner` and its call sites), `main.rs` 8,006 (touched at the existing `resolve_mesh_entity_clips` / `distinct_mesh_models` seams, extending the resolve pass rather than adding one).
