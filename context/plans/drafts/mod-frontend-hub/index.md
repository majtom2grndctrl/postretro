...
Add `frontend: Option<Frontend>` to `ModManifestResult` (runtime.rs:56) and `StagedManifest` (staged_manifest.rs:57); `commit_staged_manifest_result` replaces the `StagedManifest.frontend` field on the same branch as `ui_trees`/`theme` (replaced whole each staged commit — not a dedicated `replace_*` branch like `maps`/entities). Drain it in `run_mod_init_quickjs` (runtime.rs:1485, beside `drain_ui_trees_js`/`drain_theme_js` ~1629–1638) and `run_mod_init_luau` (runtime.rs:1709). Commit it at the same successful-staged boundary as UI trees and theme. The committed block is also stored in a durable app-side field `self.frontend: Option<Frontend>`, set at the same successful-staged commit boundary as `ui_trees`/`theme`; the game-flow arms (Task 5) read `backgroundLevel` from it. A structurally-invalid `frontend` field aborts mod-init like `maps`/`theme`/`reactions` (the drain callers set `out = Err(...); return;`); only sub-field degradation (e.g. a single bad `camera` sub-field) is logged-and-skipped.

### Task 3: Mod boot handle (`--mod` flag)

Add a `--mod <path|id>` CLI argument that sets `content_root` (extend the existing `--content-root`/bare-map parsing in `resolve_map_path`/`content_root_arg`); `run_mod_init` (runtime.rs:888) then drains the selected mod's `frontend` block (Task 2). With `--mod` and no map argument, boot proceeds Splash → `Frontend` with the selected mod's `frontend` block committed; menu presentation is Task 4's deliverable. A bare map-path argument remains the dev raw-path bypass (`LevelSource::Path`) that loads straight into `Running` under the selected (or default) mod. Out of scope: mod-browser UI and persisted last-mod selection.

### Task 4: Frontend population + menu camera

Present the committed `menuTree`: resolve it through the UI registry (`ModalStack`, modal_stack.rs) and push it as a capturing modal; fall back to the engine-default menu when absent (new tier entry alongside `hud`/`pauseMenu` fallbacks). Apply the declared static camera pose by writing `self.camera.{position,yaw,pitch}` (camera.rs:86) — the same fields `install_level_payload` writes at spawn (startup/lifecycle.rs:638; camera writes at ~921-924), so no new camera plumbing. The menu pose is applied after any backdrop install completes (and re-applied while the menu modal is top), so the install-time camera teleport (lifecycle.rs:921-930) does not clobber it. Suppress player control via the capture-mode path (`reconcile_ui_focus` main.rs:3502). Camera orbit is out of scope — static pose only.
