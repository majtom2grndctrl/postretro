...
- **Spawn ordering (verified — load-bearing):** lights spawn in the renderer install (`LightBridge::populate_from_level_with_influences`, `startup/lifecycle.rs`) **before** movers spawn (`spawn_loaded_kinematic_movers`, Segment B, `lifecycle_world_cpu.rs`). So at the `populate_from_level` call site the movers do **not** exist and no `mover_id→EntityId` map can. Carrier resolution must be a **distinct pass after** `spawn_loaded_kinematic_movers`, setting `LightComponent.carrier` on the already-spawned light entities via `LightBridge::entity_for_map_index(alpha_light_index)`. `populate_from_level*` is called on multiple load/reload paths (`lifecycle.rs`, `session/mod.rs`) and unload clears bridge+movers (`lifecycle_net.rs`) — resolution must run on every mover-spawn path, before that level's first `LightBridge::update`.
...
| Carried **spot** on a spinning mover | resolved at compile (mover `spin_speed_deg_s != 0`) | Warn (spot + spinning carrier); light binds and carries position (orbit), cone aim **not** re-rotated (deferred). |
| Carried **spot** on a translating mover | position changes each frame → dirty fires; aim as-authored | Cone tracks position and holds authored aim; re-uploads because position moves. |
| Reload / host level change | load → carrier set; unload (`lifecycle_net.rs` clears bridge+movers) → reload → resolution re-runs | Each carried light re-resolves against the freshly spawned mover; no carrier retains a pre-reload `EntityId`. |
| Named mover fails / absent from map | mover spawn returns `Err` (registry exhausted) or `mover_id` absent | Carrier degrades to **unbound** (authored origin) with a warning; never bound to a misaligned entity. |
| Mover geometry culled, light in view | mover AABB outside `visible_cells` so its draw is skipped | Light still composes: bridge iterates its own list; `interpolated_transform(mover)` always available (mover always spawned). |
