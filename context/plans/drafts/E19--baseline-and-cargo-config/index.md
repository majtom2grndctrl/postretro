...
- [ ] The `dev-fast` profile decision is recorded (adopted or rejected) with the dependency-`opt-level`/debug-info compile-time comparison numbers that drove it. The decision rests on compile-time evidence only: reduced debug-info is runtime-neutral, and any dependency-`opt-level` reduction's runtime-feel check is deferred to a manual run on a GPU host (the orchestrate/CI environment is headless — no GPU, no frame timer), noted as pending rather than run here.
