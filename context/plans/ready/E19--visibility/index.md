...
- Deps (authoritative list in Task 1): normal — `postretro-level-loader` (`LevelWorld`, `CellData`, `FaceMeta`) + `glam`; dev-only — `postretro-render-data` **and** `postretro-level-format`, both named solely inside the moved test modules. The normal build carries neither dev edge.
...
- [ ] The relocated visibility/portal unit tests pass unchanged — a pure file move introduces no logic edit, so `determine_visible_cells` is behavior-preserving by construction (the in-file tests are the fixture; there is no separate golden corpus).
...
**Deps.** `[dependencies]`: `glam`, `postretro-level-loader` (for `LevelWorld`, `CellData`, `FaceMeta` — depend on `LevelWorld` directly). `[dev-dependencies]`: `postretro-render-data` **and** `postretro-level-format` — both are named ONLY inside the moved `#[cfg(test)] mod tests`: render-data `geometry`/`material` at `visibility.rs:729-730`/`portal_vis.rs:766`; level-format `TextureCacheKeysSection` at `visibility.rs:813,873`/`portal_vis.rs:765`. `postretro-level-loader` does **not** re-export `postretro-level-format`, so the tests name it directly — it needs its own dev edge. Neither dev crate may be a normal dep (AC2 gates both under `--edges normal`).

**Re-exports + importers.** Declare both modules in the crate's `lib.rs` and expose the externally-consumed symbols at the `postretro_visibility::` crate root: `VisibleCells`, `CameraCullVisibility`, `VisibilityPath`, `VisibilityResult`, `VisibilityStats`, `determine_visible_cells`, `extract_frustum_planes` (consumed at `main.rs:110-112,2143,2161`). In the binary, delete `mod visibility;`/`mod portal_vis;` (`main.rs:51`/`main.rs:38`) and re-point every `crate::visibility::`/`crate::portal_vis::` **and** bare `visibility::`/`portal_vis::` path (e.g. `main.rs:2143,2161`) to `postretro_visibility::`. That is 11 in-binary importer files — including the still-in-binary GPU cull modules `compute_cull.rs` and `candidate_cull_mirror.rs` (they import `VisibleCells`; they don't leave until `E19--renderer-gpu`) and `scripting/systems/mesh_render.rs`. Lean on `cargo build --workspace` to surface every stale path; a `crate::`-anchored grep misses the bare `main.rs` calls, so the compiler — not grep — is the completion gate. Leave the unrelated `render/ui/tree/tests/visibility.rs` untouched.
