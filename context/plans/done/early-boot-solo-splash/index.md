...
- [ ] On launch, the first presented frame is black and is recorded before network socket construction, audio init, debug UI init, mod init, level worker dispatch, and built-in UI asset loading that runs after the window. The script runtime is the one session-lifetime system constructed **pre-window** (before first pixels), so `first_black_frame` is recorded *after* `script_runtime_ctor`, not before it: primitive closures capture `ScriptCtx` clones and the runtime is held for the whole session, so it must exist before the app is constructed and the event loop runs. (Built-in UI JSON registration likewise happens in the pre-window `SessionServices` build today; the durable ordering claim this AC pins is that the first black frame precedes net/audio/debug-UI/mod/level-worker work, which it does.) Splash rendering returns a presented/not-presented outcome; `first_black_frame` is recorded only after command submission and a successful swapchain present path.
