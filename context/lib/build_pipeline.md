...
**ChunkLightList (id 23):** per-chunk index lists into the static (baked-tier) spec-light set — dynamic lights are excluded, riding the runtime shadow-map path. When a chunk overlaps more lights than the per-chunk cap, overflow eviction keeps the highest-influence lights, ranked by the same influence metric the runtime applies per fragment; keep the compiler and runtime rankings aligned, since a change to one that diverges from the other silently degrades which lights a chunk retains. Because neighboring chunks then keep the same lights, specular highlights stay continuous across chunk faces — the cap only sets how often eviction bites, not whether the kept set is correct. _Planned, not yet built: current eviction keeps the lowest-index lights, so adjacent chunks retain different subsets and specular cuts off mid-surface at chunk faces on dense maps._
