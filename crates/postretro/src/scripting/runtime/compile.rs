...
use crate::scripting::quickjs::run_script;
...
pub(super) fn compile_start_script(ts_path: &Path, js_path: &Path) -> Result<(), String> {
    let compiler = crate::scripting::watcher::TsCompilerPath::detect().ok_or_else(|| {
        "scripts-build not found — install it on PATH or ship it next to the engine binary"
            .to_string()
    })?;
    crate::scripting::watcher::run_ts_compiler(&compiler, ts_path, js_path)
}
...
pub(super) fn scan_and_compile_stale_ts(script_root: &Path, mod_root: &Path) {
    ...
    let compiler = match crate::scripting::watcher::TsCompilerPath::detect() {
        Some(c) => c,
        None => {
            log::warn!(
...
pub(super) fn visit_ts_files_shallow(
    dir: &Path,
    compiler: &crate::scripting::watcher::TsCompilerPath,
    compiled: &mut u32,
    failed: &mut u32,
) {
...
fn compile_one_unconditional(
    ...
    let js_path = crate::scripting::watcher::compiled_output_for(path);
    match crate::scripting::watcher::run_ts_compiler(compiler, path, &js_path) {
        Ok(()) => {
