...
- `components.behavior` descriptor block: named states (`animation`, `motion` verb, optional `action`, ordered `transitions`), a top-level ordered `interrupts` list (any-state transitions, evaluated first), `initial` state, an `attack` tuning block (`damage`, `range`, `cooldownMs`) consumed by the `attack` action verb, and an optional `engagementRadius`. The two ranges are separate concerns: `attack.range` gates damage, `engagementRadius` sets where engaged agents stand (combat-slot spread) for every engaged state, attacking or not, and falls back to `attack.range` when unauthored.
...
- Removing the vestigial `death` state map entry from legacy `ai` descriptors — parsed and lowered as today; cleanup is separate. `deathDespawnMs` stays a legacy-`ai`-only field: `BehaviorGraphDescriptor` has no despawn field at all, because despawn timing is owned by the E16 death/despawn effect path and a second, unread spelling of it on the behavior block was a trap for authors.
- An engine-side leash or acquisition range for AUTHORED graphs. `select_target` has no range limit, and an authored graph carries `leash_range: None` by design, so a `chaseTarget` state authored with no exit guard yields a level-wide pursuer that validates cleanly. v1 answer: an authored graph owns BOTH engagement and disengagement through its own guards over `@brain.targetDistance` (the reference enemy is the pattern), documented in `docs/scripting-reference.md`. Disengagement has two halves, because the no-target distance sentinel is one-directional: `le`/`lt` range guards read false untargeted (so entry edges need no `hasTarget` conjunction), but `gt`/`ge` guards read TRUE, which makes a bare leash guard fire on target loss. A graph therefore also authors a stand-down interrupt on `!hasTarget` (spelled `select(hasTarget, false, true)`; no `not` opcode), first in interrupt order, so target loss returns it to `initial` in one tick instead of walking it through a travel state. The lowering emits exactly this edge for legacy brains. A `leashRange` field on the behavior block was considered and deferred — it would be a second spelling of disengagement that silently outranks the guards. The successor spec (`E10--enemy-aggro-model`, from `research/enemy-aggro-model.md`) owns the question of whether acquisition range belongs to the engine floor or to authored guards.
...
Author the reference enemy (`sdk/behaviors/reference/entities.{ts,luau}`) as an explicit `components.behavior` graph reproducing the current idle/alert/attack tuning, plus the `!hasTarget` stand-down interrupt and an explicit `engagementRadius`; keep the pose-fixture enemy on legacy `components.ai`. Its comments are the de-facto authoring documentation — they must teach the sentinel asymmetry and the range/spread split, and the two spellings must stay identical. Port the pure-core unit tests in `ai_tests.rs` to lowered-graph equivalents; add fixture assertions for behavior-identity (legacy vs authored graph — the Rust transcription of the reference graph is that test's oracle and has to track the SDK files), the no-pawn stand-down from the attack state, interrupt priority, commitment window, `@state` guard end-to-end via an impact policy — author a test-only impact policy that writes a `@state.*` field on the fixture enemy, then assert the brain transitions on the next AI tick, serde round-trip/rebind, and the alloc-probe eval assertion. Update the agent diagnostics overlay to label agents with the authored state name. Regenerate and commit both typedef fixtures.
...
| Engagement radius | `BehaviorGraphDescriptor::engagement_radius: Option<f32>` | `"engagementRadius"` (falls back to `attack.range`, else 2 m) | `engagementRadius?` | `engagementRadius?` | n/a |
...
| Self-targeting interrupts skipped during evaluation; an interrupt cannot re-enter its own state. A state-local self-edge is instead a validation error — first-true-wins makes it a silent blocker of every transition declared after it | Task 5 (evaluator), Task 2 (validation) | Any future guard side-effect must not rely on self-interrupt re-entry | AC 2, 4 |
| Disengagement is graph-authored in two halves: range guards plus a `!hasTarget` stand-down interrupt first in order. The no-target distance sentinel makes `gt`/`ge` guards read true, so range guards alone stand down late and through the wrong state | Task 4 (lowering), Task 6 (reference enemy) | Any authored graph whose only exits are `gt`/`ge` range guards; both reference authorings must carry the interrupt | AC 7 |
| `engagementRadius` (slot spread) and `attack.range` (damage gate) are separate knobs; the fallback chain field → `attack.range` → 2 m is what preserves legacy parity and keeps a pure-pursuit graph from resolving to a zero-radius ring with no slots | Task 2 (descriptor) | Any code reading `attack.range` for spacing, or `engagement_radius()` for damage | AC 1, 7 |
...
export const grunt = defineEntity({
  ...
  components: {
    ...
    behavior: {
      ...
      engagementRadius: 2,
      interrupts: [
        // Stand down on target loss, first so it outranks every range guard:
        // the no-target distance sentinel makes the `gt` leash below true.
