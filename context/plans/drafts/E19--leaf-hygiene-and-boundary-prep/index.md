...
- [ ] `Frustum`, `FrustumPlane`, and `extract_frustum_planes` are `pub` (the complete binary-side surface of the future `postretro-visibility` crate — `rg "visibility::" crates/postretro/src --glob '!visibility.rs' --glob '!portal_vis.rs'` names no other `pub(crate)` item); no other visibility change — `portal_vis.rs`-only items stay `pub(crate)`.
