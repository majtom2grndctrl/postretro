...
- Runtime compute pre-pass that composes the animated-lightmap-contribution atlas. Inputs: the three weight-map storage buffers, a CPU-built `dispatch_tiles` storage buffer (see below), the animation-descriptor storage buffer, the frame-time uniform (from the per-frame uniform bound at group 0). Output: an `Rgba16Float` storage texture (created with `STORAGE_BINDING | TEXTURE_BINDING | COPY_DST`) matching the lightmap atlas dimensions. Compose uses a dedicated compute pipeline bind group layout: group 0 is the frame-time uniform (reused from the forward layout); a compute-exclusive group carries the four storage buffers (three weight-map buffers plus `dispatch_tiles`), the descriptor buffer, and the storage-texture write view for the animated atlas. The forward pass binds the same animated-atlas texture as a sampled view on bind group 4 alongside the static lightmap. Adapter feature check at init matches the SH volume path. Dispatch is per 8×8 tile: at map-load time the engine expands every `chunk_rect` into one or more `DispatchTile { chunk_idx, tile_origin_x, tile_origin_y, _pad }` entries covering the rect; dispatch count is `dispatch_tiles.len()`, workgroup size is `@workgroup_size(8, 8, 1)`. Threads outside the chunk rect early-out. Chunks smaller than 8×8 pay one workgroup; larger chunks scale linearly; uncovered atlas area pays no thread cost. An explicit `encoder.clear_texture` precedes compose each frame; uncovered texels stay zero.
...
- `AnimationDescriptor.brightness` and `.color` evaluation on the GPU uses **Catmull-Rom** interpolation. Descriptor storage unchanged; the sampling function is the prerequisite. Sibling draft `context/plans/drafts/animated-curve-eval/` owns the evaluator and the shared WGSL helper used by both the SH animation pass and this compose pass. This spec depends on it and must not promote to `ready/` before the evaluator plan lands.
...
In `lightmap_bake`, filter the `static_lights` pass to `!light.is_dynamic && light.animation.is_none()`. Update the existing test `static_flag_bakes_but_dynamic_does_not` to cover the animated case as well. No static-atlas consumers read animated contribution today, so the removal is a clean break.
...
New WGSL compute shader `animated_lightmap_compose.wgsl` with `@workgroup_size(8, 8, 1)`. At map-load time a CPU pass in `animated_lightmap.rs` expands `chunk_rects` into a flat `Vec<DispatchTile>` where `DispatchTile = { chunk_idx: u32, tile_origin_x: u32, tile_origin_y: u32, _pad: u32 }` — one entry per 8×8 tile needed to cover each chunk rect (`ceil(rect.width / 8) × ceil(rect.height / 8)` tiles per chunk). Dispatch count = `dispatch_tiles.len()`. Per workgroup: read `tile = dispatch_tiles[workgroup_id.x]`, then `rect = chunk_rects[tile.chunk_idx]`. Per thread: `rect_x = tile.tile_origin_x + local_invocation_id.x`, `rect_y = tile.tile_origin_y + local_invocation_id.y`; early-out if `rect_x >= rect.width || rect_y >= rect.height`; else `texel_idx = rect.texel_offset + rect_y * rect.width + rect_x`; iterate the texel's light list; sample each descriptor's brightness and color via the shared Catmull-Rom helpers at the current cycle time; multiply by weight and `active`; accumulate and `textureStore` once at `(rect.atlas_x + rect_x, rect.atlas_y + rect_y)`. Frame time comes from the existing frame-uniform buffer.

Each frame the encoder issues `clear_texture` on the animated atlas before dispatching compose; uncovered atlas texels stay at zero.

Renderer owns this pass. Runs inside Render, after the BVH cull pass and before the depth prepass. wgpu infers the storage→sampled barrier from the bind-group usage change (compute write → fragment sample), so no explicit pipeline barrier is needed. The animated-lightmap storage texture is created with `STORAGE_BINDING | TEXTURE_BINDING | COPY_DST` (COPY_DST required for `clear_texture`). Two views of the same texture: the compose pass binds the write view as a storage texture; the forward pass binds the read view as `texture_2d<f32>` on bind group 4 alongside the static atlas (same pattern as the SH volume pass). Forward shader composes `static_directional_shaded + animated_sample` at the same UV; the animated term is already pre-shaded irradiance (Task 3), so no N·L is applied at runtime. The `POSTRETRO_GPU_TIMING` pass-list scaffold gains an entry for the compose pass.

### Task 6: Shared Catmull-Rom WGSL helper

Consume the `sample_curve_catmull_rom` (scalar brightness) and `sample_color_catmull_rom` (RGB color) helpers landed by the `animated-curve-eval` draft. Both helpers must be in scope for that plan before Spec 2 promotes. Factor the existing SH animation pass to call the shared scalar helper in the same commit, so one WGSL function drives every animated descriptor consumer. No algorithm work in this spec — pure integration.
...
GPU binding shape (two layouts): **Compose compute pipeline** — group 0: frame-time uniform; compute-exclusive group: `chunk_rects` storage buffer, `offset_counts` storage buffer, `texel_lights` storage buffer, `dispatch_tiles` storage buffer, `descriptors` storage buffer, `animated_lm_atlas: texture_storage_2d<rgba16float, write>`. **Forward fragment pipeline** — group 4 gains one additional binding: `animated_lm_atlas` as `texture_2d<f32>` sampled view (two views of the same texture, same as the SH volume pattern).
...
- `postretro-level-format/src/sh_volume.rs` — `AnimationDescriptor` gains `active`.
...
- `postretro/src/render/animated_lightmap.rs` — new compose-pass module.
- `postretro/src/shaders/animated_lightmap_compose.wgsl` — new.
- `postretro/src/shaders/forward.wgsl` — add animated-contribution sample.
- `postretro/src/shaders/sh_animation.wgsl` (or equivalent) — add `active` flag check so inactive lights contribute nothing to the SH volume.
- `postretro/src/render/mod.rs` (or wherever `POSTRETRO_GPU_TIMING` pass entries are registered) — add compose pass entry.

## Open questions

1. **Animated-atlas resolution.** Matching the static lightmap dimensions (1024² today) simplifies sampling: one UV, one fetch per fragment. Halving to 512² would quarter compose cost and animated-atlas footprint, and arguably matches the retro aesthetic — flicker doesn't need per-static-texel resolution. Default for this spec: matched. Revisit if profiling shows compose is budget-dominant or if the authored flicker reads "too crisp" against low-res base textures.

2. **Debug visualization scope.** Task 7 is scope-contingent. Drop if time runs short; AC does not require it.

3. **`chunk_rects` storage as buffer vs texture.** One `ChunkAtlasRect` is 20 B. At 10k chunks the storage-buffer upload is 200 KB — fine. Maps past ~50k chunks may want a 1D texture instead. Out of scope unless profiling hits it.
...
- **`light_index` is remapped to animated-subset ordering at bake time.** Spec 1's filtered-list indices are not stored in `texel_lights`. The baker converts them to animated-subset indices matching the `AnimationDescriptor` buffer ordering. Relative order is preserved; only animated lights are included.
