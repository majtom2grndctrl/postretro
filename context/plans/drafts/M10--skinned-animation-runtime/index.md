...
- **Crossfade.** Switching states blends old → new over the *entered* state's `crossfadeMs` (default constant when unspecified; `0` = hard cut). A switch during an active fade abandons the in-flight blend: the outgoing current state — at its clip-local time, frozen at the switch — becomes the new blend source, and the older fade's contribution drops. An accepted, fade-window-bounded pop; the two-clip ceiling is never exceeded.
