...
fn main() {
    ...
    // Workspace layout is fixed: crates/postretro/ is always two levels under the repo root.
    let sdk_root = Path::new(&manifest_dir)
        .ancestors()
        .nth(2)
        .expect("workspace root two levels above crates/postretro/")
        .join("sdk/lib");
    ...
    // Directory watch is best-effort for additions/deletions; per-file directives
    // from emit_ts_rerun_directives are the reliable path.
