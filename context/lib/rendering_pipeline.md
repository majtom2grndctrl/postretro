...
| 2 | **Reserved, binding map pinned** — still unallocated in code (the pipeline layout passes an empty slot so the work *adds* a group rather than renumbering). The ready plan pair pins the layout: dynamic-direct lighting owns b0–b4 (lights, influence volumes, scripted descriptors, anim samples, params uniform — same underlying buffers forward binds), shadow receipt owns b5–b8 (spot depth array, comparison sampler, light-space matrices uniform, conditional cube array). See `plans/ready/M10--dynamic-mesh-direct-lighting/` and `plans/ready/M10--dynamic-mesh-shadow-receipt/`. |
...
- **Lighting.** The fragment samples the SH indirect baseline and the baked static-direct SH atlas (group 4, `mesh_bind_group` superset — depth-aware Chebyshev octahedral irradiance, `reject_backface = false`, Chebyshev probe-occlusion enabled, direct atlas at binding 15, `DynamicDirectParams` at binding 16). Group 2 is reserved for the dynamic-direct *light loop* — runtime dynamic-tier lights ONLY (plan decision D10; static-tier direct for movers is owned by the group-4 baked atlas), distinct from the baked static direct now in group 4. Specs are ready (see the group-2 row above); the loop evaluates the same `is_dynamic`-filtered light set forward binds, with per-light shadow receipt from the M10 pools layered on top. The vertex stage already carries the skinned world-space normal for it.
