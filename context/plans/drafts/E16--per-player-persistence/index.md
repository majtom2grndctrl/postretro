...
- **The join seed is sent during the parity stage, before the seat is minted.** The host buffers the seed until `admit_or_reclaim` succeeds, then applies it to the freshly minted seat. Sending during parity avoids the timing gap where the seat exists but the seed has not arrived.
...
- **Wire backward compatibility for the join seed.** `ClientControlMessage` is a bitcode-encoded enum. bitcode has no self-describing unknown-variant skip — an older host that attempts to decode `JoinSeed` will fail and reject the client. **Decision required (see Open Questions).** An older client that never sends a seed starts at defaults — that direction degrades cleanly.
...
- [ ] AC 9: In co-op, a connecting client sends a join seed carrying its saved per-owner values. The host applies valid entries to the client's freshly minted seat; invalid entries (type mismatch, undeclared slot, non-persist slot) warn and are skipped.
...
- [ ] AC 13: A client that does not send a seed (older engine) starts at defaults on the host. The wire backward-compatibility path for an older host receiving `JoinSeed` is covered by the resolution of the open question on wire compatibility.
...
Extend `PersistedState` with a `per_owner: BTreeMap<String, BTreeMap<String, PersistedValue>>` field, `serde(default)` and `serde(skip_serializing_if = "BTreeMap::is_empty")`. Outer key: durable slot key. Inner key: hex-encoded `PlayerClaimId`. Bump `CURRENT_STATE_VERSION` from 2 to 3. Update the version check in `overlay_persisted_state` (line 151: `persisted.version != CURRENT_STATE_VERSION`): accept both 2 and 3 — reject anything outside that range. A version-2 document has no `per_owner` field; `serde(default)` yields an empty map, so global-slot restore proceeds unchanged and per-owner slots start at defaults.
...
Retain the loaded `PersistedState` in the session. Currently `overlay_persisted_state` consumes the loaded document and discards it. Change the boot path in `splash_lifecycle.rs` to store the loaded `PersistedState` in the session alongside `StateStoreLifecycle`, as a new `Option<PersistedState>` field. The periodic save and join-seed assembly read from it; the save path updates it.
...
**Periodic save (connected client).** Add a periodic save path for connected clients. A timer (~60 s) triggers a per-owner-only save: call `collect_per_owner_state` for the local player, merge the result into the retained `PersistedState`'s `per_owner` section, and write via `save_persisted_state`. The `slots` section of the retained document is not updated — the client does not collect or save global slots. The timer resets after each save. Clean exit also triggers the same save. The periodic save fires once per frame, after all fixed ticks have drained and the `Rc<RefCell<SlotTable>>` is not borrowed — a post-game-logic, pre-render seam. The timer runs only while the client is in the Participating state; a demotion (level change) pauses the timer, re-promotion resumes it, and reconnection resets it.
...
**Wire type.** Define `JoinSeedValue` in `crates/net/src/wire/control.rs` (or a sibling module in the wire crate) with variants: Boolean(`bool`), Number(`f64`), String(`String`), Array(`Vec<f64>`) — matching `PersistedValue` minus `Unsupported`. Add `JoinSeed { slots: BTreeMap<String, JoinSeedValue> }` to `ClientControlMessage`. The wire crate (`postretro-net`) does not depend on `postretro` or `postretro-scripting-core`, so `JoinSeedValue` is a parallel type converted at the boundary. Add `From<JoinSeedValue> for PersistedValue` (wire to internal, for host apply) and `TryFrom<PersistedValue> for JoinSeedValue` (internal to wire, for client send — `Unsupported` entries are skipped with a warning) in the binary crate.

**Client side.** After content-parity agreement, the client builds the join seed from the retained `PersistedState`. Extract the `per_owner` section, filter to entries matching the client's own `PlayerClaimId`, and package as `ClientControlMessage::JoinSeed`. The seed is sent during the parity stage, before the client transitions to participating. A client with no save file (retained `PersistedState` is `None` or has an empty `per_owner`) sends a `JoinSeed` with an empty map.

**Host side.** When the host receives `JoinSeed`:
1. Buffer the seed until the seat is minted — the message arrives during parity, before the client has a seat. The host buffers at most one seed per connection; a subsequent `JoinSeed` from the same connection (e.g., client re-enters parity on level change) replaces the previously buffered seed. A buffered seed is discarded if the connection closes before the seat is minted.
2. After `admit_or_reclaim` succeeds with a **fresh** seat (not a reclaim within the hold window), apply the buffered seed:
   a. Resolve each durable key to an authored slot name via the identity ledger.
...
3. If the seat was **reclaimed** within the hold window, discard the seed with a log note — live values take precedence.
4. After apply, the next owner-private replication cycle delivers the values to the client's HUD.

The host-side handler lives in the netcode module's control-message dispatch. Thread the slot table, identity ledger, and committed membership through the handler — the same access pattern existing state-related handlers use.

A client that never sends `JoinSeed` (older engine, no save file) starts at defaults — no timeout or fallback needed. An unrecognized `JoinSeed` on an older host is handled by the shipped unknown-variant skip.

Test: a client with saved per-owner values sends a join seed; the host applies them to the client's seat; the client's HUD shows the restored values. A client with no save file sends an empty seed; values start at defaults. A reclaimed seat (hold window) does not apply the seed. Invalid seed entries (wrong type, out of range, non-persist slot) are skipped with warnings. An older host ignores the message; an older client starts at defaults. The seed is applied before `levelLoad` fires.
...
| Client connects, join seed sent during parity, seat minted | Client sends `JoinSeed` during parity stage; host buffers it; `admit_or_reclaim` mints the seat; host applies buffered seed before `levelLoad` fires | Client's restored values are live before the first `levelLoad` reaction |
...
- **Wire backward compatibility for `JoinSeed`.** bitcode has no self-describing unknown-variant skip. An older host that decodes a `JoinSeed` variant it does not recognize will reject the client connection (`transport.rs` treats any decode failure as a protocol mismatch and pushes `HandshakeOutcome::Rejected`). This blocks AC 13's original "older host ignores it" guarantee. Options: (a) bump the wire version and gate the client's `JoinSeed` send on a wire-version handshake, (b) send the seed as an opaque extension payload inside a recognized variant that older hosts already skip, (c) add an explicit unknown-variant fallback to the `ClientControlMessage` decode path before this spec ships, (d) accept the incompatibility (this engine is pre-stable; mixed-version co-op may not be a requirement). Decision needed before promotion.
- **Join seed vs. `levelLoad` ordering.** The `JoinSeed` reliable message can arrive one `poll_handshakes` cycle after parity agreement. The host may promote to Participating, mint the seat, and fire `levelLoad` before the seed arrives on the next poll. Options: (a) hold `levelLoad` on a freshly minted seat until the seed arrives or a brief timeout elapses, (b) accept post-`levelLoad` apply — values arrive via the next replication cycle, just not before the first reaction fires. If reactions read per-owner slot values at `levelLoad`, option (a) is necessary; if they only read them at runtime, option (b) is safe.
