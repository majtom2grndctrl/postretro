...
pub struct MoverPose {
    ...
    pub tick_rotation_delta: Quat,
    pub carry_yaw: bool,
    pub tick_dt: f32,
}

#[derive(Debug, Clone, Copy, PartialEq)]
pub struct MoverTranslationLeg {
...
pub struct EmptyMoverPoseSource;
...
pub static EMPTY_MOVER_POSES: EmptyMoverPoseSource = EmptyMoverPoseSource;

#[derive(Clone, Copy)]
pub struct CombinedCollisionWorld<'a> {
    pub static_world: &'a CollisionWorld,
    pub movers: &'a [MoverCollider],
    pub poses: &'a dyn MoverPoseSource,
}
...
impl<'a> CombinedCollisionWorld<'a> {
    ...
    pub fn static_only(static_world: &'a CollisionWorld) -> Self {
        Self::new(static_world, &[], &EMPTY_MOVER_POSES)
    }
}

#[derive(Debug)]
pub struct MoverCollider {
...
pub fn mover_pose_for_translation_leg(pose: MoverPose, leg: MoverTranslationLeg) -> MoverPose {
