...
- The `crouch_mode` toggle-vs-hold *field on `PlayerOptions`* and the input-layer edge derivation it drives are a DEPENDENCY (`player-options` draft), not built here. This spec only NOTES the dependency and consumes the resulting per-tick crouch-intent bit. See Open Questions.
...
- [ ] `Crouching` is mutually exclusive: while crouched the `Normal` jump/air-jump branch and the `Dash` transition behave per the crouch-jump decision (Open Question #4) — pin the chosen behavior as an AC once redlined.
...
1. **`crouch_mode` (toggle vs. hold) — DEPENDENCY on `player-options`, not a crouch decision.** The `player-options` draft owns and adds `crouch_mode` (toggle | hold, default **hold**) to `PlayerOptions` (it already reserves the field); this spec only consumes the resolved bit. The INPUT layer resolves the mode into the per-tick `crouch_intent` bit `MovementInput` consumes; the `Crouching` state never knows toggle-vs-hold. Redline: confirm default **hold**, and that the resolution lives in input (not the movement intent). Until player-options lands, Task 4 uses an interim hold-mode read.
