...
- [ ] Each new **cpu-only** crate's `cargo tree` (default features) shows **no** `wgpu`, `winit`, `glyphon` (except `postretro-ui`, which uses `glyphon` `FontSystem` for CPU text measurement), `kira`, `mlua`, or `rquickjs` (gate `postretro-lighting` with `-e normal` — it carries `rquickjs`/`mlua` as `[dev-dependencies]` for its relocated script-primitive tests). (Manual/CI gate.)
- [ ] Editing a cpu-only crate and running *its* tests does not recompile `wgpu`/`naga`/`winit`/`kira`, nor (except `postretro-ui`/`scripting-core` consumers) `mlua`/`rquickjs` (`postretro-lighting`'s first test build compiles its VM dev-deps once; warm edits do not recompile them).
...
- **`script-ffi` / handler placement.** Script-primitive *wiring* descends into its subsystem crate behind an optional `script-ffi` feature (off by default, `script-ffi = ["dep:rquickjs","dep:mlua", ...]` per `scripting.md §12`); the marshalling substrate stays in `scripting-core`; the registrar is invoked from `Session::build`. `postretro-lighting` is the first instance (`E19--lighting-cpu`) and the precedent the Epic 16 combat crate mirrors.
