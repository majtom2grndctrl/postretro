...
impl ShResidencyState {
    ...
    pub(super) fn close_residency_rows_over_scaled_writers(
        ...
        resident_rows: &BTreeSet<u32>,
        rows: &mut Vec<u32>,
    ) -> Result<(), ShResidencyDrainError> {
        let grid = self.sample_region_grid();
        rows.clear();
        for row in changed_rows {
            if resident_rows.contains(&row) {
                rows.push(row);
            }
            if let Some(writer) = scaled_writer_row(&grid, row)?
                && resident_rows.contains(&writer)
            {
                rows.push(writer);
            }
        }
...
fn resolve_regions(
    ...
    rows.clear();
    if fog_draw_all {
        rows.extend(resident_rows.iter().copied());
    } else {
        for region in regions {
            append_region_rows(&grid, *region, resident_rows, rows)?;
        }
        rows.sort_unstable();
        rows.dedup();
    }
