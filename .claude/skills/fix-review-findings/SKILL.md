...
Spawn 1–2 agents, one at a time. Provide the agent brief, plus an enumeration of likely knock-on targets. These agents are also edit-only.

### 5. Compile-and-test gate

After all edits have landed, spawn **one** test-runner agent to compile and test on the now-warm shared cache. It runs:
...
For each compile error or test failure, dispatch a fix agent — one per file for independent failures (concurrent, edit-only), sequential for anything cross-cutting. Re-run step 5 until the gate is clean or the remaining failures need a user decision. Don't have fix agents run cargo themselves — the same shared-lock contention applies.
