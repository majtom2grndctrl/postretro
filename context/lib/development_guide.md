...
**Target shape.** Boundaries sit above `entities`, extracted as measured compile pressure justifies the lift (or a dedicated-server entry point forces the sim to be a shared library). `postretro-combat-model` holds combat domain data — stat resolution, resources, augments, damage taxonomy — off the registry, plus the shot-authority and carried-loadout types that otherwise couple sim/scripting up into netcode. The fixed-tick simulation belongs in a `postretro-sim` crate formalizing the headless `simulate` seam, with collision moving into it. `postretro-netcode` sits above `postretro-sim`: a combat/AI edit only stops recompiling netcode once netcode leaves the binary, and the scripting↔sim↔netcode cycle forces it to move with sim; it keeps sole ownership of registry replication (`networking.md`). Enemy AI is fused to the scripting VM runtime (script store, reaction dispatch) and to `sim`, so it lives inside `postretro-sim` until a scripting-VM decomposition — sink the store and reaction dispatch to `scripting-core`, trait-ify the `sim`↔AI seam — carves it into a `postretro-ai` crate. The render stack reads entity state from `entities` directly, so `sim` is a sibling to the renderer over `entities`, not a dependency — the binary orchestrates tick-then-draw. Of these, `postretro-combat-model` lands first; `postretro-sim`, `postretro-netcode`, and `postretro-ai` are not built yet.
