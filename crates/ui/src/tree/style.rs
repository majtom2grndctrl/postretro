...
pub fn lerp_rgba(from: [f32; 4], to: [f32; 4], e: f32) -> [f32; 4] {
    [
...
pub fn resolve_color(value: &ColorValue, theme: &UiTheme) -> [f32; 4] {
    match value {
...
pub fn resolve_spacing(value: &SpacingValue, theme: &UiTheme) -> f32 {
    match value {
...
pub fn resolve_font(font: &Option<String>, theme: &UiTheme) -> String {
    let primary = || {
        theme
            .font("primary")
            .unwrap_or(crate::text::UI_FONT_FAMILY)
            .to_string()
    };
    match font {
        None => primary(),
...
pub fn resolve_border(border: Option<&Border>, theme: &UiTheme) -> Option<Border> {
    border.map(|b| Border {
...
pub fn build_node_style_ranges(
...
pub fn container_base_style(gap: f32, padding: f32, align: Align) -> Style {
    Style {
