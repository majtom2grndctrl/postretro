...
pub type QuickJsInstaller = Arc<dyn for<'js> Fn(&rquickjs::Ctx<'js>) -> rquickjs::Result<()>>;
pub type LuauInstaller = Arc<dyn Fn(&mlua::Lua) -> mlua::Result<()>>;

/// A single registered primitive. Clones are cheap — the closures are
/// behind `Arc`, the metadata is plain data.
#[derive(Clone)]
pub struct ScriptPrimitive {
    pub name: &'static str,
    pub doc: &'static str,
    pub signature: PrimitiveSignature,
    pub context_scope: ContextScope,
    pub quickjs_installer: QuickJsInstaller,
    pub luau_installer: LuauInstaller,
}
...
/// add new argument shapes because the trait is `pub` and the macro
/// invocation list is the one source of truth.
pub trait RegisterablePrimitive<Args> {
...
impl PrimitiveRegistry {
    ...
    pub fn register<F, Args>(&mut self, name: &'static str, f: F) -> PrimitiveBuilder<'_, F, Args>
    where
    ...
    pub fn push_manual(&mut self, primitive: ScriptPrimitive) {
        self.entries.push(primitive);
    }

    pub fn iter(&self) -> impl Iterator<Item = &ScriptPrimitive> {
        self.entries.iter()
    }
    ...
    pub fn register_type(&mut self, name: &'static str) -> TypeBuilder<'_> {
        TypeBuilder {
            registry: self,
            name,
            doc: "",
    ...
    pub fn register_enum(&mut self, name: &'static str) -> EnumBuilder<'_> {
        EnumBuilder {
            registry: self,
            name,
            doc: "",
    ...
    pub fn register_tagged_union(&mut self, name: &'static str) -> TaggedUnionBuilder<'_> {
        TaggedUnionBuilder {
            registry: self,
            name,
            doc: "",
...
impl<'r> TypeBuilder<'r> {
    ...
    pub fn brand(mut self, underlying: &'static str) -> Self {
        debug_assert!(
    ...
    pub fn generic_brand(mut self, type_param: &'static str, underlying: &'static str) -> Self {
        debug_assert!(
            matches!(self.kind, TypeBuilderKind::Unset),
...
impl<'r> EnumBuilder<'r> {
    ...
    pub fn variant(mut self, name: &'static str, doc: &'static str) -> Self {
...
pub struct TaggedUnionBuilder<'r> {
    registry: &'r mut PrimitiveRegistry,
    name: &'static str,
    doc: &'static str,
    tag_field: &'static str,
...
impl<'r> TaggedUnionBuilder<'r> {
    ...
    pub fn tags(mut self, tag_field: &'static str, value_field: &'static str) -> Self {
...
pub struct PrimitiveBuilder<'r, F, Args>
where
    F: RegisterablePrimitive<Args>,
{
    registry: &'r mut PrimitiveRegistry,
    ...
    F: RegisterablePrimitive<Args>,
{
    ...
    pub fn param(mut self, name: &'static str, ty_name: &'static str) -> Self {
