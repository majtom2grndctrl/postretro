...
- **Lightmap — per-light, per-atlas-layer partitions.** Each static light's contribution for one atlas array layer (linear irradiance + unnormalized weighted direction + coverage + raw soft visibility, full-precision) is a separate `"lightmap_layer"` entry, keyed on that light's params, its influence-bounded geometry slice, density/sample-count, atlas layout, and target atlas layer. The raw soft-visibility payload is also the ShadowmaskAtlas raw-mask source. `LAYER_FORMAT_VERSION` covers the partition payload. The warm path folds partitions one atlas layer at a time, in global light order, normalizes once per covered texel, then dilates/encodes/appends that layer — reproducing the monolithic `bake_face_chart` output byte-for-byte before BC6H. Exact in both warm and cold builds. *Not yet built:* the payload narrows to the soft visibility alone, stored only for texels the light reaches. Irradiance and weighted direction are recomputed at fold time — each is a product of the unshadowed Lambert term and that visibility, and the term is analytic — so the entry stops carrying a second copy of what the fold can derive. Coverage becomes presence: a texel in the entry is a texel the light reaches, which is the same predicate the shadowmask uses for membership.
...
- **ShadowmaskAtlas — selected-light section (second level).** A `"shadowmask_atlas"` entry memoizes the encoded `ShadowmaskAtlas` (id 42) section. Key includes selected `EntityShadowLights` order, selected `"lightmap_layer"` fingerprints, atlas dimensions, and layer count. A no-edit rebuild hits it and skips graph construction, coloring, streaming fill, quantization, and section encoding. On a miss, graph construction reruns from geometry and never reads partition cache entries. The cached fill path then reads at most one selected-light partition at a time; it populates and caches every selected-light partition, including globally dropped lights. The uncached path streams selected lights in batches of at most four. Both paths allocate one final RGBA8 output, fill atlas layers outermost with direct serial writes, and immediately drop consumed partitions or batches. Warm-only, like the lightmap section memo. Cold `--no-cache`/`--release` recomputes through the uncached shadowmask path. *Not yet built:* the lightmap fold becomes the fill's source, so no stage reads a layer entry after the lightmap stage ends. The two memos miss independently — the section key folds every light's layer fingerprint, the shadowmask key only the selected lights' — so the lightmap stage probes the shadowmask memo before taking its own section hit, and folds anyway when the shadowmask memo misses.
