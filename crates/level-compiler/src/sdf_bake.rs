...
mod tests {
    ...
    // TEMP DIAG: phantom-interior detector ---------------------------------
    //
    // Confirms-or-refutes the hypothesis that the SDF bake mis-classifies some
    // open-AIR voxels as INSIDE solid (negative signed distance) — "phantom
    // interior". Builds geometry + BSP for content/dev/maps/campaign-test.map
    // the same way `prl-build` does, then scans the voxel grid for voxels that
    // are `point_in_solid(tree, p) == true` AND whose unsigned distance to the
    // nearest triangle is LARGE (> 1.5 * voxel_size). A genuine interior voxel
    // deep in a wall is far-from-triangles AND solid (fine); the BUG is a
    // far-from-geometry voxel in a region that should be air.
    //
    // Run with:
    //   cargo test -p postretro-level-compiler --release \
    //     temp_diag_phantom_interior_detector -- --ignored --nocapture
    //
    // This is a TEMPORARY diagnostic. Delete this whole block (and the helper
    // imports it pulls in) when the investigation is complete. It changes no
    // engine/production behavior.
    #[test]
    #[ignore = "TEMP DIAG: phantom-interior detector — run explicitly with --ignored --nocapture"]
    fn temp_diag_phantom_interior_detector() {
        use crate::map_data::ShadowType;
        use crate::map_format::MapFormat;
        use std::collections::HashSet;

        // 1. Build geometry + BSP tree exactly like the real bake (main.rs).
        ...
        // Locate the SDF light (the overhead point light over the bridge).
        let sdf_light = map_data
            ...
            .map(|l| l.origin);
        println!("\n=== TEMP DIAG: phantom-interior detector ===");
        println!("SDF light origin (engine m): {sdf_light:?}");

        // 2. Reconstruct the SAME voxel grid the bake uses.
        let cfg = SdfConfig::default();
        let voxel_size = cfg.voxel_size_m;
        let brick_size = cfg.brick_size_voxels;
        let ctx = SdfBakeCtx {
        ...
        println!(
            "geometry world AABB: min=({:.3},{:.3},{:.3}) max=({:.3},{:.3},{:.3})",
            world_min.x, world_min.y, world_min.z, world_max.x, world_max.y, world_max.z
        );
        println!(
            "grid origin=({:.3},{:.3},{:.3}) brick_dims={:?} interior_voxels={:?} voxel={voxel_size}m",
            grid_origin.x, grid_origin.y, grid_origin.z, brick_dims, interior_voxels
        );

        // The "phantom interior" threshold: solid AND far from any triangle.
        let phantom_threshold = 1.5 * voxel_size; // > 0.75 m at default 0.5 m voxels.

        // 3a. Whole-map scan over every interior voxel center.
        let mut total_voxels: u64 = 0;
        let mut inside_solid_count: u64 = 0;
        let mut phantom_count: u64 = 0;
        // Collect a handful of phantom examples (cap to keep output readable).
        let mut phantom_examples: Vec<(Vec3, f32)> = Vec::new();

        // 3b. Focused sub-region around the bridge / under the SDF light.
        // World box (engine meters). Defaults bracket the task's suggested
        // region; recentred on the SDF light when one is present.
        let (mut rmin, mut rmax) = (Vec3::new(-30.0, 0.0, -60.0), Vec3::new(-10.0, 20.0, -40.0));
        if let Some(l) = sdf_light {
            let c = Vec3::new(l.x as f32, l.y as f32, l.z as f32);
            rmin = Vec3::new(c.x - 10.0, 0.0, c.z - 10.0);
            rmax = Vec3::new(c.x + 10.0, 20.0, c.z + 10.0);
        }
        let mut region_voxels: u64 = 0;
        let mut region_inside: u64 = 0;
        let mut region_phantom: u64 = 0;
        let mut region_phantom_examples: Vec<(Vec3, f32)> = Vec::new();
        // Bucket whole-map phantoms by how far outside the geometry AABB they
        // sit, to tell "edge/exterior artifact" from "playable open air".
        let mut phantom_inside_aabb: u64 = 0;
        let mut phantom_outside_aabb: u64 = 0;

        for wz in 0..interior_voxels[2] {
            for wy in 0..interior_voxels[1] {
                for wx in 0..interior_voxels[0] {
                    let p = voxel_center(grid_origin, wx, wy, wz, voxel_size);
                    let inside = point_in_solid(ctx.tree, p);
                    total_voxels += 1;
                    if !inside {
                        continue;
                    }
                    inside_solid_count += 1;
                    let unsigned = nearest_triangle_distance(&triangles, p);
                    let is_phantom = unsigned > phantom_threshold;
                    if is_phantom {
                        phantom_count += 1;
                        if phantom_examples.len() < 16 {
                            phantom_examples.push((p, unsigned));
                        }
                        // Inside vs outside the true geometry AABB (a phantom
                        // outside the geometry envelope is an exterior/padding
                        // artifact, not playable-air mis-classification).
                        let inside_aabb = (p.x as f64) >= world_min.x
                            && (p.x as f64) <= world_max.x
                            && (p.y as f64) >= world_min.y
                            && (p.y as f64) <= world_max.y
                            && (p.z as f64) >= world_min.z
                            && (p.z as f64) <= world_max.z;
                        if inside_aabb {
                            phantom_inside_aabb += 1;
                        } else {
                            phantom_outside_aabb += 1;
                        }
                    }
                    let in_region = p.x >= rmin.x
                        && p.x <= rmax.x
                        && p.y >= rmin.y
                        && p.y <= rmax.y
                        && p.z >= rmin.z
                    ...
                    if in_region {
                        ...
                        if is_phantom {
                            region_phantom += 1;
                            if region_phantom_examples.len() < 24 {
                                region_phantom_examples.push((p, unsigned));
                            }
                        }
                    }
                }
        ...
        // Count region voxels separately (cheap, no distance eval).
        for wz in 0..interior_voxels[2] {
            for wy in 0..interior_voxels[1] {
                for wx in 0..interior_voxels[0] {
                    let p = voxel_center(grid_origin, wx, wy, wz, voxel_size);
        ...
        println!("\n--- WHOLE-MAP STATS ---");
        println!("total interior voxels:     {total_voxels}");
        println!("inside_solid voxels:       {inside_solid_count}");
        println!(
            "phantom-interior voxels:   {phantom_count}  (inside AND unsigned > {phantom_threshold} m)"
        );
        println!(
            "\n--- FOCUSED REGION around bridge/SDF light (x∈[{:.1},{:.1}] y∈[{:.1},{:.1}] z∈[{:.1},{:.1}]) ---",
            rmin.x, rmax.x, rmin.y, rmax.y, rmin.z, rmax.z
        );
        println!("region voxels:             {region_voxels}");
        println!("region inside_solid:       {region_inside}");
        println!("region phantom-interior:   {region_phantom}");
        println!("\nwhole-map phantoms inside geometry AABB:  {phantom_inside_aabb}");
        println!(
            "whole-map phantoms outside geometry AABB: {phantom_outside_aabb}  (edge/padding artifacts)"
        );

        println!("\n--- REGION PHANTOM EXAMPLES (world coord, unsigned) ---");
        if region_phantom_examples.is_empty() {
            println!("(none)");
        } else {
            for (p, d) in &region_phantom_examples {
                println!(
                    "  ({:.3}, {:.3}, {:.3})  unsigned={:.3} m",
                    p.x, p.y, p.z, d
                );
        ...
        println!("\n--- PHANTOM EXAMPLES (world coord, unsigned dist to nearest tri) ---");
        if phantom_examples.is_empty() {
            println!("(none)");
        } else {
            for (p, d) in &phantom_examples {
                println!(
                    "  ({:.3}, {:.3}, {:.3})  unsigned={:.3} m",
                    p.x, p.y, p.z, d
                );
        ...
        // 4. Vertical column above a bridge-top point, to SEE whether air above
        // the bridge is flagged solid. Pick (x,z) under the SDF light (or the
        // region center) and walk a column upward, finding the bridge top by
        // the highest solid voxel below ~half height, then climbing ~10 m.
        let col_xz = sdf_light
            .map(|l| (l.x as f32, l.z as f32))
            .unwrap_or(((rmin.x + rmax.x) * 0.5, (rmin.z + rmax.z) * 0.5));
        println!(
            "\n--- VERTICAL COLUMN at (x={:.3}, z={:.3}) — (y, inside, unsigned) bottom→top ---",
            col_xz.0, col_xz.1
        );
        // Map the chosen world x/z to the nearest voxel index, then scan all y.
        let col_vx = (((col_xz.0 - grid_origin.x as f32) / voxel_size).floor() as i64)
            .clamp(0, interior_voxels[0] as i64 - 1) as u32;
        let col_vz = (((col_xz.1 - grid_origin.z as f32) / voxel_size).floor() as i64)
            .clamp(0, interior_voxels[2] as i64 - 1) as u32;
        // Find the highest solid voxel in the column (the bridge top, if the
        // column passes through the slab), then print from there up ~10 m.
        let mut highest_solid_y: Option<u32> = None;
        for wy in 0..interior_voxels[1] {
            let p = voxel_center(grid_origin, col_vx, wy, col_vz, voxel_size);
            if point_in_solid(ctx.tree, p) {
                highest_solid_y = Some(wy);
            }
        }
        let start_y = highest_solid_y.unwrap_or(0).saturating_sub(1);
        let span_voxels = (10.0 / voxel_size).ceil() as u32 + 3;
        let end_y = (start_y + span_voxels).min(interior_voxels[1].saturating_sub(1));
        match highest_solid_y {
            Some(hy) => {
                let yp = voxel_center(grid_origin, col_vx, hy, col_vz, voxel_size);
                println!(
                    "highest solid voxel in column at y={:.3} (vy={hy}); printing column up ~10 m:",
                    yp.y
                );
            }
            None => println!("no solid voxel in this column; printing low portion of column:"),
        }
        for wy in start_y..=end_y {
            ...
            let unsigned = nearest_triangle_distance(&triangles, p);
            println!(
                "  y={:7.3}  inside={:5}  unsigned={:.3} m{}",
                p.y,
                inside,
                unsigned,
                if inside && unsigned > phantom_threshold {
                    "   <-- PHANTOM"
                } else {
                    ""
                }
    ...
    // END TEMP DIAG: phantom-interior detector ------------------------------

    // TEMP DIAG: floor→light column probe -----------------------------------
    //
    // The floor directly under the bridge reads WHITE in the SDF shadow debug
    // viz (the floor→light trace finds NO occluder), yet the bridge slab sits
    // between that floor and the light and should shadow it. Sub-voxel re-bakes
    // (0.25 m) did NOT bring the shadow back, which refutes "resolution too
    // coarse" and points instead at the brick CLASSIFICATION: if the slab's
    // bricks are classified EMPTY, the runtime never samples the fine field
    // there — it steps by the per-brick `coarse_distances` clearance, which in
    // an EMPTY brick reports "far open", so the ray sails through the slab.
    //
    // This probe bakes the SAME atlas the runtime samples, auto-detects a
    // shadowed pit-floor column under the bridge (a column with floor at the
    // bottom, an air gap, then a slab above), then marches the floor→light ray
    // and dumps, per step: the containing brick's top-level CLASS
    // (EMPTY / INTERIOR / SURFACE-slot) and its `coarse_distances` clearance,
    // alongside ground truth (`point_in_solid`, unsigned triangle distance). It
    // flags the brick class at the SLAB ENTRY (where ground-truth solidity
    // first flips true) — the decisive line:
    //   * SLAB-ENTRY brick is EMPTY  → BUG CONFIRMED: slab invisible to the
    //     field; the conservative-voxelization fix targets exactly this.
    //   * SLAB-ENTRY brick is SURFACE/INTERIOR → slab IS represented; the leak
    //     is downstream (light selection, max_t clamp, or the ray geometrically
    //     threads a gap so WHITE may be correct for that pixel).
    // A fine (0.05 m) sub-voxel `point_in_solid` scan across the crossing
    // reports the slab thickness the ray actually intersects.
    //
    // Run with:
    //   cargo test -p postretro-level-compiler --release \
    //     temp_diag_floor_light_column_probe -- --ignored --nocapture
    //
    // TEMPORARY diagnostic — delete this whole block when the investigation is
    // complete. Changes no engine/production behavior.
    #[test]
    #[ignore = "TEMP DIAG: floor→light column probe — run explicitly with --ignored --nocapture"]
    fn temp_diag_floor_light_column_probe() {
        use crate::map_data::ShadowType;
        use crate::map_format::MapFormat;
        use std::collections::HashSet;

        // 1. Build geometry + BSP and bake the SAME atlas the runtime samples.
        let map_path = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .parent()
            .and_then(|p| p.parent())
            .expect("workspace root")
        ...
        let voxel_size = section.voxel_size_m;
        ...
        println!("\n=== TEMP DIAG: floor→light column probe ===");
        println!(
            "atlas: voxel={voxel_size}m brick={brick_size}vox ({brick_world}m) \
             brick_dims={dims:?} surface_bricks={}",
            section.surface_brick_count
        );

        // Decode the top-level class + coarse clearance of the brick that
        // contains world point `p`. Returns (brick_index_xyz, class, coarse_m).
        // "OOB" = outside the brick grid (e.g. above the grid near the light).
        let class_at = |p: Vec3| -> (Option<[u32; 3]>, &'static str, f32) {
            let bxf = ((p.x - origin.x) / brick_world).floor();
            let byf = ((p.y - origin.y) / brick_world).floor();
            let bzf = ((p.z - origin.z) / brick_world).floor();
            if bxf < 0.0
                || byf < 0.0
                || bzf < 0.0
                || bxf >= dims[0] as f32
            ...
            let class = if slot == BRICK_SLOT_EMPTY {
                "EMPTY"
            } else if slot == BRICK_SLOT_INTERIOR {
                "INTERIOR"
            } else {
                "SURFACE"
            };
            (Some([bx, by, bz]), class, coarse)
        };

        // 2. Locate the SDF light (the off-axis overhead point light).
        let l = map_data
            .lights
            .iter()
            .find(|l| l.shadow_type == ShadowType::Sdf)
        ...
        let light = Vec3::new(l.x as f32, l.y as f32, l.z as f32);
        println!(
            "SDF light origin (engine m): ({:.3}, {:.3}, {:.3})",
            light.x, light.y, light.z
        );

        // 3. Auto-detect shadowed pit-floor columns under the bridge. Scan a
        // window of (x,z) around the light; in each column walk the y voxel
        // range to find solid runs. A "shadowed floor" column = lowest solid
        // run (pit floor) + air gap (>= 2 voxels) + a higher solid run (slab).
        // The receiver F is the air voxel just above the pit-floor top.
        let col_xz_to_vidx = |x: f32, z: f32| -> (u32, u32) {
            let vx = (((x - origin.x) / voxel_size).floor() as i64)
                .clamp(0, interior_voxels[0] as i64 - 1) as u32;
            let vz = (((z - origin.z) / voxel_size).floor() as i64)
                .clamp(0, interior_voxels[2] as i64 - 1) as u32;
            (vx, vz)
        };
        // Returns Some((floor_top_y_world, slab_bottom_y_world)) if the column
        // matches the floor+gap+slab pattern.
        let detect_column = |vx: u32, vz: u32| -> Option<(f32, f32)> {
            // Build the solidity profile up the column.
            let mut solid = vec![false; interior_voxels[1] as usize];
            for (vy, s) in solid.iter_mut().enumerate() {
                let p = voxel_center(origin.as_dvec3(), vx, vy as u32, vz, voxel_size);
                *s = point_in_solid(tree, p);
            }
            // Lowest solid voxel = pit floor bottom.
            let floor_bottom = solid.iter().position(|&s| s)?;
            // Top of the contiguous floor run.
            let mut floor_top = floor_bottom;
            while floor_top + 1 < solid.len() && solid[floor_top + 1] {
                floor_top += 1;
            }
            // First solid above the floor run, with an air gap >= 2 voxels.
            let gap_start = floor_top + 1;
            let slab_bottom = (gap_start..solid.len()).find(|&vy| solid[vy])?;
            if slab_bottom < gap_start + 2 {
                return None; // no real air gap → not a shadowed floor column
            }
            let floor_top_world =
                voxel_center(origin.as_dvec3(), vx, floor_top as u32, vz, voxel_size).y;
            let slab_bottom_world =
                voxel_center(origin.as_dvec3(), vx, slab_bottom as u32, vz, voxel_size).y;
            Some((floor_top_world, slab_bottom_world))
        };

        // Scan a 24 m x 24 m window around the light at ~1 m spacing, collect
        // matching columns nearest the light first.
        let mut candidates: Vec<(f32, f32, f32, f32)> = Vec::new(); // (x, z, floor_top_y, slab_bottom_y)
        let half = 12.0_f32;
        let mut sx = light.x - half;
        while sx <= light.x + half {
            let mut sz = light.z - half;
            while sz <= light.z + half {
                let (vx, vz) = col_xz_to_vidx(sx, sz);
                if let Some((ft, sb)) = detect_column(vx, vz) {
                    candidates.push((sx, sz, ft, sb));
                }
                sz += 1.0;
            }
            sx += 1.0;
        ...
        candidates.sort_by(|a, b| {
            let da = (a.0 - light.x).hypot(a.1 - light.z);
            let db = (b.0 - light.x).hypot(b.1 - light.z);
            da.partial_cmp(&db).unwrap()
        });
        println!(
            "\nshadowed pit-floor columns found in ±{half} m window: {}",
            candidates.len()
        );
        if candidates.is_empty() {
            println!(
                "(none — no floor+gap+slab column near the light; widen the window \
                 or check the light placement)"
            );
            println!("=== END TEMP DIAG ===\n");
            return;
        }

        // 4. Probe the floor→light ray for the nearest few shadowed columns.
        let step = voxel_size * 0.5;
        for (ci, &(cx, cz, floor_top_y, slab_bottom_y)) in candidates.iter().take(3).enumerate() {
            // Receiver F: just above the pit-floor top (a half-voxel of air).
            let f = Vec3::new(cx, floor_top_y + voxel_size * 0.5, cz);
            let to_light = light - f;
            let max_t = to_light.length();
            let dir = to_light / max_t;
            println!(
                "\n----- COLUMN {ci}: F=({:.3},{:.3},{:.3}) → light, \
                 floor_top_y={:.3} slab_bottom_y={:.3} ray_len={:.3}m -----",
                f.x, f.y, f.z, floor_top_y, slab_bottom_y, max_t
            );
            println!(
                "   {:>7}  {:>22}  {:>9}  {:>8}  {:>6}  {:>9}",
                "t(m)", "world(x,y,z)", "class", "coarse", "solid", "unsigned"
            );

            let mut prev_inside = point_in_solid(tree, f);
            let mut slab_entry_t: Option<f32> = None;
            let mut t = 0.0_f32;
            while t <= max_t + 1.0e-3 {
                let p = f + dir * t;
                let inside = point_in_solid(tree, p);
                let unsigned = nearest_triangle_distance(&triangles, p);
                let (brick, class, coarse) = class_at(p);
                let brick_s = brick
                    .map(|b| format!("[{},{},{}]", b[0], b[1], b[2]))
                    .unwrap_or_else(|| "----".to_string());
                let entry = if inside && !prev_inside {
                    if slab_entry_t.is_none() {
                        slab_entry_t = Some(t);
                    }
                    "  <== SOLID ENTRY"
                } else {
                ...
                println!(
                    "   {:>7.3}  ({:>6.2},{:>6.2},{:>6.2})  {:>9}  {:>8.3}  {:>6}  {:>9.3}  {brick_s}{entry}",
                    t, p.x, p.y, p.z, class, coarse, inside, unsigned
                );
                prev_inside = inside;
                t += step;
            }

            // Decisive read-out: the brick class at the slab entry.
            match slab_entry_t {
                Some(et) => {
                    let pe = f + dir * et;
                    let (brick, class, coarse) = class_at(pe);
                    println!(
                        "  >> SLAB ENTRY at t={et:.3}m, brick={brick:?}, class={class}, coarse={coarse:.3}m"
                    );
                    println!(
                        "  >> VERDICT: {}",
                        match class {
                            "EMPTY" =>
                                "EMPTY brick at the slab — BUG CONFIRMED (slab invisible to field).",
                            "OOB" => "slab entry OUT OF GRID — investigate grid extents.",
                            _ =>
                                "slab IS a SURFACE/INTERIOR brick — leak is downstream, not classification.",
                        }
                    );

                    // Fine 0.05 m sub-voxel point_in_solid scan across the entry
                    // to measure the actual slab thickness the ray intersects.
                    println!("  -- fine sub-voxel solidity scan across the slab crossing --");
                    let fine_step = 0.05_f32;
                    let scan_lo = (et - voxel_size).max(0.0);
                    let scan_hi = (et + voxel_size * 2.0).min(max_t);
                    let mut ft = scan_lo;
                    let mut solid_span = 0.0_f32;
                    while ft <= scan_hi {
                        let p = f + dir * ft;
                        if point_in_solid(tree, p) {
                            solid_span += fine_step;
                        }
                        ft += fine_step;
                    }
                    println!(
                        "  -- solid path-length within [{scan_lo:.3},{scan_hi:.3}]m of entry: \
                         {solid_span:.3}m (slab too thin if << brick edge {brick_world}m)"
                    );
                }
                None => {
                    println!(
                        "  >> NO SOLID ENTRY along the floor→light ray — the ray never crosses \
                         the slab geometrically (WHITE may be correct for this receiver)."
                    );
                }
            }
        }
    ...
    // END TEMP DIAG: floor→light column probe -------------------------------
