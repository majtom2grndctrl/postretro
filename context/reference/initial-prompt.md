...
**Target BSP format:** BSP2 (`qbsp -bsp2`). BSP2 removes BSP29's geometry limits (65K faces, 32K clipnodes, +/-32K coordinate range) with no downside for a custom engine. The qbsp Rust crate auto-detects BSP format and abstracts format differences — BSP29 and BSP2 files are parsed transparently. BSP2 is the compilation target (set via `qbsp -bsp2`).
...
- Baked lighting from ericw-tools, consumed via BSPX lumps and lightmap data:
  ...
  - **Directional lightmaps** (`LIGHTINGDIR` BSPX lump, also written by `light -bspx`) storing dominant light direction per texel, enabling an approximate per-pixel specular term (single dominant-direction Blinn-Phong or similar — not physically correct multi-source specular, but convincing for retro surfaces like wet streets, metal, chrome). Cheap and fully baked. The qbsp crate does not parse this natively — access via `get_unparsed("LIGHTINGDIR")`.
  ...
  - **Volumetric light probes** (`LIGHTGRID_OCTREE` BSPX lump via `light -lightgrid`) for consistently lighting dynamic objects — billboard sprites, particles, the player's weapon — so they don't look flat against the baked world. The qbsp crate parses this natively into a typed `LightGridOctree` struct (`bspx.light_grid_octree`). Note: `-lightgrid` was developed primarily for Quake 2 and is marked experimental. Verify it produces usable output for Q1 BSP2 maps early in development. If it does not, the fallback for sprite/particle lighting is to sample the nearest lightmap face or use a simple ambient + nearest-light approximation. The `rendering_pipeline.md` spec should define both paths.
...
- Custom vertex format: position, base texture UV, lightmap UV, vertex color (for per-vertex tints — dynamic lighting contribution, editor-painted color, or debug visualization; defined as a wgpu `VertexBufferLayout`)
...
3. **ericw-tools** reads PNGs for dimensions only. qbsp may auto-add the map file's parent directory as a search path — verify during build pipeline setup. If it does, and `textures/` is alongside the `.map` file, no extra flags are needed: `qbsp -bsp2 -notex map.map`. If not, use `-path <dir>` to add search paths explicitly. The BSP stores texture headers (name + dimensions) with no pixel data.
...
- The build pipeline runs three tools in sequence: `qbsp -bsp2 -notex` (compile geometry) -> `vis` (compute visibility) -> `light -bspx -lightgrid` (calculate lighting). AO is enabled via worldspawn key `_dirt 1` (or equivalently as `-dirt 1` on the command line, since all worldspawn keys can be passed as command-line overrides) — bakes directly into lightmap data, no separate lump.
...
1. **`rendering_pipeline.md`** — The rendering pipeline in detail: what happens each frame (in order), how each BSPX lump is consumed (directional lightmaps -> specular, lightgrid -> sprite/particle lighting), BSP visibility (PVS-based leaf culling using vis data and how it determines per-frame draw sets), BSP loading pipeline (how qbsp data and BSPX lumps map to wgpu buffers and engine data structures), the custom vertex format. Note that AO is baked into lightmap data (not a separate lump), `LIGHTINGDIR` requires custom BSPX parsing via `get_unparsed()`, and `LIGHTGRID_OCTREE` is natively parsed (use the typed `LightGridOctree` API, not raw bytes). Specify degradation behavior for each optional BSPX lump — what the renderer does when the lump is absent (e.g., missing `LIGHTINGDIR` degrades to diffuse-only, missing `LIGHTGRID_OCTREE` falls back to nearest-lightmap sampling or ambient + nearest-light). Carry the `-lightgrid` experimental caveat into this doc.
...
5. **`build_pipeline.md`** — The build pipeline: TrenchBroom -> ericw-tools -> .bsp -> game loads it, specific ericw-tools flags (including the `-lightgrid` experimental caveat and early-verification note), the FGD file design for custom entities (`env_fog_volume`, `env_cubemap`, `env_reverb_zone`), surface material derivation scheme, the custom TrenchBroom game configuration
...
Additionally, produce plan files in `context/plans/`. This directory uses a lifecycle convention:
- **`ready/`** — fully specced and unblocked; an agent can pick this up and execute it.
- **`drafts/`** — conceptually scoped but not yet detailed enough to execute. Needs refinement before promotion to `ready/`.
- **`done/`** — completed plans, kept for reference.
- **`in-progress/`** — plans currently being executed.

The plan files:

7. **`context/plans/drafts/grounded-movement.md`** — Task for player controller with BSP collision, gravity, and ground detection. Note that the qbsp crate natively parses the `BRUSHLIST` BSPX lump, which provides convex brush hulls for collision detection.
8. **`context/plans/drafts/entity-types.md`** — Task to define and implement specific entity types (enemies, doors, pickups, triggers, projectiles).
9. **`context/plans/drafts/cubemap-bake-tool.md`** — Task to build the tool that renders cubemaps from `env_cubemap` positions.

Also update `context/lib/index.md` to reflect any changes to the agent router or architectural decisions. Remove the "Docs that don't exist yet are planned" caveat once all listed docs are produced. Update the `src/main.rs` file header to point at `rendering_pipeline.md` once it exists.
...
The roadmap is a medium-lived planning document: reviewed and updated at the start of each phase, deleted when all phases are complete. Phases are checked off or removed as work progresses, but the document persists across many tasks. It should note this lifecycle at the top.
