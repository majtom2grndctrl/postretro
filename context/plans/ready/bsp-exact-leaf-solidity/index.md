...
- [ ] Two wedge brushes assembled into a cuboid emit faces covering exactly the six outer cuboid planes and nothing on the diagonal plane (coplanar fragments from the two brushes may tile one plane — the metric is plane coverage, not polygon count).
...
- [ ] Every leaf's `bounds` remains finite with `min ≤ max` and contains all of that leaf's emitted face vertices within the face-clip epsilon (face vertices carry `SPLIT_EPSILON`-scale clipping noise from a different code path than the polytope; strict containment would false-fail a correct implementation).
- [ ] `cargo run -p postretro-level-compiler -- content/dev/maps/campaign-test.map -o <tmp>.prl` succeeds end-to-end (manual acceptance check; not automated by a task).
