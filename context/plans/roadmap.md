...
- [ ] **Enemy steering feel** — acceleration, deceleration, bounded turn rate, and arrival easing so agents read as moving bodies, not waypoint snaps. `context/plans/drafts/M10--enemy-steering-feel/`
- [ ] **Enemy stuck recovery** — detect route-valid physical wedges and apply deterministic tangent recovery plus budgeted replans. `context/plans/drafts/M10--enemy-stuck-recovery/`
- [ ] **Enemy locomotion animation** — choose idle vs. walk from actual agent ground speed while preserving attack/death state selection. `context/plans/drafts/M10--enemy-locomotion-animation/`
- [ ] **Enemy combat positioning** — select stable reachable combat destinations around the player instead of chasing the player's center point. `context/plans/drafts/M10--enemy-combat-positioning/`
...
- [ ] **Phase 3.5 — State replication (server-authoritative state-store slots → clients).** The generic, **schema-driven** path that feeds a client's engine-owned slot table from server authority, so the UI subscribes by slot name unchanged — the M13 decoupling seam, now across the wire. Today's gap it closes: connected clients show `player.health` as `--` because Phase 3 replicated *movement* (`Transform`/`PlayerMovementState`/authority), not stats, and the slot feeder is host/single-player-only. The wire carries slot **values keyed by the schema the scripting layer already declares** (`defineStore`), never bespoke per-stat fields — so shields / elemental shields / ammo / none and any future modder-defined stat replicate with **zero netcode change per stat**. Reuses the Mod State Store's `persist` serialization + branded `StateValue<T>` + clamp/validate and its engine-owned-readonly vs. modder-declared split (only server-authoritative slots go on the wire; client-local UI slots stay off it), and Phase 2's delta/baseline/ack + join-in-progress baseline machinery. Two scopes: **owner-private** slots (HP/ammo/shields → the owning client) and **shared/global** slots (objective / set-piece progress → all clients, late joiners via the baseline). Foundational to Phase 4 and Phase 5 (below), so it sequences before them — Phase 4's design/playtest work may overlap, but its progress-replication and Phase 5's confirmed HP both consume this tier. Connects forward to the Shields + damage-type system (Future / Gameplay) and the M14 behavior-IR recharge policy: the engine owns the per-tick recharge (server-authoritative), the modder owns the policy IR, and this tier carries the resulting value to the client. Detail-on-open; spec drafted against `context/research/netcode/` plus the mod-state-store / persist docs.
...
- [ ] **Phase 7 — Scale + dedicated-server readiness.** Validate 16 players within a host-upstream bandwidth budget (priority-accumulator; interest management via portal/PVS if needed); prove the headless server entry point (dedicated-server split). This phase's loaded/jittery, many-remote conditions are also the home for the two deferred Phase 3 remote-interpolation refinements below — per-entity delay in particular scales in severity with player count, and both are invisible on clean LAN co-op. Fold them in here as one tightly-scoped follow-up spec, **unless** a Phase 5/6 combat-under-latency playtest surfaces either sooner (a stuttering/briefly-frozen remote *enemy* reads worse than a teammate) and justifies pulling it forward:
