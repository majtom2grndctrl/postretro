...
- **`ui.createLocalState()` end-to-end (id-addressed app-side presentation cells).** The author-driven write path cannot ride the TW `NodeContext` mechanism (tweens are render-derived, never reaction-written; reactions run app-side at the game-logic stage while the renderer is written one-way via the snapshot, and positional `NodeId`s are render-built). So `localState` is a small app-side presentation-state subsystem:
  ...
  - **(c) Cell-write reaction.** A new `SystemReactionCommand` arm (sibling of `SetState`) drained at the game-logic stage into the app-side cell store; `ui.createLocalState()`'s handle `.set(v)` emits it. Distinct from `setState` (which writes the store).
  ...
  - **(e) SDK function.** `ui.createLocalState(init)` in `sdk/lib/ui/state.{ts,luau}` — an SDK-lib function like `setState` (hand-authored, mirrored into the static SDK block, drift-checked), **not** a registered primitive and **not** auto-emitted; exposed under a new `ui` namespace object (state helpers are namespaced; reactions stay bare exports). Returns a **distinct presentation handle** (`.set()` = cell-write, `.get()` = local bind ref). `State`-named (stored across frames) per `scripting.md` §11.
...
- [ ] UI registrations drain from the manifest **before** the existing mod-init / data-script context drop (no new lifecycle stage); a frame then renders the registered UI with no VM resident (the no-VM-resident half is the standing architecture invariant — a structural/review gate, not a new runtime signal).
- [ ] A modder component calling `ui.createLocalState()` renders, and a descendant widget bound `{ local: "name" }` displays the cell value. The cell value persists across frames at a stable value on a settled frame (the gameplay layer's recompute counter does not increment — asserted via the `#[cfg(test)]` `UiTree::recompute_count()` accessor — because the live value rides the snapshot, not the compared descriptor). The cell value persists across a structurally-identical retained-diff reuse keyed by its scope id; it is discarded when its declaring scope id is no longer present.
...
- [ ] The static SDK block declares `ui.createLocalState` (an SDK-lib function, drift-checked — not auto-emitted from the registry) and the generated typedefs include the manifest UI fields; the parity guard passes; `gen-script-types` reports no drift (regeneration incremental on G1a's committed output).

## Tasks

### Task 1: Manifest UI fields + dual-path bridge wiring
Add UI-tree/theme/font fields to `ModManifestResult` + the `ModManifest` typedef + parity guard (`primitives/mod.rs`) and a UI-tree field to `LevelManifest` (`data_descriptors.rs`). The four mod parsers all build `ModManifestResult`; the two level parsers build `LevelManifest` — the new fields live on those two structs, and the `ModManifest` registered typedef + parity guard must move in lockstep with `ModManifestResult` or the guard test fails. Drain via G1a's bridge fns in all four mod parsers and the two level parsers. `Depends on` G1a.

### Task 2: Tiered registry + always-on compose
Add the engine/mod tier + shadow warning to `UiTreeRegistry`; extend the `main.rs` compose step so always-on registered trees (tiered resolution) compose as base layers beneath pushed modal entries in the single composition. **Capture/focus invariant:** base/always-on layers are appended to `UiReadSnapshot.trees` but are NOT entered into the pushed modal stack, which is the sole source for `top_capture_mode`/`active_name`/`active_text_entry_target` (`modal_stack.rs`) — so a base layer never captures input or takes focus regardless of its declared `captureMode`. Level tier deferred. `Depends on` nothing (engine-side; concurrent-safe with Task 1).
...
Add the production caller: deserialize manifest `theme` → `ThemeDescriptor`, merge over `engine_default` → `set_ui_theme(merged)` (relying on its `theme_generation` bump). Add `Renderer::register_ui_font(family, ttf_bytes)` delegating to the glyphon `FontSystem` (`text.rs` registers compile-time `include_bytes!` faces today — this is the net-new runtime path), plus the runtime TTF asset read following the `ui.md` §5 asset-path rule (cwd-relative at runtime; `CARGO_MANIFEST_DIR` only under `#[cfg(test)]` — never baked into the production loader), and drain font assets. `Depends on` Task 1.

### Task 5: `ui.createLocalState()` end-to-end
(a) Additive `localState` declaration on `ContainerWidget` (a stable scope id + named cells + initials), `#[serde(default, skip_serializing_if)]` like the other optional fields so round-trip identity holds. (b) Extend the bind shape so `TextBind`/`PanelBind`/`SliderBind` accept a `{ local: "name" }` alternative beside `{ slot }` — a descriptor-model change, not just a new field. (c) An app-side `(scopeId, cellName)` presentation-cell store, seeded from declared initials and reconciled against the composed-scope-id set gathered at the `main.rs` compose step (Task 2) — cleared when a scope id leaves the composed set. (d) A `SystemReactionCommand::CellWrite` arm (sibling of `SetState` in `system_commands.rs`) drained in `App::dispatch_system_commands` (`main.rs`) into that store — **not** the slot table; `reaction_dispatch.rs` is uninvolved (it handles named/entity reactions). (e) Resolved cell values published on an additive `UiReadSnapshot` field; the renderer resolves `{ local }` binds against it in `build_draw_data_retained`. (f) The `ui.createLocalState()` SDK function — an **SDK-lib function like `setState`** (hand-authored in `sdk/lib/ui/state.{ts,luau}`, mirrored into the static SDK block, drift-checked), **not** a registered primitive and **not** auto-emitted; exposed under a new `ui` namespace object (per G1a's locked "state helpers are namespaced" decision — reactions stay bare exports). Its handle is distinct from G1a's store handle: `.set()` emits the `CellWrite` reaction, never `setState`. Never writes the authoritative store. `Depends on` G1a (handle ergonomics shape) and Task 2 (the snapshot/compose path it extends).
...
**Key files:** `scripting/runtime.rs`, `scripting/staged_manifest.rs`, `scripting/data_descriptors.rs`, `scripting/primitives/mod.rs` (parity guard), `scripting/reactions/system_commands.rs` (+ the `CellWrite` drain in `App::dispatch_system_commands`, `main.rs`), `scripting/typedef.rs`, `render/ui/modal_stack.rs` (tiers), `render/ui/descriptor.rs` (additive `localState` on `ContainerWidget` + `{local}` bind alternative on `TextBind`/`PanelBind`/`SliderBind`), `render/ui/tree.rs` (`{local}` resolution), `render/ui/mod.rs` (snapshot cell field), `render/ui/text.rs` (`register_ui_font` → `FontSystem`), `render/mod.rs` (theme/font install, compose), `main.rs` (compose step, drains, dispatch), `sdk/lib/ui/state.{ts,luau}`, `sdk/lib/index.ts`.
...
| local cell decl | additive `ContainerWidget` field (scope id + cells + init) | `"localState"` | `ui.createLocalState(init)` | `ui.createLocalState(init)` |
...
- **`localState` uses a distinct presentation handle + `CellWrite` reaction, never `setState`.** Presentation-only is the decoupling payoff (`ui.md` §6); G1a's store handle writes the store. `State`-named because stored across frames (§11) — the `liveValue()` → `ui.createLocalState()` rename M14 reserved. `ui.createLocalState()` is an SDK-lib function (like `setState`), **not** a registered primitive, exposed under a new `ui` namespace object (state helpers are namespaced; reactions stay bare exports). It follows the `create*` convention (inline-constructed, not engine-registered — distinct from `define*`; see G1a). The `{ local: "name" }` bind alternative on `TextBind`/`PanelBind`/`SliderBind` is a descriptor-model addition, not just a new field.
