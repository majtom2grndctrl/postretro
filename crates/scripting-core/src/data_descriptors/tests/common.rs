...
pub fn eval_js<F, R>(src: &str, f: F) -> R
where
...
pub fn install_ui_theme_token_validator(lua: &mlua::Lua) {
...
pub fn eval_lua<F, R>(src: &str, f: F) -> R
where
    F: FnOnce(LuaValue) -> R,
{
...
pub const JS_PLAYER_MOVEMENT: &str = r#"({
...
pub const UI_ALL_KINDS_JS: &str = r#"({
    anchor: "center",
    offset: [10.0, -20.0],
    root: {
