...
|---|---|---|---|
| On-wire/emitted | Finalized pack view will write the section | PRL container contains the section | Resource row is required for emitted/present sections, including valid-empty payloads; absent/withheld sections require no row. A valid-empty sparse companion over a nonzero id34 grid still has normal affinity dimensions. |
| Parsed-valid | Finalized section data decodes under the production codec | Section decoded and passed old validation | Participates in directory dimensions, CSR/dependency checks, ownership, descriptor-index checks, and empty-row range checks even when later runtime exposure is `None`. |
...
| Partial scale-0 edge brick; scale-1/2/3 node spans clusters | Scale-0 clipped indices in bounds; scale>0 full aligned cube is inside the grid; section-49 half-interval support only for initial cell contact; derived origin/member support; stable owner/halo | T2/T4/T6; AC3/4 |
| Closure locality regression | A 20×4×4 grid with one scale0 seed remains one brick; a scale>0 seed adds exactly its own constituent cube and never whole-grid coverage without actual adaptive-node membership | T2/T4/T7; AC3/4/9 |
| Valid base plus valid-empty id45/id48 | Zero CSR entries; normal nonzero affinity dimensions; sparse resource ranges mirror the final common covered brick set including empty rows; range count zero only when common covered set is empty | T2/T4/T6/T7; AC3/5/6 |
