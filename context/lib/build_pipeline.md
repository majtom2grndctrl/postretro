...
| ShadowmaskAtlas | 42 | When usable `EntityShadowLights` are emitted; per-selected-light baked world-visibility masks, one mask slot per selected light, with `0xFF` entries for globally dropped masks. Raw RGBA today; BC5 `.rg` with two groups side by side per layer is decided, not yet built |
