...
fn emit_ts_type(ty: &RegisteredType, out: &mut String) {
    ...
    match &ty.shape {
        TypeShape::Brand { underlying: _ } => {
            writeln!(
                out,
                "{TS_INDENT}export type {name} = number & {{ readonly __brand: \"{name}\" }};",
                name = ty.name,
            )
            .unwrap();
        }
        TypeShape::Struct { fields } => {
            ...
            if !any_doc {
                ...
                let inner_indent = "    ";
                for f in fields {
                    ts_doc_block(f.doc, inner_indent, out);
                    writeln!(out, "{inner_indent}{}: {};", f.name, rust_to_ts(f.ty_name)).unwrap();
                }
        ...
        TypeShape::StringEnum { variants } => {
            ...
            if !any_doc {
                ...
                for (i, v) in variants.iter().enumerate() {
                    ts_doc_block(v.doc, inner_indent, out);
                    let suffix = if i == last { ";" } else { "" };
                    writeln!(out, "{inner_indent}| \"{}\"{suffix}", v.name).unwrap();
                }
            }
        }
        TypeShape::TaggedUnion {
            ...
            if !any_doc {
                ...
                for (i, v) in variants.iter().enumerate() {
                    ...
                    writeln!(
                        out,
                        "{inner_indent}| {{ {tag_field}: \"{}\"; {value_field}: {} }}{suffix}",
                        v.kind,
                        rust_to_ts(v.value_ty)
                    )
                    .unwrap();
...
pub(crate) fn generate_typescript(registry: &PrimitiveRegistry) -> String {
    ...
    let types: Vec<&RegisteredType> = registry.iter_types().collect();
    ...
    match &ty.shape {
        ...
        TypeShape::Struct { fields } => {
            ...
            if !any_doc {
                ...
                let inner_indent = "  ";
                for f in fields {
                    luau_doc_line(f.doc, inner_indent, out);
                    writeln!(
                        out,
                        "{inner_indent}{}: {},",
                        f.name,
        ...
        TypeShape::StringEnum { variants } => {
            ...
            if !any_doc {
                ...
                let inner_indent = "  ";
                for (i, v) in variants.iter().enumerate() {
                    luau_doc_line(v.doc, inner_indent, out);
                    let prefix = if i == 0 { "" } else { "| " };
                    writeln!(out, "{inner_indent}{prefix}\"{}\"", v.name).unwrap();
                }
            }
        }
        TypeShape::TaggedUnion {
            ...
            if !any_doc {
                ...
                for (i, v) in variants.iter().enumerate() {
                    ...
                    writeln!(
                        out,
                        "{inner_indent}{prefix}{{ {tag_field}: \"{}\", {value_field}: {} }}",
                        v.kind,
                        rust_to_luau(v.value_ty)
                    )
                    .unwrap();
