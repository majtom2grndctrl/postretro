...
- A damage-taken accumulator on `BrainComponent`, written at the `apply_damage_with_context` chokepoint, consumed by the AI tick before graph evaluation. Captures every damage source (player weapon stage, enemy melee, the `applyDamage` script reaction) between consecutive AI evaluations.
- Trigger rule: accumulated damage since the last AI evaluation ≥ threshold, and the re-stagger cooldown elapsed → the engine writes `@state.staggered` (`registry.entity_state_mut`) as a one-tick pulse, then clears it the following tick. Checked every tick (not think-strided); quiescent entities (pending despawn per E16) are excluded from AI pass-1 and never reach this check. This is the only engine-authored write; everything downstream is graph content.
...
- [ ] A lethal hit queues deferred despawn (E16); the entity is excluded from AI pass-1 on subsequent ticks and never reaches the stagger accumulator — no pulse fires, no interrupt-ordering race exists.
