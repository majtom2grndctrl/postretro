...
It already carries a full `PlayerMovementDescriptor` — and that descriptor's fields are `NumberOrIr` / `BoolOrIr` (imports at `tuning_payload.rs:98`). **Authored IR already crosses this payload.** Carrying the switch policy's guard IR is therefore continuous with shipped practice, not a new capability, which is what makes "replicate rather than hash" affordable here.
...
## 3. Why a general timing primitive is not needed
...
## 4. Why the wheel is discrete, not analog
...
## 5. Presentation is already plumbed
...
## 7. Lifecycle — the cross-instance switch handoff
...
## 8. Observers of active-weapon state
...
## 9. Oversized files this plan touches
