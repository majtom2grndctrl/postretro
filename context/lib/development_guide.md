...
**Tooling.** The live graph is `crate-graph.md` — a generated snapshot (layers, edges, chokepoint ranking) kept fresh by the `crate-graph --check` preflight gate. Query it live with `cargo run -p xtask -- crate-graph --rdeps <crate>` for a crate's blast radius (reverse deps), or `--deps <crate>` for what it pulls in. The invariants above (nothing depends on the binary, `foundation` stays a leaf, `entities` depends only on `foundation`) are enforced by the `layering_invariants_hold` test — an upward edge or a widened chokepoint fails `cargo test`.
