...
`reviewers:N` forces N depth agents and skips the table. `effort:medium` runs depth agents at medium reasoning effort. The hygiene+drift agent always runs at medium reasoning effort, unaffected by overrides.
...
- **Each depth agent:** a `worker` agent with `model: "gpt-5.5"` and the specified `reasoning_effort` (default: high). Pass full content of `.Codex/skills/code-review/SKILL.md`, then its lens prompt (below), then the specific flow or surface from triage.
- **Hygiene + drift agent:** a `worker` agent with `model: "gpt-5.5"` and `reasoning_effort: "medium"`. Pass full content of `.Codex/skills/code-review/SKILL.md`, then the hygiene+drift prompt (below).
