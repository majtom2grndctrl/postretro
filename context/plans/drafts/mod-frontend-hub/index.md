...
> Prerequisites: `drafts/runtime-level-lifecycle` (load/unload cycle, Frontend/Loading states), `drafts/mod-map-catalog` (`defineMod` + the map catalog this plan's level-select and `loadLevel` read), and `drafts/reaction-composition` (mod-global reaction tier — so death/level-flow reactions are declared once, not per level). Builds on the manifest-return, staged-reload, and tiered-registry contracts established by `in-progress/production-gameplay-hud` and `in-progress/production-pause-menu`.

## Goal

Let a mod declare its frontend through a `frontend` manifest block (on the `defineMod` builder from `mod-map-catalog`): which menu UI shows at start, an optional background level behind it, and where the camera sits during menus. Add a catalog-driven level-select, and wire the bidirectional `Frontend ⇄ Running` transitions (start game, quit to menu, died→menu, level complete) on top of the runtime level lifecycle.

## Scope

### In scope

- **`frontend` block on the `defineMod` manifest** — the `defineMod` builder itself comes from `mod-map-catalog`; this plan adds the `frontend` field and its types to the manifest the builder already types.
...
- **Catalog-driven level-select:** the frontend reads the map catalog (`mod-map-catalog`) to list/filter loadable maps by `name`/`tags` and start one via `loadLevel(id)`. Pre-load discovery — no map is loaded to populate the list.
- **Game-flow vocabulary (the exposed options):** a small closed set of engine-owned system reactions the SDK exposes — `loadLevel({ map })` (carries a **catalog id**, resolved via `mod-map-catalog` to a path; queued/drained like `openMenu`/`showDialog`, `scripting.md` §10.4), `restartLevel()` (reload the active map), and `returnToFrontend()` (unload → Frontend). Each drives a lifecycle load/unload request. The typedef is the contract, and the set is **designed to grow** (see out of scope).
- **Player-death handling — author's choice from the vocabulary:** the engine fires the existing `playerDied` event (health.rs:15) and bakes no death policy. The mod binds it to whichever game-flow verb it wants (`restartLevel()` for the simple case) or to `openMenu({ tree })` for a death screen whose buttons invoke the verbs. The death reaction is declared once in the **mod-global tier** (`reaction-composition`), optionally scoped per mode (campaign restart vs deathmatch respawn via disjoint tags) — never copy-pasted per level. Level-complete uses the same path via an `onStateCrossing` watcher (there is no built-in `levelComplete` event).
- **Quit-to-menu button:** reserved argument-less `ui.quitToMenu` action (mirrors `ui.exitToDesktop`) — a thin button alias over `returnToFrontend`, so the engine-fallback menu quits without any registered reaction.
...
- [ ] The `frontend` block is accepted on the `defineMod` manifest, type-checks in TS and Luau, and `gen-script-types` emits its declarations with the drift test passing (the `defineMod` builder itself is covered by `mod-map-catalog`).
...
- [ ] The level-select lists the catalog's maps by `name`, filterable by `tags`, without loading any of them; starting one routes through `loadLevel(id)`.
- [ ] The SDK exposes `loadLevel`, `restartLevel`, and `returnToFrontend` as bindable game-flow verbs. `restartLevel()` reloads the active map; `returnToFrontend()` unloads to the menu; a `ui.quitToMenu` button reaches `returnToFrontend` without a registered reaction.
...
- [ ] CPU tests cover `defineMod` identity/round-trip, `frontend` drain, staged replace/omit, fallback reveal, and `loadLevel`/`ui.quitToMenu` routing. Manual launch verifies boot→menu→start→play→quit→menu→start-again.
...
Add the optional `frontend` field to the `ModManifest` type in both typedef blocks (`TS_SDK_LIB_BLOCK` typedef.rs:678, `LUAU_SDK_LIB_BLOCK` typedef.rs:1761) and declare the `Frontend` and menu-camera types. The `defineMod` builder is delivered by `mod-map-catalog`; this plan only extends the manifest it already types. Add SDK-parity and typedef-drift coverage for the new field.

### Task 2: Drain the `frontend` block into engine state

Add `frontend: Option<Frontend>` to `ModManifestResult` (runtime.rs:45) and `StagedManifest` (staged_manifest.rs:55). Drain it in `run_mod_init_quickjs` (runtime.rs:1445, beside `drain_ui_trees_js`/`drain_theme_js` ~1589–1607) and `run_mod_init_luau` (runtime.rs:1639). Commit it at the same successful-staged boundary as UI trees and theme. Malformed `frontend` is logged and skipped (degrade, never abort — matches UI-tree/theme handling).

### Task 3: Frontend population + menu camera

When the lifecycle enters Frontend (prereq), resolve the committed `menuTree` through the UI registry (`ModalStack`, modal_stack.rs) and push it as a capturing modal; fall back to the engine-default menu when absent (new tier entry alongside `hud`/`pauseMenu` fallbacks). Apply the declared static camera pose by writing `self.camera.{position,yaw,pitch}` (camera.rs:86) — the same fields `install_level_payload` writes at spawn (main.rs:3759), so no new camera plumbing. The static pose is the required deliverable. If an orbit is declared, drive the pose from a thin keyframe sampler each frame; if that reuse turns out heavier than a thin sampler, ship static-only and defer orbit, recording the call in Task 5. If `backgroundLevel` is set, enqueue its load through the lifecycle request queue but stay in Frontend (player control suppressed via the capture-mode path, `reconcile_ui_focus` main.rs:4174).

### Task 4: Game-flow vocabulary and transitions

Add the game-flow system reactions (no entity target; queued and drained like the `openMenu`/`showDialog` system reactions, `scripting.md` §10.4): `loadLevel` (carries a `map` catalog id → resolved via `mod-map-catalog` to a path → enqueues a lifecycle load request), `restartLevel` (no arg → reload the active map), and `returnToFrontend` (no arg → lifecycle unload → Frontend). Add the reserved `ui.quitToMenu` button action — a constant in `render/ui/actions.rs` (beside `EXIT_TO_DESKTOP_ACTION`), a `UiButtonAction` variant, classification in `classify_ui_button_action` (main.rs:993) and routing in `route_ui_button_action` (main.rs:1025) to the `returnToFrontend` engine op, plus SDK constants (`QUIT_TO_MENU_ACTION`) in `sdk/lib/ui/reactions.{ts,luau}` — so engine-fallback menus quit without a registered reaction. Wire the death path: the existing `playerDied` event (health.rs:15) binds to any game-flow verb (or to `openMenu` for a death screen) through the existing reaction registration; document the simple-restart and death-screen patterns, plus level-complete via `onStateCrossing`. Keep the game-flow set open for future `respawnAtCheckpoint`/`loadLastSave` — no implementation, no foreclosure.
...
|---|---|---|---|---|---|
| `defineMod` (from `mod-map-catalog`) | n/a (SDK only) | n/a | `defineMod()` | `defineMod()` | this plan adds the `frontend` field it types |
...
| background level | catalog id → `PathBuf` | `"backgroundLevel"` | `backgroundLevel` | `backgroundLevel` | optional; resolved via `mod-map-catalog` |
| menu camera | `Camera{position,yaw,pitch}` writer | `"camera"` | `camera` | `camera` | static pose + optional keyframe orbit |
| load verb | `loadLevel` system reaction | `{name:"loadLevel",args:{map}}` | reaction name | reaction name | `map` is a catalog id (NOT a `ui.*` action) |
...
| quit button | `UiButtonAction::QuitToMenu` | `"ui.quitToMenu"` | `QUIT_TO_MENU_ACTION` | `QUIT_TO_MENU_ACTION` | reserved, argument-less; aliases `returnToFrontend` |
...
export function setupMod() {
  return defineMod({
    ...
    entities,
    theme,
    // maps: [...] — the catalog (mod-map-catalog) classifies levels and feeds level-select
    uiTrees: [
...
export const startCampaign = defineReaction({
  name: "startCampaign",
  steps: [loadLevel({ map: "e1m1" })],   // catalog id, resolved via mod-map-catalog
});
...
//   steps: [openMenu({ tree: "deathScreen" })]
// Future verbs slot in here: respawnAtCheckpoint(), loadLastSave().
```

## Open questions

- `frontend` in `StagedManifest` (debug hot reload) — `fonts` is currently *omitted* from `StagedManifest` (staged_manifest.rs:55). Confirm `frontend` is worth the staged lane or, like fonts, commits cold-only. Leaning include it (camera/menu iteration benefits from reload).
- Reaction scope for death/level-flow — **resolved**: handled by `drafts/reaction-composition` (mod-global reaction tier + per-reaction level-tag scope). The death reaction is declared once in `defineMod`, optionally scoped per mode. This plan consumes that tier.
