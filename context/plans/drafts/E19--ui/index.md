...
- `postretro-ui`: move the CPU `render::ui` subtree —
  - descriptor surface (already `pub use postretro_scripting_core::ui::descriptor::*` — keep the dependency, drop the dead local files per `E19--leaf-hygiene-and-boundary-prep`),
  ...
  - the CPU output/wire types currently co-located in `ui/mod.rs`: `UiInstance`, `UiDrawList`, `UiDrawData`, `UiUniform`, `UiText`, `FocusRect`, `FocusRectList`, `FocusGroup`, `FocusNeighbors`, `NodeInteraction`, `UiReadSnapshot` (carrying `descriptor::CaptureMode` per `E19--leaf-hygiene-and-boundary-prep`),
...
## Sequencing
**Phase 1:** Task 1, then Task 2, then Task 3. Needs `E19--leaf-hygiene-and-boundary-prep` (UiCaptureMode inversion). Independent of the other Milestone 1–2 CPU specs (`E19--render-data`, `E19--level-loader`, `E19--visibility`, `E19--lighting-cpu`). Milestone 2.
