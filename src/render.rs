...
fn fs_main() -> @location(0) vec4<f32> {
    // Cyan wireframe for cyberpunk aesthetic.
...
impl Renderer {
    ...
    ///
    /// `bsp_world` is `None` when no map file was loaded (renders clear color only).
    /// `force_line_list` overrides polygon mode detection and uses the LineList fallback.
    pub fn new(
        window: &Arc<Window>,
        bsp_world: Option<&BspWorld>,
            ...
            bsp_world.filter(|w| !w.vertices.is_empty() && !w.indices.is_empty())
        {
            ...
            let indices = match wireframe_mode {
                WireframeMode::PolygonModeLine => world.indices.clone(),
                WireframeMode::LineList => {
                    build_line_list_indices(&world.indices, &world.face_meta)
                }
            };

            let count = indices.len() as u32;
            (
                bytemuck_cast_slice_f32x3(verts),
            ...
            (
                vec![0u8; 12], // one dummy vertex (3 floats)
...
fn build_line_list_indices(tri_indices: &[u32], face_meta: &[crate::bsp::FaceMeta]) -> Vec<u32> {
    ...
    for face in face_meta {
        let offset = face.index_offset as usize;
...
mod tests {
    use super::*;
    use crate::bsp::FaceMeta;

    #[test]
    fn build_line_list_from_single_triangle() {
        // One triangle: indices [0, 1, 2], face with offset=0, count=3.
        let tri_indices = vec![0, 1, 2];
        let face_meta = vec![FaceMeta {
            index_offset: 0,
            index_count: 3,
            leaf_index: 0,
        }];

        let lines = build_line_list_indices(&tri_indices, &face_meta);
    ...
    fn build_line_list_from_quad() {
        // Quad fan: (0,1,2), (0,2,3) -> edges: 0-1, 1-2, 2-3, 3-0
    ...
    fn build_line_list_from_pentagon() {
        // Pentagon fan: (0,1,2), (0,2,3), (0,3,4) -> edges: 0-1, 1-2, 2-3, 3-4, 4-0
    ...
    fn build_line_list_multiple_faces() {
        // Two triangles at different offsets.
    ...
    fn build_line_list_degenerate_face_skipped() {
        // Face with < 3 indices should be skipped.
