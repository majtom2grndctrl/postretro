...
| Stream permits (8), one read issuer plus a decode pool (half the cores, 1–3), and decoded bytes per drain (8 MiB, always at least one cluster) | More permits give the issuer a wider window to order and coalesce, and may shorten visible misses, but raise concurrent I/O/decode buffers. A larger byte budget installs more per drain at a higher per-frame CPU cost; less reduces transient load at the cost of latency. | Runtime policy constants; keep the host-phase and per-frame bounds in view. Tune from the `[SH streaming]` log line or the dev-tools Streaming tab. |
| Read coalescing (256 KiB gap, 16 MiB span) | A wider gap merges more chunks into one read but discards more unrequested bytes; a larger span cap lengthens single reads. | Runtime policy constants; compare coalesced reads against gap bytes. |
