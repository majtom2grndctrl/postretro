...
- **Moving-light shadow-depth invalidation.** Task 7 caches static-world depth and restores it per frame for *fixed* lights (measurement-gated). Re-rendering that cache when a light's transform changes — projectile-attached / moving lights — is deferred to the *Moving-light shadow-depth invalidation* roadmap line (Rendering and visual polish). Task 7 builds the cache + restore seam so the invalidation path is purely additive.
...
- [ ] The per-frame spot-depth world re-render cost is measured (`POSTRETRO_GPU_TIMING`) and recorded; static-depth caching (Task 7) is implemented only if that cost justifies it, and when implemented, a fixed light re-renders its cached static world depth only on slot (re)assignment — not every frame. Measure-and-report; cutting Task 7 is an acceptable outcome.
...
Render entity occluders into each occupied spot-pool slot's depth pass, on top of the slot's world-depth baseline, within the per-slot "Spot Shadow Depth Pass" render pass (`CompareFunction::Less` composites world + entity into one layer). The baseline is established either by the shipped per-frame `shadow_cull.draw_slot_indirect` world draw (today) or, if Task 7's measurement justifies it, by restoring the slot's cached static depth — entity draws compose on top either way, so this task is written against "world baseline already in the slot." Set the skinned-depth pipeline (Task 1) and entity instance draws after that baseline. The slot's light-space matrix is the `slot_cone_matrices` entry already stashed on `SpotShadowPool` in `update_dynamic_light_slots`. Render entity occluders only for slots whose light has `casts_entity_shadows` (Task 3) — a dynamic light with the toggle off keeps its world shadow but draws no entities. Cull entity instances per slot on the CPU: transform each instance's model bound (Task 1) by its `transform` and test against the slot's cone frustum via `cone_frustum_planes` + `aabb_intersects_frustum`. Add tunable-radius PCF to `sample_spot_shadow`. World forward sampling is unchanged — verify entity shadows appear on world receivers for dynamic spots.
...
### Task 7: Static-depth caching for spot slots (measurement-gated)
The spot depth pass re-renders **static world geometry** into every occupied slot every frame (`render/mod.rs` ≈4300: `LoadOp::Clear` then `draw_slot_indirect`), then Task 2 composes the always-moving entity occluders on top. For fixed lights the world half is pure redundant per-frame work — vertex + cull + raster, not just fragment writes (so a scissor/dirty-rect trick on the existing draw saves nothing material). Eliminating it needs a cached static-depth copy restored each frame before the entity draws.

**Measure first.** Before building the cache, record the spot-depth world re-render cost via `POSTRETRO_GPU_TIMING=1` (per-pass GPU time) on a representative dev map at realistic shadowed-spot counts. On a 4 GB floor with the 384 MB (96-slot) live pool, spending VRAM to cache a cheap depth-only cone-culled draw would itself be the sloppy choice — cache only if the measured cost justifies it.

**Approach (if justified): A — dirty-gated static cache + per-frame restore.** A renderer-owned static-depth cache (`Depth32Float`, `COPY_SRC` + render target, **not** a sampled binding — forward keeps sampling the one live pool), rendered for a slot only when its light-space matrix is (re)assigned. Each frame: blit the slot's cached static depth into the live slot layer, then draw entity occluders (Task 2) on top. Rejected alternative **B** (separate static + dynamic *sampled* layers, `min()` at sample time) burns one of the scarce ~14/16 sampled bindings. Size the cache to **concurrent slots-with-entities** (a small ranked sub-pool — e.g. 16 slots ≈ 64 MB — not all 96), since only slots with an entity in cone need a restorable baseline. Build the cache + restore seam so the *Moving-light shadow-depth invalidation* roadmap line (re-render on light transform change) is additive.
...
**Phase 2.5 (sequential, gated):** Task 7 — measure the spot-depth world re-render; build static-depth caching only if the measurement justifies it. Consumes Task 2's pass structure; off the north-star path.
...
Spot shadows (north star: enemy casts a shadow) land at end of Phase 2. Task 7 (caching) and the point work (Phases 3–4) are additive, cuttable tails.
...
- **Static-depth caching: worth it, and at what size? (Task 7).** Resolve by measurement, not estimate: with `POSTRETRO_GPU_TIMING=1`, is the per-frame spot-depth world re-render expensive enough to justify a static-depth cache, given the cache costs VRAM on the 4 GB floor (≈4 MB × cached-slot count)? If yes, how many concurrent slots-with-entities must the static-cache sub-pool cover (sizing the VRAM)? Decide during Phase 2.5; cutting Task 7 entirely is acceptable if the re-render measures cheap.
