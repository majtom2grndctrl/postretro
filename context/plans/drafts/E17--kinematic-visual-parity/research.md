...
- **Shared material-shading WGSL.** `blinn_phong` / `sample_normal` / oct-decode / TBN are binding-agnostic leaf helpers (take all inputs as args) — the §8 share-by-concatenation case. The `skin_matrix` duplicate precedent does **not** apply (it duplicates only because it reads module-scope `bone_palette` at a fixed binding). Extract to one shared snippet consumed by forward + skinned_mesh + mover, guaranteeing parity by construction rather than a coarse fingerprint test.
