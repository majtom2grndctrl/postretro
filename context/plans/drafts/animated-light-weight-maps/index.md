...
In `lightmap_bake`, filter the `static_lights` pass to `!light.is_dynamic && (light.animation.is_none() || light.bake_only)`. `bake_only` animated lights stay in the static bake — they have no weight-map entry (excluded from Spec 1's chunk list), so the static atlas is their only contribution path. Non-`bake_only` animated lights are removed entirely; their contribution moves to the compose path. Update the existing test `static_flag_bakes_but_dynamic_does_not` to cover the animated case as well.
...
- **`bake_only` animated lights stay in the static bake.** They are excluded from Spec 1's chunk list so cannot have weight-map entries; the static atlas is their only contribution path. Contribution is frozen at bake-time phase — no runtime animation. Non-`bake_only` animated lights are removed from the static bake entirely.
