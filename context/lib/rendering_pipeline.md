...
**Dynamic direct.** Dynamic lights run a diffuse-only per-fragment loop with an influence-volume early-out; no surface, including movers, receives dynamic-light specular. Dynamic spot lights cast shadow maps (depth texture 2D-array pool, comparison sampler). Dynamic point lights cast cube-array shadows (6-face depth pool): each occupied face renders cone-culled static world geometry plus entity occluders, mirroring the spot pool's occluder split. Sun/directional lights cast no dynamic shadows. Light sources: FGD entities (`light`, `light_spot`, `light_sun`) and gameplay effects. Clustered forward+ binning deferred until profiling shows the flat loop bottlenecks. A dynamic-tier light's projection is fixed at level load, so its world shadow depth is invariant frame to frame. A per-slot world-depth cache reuses it: world depth renders only when a slot's occupant changes, and both world and entity receivers sample it, keyed on occupant identity and parallel to the promoted-static cache. Entity occluders still redraw each frame. Decided, not yet built.
