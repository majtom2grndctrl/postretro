...
- [ ] Billboard and fog (`reject_backface = false`) open-area output is visually unchanged from baseline — all 8 corners always contribute, so the only difference is manual `textureLoad` + fp32 trilinear weighting vs. the hardware sampler on `Rgba16Float`; delta is sub-perceptual. This is the genuine no-op path.
- [ ] Forward (`reject_backface = true`) open-area output is expected to differ from the hardware-trilinear baseline: backfacing corners are dropped even with no walls, so roughly half the 8 corners are correctly excluded for any angled surface. That difference is the intended correctness gain — do not assert visual equivalence to the old sampler for forward in open areas.
...
- [ ] Under the `dev-tools` feature, the `MarkerMode::Validity` overlay renders baked per-probe validity correctly at a known valid probe and a known invalid (in-wall) probe. This validates the *bake* (the CPU-side `ShVolume::validity` mirror), not the GPU compose-pass alpha propagation: the packer→band-0-alpha encoding is covered by the `pack_probes_writes_two_state_validity_into_band0_alpha` unit test, and a compose-pass propagation regression (Task 1.2) would resurface as the through-wall bleed checked above. There is no direct GPU band-0-alpha readback — `decode_l0` returns RGB only — and a manual GPU spot-check of that channel was judged not worth building.
