...
- **The augment / `defineAugment` modifier layer.** A separate Weapon Systems item; `effective()` gains only the dynamic-spread composition, not a general modifier fold.
...
- [ ] **AC1** A weapon with `bloomPerShotDegrees > 0` fires its first shot from rest at its base `spreadDegrees` cone; each successive sustained shot samples a wider cone, up to `spreadDegrees + bloomMaxDegrees`.
...
- [ ] **AC3** With all new spread fields at their 0 defaults, cast directions are byte-for-byte identical to current static behavior (base `spreadDegrees`; exact axis when base is 0).
...
- [ ] **AC12** `hud.reticle` shows a spread ring whose radius tracks `player.spread` with a tween: minimum at rest, visibly expanding during sustained fire and while moving, easing back as accuracy recovers.
...
- Constant near `crates/postretro/src/weapon/spread.rs`: `MAX_EFFECTIVE_SPREAD_DEGREES` (engine ceiling, e.g. 45.0 — matches the static validation cap). The movement reference speed is not an engine constant — it is the pawn's own authored `ground_params.speed.run`, so a designer who retunes movement automatically rescales weapon movement-spread and `movementSpreadDegrees` reads as "spread at full run."
...
- Axis tilt: rotate the aim direction upward (about the camera-right axis, in world space) by `spread_vertical_bias * effective_degrees` before `sample_cone_direction`; at bias 0 the axis is unchanged.
...
| 2 | Sustained fire | per shot: grow after sampling, cap at `bloomMaxDegrees` | Cone widens shot-to-shot up to `spreadDegrees + bloomMaxDegrees` (AC1) |
