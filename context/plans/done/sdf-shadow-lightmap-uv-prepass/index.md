...
The chosen mechanism: a new **lightmap-UV render target on the depth pre-pass** (MRT slot), full resolution, `Rg16Float`. The pre-pass already runs the vertex stage that has `lightmap_uv_packed` in flight; adding one fragment that writes the interpolated UV is nearly free per pixel and benefits from early-Z (the written UV is the visible-surface UV). The half-res shadow pass samples this target at half-res pixel centers and uses the lightmap UV to index `static_lm_direction` and `animated_lm_direction`.

**Load-bearing decision — write on the pre-pass, not the forward pass.** Forward fragments are already expensive (SH lookups, lightmap fetches, normal-mapped shading, dynamic-light loop). Adding bandwidth there compounds the cost class that originally killed inline SDF (`done/sdf-static-occluder-shadows/research.md` — "Why SDF was removed"). Pre-pass fragments currently do **zero** shading work — one Rg16Float write is the minimum incremental cost and benefits from early-Z. Forward stays single-color-target.
...
| B | Lightmap-UV gbuffer on pre-pass MRT | **Chosen** | Exact per-texel direction. ~8 MB at 1080p Rg16Float. Pre-pass write benefits from early-Z; forward stays single-target. |
...
- A new **full-resolution lightmap-UV render target** (`Rg16Float`), allocated alongside the depth attachment and resized in lockstep with it on surface resize.
...
- A pre-pass pipeline gaining one color attachment with `LoadOp::Clear` to `(-1.0, -1.0)` — see *Sentinel and consumer behavior*.
...
- [ ] The depth pre-pass pipeline declares one color target of format `Rg16Float`; the renderer allocates a matching texture at surface size and recreates it on resize. Asserted via the pipeline descriptor and a resize-path unit test. [T1, T2]
...
- [ ] The pre-pass color attachment uses `LoadOp::Clear` with `r = -1.0` and `g = -1.0` (the negative sentinel). Asserted via render-pass descriptor inspection in a unit test. [T2]
...
| Task | Covering ACs |
|---|---|
| T1 | pipeline declares Rg16Float MRT; shader exports fragment; GPU timing budget |
| T2 | resize allocates new target; clear color = (-1,-1,_,_); GPU timing budget |
...
Pipeline-side: `depth_prepass_pipeline` (`render/mod.rs:1635`) gains a `fragment: Some(...)` block declaring one color target of format `Rg16Float`, no blend. The pipeline layout is unchanged — fragment stage reads from no resources. Depth-stencil state is unchanged — still `depth_compare: Less` with `depth_write_enabled: true` (do not mirror forward's `Equal`).
...
if (lm_uv.x < 0.0 || lm_uv.y < 0.0) {
    // Pre-pass sentinel (Rg16Float cleared to (-1,-1)) — no fragment wrote this pixel. Bail to fully lit.
...
- **One MRT, full-res, Rg16Float.** ~8 MB at 1080p; ~500 MB/s bandwidth at 60 fps. Well within the 2020 MBP budget — pre-pass writes hit early-Z so the per-fragment cost is effectively one ROP write. `Rg16Float` is the baseline renderable color format (`Rg16Unorm` is not renderable as a color attachment without the `texture-formats-tier1` feature) — see *Resolved decisions — MRT format*.
- **Sentinel = (-1.0, -1.0).** Negative UVs are unreachable by any real fragment (real UVs are `[0,1]`) — see *Resolved decisions — MRT format*. The shadow-pass consumer reads the sentinel and bails to `FULLY_LIT`.
...
**MRT format — `Rg16Float`, not `Rg16Unorm`.** Shipped as `Rg16Float`. The original spec proposed `Rg16Unorm` (a literal round-trip for the `u16/65535` lightmap-UV packing in `forward.wgsl:273–275`, with uniform ~1.5e-5 precision), and anticipated falling back to `Rg16Float` only if chart-edge precision near 1.0 proved visibly insufficient. The actual reason for the fallback is different: `Rg16Unorm` is **not renderable as a color attachment** on baseline wgpu / AMD discrete adapters — `RENDER_ATTACHMENT` usage requires the `texture-formats-tier1` feature, which the target adapters lack. `Rg16Float` is a baseline renderable color format needing no feature, so it was used unconditionally rather than as a precision fallback. `Rgba16Float` doubles memory for no gain; `Rg11B10` loses precision below 0.0005 — too tight.

Sentinel is `(-1.0, -1.0)`, not `(0.0, 0.0)`. This is more correct, not merely a format workaround: chart-less real vertices legitimately write UV `(0,0)` (`crates/postretro/src/geometry.rs` ~lines 19-22 — lightmap UV is zero on vertices that received no chart), so a `(0,0)` sentinel would falsely flag real geometry as "no fragment." Negative UVs are unreachable by any real fragment (real UVs are `[0,1]`), and `Rg16Float` represents `-1.0` exactly. The shipped `lightmap_uv_clear_is_negative_sentinel` test asserts the clear is `(-1,-1)`.
