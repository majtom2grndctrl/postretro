...
- [ ] A bare read or write of a per-owner slot fails at load with a diagnostic naming the slot; so does an owner-addressed access to a global slot, so does `network: "ownerPrivate"` declared without per-owner cardinality, and so does `perOwner` declared alongside `accumulate`. Per shipped `mod_init.rs` behavior, this rejects the whole declaration set at mod-init — the manifest does not partially load.
