...
**Socket-mount verification.** Verify weapon/prop socket-mount orientation in the engine frame — never against a Blender render. Blender's glTF importer reorients pose bones and flips the up-axis; the engine loader consumes raw glTF joint world matrices as authored (spec-correct), so a mount that looks right in Blender can still be wrong in-engine. Mandatory tool: `crates/model/examples/socket_dump.rs` (`cargo run -p postretro-model --example socket_dump -- <model.gltf> [clip] [socket] [time] [weapon.gltf]`) mounts the weapon exactly as the runtime does and reports `barrel·+Z` (want ≈ 1.0, forward), `barrel·+Y` (want ≈ 0, level), `up·+Y` (want ≈ 1.0, unrolled), plus a suggested corrective `--rotate-euler` to feed back into the `prop_to_gltf.py` bake. It identifies the muzzle geometrically (thin end of the barrel's long axis) — an earlier farthest-vertex heuristic could pick the stock butt instead, reporting a false-positive `barrel·+Z ≈ 1.0` for a weapon that actually faced backward.
