...
- Resolved: the `levels` selector is a per-reaction field — the engine model — and `scopeReactions` is sugar that stamps it. AC, Scope, and the boundary inventory all commit to the per-reaction field.
