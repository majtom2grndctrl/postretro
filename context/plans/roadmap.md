...
- [ ] **Weapon primitives** — engine-level weapon primitive surface. Hitscan resolution mode first: raycast the Milestone 7 collision world, spawn an impact, emit a typed hit event (damage intent). Projectile is a sibling resolution mode, deferred unless the slice needs it — not faked with a fast projectile. First weapon authored in the SDK as a reference behavior (script declares parameters, Rust runs the fire logic). Testable against the static world (wall impacts) before any enemy exists; the hit event's health/kill consumer lands with the enemy entity. A foundation to refine, not a stub. `context/plans/drafts/M10--weapon-primitives/`
- [ ] **Stub sound layer (Nintendo-style SFX)** — placeholder retro SFX (fire, impact, footstep, alert, pain) wired through entity-emitted sound events. Lands early so weapons and enemies emit into a live sink rather than dead wiring — you hear combat from the moment it exists. Explicitly a stub: the generic sound-event hook is durable, the assets and backend are placeholder. The real spatial audio foundation lands in Milestone 11.
...
- **Emissive / fullbright surfaces** — texture-driven self-lit surfaces. Not started. An earlier `neon_`-based lighting-replacement stub was removed as incorrect; the correct approach is additive HDR emissive + bloom, designed alongside the post-processing pass. Deferred — see `plans/drafts/emissive-surfaces/`.
