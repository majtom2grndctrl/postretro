...
In the forward fragment shader, multiply each shadowed light's direct contribution by the upsampled shadow factor — including the **static lightmap term**, gated on the unshadowed-mode flag from Task 2a/3 (when the lightmap is shadowed, e.g. a legacy PRL, skip multiplying the static term to avoid double shadows). Upsample inline with a depth-aware 2×2 bilateral filter (no extra full-res target). Note: the `fog_composite.wgsl` bilateral upsample this could once have copied was reverted in `f50314d` for perf — re-derive the filter, don't expect existing code (`research.md`). Sample the shadow-factor buffer via a new binding in the shadows bind group (group 5); the shared group-5 BGL and every pipeline that uses it must be updated together with the new binding's `visibility`. Gate application on the `SdfShadowMode` uniform (off → no shadow factor applied).
