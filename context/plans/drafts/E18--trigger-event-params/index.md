...
- **`levelLoad`/crossing sources publishing trigger-event inputs** — a `TriggerEventParams` reaction on those sources rejects at install (mechanism owned by the sibling spec).
...
- [ ] `onTriggerEvent({ tag }, "enter", [r])` fires identically to a brush `on_fire` KVP naming the same reaction — same fire context, same commands (headless equivalence test).
...
| trigger-event observer | trigger-event descriptor + install into `TriggerBindingTable` | `triggerEvents: [{ tag, event: "enter"\|"exit", fire, levels? }]` on `setupLevel` return / `ModManifest` | `onTriggerEvent({ tag }, event, fire)` | same | brush `on_fire`/`on_exit` KVPs remain the map-authored referrer |
