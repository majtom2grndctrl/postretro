...
| # | Stage | Notes |
|---|-------|-------|
| 1 | Init logging and boot clock, parse args, resolve selected mod/content root + optional map path | `--mod <path>` / `--mod=<path>` is the shipped single-mod boot handle. It selects the mod root. A bare map argument remains the raw-path dev bypass under that root. |
...
| 7 | Present the frontend or enqueue the boot map load | No CLI map → frontend menu. If the committed frontend declares a background level, the menu stays open while that catalog id loads behind it. CLI map → Loading. |
...
| 9 | Steady per-frame loop: Input → Game logic → Audio → Render → Present | Running draws the level. Frontend draws a world-less frame and frontend-safe UI. A frontend menu over a loaded backdrop runs through Running with UI capture and the menu camera pose held. |
...
- **Early boot solo splash.** The planned splash architecture moves back out of the UI system. Minimal pre-event-loop work is logging, boot timing, raw argument collection, and content-root/boot-map selection. `EventLoop::new` should run before session-lifetime scripting bootstrap, net endpoint construction, Rust-only handler registries, options I/O, input seeding, and built-in UI registration. Resume creates the window and a boot-ready renderer with only the surface/device/queue/config plus direct splash pass; the first successful present is black, then bounded splash decode/upload runs, then the first successful logo present runs if the built-in image is valid. Missing or malformed splash assets warn and keep the boot frame black. Deferred session startup, stale-script work, audio, debug UI, net endpoint setup, mod init, frontend presentation, and boot-map worker dispatch run only after the visible logo frame, or after the warning/fallback black frame when the logo is unavailable. Full renderer initialization completes before splash clear and before Frontend, Loading completion, Running, UI pass, or scene rendering can execute. Suspend/resume must preserve the pending session owner until commit, keep any installed session bundle, and re-present black/logo as needed before continuing full renderer initialization.
- **Mod-supplied splash override.** No override hook is active until a splash-override feature ships.
