...
- **The join seed is sent during the parity stage, before admission.** The host buffers the seed until admission succeeds, then applies it to the freshly minted seat before the first `levelLoad` fires. Sending during parity avoids the timing gap where the seat exists but the seed has not arrived.
...
- **`JoinSeedValue` is a wire-level enum in `postretro-net`.** Same variants as `PersistedValue` (Boolean, Number, String, Array). Converted at the boundary in the binary crate. Avoids a crate dependency cycle — `postretro-net` does not depend on `postretro` or `postretro-scripting-core`.
- **No wire version bump for the join seed.** `ClientControlMessage` is an extensible enum. An older host ignores the variant. An older client never sends one — the feature degrades to pre-persistence behavior.
...
- [ ] AC 13: A host that does not recognize the join seed message (older engine) ignores it; the client starts at defaults. A client that does not send a seed (older engine) starts at defaults on the host.
- [ ] AC 14: Abnormal termination may lose up to ~60 s of unsaved per-owner changes for a connected client, matching the periodic save interval. Single-player/host abnormal termination may lose all unsaved changes, matching the existing persistence contract.
...
**Collect.** Add `collect_per_owner_state` in `state_persistence.rs`. For each `perOwner + persist` slot (filtered by `is_persisted_per_owner_slot`), read the local seat's per-owner entry via `record.per_seat_value(local_seat)`, convert to `PersistedValue`, and insert under `(durable_key, hex_player_id)`. The function takes `&SlotTable`, `Option<&StoreIdentityLedger>`, `&BTreeSet<String>` (committed membership), `local_seat: Seat`, and `local_player_id: [u8; 16]`. Returns a `BTreeMap<String, BTreeMap<String, PersistedValue>>` (the `per_owner` shape).
...
**Wire type.** Define `JoinSeedValue` in `crates/net/src/wire/control.rs` (or a sibling module in the wire crate) with the same variants as `PersistedValue`: Boolean(`bool`), Number(`f64`), String(`String`), Array(`Vec<f64>`). Add `JoinSeed { slots: BTreeMap<String, JoinSeedValue> }` to `ClientControlMessage`. The wire crate (`postretro-net`) does not depend on `postretro` or `postretro-scripting-core`, so `JoinSeedValue` is a parallel type converted at the boundary. Add `From<JoinSeedValue>` for the internal `PersistedValue` (or equivalent conversion) in the binary crate.
...
1. Buffer the seed until admission completes — the message arrives during parity, before the client has a seat.
