...
- Depth is authored in **meters**, not texture space: brush UV scale is unconstrained in TrenchBroom, so a texture-space scale would give one material different physical depth on differently scaled brushes. The shader solves world-units-per-UV-unit per fragment from `dpdx(world_position) / dpdx(uv)` and takes the march axes from that same Jacobian, so the axes and the scale cannot disagree.
...
- **Player quality tier.** `PlayerOptions::surface_depth_quality` (`player_options.md` §4) is off/low/high and applies by **rewriting the per-material uniform buffer** with `queue.write_buffer` — never by rebuilding bind groups (gameplay allocates nothing, `resource_management.md` §8.2) and never by growing the group-0 `Uniforms` ABI. `Off` packs the historical all-zero second row, so the render is byte-identical to the pre-feature engine. `Low` keeps the carve depth and terracing (side faces and depth AO fall out of the primary march for free), caps the step count, halves the fade distance, and sets the self-shadow budget to zero. `High` is the per-prefix values verbatim. That budget therefore rides in the packed march word rather than a WGSL `const`: there is no shader-variant system (§8), so anything a tier must switch off has to reach the shader as data. The derivation is GPU-free and unit-tested in `postretro-render-cpu::{surface_depth, material_plan}`; the renderer only performs the write.
...
- **Depth is authored in meters, but the unit is a switch.** `SURFACE_DEPTH_TEXEL_MODE` (`crates/render-data/src/material.rs`, mirrored in `surface_depth.wgsl` and pinned by `shader_constants_match_the_cpu_reference`) selects whether `depth_meters` is a world distance or a count of albedo texels; each unit has its own authoring table and its own cap, and flipping the constant selects both. Meters keeps a material's physical depth constant however a mapper scaled the face. Texels keep the carve at a fixed depth in the lattice the retro look is built on, and make the march's worst case resolution- and scale-independent — horizontal travel is `N * tan(theta)` texels with the texel rate cancelling out, so a high-resolution texture on a small brush can no longer exhaust the step budget. Meters is the shipped default; which one actually looks right is unmeasured.
- **Every tuning dial is a guess.** Per-prefix carve depths, the AO strength, the LOD fade window, and the `Low` tier's step cap and fade scale were chosen to be plausible and bounded, not measured against a frame.
- **No cost measurement exists.** No `POSTRETRO_GPU_TIMING` run (§12) has ever included this march. Per-fragment cost at the §10 perf floor is unknown, and the step caps are budgets nobody has priced.
