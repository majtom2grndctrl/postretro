...
>
> **Premise correction vs. earlier planning notes:** facing *arbitration* already exists and is correct in the enemy-AI tick. Agents in `Alert`/`Attack` face their **velocity heading** while moving (XZ speed above `MOVE_SPEED_EPSILON`, the shared facing/locomotion epsilon) and face the selected target when stopped-but-engaged; `Idle`/`Death` facing is untouched. This spec does not change who the agent looks at. The gap is *how fast*: the chosen rotation is snapped into `Transform.rotation` in one tick.
...
- The facing block in `crates/postretro/src/scripting/systems/ai.rs` computes a target rotation via `yaw_rotation_toward` and writes it directly to `Transform.rotation`. No gameplay-side slew exists.
...
- `run_ai_tick` receives the fixed tick delta as `tick_dt`, in scope throughout the facing block, so the per-tick clamp is `FACING_TURN_RATE * tick_dt`.
...
- **Tuning constant.** `FACING_TURN_RATE` (rad/s) as a module constant beside `MOVE_SPEED_EPSILON`, following the steering-constant precedent.
- **Cross-spec constraint.** Facing must not lag the body's own movement: `FACING_TURN_RATE` ≥ the steering max turn rate (`agent_steering::MAX_TURN_RATE`, currently `std::f32::consts::TAU`), asserted by a unit test comparing the two constants. That test must reference the steering constant, so widen it from private to `pub(crate)` — the sole edit outside the facing block this spec allows.
...
In the existing facing block in `ai.rs`, keep the existing arbitration exactly as-is — `yaw_rotation_toward` still returns `Option<Quat>`, including its zero-length `None` guard. On `Some(target)`, extract the target yaw from that quat and the current yaw from `Transform.rotation` the same way (rotate `MESH_FORWARD` by the quat, `atan2` the XZ), apply `slew_yaw` with max delta `FACING_TURN_RATE * tick_dt` (`tick_dt` param of `run_ai_tick`, in scope in the block), and write back `Quat::from_rotation_y(slewed)`. On `None`, leave facing untouched, exactly as today — this is what keeps the zero-length guard tests green. Add `FACING_TURN_RATE` (rad/s) beside `MOVE_SPEED_EPSILON` — start at ~2–3× the steering max turn rate (the AC only pins the floor; final value is tuned in playtest). Widen `agent_steering::MAX_TURN_RATE` from private `const` to `pub(crate)` so the ≥-steering-turn-rate unit test can reference it — this one-line visibility change is the sole edit outside the facing block. Existing facing tests that asserted single-tick snap (in `ai_tests.rs`) are updated to loop the tick and drive to convergence, then assert; all other ai tests stay green unchanged.
