...
- **`postretro-ui*`** (cpu-only, `E19--ui`) — `render::ui` CPU subtree + `UiTexture`. Depends on `scripting-core` (descriptor model), `entities` (`SlotValue`, used across `tree/*`), `taffy`, `cosmic-text` (direct, for `FontSystem`/measurement — no `glyphon`). No `input`, no wgpu; `mlua`/`rquickjs` ride in transitively via `scripting-core` (Decision 13).
- **`postretro-render-cpu*`** (cpu-only, `E19--render-cpu`) — the harvest: CPU islands from `render/` (`frame_uniforms`, `mesh_instances`, `material_plan` CPU half, `fog_mask`, `lighting/chunk_list`, the CPU halves of `loaded_texture`/`sdf_*`/`sh_volume`/`sh_compose`/`animated_lightmap`/`screen_effects`; `splash` stays binary-side — it returns `UiTexture` and takes the binary `SplashSource`), the `fx::{smoke,fog_volume}` data, and the mesh/SH CPU types scripting imports. Carries WGSL binding constants with their packers. Depends down on `level-loader`, `level-format`, `render-data`, `visibility`, `entities`, and `model` (`gltf_loader::Submesh`/`ModelHandle`/`sample_params`, consumed by `material_plan`/`mesh_instances`) — the `render-cpu → model` edge is not a drawn row in the ASCII above, where `render-cpu` and `model` render as peers; `lighting` rides as a `[dev-dependencies]` edge (test-only, `light_space_matrix`).
...
- [ ] Each new **cpu-only** crate's `cargo tree` (default features) shows **no** `wgpu`, `winit`, `glyphon`, `kira`, `mlua`, or `rquickjs` — except `postretro-ui`, which pulls `mlua`/`rquickjs` transitively through `postretro-scripting-core` (Decision 13). (Manual/CI gate.)
...
- [ ] WGSL byte-layout contracts hold: `group3_shader_bindings`, `shader_tests`, and the `frame_uniforms` byte-layout tests (which travel to `postretro-render-cpu` with `FrameUniformData`) pass — any moved packer carries its binding-index/stride constants.
- [ ] No net-new `unsafe` (pre-existing `unsafe` travels with moved code).
- [ ] No `wgpu` call lands outside `postretro-renderer`. After the terminal spec, `wgpu` is absent from the `[dependencies]` of every non-renderer crate — the binary included — and `cargo build --workspace` passes, so none can name a wgpu type by construction; the binary's present loop names only the opaque handle (the dev-tools debug-UI interleave moves behind it too). (`rg wgpu` alone is not the gate — it trips on doc-comments/timing-label strings; use the Cargo.toml dep check plus `rg 'wgpu::'`.) See `E19--renderer-gpu`.
...
- **`FontSystem` borrow seam.** `postretro-ui` owns the `FontSystem` (`cosmic_text::FontSystem`, built by `build_font_system`); the renderer's `UiTextRenderer` borrows it `&mut` across the once-per-frame snapshot seam to prepare `glyphon` text. `postretro-ui`'s `cosmic-text` must stay version-unified with the `cosmic-text` `glyphon` pulls renderer-side, so the `FontSystem` type identity holds across the borrow. (Detail: `E19--ui` Task 3.)
- **WGSL byte-layout.** Binding-index/stride constants travel **with** their CPU packers into `postretro-render-cpu`; the `group3_shader_bindings`/`shader_tests` guards (renderer-side) and the `frame_uniforms` byte-layout tests (in `postretro-render-cpu`) must pass unchanged. Pin constants, not offsets (per `context_style_guide.md` — state the constraint, not the layout).
...
12. **`postretro-ui` text dependency: `cosmic-text` direct, not `glyphon`** — `postretro-ui` depends on `cosmic-text` directly for `FontSystem` and text measurement/shaping; `glyphon` does not appear in `postretro-ui` at all. `glyphon` pulls `wgpu` unconditionally, so any cpu-only crate that depends on it — even for a CPU-only re-export like `FontSystem` — transitively compiles wgpu, which is a hole in the firewall, not an exception to it. `cosmic-text` is pure CPU; `glyphon` wraps it with a GPU text atlas/renderer and stays renderer-only, which also satisfies "Renderer owns GPU." `postretro-ui` owns the `FontSystem`; the renderer's `UiTextRenderer` borrows it across the snapshot seam (see **Cross-boundary contracts**). (Found by the `E19--ui` re-grounding review; affects `E19--ui`/`E19--renderer-gpu`.)
13. **`postretro-ui` VM coupling — accepted, not firewalled** — `postretro-ui` depends on `postretro-scripting-core` for the descriptor model, which packages `mlua`/`rquickjs` non-optionally, so `postretro-ui`'s `cargo tree` shows the VMs. Accepted as a documented exception. Principle: lean, and defer abstraction to a measured need (cf. Decision 6). The UI runtime is VM-free by design — scripts declare, the VM drops after load, no per-frame callbacks (`ui.md`) — so the coupling is a packaging artifact of scripting-core, not a semantic dependency; the split's warm-edit win still lands, since the VM rlibs are cached upstream and never recompile on a UI edit. Gating scripting-core's VMs behind an off-by-default `script-ffi` feature, or extracting a VM-free descriptor leaf below it, stays available if the VM cold-build/CI cost ever measures as a problem — deferred, not forced. `cpu-only` here means wgpu-free; VM presence does not violate that label. (Found by the `E19--ui` implementability review; affects `E19--ui`. Reconciles the global-AC line with its pre-existing consumer carve-out.)

## Open questions

None remaining — all thirteen settled (see **Decisions**). Residual implementation-time confirmation (not a design question) lives in the specs: whether any remaining `postretro-lighting` packer references `render-data` geometry types. (`cone_frustum`'s home was a real design question — now decided: `render-data`, Decision 9. Whether `postretro-ui` needs an `entities` dep is likewise resolved — unconditional, `SlotValue`.)
