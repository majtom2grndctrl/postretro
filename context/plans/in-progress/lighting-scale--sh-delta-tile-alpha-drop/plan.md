...
| M1 Real-map disk/payload/storage size row | `campaign-test` plus resolved id-45 map; compiler summaries, dev-tools footprint log, and file sizes recorded in `research.md` | manual-measurement | partial: exact cold artifact and verbatim-upload sizes recorded; dev-tools log blocked by unrelated DiagnosticsTab compile error |
...
1. **Dev-tools storage log (M1/A7).** First resolve the unrelated existing
   `DiagnosticsTab::ALL` array-length compile error at
   `crates/renderer/src/render/debug_ui/mod.rs:1236`. On the same machine, launch the
   pre-change and branch `campaign-test.prl` artifacts with `--features dev-tools` and capture
