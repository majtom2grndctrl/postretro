...
| 4 | On resume: create window (**hidden**), init the **boot-ready** renderer (surface/device/queue/config + boot splash pass), enter Splash, request first redraw | Fires once on desktop; guarded against re-entry. The window is created hidden (`with_visible(false)`) and revealed only after the first splash frame presents (§ "Window reveal" below) — this kills the Windows pre-first-present white flash. Device creation already requests the FULL feature/limit set (wgpu features can't be added later). Adapter checks the boot splash needs run before the first splash draw; hard renderer requirements fail fast with a named message (`rendering_pipeline.md` §4). No audio, debug UI, net, mod, or level work in this path. |
...
- **First frame (splash-color clear).** Paint the first frame (no logo bound — a `SPLASH_CLEAR_COLOR` clear) into the still-hidden window. After it presents: record `first_black_frame`, **reveal the window** (`set_visible(true)` — see "Window reveal" below), then decode and upload the splash image (bounded CPU work, single-threaded). A missing or malformed splash asset warns and keeps the frame at the splash-color clear — never aborts boot. No mod, session, audio, net, or level work yet.
...
### Window reveal (no white flash)

The window is created **hidden** (`window_attributes()` → `with_visible(false)` in `main.rs`) and revealed (`Window::set_visible(true)`, via `App::reveal_window_after_first_present`) only on the **presented** branch of the first splash frame (`run_splash_frame_zero`, right after the `first_black_frame` mark). On Windows the OS paints a freshly-created window with its class background brush (white) before the app's first wgpu frame is on screen; that pre-first-present flash is what a default-visible window shows. Creating the window hidden and revealing it after the first successful present means the first visible pixels are the splash-color frame — no white flash. Window visibility is a winit/app-layer concern (no GPU call, no `unsafe`); the renderer stays the sole GPU owner. The reveal fires on the presented branch only — a transient `NeedsRedraw` returns before the reveal, so a present-retry never shows a white/blank window — and on **every** window creation, since suspend→resume resets to Booting and replays the splash loop from frame 0 (`set_visible(true)` is idempotent).
...
`StartupTimings` records ordered named stage marks (logged as `[Startup] …`). Boot marks in order: `args_parsed`, `event_loop_created`, `window_created`, `wgpu_init` (boot-ready renderer), `first_black_frame`, `splash_decoded`, `splash_uploaded`, `first_splash_frame`, `audio_init_complete`, `script_runtime_ctor` (post-first-pixel, inside `Session::build`), `net_endpoint_complete`, `session_init_complete`, `renderer_full_init_complete`, and (CLI-map boot) `boot_worker_dispatch`. `first_black_frame` / `first_splash_frame` are recorded only on the presented branch (after command submission + successful present); the window is created hidden and revealed on the same presented branch as `first_black_frame` (see "Window reveal"). `first_black_frame` precedes the net/audio/debug-UI/mod/level-worker marks **and** `script_runtime_ctor` — the script runtime is now built behind first pixels in `Session::build`.
