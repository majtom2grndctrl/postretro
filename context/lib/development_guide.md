...
**Fixed-tick gameplay lives in `postretro-sim`.** It owns navigation, triggers, combat resolution, and the scripting runtime/handlers that drive the headless simulation seam. Collision, movement, and kinematic movers live in the lower `postretro-physics` substrate. Enemy policy lives in `postretro-ai` above sim and is injected into the tick through sim's synchronous effects contract. The binary supplies data at that boundary; sim never depends upward on AI, binary input, UI, renderer, or application state.

**Target shape.** `postretro-combat-model` holds combat domain data — stat resolution, resources, augments, damage taxonomy — off the registry, plus shot-authority and carried-loadout data. `postretro-physics` owns collision, movement, and kinematic movers below `postretro-sim`. `postretro-sim` owns fixed-tick orchestration and exposes the synchronous AI-effects boundary; `postretro-ai` holds enemy policy above it; `postretro-netcode` depends on sim and has sole ownership of registry replication and authoritative network gameplay (`networking.md`). Normal dependencies point downward: netcode → sim → physics and lower crates, while only the binary composes sim with AI. Neither sim nor netcode depends on AI, the binary, renderer, UI, or input. The render stack reads entity state from `entities` directly, making sim a sibling to renderer; the binary orchestrates input → game logic → audio → render → present.
