...
- [ ] **A — UI render pass + thin vertical slice.** One hardcoded descriptor (9-slice panel + baked-bitmap-font text) drawn end-to-end through a real new `render/ui/` peer pass: design-resolution target with nearest upscale, pixel snapping, alpha-blended into the final color target after scene / before present (research §4, §13). Runs alongside the egui debug overlay, not replacing it yet. **Locks** — render-pass placement in frame order, the quad/9-slice pipeline shape, the design-res target, the once-per-frame published read handle, baked bitmap font as the default; **and pulled forward**: the Input-stage UI-dispatch seam plus the modal capture-vs-passthrough contract (research §4 — a UI click resolved on frame N must reach game logic on frame N+1; a frame-ordering contract, not an interaction feature); **and** the automated test strategy for a GPU-drawn UI — taffy layout-tree assertions below the draw call, and/or golden-image / headless draw-list snapshots — prove the harness here, not later. Narrow real version of each, built in place.
- [ ] **B — Descriptor model + retained tree + layout.** serde descriptor structs ↔ Rust enum variants, Rust-owned retained tree, taffy layout (research §5, §7), anchor + offset, dirty-tree relayout, core widget vocab (text / panel / image / vstack / hstack / grid / spacer, research §6). Built in Rust and tests, no script ingestion yet. **Locks (pulled forward):** the descriptor **wire format** as a first-class deliverable with a Boundary Inventory (Rust ↔ JS ↔ Luau ↔ wire casing) and the discriminated-union-per-kind decision (research §6, §15) — not deferred to the SDK goal. Reserves an abstract layout **measure** seam so glyphon shaped text (Goal D) slots in without a layout-engine rewrite: bitmap font has trivial advance widths, shaped text does not. Depends on A.
- [ ] **C — State system (the decoupling seam).** `defineState`, branded `StateValue<T>`, the slot table, engine-owned-readonly vs. modder-declared, the once-per-frame published read handle, value diffing for layout invalidation, clamp / validate (research §9). Ships the static proxy populating `player.health` / `player.ammo`. **Publishes the engine-owned slot schema as the contract Milestone 10 honors.** Owns the persisted-slot save **wire format** (`persist: true` slots, research §9) — a real format, not a one-word mention. Depends on B.
- [ ] **D — Text shaping + theming.** glyphon shaped text + glyph atlas, TTF font registration, the theme-token table; widgets reference tokens by name (research §8). Fills B's measure seam. Depends on C.
...
The spine A → B → C is sequential. Then D → (E ‖ F): D before E (token dependency), F concurrent with E, with the F/D layout-module merge-coordination noted. G1 converges D + E + F. Then BIS and SE (SE needs compositor coordination with Milestone 9/10). G2 trails. The deferred set stays detail-on-open.
