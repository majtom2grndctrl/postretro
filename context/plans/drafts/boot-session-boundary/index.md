...
- [ ] The entire `Session` is constructed in the post-first-pixel install path, not before the window. Net endpoint, audio, debug UI, and script runtime all build there. The post-first-pixel ordering of the audio/net/debug-UI marks relative to `first_black_frame` is unchanged from the predecessor.
...
- [ ] Suspend/resume commits session install exactly once: the install consumes a one-shot guard so a resumed boot does not rebuild an already-installed `Session`; resume re-presents black/logo and reruns renderer completion only.
...
**Single-commit guard.** Guard `Session` install with a one-shot consume (the predecessor's `take_once` on `pending_session` is the model): a resumed boot whose `app.session.is_some()` skips rebuild and reruns only renderer completion. This is the producer for the suspend/resume AC.
