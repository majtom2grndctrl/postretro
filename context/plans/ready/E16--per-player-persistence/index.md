...
5. After `Participating` has applied or defaulted the seed, the host marks that connection's seed as consumed. A post-apply `JoinSeed` is dropped, not applied. A demotion (level change) clears the consumed marker and the buffer entry, so a client re-entering parity can send a fresh seed for the next `Participating` transition.
