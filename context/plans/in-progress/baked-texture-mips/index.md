...
Move texture mip-chain generation out of the renderer and into `prl-build`. Mip pyramids bake once at compile time in linear color space, land in per-texture sidecar `.prm` files under `<workspace>/.build-caches/prm-cache/`, and upload directly at runtime. Zero CPU filtering at level load, gamma-correct mips that don't darken midtones, mips become content-addressed asset data shared across every map that references the texture.

## Scope

### In scope

- New `.prm` (PostRetro Material) sidecar file format. One file per unique (diffuse PNG content) bundle, content-addressed by `blake3(PNG content)`, stored under `<workspace>/.build-caches/prm-cache/<hex>.prm`. Cross-mod dedupe is intended: two mods that ship the same diffuse PNG bytes share one `.prm`. Bundles diffuse + specular + normal slots; each slot optional.
...
- `.build-caches/` is workspace-rooted, gitignored, regenerated on demand. Contains `prl-cache/` (existing stage-output cache, migrated here from the workspace root) and `prm-cache/` (new texture sidecars). The compiler writes both; the runtime reads only from `prm-cache/`.
...
- [ ] `prl-build` writes one `.prm` file per unique diffuse-bundle into `<workspace>/.build-caches/prm-cache/`, named `<blake3-hex>.prm`. Repeated builds with unchanged source PNGs skip mip-bake and file rewrite — the writer still re-hashes source PNGs and reads the existing `.prm` header's `bundle_hash` to compare.
- [ ] PRL carries a `TextureCacheKeys` section: one 32-byte blake3 per entry in `TextureNamesSection`, same ordering. PRL header `version` is 4. Loading a PRL with `version != 4` fails with `UnsupportedVersion`. Task 1's v3-reject test covers the common case.
...
- [ ] `downsample_2x`, `mitchell_netravali`, and `mip_level_count_for` are absent from the renderer crate after this plan lands; `upload_texture_data` uploads caller-supplied levels verbatim with no per-level transformation.
...
Add a `prm` module to `postretro-level-format` owning the `.prm` wire format: `PrmFile`, `PrmHeader` (parsed header fields), `PrmSlot` (per-slot data struct), `PrmSlots` (bitflags type for `slot_mask`), `PrmFormat`, `PrmReadError`, and a `STAGE_VERSION` constant. The parser API is `PrmFile::from_bytes_partial(data: &[u8]) -> (Result<PrmHeader, PrmReadError>, [Result<PrmSlot, PrmReadError>; 3])` — header errors are whole-file; per-slot errors are isolated so the runtime can keep cleanly-parsed slots and substitute placeholders only for the failing ones. Slot positions in the returned array follow the wire order: index 0 diffuse, 1 specular, 2 normal; absent slots are `Err(PrmReadError::NotPresent)`. Both the compiler-side writer (Task 2) and runtime-side reader (Task 3) import these types from level-format — the parser is not reinvented in the runtime crate. `PrmFile::to_bytes` / `from_bytes` follow the hand-rolled `Vec<u8>` / byte-slice convention used by other level-format sections (see `texture_names.rs`). No `postcard` dependency added to `postretro-level-format` (the `level-compiler` crate already uses postcard for its own purposes; that's untouched).
...
Add `texture_mips.rs` next to `texture_validation.rs`. The wire-format types live in `postretro-level-format::prm` (Task 1); the writer imports them. Entry point takes the deduplicated texture-name list, the texture root path, and the cache root (`<workspace>/.build-caches/prm-cache/`). For each name:

1. Resolve `<name>.png`, `<name>_s.png`, `<name>_n.png` via case-insensitive lookup (this task also lifts `build_name_to_path_map` from `crates/postretro/src/texture.rs` into `postretro-level-compiler` so the compiler can call it; Task 4 removes the now-dead runtime copy).
2. Compute `key = blake3(diffuse_png_bytes)` if diffuse exists. Otherwise `key = blake3([bit_index_byte] || first_present_png_bytes)`, where `bit_index_byte` is `0x01` for specular-only or `0x02` for normal-only. The tag byte prevents collisions between specular-only and normal-only textures that share PNG bytes. Slot probe order: diffuse → specular → normal. Hash inputs are raw on-disk PNG file bytes — no decode/re-encode normalization. The blake3 reflects the file, not the decoded pixels. If all three slots are absent for a name, return `[0u8; 32]` as the key and emit no `.prm` file; the runtime's zero-key skip path (see `TextureCacheKeysSection` body) handles it.
...
4. Otherwise build mip chains per slot in linear `f32`, encode per *Wire format* below, write atomically — tempfile name `<hex>.prm.tmp.<pid>` in the same `prm-cache/` directory as the final file (per-process suffix avoids collisions if `prl-build` is ever invoked concurrently), then `std::fs::rename` for cross-platform atomic replacement. Write or rename errors surface as build failures, not warnings.
...
`UiTexture` carries `{ data, width, height }`. `splash::load_splash` (`render/splash.rs:22`) returns `Result<UiTexture>`; `splash::upload_splash_texture` (`render/splash.rs:51`) and `install_splash_from_loaded` accept `&UiTexture`. The two `install_splash_from_loaded` call sites in `crates/postretro/src/main.rs` (lines 1220 and 1289) thread `UiTexture` through. `LoadedTexture` becomes a thin wrapper over the parsed `.prm` slot data + GPU handles (per the bullet below). The magenta-checker placeholder remains in the `LoadedTexture` family (it's a world-material fallback, not UI) with `mip_count = 1`.

- Import `PrmFile::from_bytes_partial` from `postretro-level-format::prm`. The header result drives file-level fallback; the per-slot `[Result<PrmSlot, PrmReadError>; 3]` array drives per-slot fallback. Cleanly-parsed `PrmSlot`s expose `(format_tag, width, height, level_count, payload)` for upload.
...
- Upload per level via `Queue::write_texture` with `bytes_per_row = bpp * level_width`, `rows_per_image = level_height`; the `.prm` payload bytes upload slice-by-slice. Source-side row stride has no alignment requirement for `write_texture`, so no staging buffer is needed.
- `LoadedTexture` becomes a thin wrapper over the parsed `.prm` slot data plus GPU handles, living in `crates/postretro/src/render/loaded_texture.rs`. The move preserves `context/lib/index.md` §2's "Renderer owns GPU" invariant: this plan adds GPU handles to `LoadedTexture` (it carried only CPU-side bytes before), and putting the type inside the renderer module keeps those new handles from leaking into `ui_texture.rs` (which is CPU-only and now UI-only). The `is_placeholder` field is preserved (load-bearing for sibling-probe skipping in the pre-rename `texture.rs:184,224`; the probe logic follows `LoadedTexture` into the new file).
- `load_textures` rewrites: for each entry in `TextureNamesSection`, look up the blake3 from `TextureCacheKeysSection`, open `<workspace>/.build-caches/prm-cache/<hex>.prm`, decode, upload each level per slot. The texture-root PNG scan disappears from the runtime. (`texture_names` is `Vec<String>`; the runtime's current `Vec<Option<String>>` wrap at `startup/worker.rs:80-84` drops away.)
...
- Delete the PNG-fixture-driven tests from the pre-rename `crates/postretro/src/texture.rs`: the five `build_name_map_*` tests (which follow `build_name_to_path_map` into the compiler), `load_textures_loads_matching_pngs`, `load_textures_case_insensitive_match`, `load_textures_missing_produces_checkerboard`, `load_textures_none_entry_produces_checkerboard`, `load_textures_corrupt_png_produces_checkerboard` (line 444), `load_textures_empty_names_produces_empty_set` (line 483), the `None` slot in `load_textures_preserves_index_order`, and the eleven sibling-probe tests at lines 528–718. 22 of 27 tests in the file go (5 `build_name_map_*` + 6 `load_textures_*` + 11 sibling-probe). The four `checkerboard_*` tests at lines 290–328 move with the placeholder logic to `crates/postretro/src/render/loaded_texture.rs`. Replacement: `.prm`-fixture tests built via a `#[cfg(test)]` helper module in `postretro-level-format::prm` that emits in-memory `.prm` byte blobs.
...
- Migrate the workspace stage cache: change the hard-coded default-dir construction in `crates/level-compiler/src/main.rs` (comment at line 113, `.join(".prl-cache")` at line 129) from `<workspace>/.prl-cache/` to `<workspace>/.build-caches/prl-cache/`. `cache.rs` is untouched — `StageCache::new` already takes a generic path. Update `.gitignore` from `.prl-cache/` to `.build-caches/`. `--cache-dir` semantics unchanged (still overrides the stage cache root only).
...
- Workspace-rooted cache (`<workspace>/.build-caches/prm-cache/`) is safe across mods because the filename is `blake3(PNG content)`: two mods shipping different PNG bytes under the same texture name produce different filenames; two mods shipping identical PNG bytes dedupe to one `.prm`. Per-mod isolation in the directory layout is unnecessary.
...
[u8; 32] bundle_hash                           -- blake3 over `slot_mask` byte followed by `(bit_index_byte, source_png_file_bytes)`
                                               ...
                                               -- `source_png_file_bytes` are raw on-disk PNG bytes (matches Task 2 step 2).
                                               -- The filename key (Task 2 step 2) and `bundle_hash` are intentionally distinct.
                                               -- Filename is the addressing key (cheap, diffuse-content-only).
                                               -- `bundle_hash` is the cache-validity fingerprint (covers every present slot + slot_mask).
                                               -- They never compare equal even for diffuse-only bundles.
...
-- Per-slot block header is 12 bytes (1 + 1 + 2 + 2 + 1 + 1 + 4), followed by `payload_bytes` of level data.
                                               -- Block order follows slot_mask bit index (0=diffuse, 1=specular, 2=normal), NOT format_tag numeric order; the two indices are independent encodings.
...
- Return `PrmReadError::Truncated` if the byte slice ends mid-header or mid-payload before all declared bytes are consumed.
- Reject if `width > 4096 || height > 4096` (per-slot) with `PrmReadError::DimensionTooLarge { slot: u8, width: u16, height: u16 }`. Matches the engine's working texture size cap and prevents `u32` overflow in `payload_bytes` arithmetic for tag-0/tag-1 (4 bytes/texel).
- `bytes_per_pixel`: tag 0 → 4, tag 1 → 4, tag 2 → 1.

`PrmReadError` variants: `BadMagic { found: [u8; 4] }`, `UnsupportedVersion { version: u8 }`, `StageVersionMismatch { expected: u8, found: u8 }`, `ReservedSlotBitsSet { mask: u8 }`, `DimensionTooLarge { slot: u8, width: u16, height: u16 }`, `LevelCountMismatch { slot: u8, expected: u8, found: u8 }`, `PayloadBytesMismatch { slot: u8, expected: u32, found: u32 }`, `TotalBodyBytesMismatch { expected: u32, found: u32 }`, `Truncated`, `NotPresent`, `Io(std::io::Error)`. `Io(std::io::Error)` is reserved for the loader layer that wraps `std::fs::read` before parsing; the byte-slice parser itself never produces it.

Level load never fails on a `.prm` parse error. The runtime calls `PrmFile::from_bytes_partial` and inspects the header result and the three per-slot results separately. Header errors (bad magic, version mismatch, stage_version mismatch, total_body_bytes mismatch, truncated header) fail the entire `.prm` — the runtime substitutes per-slot placeholders for all three slots and logs one `warn!` naming the texture and the header failure. Per-slot errors (level_count or payload_bytes arithmetic mismatch, dimension-too-large, truncated slot payload) fall back to that slot's placeholder; other slots that parsed cleanly are used; each failing slot logs its own `warn!`.
...
- A new helper `resolve_prm_cache_root(map_path: &Path) -> PathBuf` returns `cache::find_workspace_root(map_path).join(".build-caches").join("prm-cache")`; it lives next to `resolve_texture_root` in `main.rs` and reuses the workspace-root walk from `cache.rs`. `pack.rs` gains a `texture_root` and `cache_root` argument; the compiler threads `resolve_prm_cache_root`'s return value in. The existing `--cache-dir` / `--no-cache` CLI flags continue to govern only the stage cache (`.build-caches/prl-cache/`); they do NOT affect `.prm` output, which always lives at `.build-caches/prm-cache/`.
- The compiler's `cache.rs` `StageCache` mechanism is unused for textures — the on-disk `.prm` file (content-addressed filename) IS the texture cache. Existence + parse-success at `<cache>/<hex>.prm` is the cache hit signal; the `bundle_hash` field in the header is the input-fingerprint check for the all-slots-unchanged path. The stage-cache default directory (constructed in `crates/level-compiler/src/main.rs:113,129`) migrates from `<workspace>/.prl-cache/` to `<workspace>/.build-caches/prl-cache/` so all build caches share one root. `cache.rs` itself stays generic (`StageCache::new` accepts any path).
- Crate split for the `.prm` wire format: `postretro-level-format::prm::{PrmFile, PrmSlot, PrmFormat, PrmReadError}` is the surface area shared by writer (level-compiler) and reader (postretro runtime). `postretro-level-compiler` is binary-only (no `[lib]` target), so the level-format crate is the only viable shared home. `postretro-level-format::prm::STAGE_VERSION` (`u8`) is the canonical constant; `texture_mips` imports it directly. No re-export. The `u8` width diverges from `build_pipeline.md` §Build Cache's `u32` `STAGE_VERSION` convention; the `.prm` header packs tightly and owns its own version semantics.
- The global `base_sampler` field at `render/mod.rs:521` is removed; the `mip_count_samplers` pool replaces it.
- Runtime loader (`crates/postretro/src/startup/worker.rs::run_worker`): replace the texture-decode block at lines 73–88 (the `texture_root` assignment is at line 75) with `prm_cache_root = content_root.parent().unwrap().parent().unwrap().join(".build-caches").join("prm-cache")` (two parents up from `content/<mod>/` lands at `<workspace>/` in the dev layout; shipping layouts are out of scope per *Out of scope*). Pass `&world.texture_names` (`Vec<String>`) and `prm_cache_root` to the new `load_textures` signature (now `crates/postretro/src/render/loaded_texture.rs::load_textures`). Drop the `Vec<Option<String>>` wrap at lines 80–84 — the runtime previously synthesized `None` for missing PNGs; with the `.prm` upload path every entry corresponds to a present `String`. `TextureNamesSection` itself is unchanged.
- `.gitignore`: update line 27 from `.prl-cache/` to `.build-caches/` (covers both `prl-cache/` and `prm-cache/` subfolders at any depth). Confirm at promotion.

## Open questions

- **Cache directory deletion as a recovery step.** Users hitting `warn!` logs naming a texture should `rm -rf <workspace>/.build-caches/prm-cache/` (sidecar cache only — the sibling `prl-cache/` is stage-output cache governed by `--cache-dir`) and rebuild. Document at promotion time in `build_pipeline.md`.
