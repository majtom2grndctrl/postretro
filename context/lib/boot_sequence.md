...
| Active per-level reactions/crossings (`data_registry.reactions` / `crossings`), accumulator bindings, and level-scope UI trees | Level unload. Active sets recompose on the next level load from current globals plus that level's catalog tags. |
