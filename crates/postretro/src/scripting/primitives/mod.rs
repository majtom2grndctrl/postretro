...
pub(crate) fn register_shared_types(registry: &mut PrimitiveRegistry) {
    ...
    registry
        ...
        .doc("First-person weapon position in metres from screen center. `right`, `up`, and `forward` map to camera-space +X, +Y, and -Z respectively. Omitted fields default to 0; omitting the whole placement keeps the legacy BASE_OFFSET.")
    ...
    registry
        ...
        .doc("Authored first-person weapon placement. Position is in metres from screen center and rotation is in degrees. Omit `placement` on a weapon to preserve the legacy BASE_OFFSET with zero rotation.")
    ...
    registry
        ...
        .field("placement?", "WeaponPlacementDescriptor", "Optional first-person placement. Position uses metres from screen center (right/up/forward map to +X/+Y/-Z) and rotation uses degrees. Omit it for the legacy BASE_OFFSET and zero rotation; it never changes the third-person hand socket.")
