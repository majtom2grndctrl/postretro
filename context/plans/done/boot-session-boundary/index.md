...
- [ ] Each `ApplicationHandler` event method and frame handler obtains session state through at most one `self.session.as_mut()` (or `as_ref()`) destructure at entry. No `session_mut()`-style accessor is introduced. Two-part gate: a mechanical grep asserts zero `session_mut(` tokens; the at-most-one-destructure-per-handler check is manual review (grep can count `self.session.as_` per file but cannot scope per handler body).
...
**Failure path.** On `Err` from `Session::build`: store the error in `exit_result`, log it, request `event_loop.exit()`, and early-return from `run_splash_frame_one` so no later step that frame (renderer full-init, frontend/loading transition) runs against a `None` session — mirroring `finish_renderer_full_init`'s failure handling. This means `install_pending_session` and `PendingSessionInit::install` (today returning `()` and called bare in `run_splash_frame_one`) change to return a failure signal (`Result`/`bool`) the caller checks; place the `Session::build` call and its check ahead of the existing `finish_renderer_full_init` check. Subsequent redraws observe `BootState` unchanged until the loop exits.
