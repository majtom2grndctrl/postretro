...
- [ ] **Dynamic mesh shadow casting** — NPC meshes render into the existing real-time dynamic shadow pool (the 12-light pixmap budget in `lighting/spot_shadow.rs`, which today renders only static world geometry), via a skinned-depth variant of the mesh pass. Independent of the in-flight static-lighting (SDF) rewrite — the pool is stable. *Can defer* — not on the walk/attack/die critical path — but the pass is built depth-reusable from day one, so deferring costs nothing later. Grounds enemies visually.
