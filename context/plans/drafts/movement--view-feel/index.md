...
- Sway: continuous, non-periodic yaw/pitch wander from summed incommensurate sines, base amplitude scaled up by movement speed.
...
- A single engine-side global view-feel scale (0–1, default 1.0) multiplying all output — the accessibility seam for the future M13 settings menu.
...
**D6 — Accessibility scale is a seamed engine value, not a descriptor field.** Per-class feel is the author's job (descriptor); the player's comfort override is a single runtime scale in `[0,1]`, default 1.0, multiplying every motion's output. It ships as one named engine value (hardcoded-but-seamed) with no UI; the M13 settings menu drives it later. `scale = 0` zeroes all view feel — the off switch.
...
- [ ] Bob output is zero when horizontal speed is at or below `speedThreshold`, and its amplitude increases with horizontal speed above the threshold (self-gating verified).
...
- [ ] Sway output is bounded by its effective amplitude, is non-zero while moving, and its effective amplitude increases with speed when `speedScale > 0` (and is constant when `speedScale = 0`).
- [ ] The global view-feel scale multiplies all output: at `scale = 0` bob, tilt, and sway all produce zero offset/roll regardless of velocity; at `scale = 1` they produce the unscaled values.
...
- [ ] Descriptor parsers reject invalid fields symmetrically in both JS and Luau with matching field-path error text: `bob.frequency`, `tilt.speedReference`, `tilt.tension`, and `sway.frequency` reject missing/non-finite/non-positive (zero rejected); `bob.verticalAmplitude`, `bob.lateralAmplitude`, `bob.speedThreshold`, `sway.amplitude`, and `sway.speedScale` reject missing/non-finite/negative (zero allowed); `tilt.maxAngle` rejects missing/non-finite/out-of-`[0, 90]`.
...
| bob cycle rate | `frequency: f32` | `frequency` | `frequency` | `frequency` | n/a |
...
## Open questions
- **Bob frequency units.** Spec'd as distance-phased (phase advances with horizontal distance), so `frequency` reads as cycles per unit travel and is self-gating. An alternative is cycles-per-second scaled by `speed / ground.speed.run` (couples view feel to the run-speed field). Distance-phased is chosen for decoupling; revisit if authors find the unit unintuitive.
- **Sway axes.** v1 perturbs yaw + pitch only. A faint roll-sway component (distinct from tilt's velocity roll) could add to the creature feel later — additive, no field-set change beyond an optional axis weight. Deferred.
- **Global scale → settings UI.** The scale seam ships at default 1.0 with no UI. The M13 settings menu owns the player-facing slider and any per-motion breakdown (separate bob / tilt / sway sliders, matching DOOM TDA). Flagged so M13 wires the existing seam rather than re-deriving it.
