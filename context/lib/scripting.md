...
**Compile-time light membership.** For both QuickJS and Luau map data scripts, the build evaluates `setupLevel` against the map lights, derives targets of `setLightAnimation` across returned reactions, and stubs runtime-only primitives. Static targets reserve animated baked-light membership; dynamic targets remain runtime-only. Curves remain a runtime contract and are never baked.
