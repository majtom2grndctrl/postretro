...
impl MovementScope {
    ...
    pub(crate) fn refresh(&mut self, component: &PlayerMovementComponent, elapsed_ms: f32) {
...
mod tests {
    ...
    #[test]
    fn refresh_allocates_nothing() {
        // The per-tick snapshot refresh runs on the movement hot path; it must not
        // allocate. Uses the IR substrate's alloc-probe harness.
        let component = seeded_component();
        let mut scope = MovementScope::for_validation();
        // Warm any one-time lazy state before arming the probe.
        scope.refresh(&component, 0.0);

        let snapshot = AllocSnapshot::arm();
        scope.refresh(&component, 25.0);
        let allocs = snapshot.allocs_since();

        assert_eq!(
            allocs, 0,
            "snapshot refresh must perform zero heap allocations"
        );
    }
}
