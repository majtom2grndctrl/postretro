...
- [ ] **glTF mesh loading** — runtime loader: read glTF mesh geometry and skinning vertex attributes (positions, normals, UVs, joint indices, weights) via the `gltf` crate into engine-side structs and GPU buffers in the spike's locked layout. Material textures resolve to their `.prm` equivalents through the existing texture pipeline (external PNG reference → `blake3` cache). `extras` metadata is read here and carried onto the entity. Renderer consumes handles, never raw glTF. Skin weights ride in the vertex buffer unused until the skeleton lands. Shape ≈ the runtime-loader half of *Baked Texture Mips*, without the bake stage.
...
- [ ] **glTF skeleton + clip loading** — extend the loader to read the joint hierarchy, inverse-bind matrices, and animation clips into engine-side structs. Feeds only the animation runtime, so it is deferred to here — the mesh pass lands on static loading alone. Smaller than the mesh loader; keyframe sampling itself lives in the animation runtime.
...
- [ ] **Enemy entity + damage surface** — minimal enemy primitive: spawnable, carries HP, consumes the weapon's `DamagePayload`, dies at zero HP. Pure Milestone 6 entity model and damage events — no render, nav, or AI dependency. Shootable as a static proxy, so it gives the shipped weapon a target the day it lands.
