...
- [ ] Staged hot reload replaces the mod-global reaction set atomically and recomposes the active set against the current level's tags; failed and stale staged results are a no-op, preserving both the prior global definitions and the active set.
...
- [ ] The script-syntax example type-checks against the regenerated SDK typedefs, and `scopeReactions(tags, list)` returns each reaction with `levels` set to `tags`.
- [ ] No new `unsafe`.
