...
**Variable base-probe density.** PRL v10 irradiance data and its direct-SH companion use one stored-atlas geometry for their at-rest and composed data. Each 4×4×4 probe brick stores its valid L0 probes, eight L1 corner slots, or one L2 mean tile. Bake-time classification chooses the level, preserves detail near protected or high-error regions, and never stores base data more coarsely than a present grid-matched id 27, id 41, or id 45 delta section with an entry in that brick. Compose writes only stored slots. The shared sampler resolve reconstructs each requested probe from its L0 tile, L1 corners, or L2 mean; world, entity, billboard, fog, and diagnostics therefore see the same irradiance field. Load-derived probe metadata supplies validity, density level, and stored location while retaining probe-depth visibility data. Laptop shared-memory iGPUs are the binding smoothness floor, below §10's desktop performance floor.
...
**Target hardware.** The renderer targets mid-2020 mid-range discrete GPUs — the envelope the lean wgpu pipeline is built toward. **Perf floor** (must hold an acceptable framerate): NVIDIA GTX 16-series (Turing, e.g. GTX 1660 Super). No RT cores at this tier, so SDF shadows sphere-trace in compute (§4) and hardware ray tracing stays a non-goal (§13). **Compatibility floor** (must run, not perf-tuned): AMD Radeon Pro 5500M-class (RDNA1, the 2020 16-inch MacBook Pro discrete GPU) on the Metal backend; a live-tunable quality panel (dev-tools) explores settings on this class. Perf-gated renderer decisions — SDF shadow budgets and the like (§4) — are measured against this envelope; measured per-pass numbers live with the `POSTRETRO_GPU_TIMING` diagnostics (§12), not here. *(Variable base-probe density sets a lower binding floor for its own footprint work: laptop shared-memory iGPU smoothness, below this desktop perf floor — an owner-stated divergence for that feature, not a change to the renderer-wide target. See §4 "Variable base-probe density".)*
