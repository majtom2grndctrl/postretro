...
There is one `WieldableTuningPayload` (`tuning_payload.rs:21-29`) per inventory slot, and each gains exactly two scalar fields, `magazine: u32` and `reserve: u32`, describing that slot. `WieldableTuningPayload` and `TuningPayload` (`crates/postretro/src/netcode/tuning_payload.rs`) derive serde `Serialize`/`Deserialize`, and `encode_tuning_payload`/`decode_tuning_payload` use serde_json, so fields are name-keyed and field order is irrelevant — there is no positional tagging to append after. The payload is an opaque `Vec<u8>` inside `ServerControlMessage::Tuning`, so this changes no envelope — only the blob's inner shape. A shape change is gated by `TUNING_PAYLOAD_EPOCH`, checked in `decode_tuning_payload`, which is documented as independent of the bitcode wire version; bumping it is required, and an unbumped change fails at `decode_tuning_payload`, not at admission.
...
|---|---|---|---|
| I1 — A seat is released only by hold expiry; no exit from participating, including a close, releases it | Task 2 (seat table), Task 7 (release path) | `networking.md:98` clears all slot state on *any* exit from participating, and a level change is now such an exit; a release rule keyed to "leaves participating" releases every seat at every level change; a close starts a hold rather than a release | AC-SEAT-2, AC-CARRY-1 |
...
| I5 — The roster is rebuildable from the client-asserted player ids alone | Task 6 | Session id and seats are host-minted and cannot survive a host change (`plans/roadmap.md:202`); a roster deriving anything from a seat that is not also derivable from its player id breaks this | AC-ROSTER-2 |
