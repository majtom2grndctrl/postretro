...
- **Code reviewers:** 2 `worker` agents with `model: "gpt-5.5"` and `reasoning_effort: "high"`
- **Comment drift checker:** 1 `worker` agent with `model: "gpt-5.5"` and `reasoning_effort: "medium"`
- **Total:** 3 agents in parallel

Override with arguments:
- `reviewers:3` — run 3 code review agents instead of 2
- `effort:medium` — use GPT 5.5 medium reasoning effort for code reviewers instead of high

Comment drift checker: always 1 `worker` agent with `model: "gpt-5.5"` and `reasoning_effort: "medium"`, unaffected by overrides.
...
- `effort:high|medium` — reasoning effort for code review agents (default: high)
...
Full content of `.Codex/skills/code-review/SKILL.md` as instructions, plus review target. Use `model: "gpt-5.5"` with the specified reasoning effort (default: high).

**Comment drift agent (1 instance, always GPT 5.5 medium reasoning effort):**
...
**Panel:** N code reviewers (reasoning effort) + 1 comment drift checker (medium effort)
