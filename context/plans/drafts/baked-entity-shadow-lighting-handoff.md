# Handoff — baked-light entity shadows & entity lighting (post-M10)
...
>
> **Origin:** a working session reviewing the M10 dynamic-mesh-shadows branch (`claude/dynamic-mesh-shadows-jvl96j`). The review and its fix-ups are committed. These are the *forward* plans the conversation surfaced.

## The mental model you arrived at

There are **two orthogonal capabilities** an entity (enemy mesh) can have with respect to a light, and M10 only built one of them:

1. **Entity → world shadow** (enemy throws a shadow on the floor) — **shipped by M10**, dynamic lights only.
...
- **What:** wire the Milestone-5 flat per-fragment runtime-light loop into the skinned-mesh shader, filling the reserved group-2 slot (`render/mesh_pass.rs` group 2 = "SH ambient + dynamic direct"), so dynamic lights illuminate entities on top of their baked SH indirect.
...
- **What:** once group 2 opens the per-light term, attenuate it per dynamic light by sampling that light's existing shadow map — M10's spot 2D-array (`lighting/spot_shadow.rs`) and point cube-array (`lighting/cube_shadow.rs`) pools — but consumed from the **entity** shader side rather than only the world shader. Gives crisp **world→entity** and **entity→entity** shadows.
- **Why:** purely additive on top of group 2 + M10's pools; depends on group 2 (nothing to attenuate before then). This was the genuinely *unhomed* half — the illumination half already had `roadmap.md:207`; the receipt half did not, so you added it.
...
1. **Stage 1 — Path A hybrid.** Budget-free, reuses sdf term + M10 pools, no texture work. Proves the authoring model (`_shadow_type sdf` + `_cast_entity_shadows` on a fixed light) and the fixed-light entity-shadow plumbing.
...
3. Should the M10 "physically impossible" wording be **corrected** (it's an overstatement) or left as a deliberate simplification steering authors to dynamic lights?
...
- `crates/postretro/src/lighting/mod.rs:60` — `entity_occluder_eligible = casts_entity_shadows && is_dynamic` (the M10 gate).
...
- M10 pools: `lighting/spot_shadow.rs`, `lighting/cube_shadow.rs`; the static-depth cache is M10 Task 7.
