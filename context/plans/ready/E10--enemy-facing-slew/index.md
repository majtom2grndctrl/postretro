...
In the facing block (`ai.rs:573-607`), derive the target yaw from the existing arbitration's chosen direction (the same `atan2` XZ extraction, applied to the arbitrated direction rather than the current quat), extract current yaw from `Transform.rotation` (rotate `MESH_FORWARD`, `atan2` XZ), apply `slew_yaw` with `FACING_TURN_RATE * tick_dt` (`tick_dt` param of `run_ai_tick`), and write back the yaw quat. Add `FACING_TURN_RATE` beside `MOVE_SPEED_EPSILON` with the ≥-steering-turn-rate unit test (which references `pub(crate)` `agent_steering::MAX_TURN_RATE`). Existing facing tests updated only where they asserted single-tick snap.
