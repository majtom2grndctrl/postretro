...
| `switch` | brush | Visible, solid, pressable brush; compiles as static world geometry **and** a `use`-activation trigger volume grown past the brush's open faces by `use_reach`; the geometry never moves | `name`, `use_reach` (press margin per open face, map units; default 24 ≈ 0.61 m; must be > 0 and ≤ 128, outside that range is a compile error), `target_tag`, `on_fire`, `on_exit`, `command` (`start`/`stop`/`reverse`/`go_to_path_node`), `command_arg`, `fire_mode` (`once`/`multiple`), `rearm_ms`, `enabled_on_spawn`, `_tags`. No `activation` — always `use`; an authored one warns and is discarded |
...
- **`switch`** — compile-time sugar over two shipped mechanisms; no runtime type, no PRL addition. The same brushes fold into the static world brush set (the switch renders, takes baked light, and collides like worldspawn brushwork) *and* resolve to a trigger volume with `activation` forced to `use`, whose AABB then grows by `use_reach` on each face whose adjacent space is open. Growing is part of the mechanism: use-activation is a capsule/AABB intersection test and the brush is solid, so the volume must reach past the switch face into the space the player occupies. Testing each face's adjacent space is what keeps it from reaching *through* a wall — the intersection test measures from the capsule centerline, so a face's effective reach is the margin plus the capsule radius, which outreaches a standard wall and makes a uniformly grown volume pressable from the room behind. A wall-flush switch therefore grows no rear margin, and one enclosed on every face grows none at all and warns. Accepted residual: only the space immediately against a face is probed, so a switch floated off a wall still grows across the gap behind it. Because a switch *is* a trigger volume at the component layer, `trigger_volume` component queries also return switches.
