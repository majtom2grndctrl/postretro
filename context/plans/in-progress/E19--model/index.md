...
- Depend on `postretro-render-data` (required — `mesh.rs` uses `cone_frustum::Aabb` for `SkinnedMesh` bounds), `postretro-level-format` (features `["gltf-resolve"]` — `gltf_loader.rs` uses `gltf_resolve::resolve_material_base_color_path` and `octahedral`), `glam`, `gltf`, `serde`, `serde_json`, `blake3`, `thiserror`, and `bytemuck`. No `postretro-entities`/`postretro-scripting-core` dep — the only file that pulled them (`animation_reactions.rs`) relocates out (see Decision), keeping the crate a VM-free CPU leaf.
