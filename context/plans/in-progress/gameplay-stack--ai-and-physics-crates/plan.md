...
- Post-split test-name set: 7,249 unique names. After normalizing only the intentional `scripting_systems::ai::` removal and `kinematic_mover::commands::` → `mover_commands::` relocation, all 7,246 baseline identities remain and three proofs were added; the implementation diff contains no `#[ignore]` change.
...
| A8 test-name superset | Capture post-split `cargo test --workspace -- --list`, normalize to unique names, and `comm -23 baseline post` expecting empty output; compare ignored markers separately | passed — 7,246 retained, 3 added, 0 missing, no ignore diff |
