...
**Contracts first.** A few decisions here are expensive to reverse: the vertex format (rigid + skinned), the model bake container (made extensible like `.prm`'s `format_tag`), the mesh render-pass shape (reusable in the depth/shadow pass), and the `MeshComponent` entity surface. Design these to the endpoint now. A front-loaded spike locks them before the production bake exists. The render path is **skinned-capable from the start**; a rigid mesh is the degenerate single-bone case, so future props, pickups, and weapon viewmodels reuse the same pass without a separate tier.
...
- [ ] **Model pipeline spike** — prove the full risky path end-to-end with hand-authored data: one skinned mesh, animated, SH-lit (binds the shared `sh_sample.wgsl` group), casting a CSM shadow, culled through a portal. Throwaway code; the deliverable is locked durable contracts — vertex format (rigid + skinned), bone-palette layout and limits, mesh-pass shape, bake-container shape — captured in `context/lib/` (`rendering_pipeline.md`, `build_pipeline.md`, `entity_model.md`). Front-loaded because everything downstream rides on these decisions.
- [ ] **glTF model bake** — prl-build stage: glTF → a baked model container (mesh, skin weights, skeleton, animation clips), `format_tag`-extensible from day one like `.prm`. Source glTF stays the authoring source of truth; conversion is implicit during prl-build. The biggest single chunk. Shape ≈ *Baked Texture Mips* (new container + new bake stage + runtime loader). Consumes the spike's locked format.
- [ ] **Mesh render pass + `MeshComponent`** — new render pipeline drawing per-entity meshes at the interpolated transform, SH-lit, portal/frustum-culled (camera-leaf lookup locates the entity cell). Skinned-capable: binds a bone-matrix palette, renders in bind pose for now. `MeshComponent` carries a model handle; classname wiring spawns mesh-bearing entities from a map. Pass designed reusable for the depth/shadow path (`depth_prepass.wgsl` precedent). Shape ≈ *Emitter Entity* (component + render integration + classname routing), minus its reuse of the billboard pass — this pass is genuinely new.
...
- [ ] **Skinned animation runtime** — per-frame clip sampling → bone-matrix palette upload; a shallow animation state surface (idle / locomotion / attack / death with crossfade). Engine owns sampling and palette; the state surface is small and script-drivable. Shape ≈ *Animated SH Volumes* (format + runtime + shader, one multi-task plan). Depends on the bake (clips) and the mesh pass (palette binding).
...
- [ ] **Enemy entity + damage surface** — minimal enemy primitive: spawnable, carries HP, exposes a hittable volume the weapon raycast resolves against, consumes the weapon's `DamagePayload`, dies at zero HP. Builds on the Milestone 6 entity model and damage events; no navigation, no rendering dependency for the logical surface (a `MeshComponent` makes it visible once the render track lands). Gives the shipped weapon a live target.
...
- [ ] **Enemy AI behavior** — simple state machine (idle → alert → attack → death), authored in the SDK as a reference behavior. Drives navigation (move toward player), attack (emit a damage hit at the player), and animation state (select the clip per logical state). The convergence point: depends on the enemy entity surface, the navigation foundation, and the skinned animation runtime. A foundation to refine, not a stub.
