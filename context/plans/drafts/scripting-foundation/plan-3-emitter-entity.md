...
| `sdk/lib/` | New files: `emitter.ts`, `particles.ts`. Luau ports (`emitter.luau`, `particles.luau`) are the final sub-plan. |
...
**Scope:** Two hand-authored TypeScript files under `sdk/lib/`. No primitives called from vocabulary beyond what Plans 1, 2, 3 register.

### Files

#### `sdk/lib/emitter.ts` — `emitter()` component constructor
...
```typescript
// Proposed design — ships as sdk/lib/emitter.ts
defineEntity({
  components: [
    emitter({
...
#### `sdk/lib/particles.ts` — pre-built emitter configurations
...
3. Place under `sdk/lib/` — the SDK-source directory Plan 2 established.
...
- [ ] `emitter.luau` and `particles.luau` exist under `sdk/lib/`.
