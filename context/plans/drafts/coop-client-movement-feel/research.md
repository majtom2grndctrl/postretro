...
`drop_stale` (`:290`) retains only commands strictly newer than the cursor; `enqueue`'s stale check (`:255`, `client_tick_le`) drops any later arrival at or below the cursor. Input is drained once per render frame (`net_poll_and_apply` → `host_handle_client_messages` → `ingest` at `netcode/mod.rs:1745`, reached via the snapshot-apply stage at `main.rs:2446`) and `resolve_tick` runs once per fixed tick (`main.rs:2492`/`:2722` → `host_resolve_remote_commands` at `command_queue.rs:541`/`:550`), so the two 60 Hz clocks are independent and run ~1 tick out of phase.
