...
- Extract an upper **entities** crate (`postretro-entities`): the entity registry, `ComponentValue`, the remaining components, `ScriptCtx`, the slot table + engine-state catalog, provenance, the system-command queue, the **reaction/crossing descriptor types** (they reference `EntityId` — see the partition rule below), and `DataRegistry`. Depends on the foundation.
- **Descriptor partition rule:** a descriptor type that references `EntityId` lives in the **entities** crate (with the registry); `EntityId`-free descriptor types live in the **foundation**. The implementer verifies each descriptor by this rule — do not rely on the enumerations above being exhaustive.
...
- [ ] Placement holds: `DamagePayload`/`NavAgentParams`/`ModMapEntry` and the `EntityId`-free descriptors resolve from `postretro-foundation`; the reaction/crossing descriptors (which reference `EntityId`), `apply_damage`/`attach_agent`, and `EntityId` itself resolve from `postretro-entities`; `weapon`/`nav` define neither sunk POD; `MenuCamera`/`Frontend` stay in `postretro`.
...
- **Descriptor partition by `EntityId`-reference.** Descriptor types that reference `EntityId` (the reaction/crossing descriptors) live in `postretro-entities` with the registry; `EntityId`-free descriptors live in `postretro-foundation`. This breaks the `SequenceStep → EntityId` cross-crate cycle by placement rather than by sinking `EntityId` down (which would change the thesis — `EntityId` stays the entities crate's opaque handle).
