...
The exact wire representation — extend `KinematicGeometry` (id 43, version bump) to carry members, add a new instanced section, or a member-reference field on `MapEntity` (id 29) — is a per-spec **Wire format** decision, deliberately not fixed here (`context_style_guide.md`: state the constraint, not the layout).
...
**1 — `E17--assembly-carried-members` (foundation + first consumer).** Introduce the canonical assembly (`MapAssembly` sketch above) and prove it end-to-end with one real consumer: a dynamic light that travels with a kinematic mover. Primary authoring is an explicit binding on the light (a KVP naming the mover, mirroring how `trigger_volume.target_tag` binds a mover — no `func_group` dependency, so this spec does not need spec 2). The compiler peels the bound light off the top-level `MapLight` path with its mover linkage and authored local offset; the runtime composes the light's world pose from the mover's `Transform` each tick after `run_kinematic_mover_tick`, before render, and from the *interpolated* pose at render time. Only dynamic-tier lights may be carried — a baked light bound to a mover is a named compile error (its contribution is baked static; it cannot move). This is the thin vertical slice through adapter → canonical → PRL → runtime that falsifies the assembly boundary while rewrites are cheap; it decides the member representation — whether to extend the E21 attachment relation (`research.md §6`) to a mover-`Transform` source and a live-entity child, or a bespoke parent link — with the light's mover-local offset derived from its authored position rather than an offset KVP (Alternatives rejected), and whether members ride `KinematicGeometry` id 43 (version bump) or a sibling section. Client vantage adds no wire: the client re-derives the mover `Transform` from already-replicated phase (E17-A) and composes the light locally — a claim the spec must warrant against the replication path, not assert (`research.md §7`). Edge coverage per `research.md §8` (reversal, completion, zero/two ticks per frame, unresolved binding).
