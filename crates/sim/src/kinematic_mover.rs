...
pub struct MoverTickStateTable {
...
impl MoverTickStateTable {
    ...
    pub fn publish(&mut self, mover_id: u32, state: MoverTickState) {
        self.states.insert(mover_id, state);
    }

    pub fn get(&self, mover_id: u32) -> Option<&MoverTickState> {
        self.states.get(&mover_id)
    }

    pub fn terminus_events(&self) -> impl Iterator<Item = (MoverEventKind, u32)> + '_ {
    ...
    pub fn split_for_blocking(&mut self) -> (MoverTickPoseSource<'_>, &mut MoverBlockingState) {
        (
            MoverTickPoseSource {
                states: &self.states,
...
pub struct MoverTickPoseSource<'a> {
...
pub fn run_kinematic_mover_tick(
...
pub fn mover_pose_for_current_phase(
    transform: Transform,
...
pub fn mover_is_docked_closed(mover: &KinematicMoverComponent) -> bool {
...
pub fn advance_mover_phase_one_tick(
