...
- [ ] A bare read or write of a per-owner slot fails at load with a diagnostic naming the slot; so does an owner-addressed access to a global slot, so does `network: "ownerPrivate"` declared without per-owner cardinality, so does `perOwner` declared alongside `accumulate`, and so does `perOwner` declared alongside `persist`. Per shipped `mod_init.rs` behavior, this rejects the whole declaration set at mod-init — the manifest does not partially load.
...
| A per-owner value survives a disconnect within the hold window, and is dropped only when its seat is released — expiry or a losing stale-duplicate reclaim, never merely on disconnect | `E15` (hold window, `CarriedState` parity, `release_seat`'s two call sites), Task 1 (both call sites wired to clear per-owner storage) | Wiring only the expiry path leaves a stale duplicate's per-owner values leaked forever, since seat numbers are never reused | AC 11, AC 12 |
...
| Cardinality and replication stay independent | Task 2 (separate declarations, several cross-checks) | A later shortcut that infers one from the other re-fuses the axes | AC 5, 6 |
