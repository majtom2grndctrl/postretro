...
pub enum EngineStateCapability {
    Readonly,
    Writable,
}
...
pub enum EngineStateValueType<'a> {
    Number,
    Boolean,
    String,
    Enum { values: &'a [&'a str] },
...
#[derive(Clone, Copy, Debug, PartialEq)]
pub struct EngineStateCatalogEntry<'a> {
    ...
    pub sdk_path: &'a [&'a str],
    pub value_type: EngineStateValueType<'a>,
    pub default: EngineStateDefault<'a>,
...
pub enum EngineStateCatalogError {
...
#[derive(Clone, Debug)]
pub struct EngineStateCatalog {
    entries: Vec<EngineStateCatalogEntry<'static>>,
    tree: EngineStateTree,
}

impl EngineStateCatalog {
    pub fn entries(&self) -> &[EngineStateCatalogEntry<'static>] {
        &self.entries
    }

    pub fn tree(&self) -> &EngineStateTree {
        &self.tree
    }

    pub fn store_declarations(&self) -> Vec<(String, Vec<(String, SlotRecord)>)> {
...
pub fn engine_state_catalog() -> Result<EngineStateCatalog, EngineStateCatalogError> {
