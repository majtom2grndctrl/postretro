//! CPU-only lighting packing and reachability math.
//!
//! This crate owns shader-facing byte packing and light influence predicates.
//! It intentionally contains no wgpu types; renderer-owned GPU pools stay in
//! `postretro`.
...
pub fn pack_light_with_slot(light: &MapLight, slot_index: u32) -> [u8; GPU_LIGHT_SIZE] {
    ...
    write_u32_as_f32(&mut bytes, CUBE_SLOT_BYTE_OFFSET, NO_SHADOW_SLOT);
...
pub fn pack_light(light: &MapLight) -> [u8; GPU_LIGHT_SIZE] {
    pack_light_with_slot(light, NO_SHADOW_SLOT)
}
...
mod tests {
    ...
    fn pack_point_light_encodes_position_color_type() {
        ...
        assert_eq!(read_u32(&bytes, 56), NO_SHADOW_SLOT);
    }
    ...
    fn cone_slots_default_to_sentinel_for_point_light() {
        ...
        assert_eq!(read_u32(&bytes, 56), NO_SHADOW_SLOT);
        assert_eq!(read_u32(&bytes, CUBE_SLOT_BYTE_OFFSET), NO_SHADOW_SLOT);
    }
    ...
    fn pack_lights_with_slots_no_slot_sentinel() {
        let lights = vec![sample_point(), sample_spot()];
        let slots = vec![NO_SHADOW_SLOT, 2u32];
        let mut bytes = Vec::new();
        pack_lights_with_slots_into(&mut bytes, &lights, &slots);
    ...
    fn pack_lights_with_slots_into_identical_inputs_produce_byte_equal_outputs() {
        let lights = vec![sample_point(), sample_spot(), sample_directional()];
        let slots = vec![0u32, 1u32, NO_SHADOW_SLOT];
