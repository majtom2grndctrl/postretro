...
Define the `BindingScope` trait — bind-time `resolve_input(name) -> Option<InputHandle>` / `resolve_output(name) -> Option<OutputHandle>`, eval-time `read(InputHandle) -> IrValue` / `write(OutputHandle, IrValue)` — so the evaluator never holds a global namespace. **Writability is a bind-time scope capability:** a scope grants a write handle only for outputs it permits, so a buffer that targets a forbidden output fails to bind (degrades) rather than being policed each tick; the evaluator is never "privileged or not" — the scope decides what it exposes. Data-integrity validation (declared type / range clamp) always applies at the write itself, independent of writability. Implement the **eval** pass over an `IrProgram`: pure, total, bounded, zero per-eval allocation, no string lookups (names already resolved to handles at bind). Arithmetic is total (÷0 → 0, non-finite guarded, `clamp` tolerates inverted bounds). Ship a store-backed scope adapter (reads slots by dotted name; grants write handles per the slot's declared writability, delegating the write to the store's validated engine write-by-name API) and a read+write **test stub scope** exercising both paths. No real adopter consumes the evaluator in this plan.

### Task 3: Builder vocabulary + typedef emission + dual-runtime parity

Register the `IrNode` union through the type registry (`register_tagged_union` precedent) so the vocabulary emits into `postretro.d.ts` / `postretro.d.luau` — the typedef is the contract. The builders themselves are **pure `sdk/lib` constructors** in each runtime, typed against that emitted union: constructing a node is data assembly that returns plain IR data through the normal `setupMod` / `setupLevel` return, **not** an FFI primitive call (`scripting.md` §12 — no side-effect FFI from imports; building a node binds nothing and evaluates nothing). This mirrors descriptor authoring: TS/Luau types guard at author time, Rust serde + bind-time validation is the authority. Add the **parity contract test**: the same authored expression in TS and Luau produces identical IR JSON (the dual-runtime test pattern the per-runtime install tests seed).
...
- **Scope.** `BindingScope` trait (read + write, both optional-capability). Writability lives in `resolve_output` — granting a handle *is* the authorization; a denied output fails bind. Store adapter delegates writes to `write_store_slot` (validated/clamped) and grants handles per declared writability. Movement's scope (plan 3) exposes `speed` / `grounded` / `chargesRemaining` / … from the movement component **without** routing through `worldQuery` — the engine-internal invariant (`entity_model.md` §7b) holds because the scope is an engine-side binding, not the script-facing slot table.
...
These were settled by zooming out to the engine's core rule — *declare as data, Rust evaluates, the VM drops; the IR is not special, it is the consolidation of the existing substrate* (`scripting.md` §11, §241).

- **Builder mechanism → pure SDK constructors.** Opcode builders are plain `sdk/lib` constructors typed against the registry-emitted `IrNode` union, not registry primitives. Constructing a node is data assembly with no runtime effect, so an FFI primitive call would violate the no-side-effect-import rule (§12) for no gain. The union type in the typedef carries the contract; the constructors author against it (the descriptor precedent).
...
- **Compound conditions — empirical, designed-for.** Plan 3 (movement value expressions) likely needs only single-comparison `select` conditions; `and`/`or`/`not` are deferred from v1 but near-certain by plan 4 (trigger-predicate consolidation). Decision already taken: keep them out of v1, design the node type system to admit a combinator opcode additively (see non-goals). The only thing left to confirm is empirical — that plan 3's real expressions don't pull them forward.
