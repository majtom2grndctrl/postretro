...
**TriggerVolumes (id 44):** optional section for invisible brush triggers. Each record carries a world-space AABB, activation (`touch` or `use`), target tag, closed mover-command vocabulary, fire mode, rearm delay, enabled-on-spawn state, tags, and named `on_fire`/`on_exit` events. Version 2 appends the event names; version 1 decodes both as empty. Runtime preserves direct mover commands and dispatches bound event reactions. Trigger brushes are not present in static `Geometry`, `Bvh`, static collision, lightmaps, SDF, portals, or navmesh.
