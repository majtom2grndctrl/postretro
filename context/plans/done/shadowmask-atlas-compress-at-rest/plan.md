...
| MN6 second-group shadows after reload and a dev level cycle | owner, in-engine | manual | pass (owner, Windows, 2026-09-25) — menu cycle movement-feel → combat-demo → kinematic-platforms → campaign-test → movement-feel showed shadowmasks as expected. campaign-test was first fully lit from a pre-change bake (the designed retired-format fallback) and correct after a re-bake |
| MN7 level cycle between differing lightmap widths and back | owner, in-engine | manual | pass (owner, Windows, 2026-09-25) — same four-level menu cycle and back |
