...
- **Tests.** Both-runtime declaration parity, per-slot-type schema validation, validation/clamp, readonly rejection, persistence round-trip, slot-table never-cleared (unit test over the existing clear paths — `data_registry.clear()` and the table's own teardown — asserts it is untouched).
