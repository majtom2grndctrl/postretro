...
Boot is staged so first pixels reach the window at the earliest practical point, then heavier subsystems initialize behind the visible splash. A minimal pre-window setup pass builds session-lifetime state up to the script runtime; the winit-driven splash state machine (Booting → Splash → Frontend/Loading/Running) paints a black frame, then the logo, and only **after** the first visible frame does it run deferred session install, audio/debug-UI/net setup, mod init, full renderer initialization, and level-worker dispatch.
...
| 2 | Create the event loop, then build the session services: `ScriptCtx`, primitive registry (`register_all`), `ScriptRuntime`, SDK-type emission, Rust-side registries, player options I/O, input seeding, built-in UI JSON registration | The event loop is created **before** the scripting bootstrap and net setup. The script runtime is constructed ONCE, **before the window** — the only session-lifetime system built pre-window. Primitive closures capture `ScriptCtx` clones; held for the whole session, never recreated. |
...
| 4 | On resume: create window, init the **boot-ready** renderer (surface/device/queue/config + boot splash pass), enter Splash, request first redraw | Fires once on desktop; guarded against re-entry. Device creation already requests the FULL feature/limit set (wgpu features can't be added later). Adapter checks the boot splash needs run before the first splash draw; hard renderer requirements fail fast with a named message (`rendering_pipeline.md` §4). No audio, debug UI, net, mod, or level work in this path. |
...
`StartupTimings` records ordered named stage marks (logged as `[Startup] …`). Boot marks in order: `args_parsed`, `event_loop_created`, `script_runtime_ctor` (pre-window), `window_created`, `wgpu_init` (boot-ready renderer), `first_black_frame`, `splash_decoded`, `splash_uploaded`, `first_splash_frame`, `audio_init_complete`, `net_endpoint_complete`, `session_init_complete`, `renderer_full_init_complete`, and (CLI-map boot) `boot_worker_dispatch`. `first_black_frame` / `first_splash_frame` are recorded only on the presented branch (after command submission + successful present). `first_black_frame` precedes the net/audio/debug-UI/mod/level-worker marks but **not** `script_runtime_ctor` — the script runtime is built before the window.
