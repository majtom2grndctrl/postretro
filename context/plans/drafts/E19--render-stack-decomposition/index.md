...
- **`postretro-ui*`** (cpu-only, `E19--ui`) — `render::ui` CPU subtree + `UiTexture`. Depends on `scripting-core` (descriptor model), `entities` (`SlotValue`, used across `tree/*`), `taffy`, `cosmic-text` (direct, for `FontSystem`/measurement — no `glyphon`). No `input`, no wgpu.
...
- [ ] Each new **cpu-only** crate's `cargo tree` (default features) shows **no** `wgpu`, `winit`, `glyphon`, `kira`, `mlua`, or `rquickjs`. (Manual/CI gate.)
