...
**FFI marshalling is an orphan-rule boundary.** A VM marshalling impl for a floor-owned type lives in that floor crate when Rust's orphan rule requires ownership. Floor crates keep those impls behind optional `script-ffi` features, off by default; the runtime enables the upper floor feature, which forwards as needed. Foreign types wrap in local newtypes. Runtime-side descriptor converter functions that are VM-coupled live in `postretro-scripting-core`; subsystem handler wiring in `postretro` calls down into that substrate for script argument/result newtypes. Default builds have no VM deps — editing a handler or bridge never rebuilds them. **That is the compile firewall.**

**Handler placement.** Script-callable handlers co-locate with the subsystem they expose (subsystem module trees within the engine binary, not new crates). Distinguish two things. The **marshalling substrate** — the marshalling newtypes + their FFI impls, the registry types, the converters, the VM runtimes, the typedef generator — lives in the VM crate (`scripting-core`). The **handler wiring** — the `register_*` registrar functions + their per-primitive closures — co-locates with the subsystem in the binary, alongside the pure logic. The governing reason: the VM crate sits below the binary in the one-way dependency, so any code that calls a subsystem function must live in the binary; the registrar wiring therefore goes up with the logic, while only the reusable marshalling substrate stays in the VM crate. Reaction handlers (VM-free) relocate whole; primitive handlers relocate their pure logic **and** their wiring, leaving the marshalling newtypes + registry machinery in the substrate — the binary-side wiring decodes script args via those newtypes, calls the same-crate subsystem fn with native Rust args (never VM types), and encodes the result via those newtypes. Aggregation stays explicit (no `inventory` / `linkme`): the registrars, now binary-side, are invoked from the single runtime construction site (`Session::build`).
