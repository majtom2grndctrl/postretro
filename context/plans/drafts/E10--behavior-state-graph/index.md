...
- Engine-owned floor unchanged: target selection/retention/hysteresis, think-stride, aggro gate (`updateEnemyState`; closed gate forces `initial` + clear steering), combat-slot resolution, facing/locomotion arbitration, damage chokepoint, host-only evaluation (client skip gate untouched), E16 death/despawn ownership.
...
Behavior-preserving split of `crates/postretro/src/scripting/systems/ai.rs` (969 lines) into a `systems/ai/` module directory: the pure transition core (`evaluate_transition`, `TransitionResult`, `SteeringIntent`, stride/hysteresis helpers), target selection (`select_target`, `TargetPawn`, candidate helpers), and the tick orchestration (snapshot/compute/apply passes, steering application, damage, animation, facing). Public surface (`run_ai_tick_with_navigation_and_impact`, `ENEMY_ATTACK_EVENT`, test-visible helpers) re-exported so `sim/mod.rs` and `ai_tests.rs` change imports only. No behavior change; full suite green.

### Task 2: Foundation descriptor + parsing

`BehaviorGraphDescriptor` in `postretro-foundation` (`data_descriptors/types/`): `initial: String`, `states: BTreeMap<String, BehaviorStateDescriptor>` (`animation: String`, `motion: MotionVerb`, `action: Option<ActionVerb>`, `transitions: Vec<TransitionDescriptor>`, `on_enter: Option<String>`), `interrupts: Vec<TransitionDescriptor>`, `attack: Option<AttackTuning>`. `TransitionDescriptor { to: String, when: IrNode }` — raw `IrNode` per the descriptor-partition rule; every referenced type is foundation-resident so the descriptor stays in foundation. `MotionVerb`/`ActionVerb` are closed serde enums per the boundary inventory. Structural validation at parse: names resolve, no duplicates, numeric `attack` fields finite/positive (damage ≥ 0). Add the `behavior` key to `EntityTypeDescriptor` and both twin parsers (`scripting-core` `js/entity.rs`, `lua/entity.rs` — both funnel through `serde_json`, so divergence is impossible), including the `ai`+`behavior` mutual-exclusion error. Extend the typedef generator (`scripting/primitives/mod.rs` descriptor fields) so `behavior` and its types emit into both typedefs; guard fields emit as `RuntimeValue`.

### Task 3: BrainScope

A `BindingScope` implementation in `postretro-entities` beside `BrainComponent` (it must name `EntityStateComponent`, so it cannot sit in foundation; the bound program lives on the runtime state that binds it, so the scope sits with the component). Fixed input table (`MovementScope` precedent — name→index handles, order load-bearing): `@brain.hasTarget` Bool, `@brain.targetDistance` Number (sentinel `1.0e9` when no target), `@brain.timeInStateMs`, `@brain.attackCooldownMs`, `@brain.acquisitionDue` Bool, `@brain.health`, `@brain.maxHealth`. `@state.`-prefixed names route to `EntityStateComponent` via interned name→index handles with per-tick snapshot refresh (`EntityScope` routing precedent, read path only). No outputs (`resolve_output` → `None` — guards are read-only). Ships `for_validation()` (type-correct zeros, for declaration-time bind checks) and `refresh(...)` repopulating the fixed slots and state snapshot allocation-free each evaluated entity. Declaration-time validation: both runtimes bind every authored guard against `for_validation()` at descriptor validation and reject with pathed `BindError` context — the dash `bind_dash_node` mirror.

### Task 4: Brain generalization + spawn bind

`BrainComponent` carries the graph: retained `BehaviorGraphDescriptor`, current state (stable index into a resolved state list), `time_in_state_ms: f32`, and bound guard programs (`Vec` parallel to interrupts + per-state transitions) marked `#[serde(skip)]` with always-true `PartialEq` (the `DashPrograms` pattern) — rebind on deserialize/spawn from the retained graph; bind failure warns once and disables the affected transition (fall-back-to-native posture). Engine substates stay as-is: `attack_cooldown_remaining_ms`, `think_stride_counter`, `aggro_armed`, `acquired_target`, `combat_slot`, `combat_slot_hold_ticks`. The attach site (`builtins/data_archetype.rs` ai-block, line ~479) becomes: legacy `AiDescriptor` → `lower_ai_descriptor(&AiDescriptor) -> BehaviorGraphDescriptor` (foundation fn, Task 2's types; guards built as `IrNode` trees Rust-side reproducing `evaluate_transition` edge-for-edge, detection/leash edges conjoined with `@brain.acquisitionDue` via `select`) → one `BrainComponent` shape; authored `behavior` attaches directly. Spawn-time animation validation walks the graph's states instead of `LogicalState::ALL`. `attach_agent` keeps consuming `move_speed` (legacy) or a `behavior.moveSpeed` field mirrored into the same parameter.

### Task 5: Graph evaluator in the tick

Replace the `evaluate_transition` call path with graph evaluation inside the existing snapshot/compute/apply tick: per evaluated enemy, `refresh` the BrainScope, evaluate `interrupts` then current-state `transitions` in order, first `IrValue::Bool(true)` wins (the impact `group.when` eval shape); the selection step is one pure function `select_transition(graph, bound, scope) -> Option<StateIndex>` — the pinned pluggable seam. Apply the target state's `MotionVerb` as today's `SteeringIntent` mapping (`chaseTarget`→Chase with combat-slot preference, `hold`→Clear, `freeze`→Hold), run the `attack` action (cooldown-gated `apply_damage_with_context`, `ENEMY_ATTACK_EVENT`, target-alive check — today's rules), request the state's animation through the existing switch/warn path with the locomotion latch (a `chaseTarget` state at rest selects the graph `initial` state's animation, preserving today's Alert-at-rest→idle behavior), reset `time_in_state_ms` and fire `on_enter` on state change. Plumbing: the tick's event return widens from `Vec<&'static str>` to owned names to carry authored `on_enter` addresses — the one caller is `sim::simulate_tick` (`sim/mod.rs:293`) and its post-tick `fire_named_event` drain. Aggro gate closed forces `initial` + Clear before evaluation, as today's forced Idle. Stride, target selection, hysteresis, combat slots, facing, and the E16 pass-1 skips (deferred despawn, downed) are untouched.

### Task 6: Reference graph + tests + diagnostics

Author the reference enemy (`sdk/behaviors/reference/entities.{ts,luau}`) as an explicit `components.behavior` graph reproducing the current idle/alert/attack tuning; keep the pose-fixture enemy on legacy `components.ai`. Port the pure-core unit tests in `ai_tests.rs` to lowered-graph equivalents; add fixture assertions for behavior-identity (legacy vs authored graph), interrupt priority, commitment window, `@state` guard end-to-end via an impact policy, serde round-trip/rebind, and the alloc-probe eval assertion. Update the agent diagnostics overlay to label agents with the authored state name. Regenerate and commit both typedef fixtures.
...
| Motion verb | `MotionVerb::{ChaseTarget, Hold, Freeze}` | `"chaseTarget"` / `"hold"` / `"freeze"` | same strings | same | n/a |
| Action verb | `ActionVerb::Attack` | `"attack"` | same | same | n/a |
| Transition | `TransitionDescriptor { to, when: IrNode }` | `{to, when}`; `when` is the op-tagged IR object | `when: RuntimeValue` | same | n/a |
| Attack tuning | `AttackTuning` | `attack.{damage,range,cooldownMs}` | same | same | n/a |
...
| Lowered legacy graph is behavior-identical to v0 `evaluate_transition` | Task 4 (guard construction), Task 5 | Acquisition-gated edges must conjoin `@brain.acquisitionDue`; stride/hysteresis stay engine-side | AC 1, 7 |
| Bound programs are derived data: never serialized, rebind from retained graph, equality-neutral | Task 4 | Serde/replication of `BrainComponent`; hot reload | AC 10 |
