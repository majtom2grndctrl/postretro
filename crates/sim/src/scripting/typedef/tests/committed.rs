...
fn committed_sdk_types_match_current_registry() {
    ...
    assert_eq!(
        committed_ts, ts,
        "sdk/types/postretro.d.ts is out of date — re-run `cargo run -p postretro-sim --bin gen-script-types` and commit the result"
    );
    assert_eq!(
        committed_luau, luau,
        "sdk/types/postretro.d.luau is out of date — re-run `cargo run -p postretro-sim --bin gen-script-types` and commit the result"
    );
}
