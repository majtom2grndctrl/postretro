...
In `lightmap_bake`, filter the `static_lights` pass to `!light.is_dynamic && light.animation.is_none()`. All animated non-dynamic lights move to the compose path. Also drop the `!bake_only` guard from Spec 1's chunk-list filter in `animated_light_chunks.rs` (retroactive Spec 1 change — see Settled decisions). Also change the `main.rs` call site to pass lights filtered by `!is_dynamic` (not `!bake_only`) to `build_animated_light_chunks`, matching the filter `sh_bake.rs` uses for the descriptor buffer. This is what aligns the two namespaces so no per-entry remap is needed at bake time. Update the existing test `static_flag_bakes_but_dynamic_does_not` to cover both animated and `bake_only` animated cases.
...
1. Resolve the chunk's atlas-texel rectangle from the face's chart placement and the chunk's UV sub-region. Round outward to integer texel boundaries (floor min, ceil max). Assert that no two chunks on the same face produce overlapping atlas rectangles after rounding — requires the UV chunk packer to maintain a 1-atlas-texel gap between adjacent chunk UV boundaries within a face (see Settled decisions). If the assert fires, the fix is in the UV packer, not the weight baker.
2. For every texel in that rectangle, for every animated light in the chunk's light list: use the same supersampling strategy as `lightmap_bake` (same sample count, distribution, and per-sample shadow ray); average weight across samples. For each sample: recompute the world position and normal using the same chart-rasterization math as `lightmap_bake` — these values are not persisted and must be derived fresh from the chart placement and UV coordinates. If this math is currently inline in `lightmap_bake`, extract it into a shared crate-internal helper (e.g. `chart_raster.rs`) callable from both bakers. Copy-paste is not acceptable; the two bakers must share one implementation to avoid drift at chunk boundaries. Compute unshadowed Lambert contribution (distance falloff × N·L × spotlight cone — identical to `lightmap_bake`). Run a shadow ray against the shared BVH using the same epsilon/bias as `lightmap_bake` to avoid self-intersection. Uses the same `Bvh` instance as `lightmap_bake` in the same compile invocation — no separate BVH build. Emit `(light_index, weight)` when visible and weight > 1e-6 (drop numerical-noise contributions to avoid inflating per-texel counts toward the cap). Chunk-list indices directly reference descriptor buffer slots — no remap (see Settled decisions). Emit a zero-count entry for texels where no light contributes so `offset_counts.len()` matches the total rect area invariant. Also emit a zero-count entry for rect texels that fall outside the face's UV chart (an artifact of outward rounding) — no valid world position or normal exists for them, so no Lambert computation is attempted.
...
Each frame the encoder dispatches a compute pre-clear pass over the full animated atlas (`ceil(atlas_width / 8) × ceil(atlas_height / 8)` workgroups, covering every texel regardless of chunk coverage) before dispatching compose (writes `vec4(0.0)` to every texel via compute — no adapter feature required); uncovered atlas texels stay at zero. Implementer may combine the clear and compose into one shader or use a separate clear shader.
