...
- [ ] Enforcement: a reaction whose IR reads `@rising`, fired from the `levelLoad` address or bound to a trigger `on_fire`, is rejected at level install with a warning and never fires; other reactions on the same source still fire, and the *same* `@rising` reaction subscribed to both a crossing (binds, fires) and `levelLoad` (skipped) in one level holds both dispositions simultaneously.
...
No new binary/PRL section, no FGD KVP. Captured dispatch values ride the existing system-command queue as `(String, IrValue)` pairs (state the constraint: entities-crate command, foundation value type — exact field layout implementation-chosen). The subscription key rides alongside as a stable install-order `u32` in a named command field (exact spelling implementation-chosen).
