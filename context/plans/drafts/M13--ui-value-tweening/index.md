...
- **Tween wire format.** Optional `tween` sub-object on the existing binds — `TextBind` and `PanelBind` (`render/ui/descriptor.rs`): `{ durationMs, easing, from? }`. `easing` is a closed enum: `linear`, `easeIn`, `easeOut`, `easeInOut` (cubic). `from` is an optional initial display value — a number on a text bind, a length-4 linear RGBA array on a panel bind. Tween-less binds keep their exact current wire form (`skip_serializing_if`).
- **Time source.** `UiReadSnapshot` gains `time_seconds: f64`, filled by the App from its existing dt-accumulated `script_time` (deterministic, never wall-clock — the `StaticUiProxy` precedent). `0.0` default on the splash path. Threaded into the retained build (`UiPass::layout_gameplay_tree` → `UiTree::build_draw_data_retained`); CPU tests feed synthetic times.
...
- **Demo + CPU gate.** `demo::build_demo_descriptor`: the health text bind gains `tween { from: 0, durationMs: 1200, easing: easeOut }` (HP visibly counts 0 → 100 on level load); the flash panel bind gains `tween { durationMs: 150, easing: easeInOut }` (the 500 ms proxy toggle eases instead of stepping). The `demo_ui_gate_test` assertions are updated for the tweened behavior. CPU tests with synthetic times cover: first-resolve `from` flourish, mid-flight retarget, easing monotonicity, exact-target settle, post-settle no-rebuild, the non-numeric snap-with-warn, and unbound/tween-less nodes unaffected.
...
- [ ] `UiReadSnapshot` carries a frame time in seconds sourced from the App's dt-accumulated clock; the splash path publishes `0.0`; no wall-clock read exists in the tween path.
...
- [ ] A tween declared on a slot that resolves to a non-tweenable shape (e.g. a string on a text bind) renders via the unchanged C resolution with one warning per tree build.
...
- [ ] No store write originates from the UI module: tween state lives in the renderer's retained tree; `write_store_slot` call sites are unchanged.

## Tasks

### Task 1: Tween wire format
In `render/ui/descriptor.rs`: the `tween` sub-structs on `TextBind` (`from: Option<f32>`) and `PanelBind` (`from: Option<[f32; 4]>`) sharing one `Easing` serde enum (camelCase wire), all `skip_serializing_if` so tween-less binds round-trip unchanged. Round-trip tests per shape plus the no-wire-break fixture assertions. Pure data.

### Task 2: Snapshot time plumbing
`UiReadSnapshot` (`render/ui/mod.rs`) gains `time_seconds: f64` (default `0.0`); `with_gameplay_tree` gains the time argument — callers updated: the publish site in `main.rs` (passes `self.script_time`) and the gate tests (synthetic times). `UiPass::layout_gameplay_tree` and `UiTree::build_draw_data_retained` gain the time parameter; the gameplay record block in `render/mod.rs` passes `self.ui_snapshot.time_seconds`. The fresh path (`layout_tree`/`build_draw_data`) takes no time — inertness falls out structurally.

### Task 3: Tween runtime in the retained diff
In `render/ui/tree.rs`: extend `NodeContext::Text`/`Panel` with the display state (display value, tween start value/time, target, warn-once latch); the four cubic easing functions as module-local helpers; rewrite `resolve_bindings` into the tween driver per the Scope mechanics (retarget / first-resolve / in-flight advance / exact settle), classifying each frame's change through the existing `BindingDiff` so the relayout/redraw split and `mark_dirty` path are reused, not duplicated. The in-flight text string goes through `last_resolved` so the measure seam (`measure_node`) shapes the displayed value unchanged. Depends on Tasks 1 + 2.

### Task 4: Demo + CPU gate
Add the two tweens to `demo::build_demo_descriptor`; update `demo_ui_gate_test` for the tweened frame sequence; extend the CPU harness with the synthetic-time suite from the AC list. The `StaticUiProxy` is untouched — its step-toggle output is exactly what the panel tween smooths.
...
TW and `M13--fonts-theming` are planned as one concurrent wave. Unavoidable shared files: `render/ui/descriptor.rs` (TW edits the bind structs; D edits widget field types), `render/ui/tree.rs` (TW adds tween state to `NodeContext` and rewrites `resolve_bindings`; D adds resolved theme values to `NodeContext` and `build_node`), and `render/ui/mod.rs` (TW widens signatures with time; D widens them with the theme). Conflicts are additive — new fields, new parameters — not semantic. Run the two in isolated worktrees and merge-coordinate those three files at integration; whichever lands second rebases its signature changes mechanically.
...
| duration | `duration_ms: f32` | `durationMs` (milliseconds) | `durationMs` | `durationMs` |
