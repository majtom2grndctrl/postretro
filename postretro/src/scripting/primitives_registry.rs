...
//
// Registering a Rust function once installs it into every QuickJS context and
// every `mlua::Lua` state created later. `RegisterablePrimitive` enforces the
// `Result<_, ScriptError>` return shape at compile time; per-arity impls are
// generated by the `impl_registerable!` macro below.
