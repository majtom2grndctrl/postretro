...
- [ ] A trigger-bound reaction with a consequential step behind an `on_complete` chain or `Progress` reaction logs a bind-time warning; that buried step executes on the app drain, while the reaction's top-level consequential steps still execute in-tick.
...
- [ ] `prl-build` warns (does not fail) on a `trigger_volume` with neither `target_tag` nor `on_fire` (still emits the record; the compiler `log::warn!` is a review/grep gate — `log_capture` is not available in `level-compiler`).
...
- In-tick consequential execution covers a trigger-bound reaction's top-level `Primitive`/`Sequence` steps only. Steps reached through an `on_complete` chain or behind a `Progress` reaction are deferred by the reaction model and drain app-side; the binder warns when one is buried so the boundary is author-visible. Same-tick atomicity is authored by composing steps in one reaction.
