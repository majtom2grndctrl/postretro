...
- [ ] A tag containing `.` is rejected at level install with a warning and declares no occupancy slots. A crossing over an undeclared-tag occupancy slot hits the shipped warn+skip and registers no watcher (inert-safe degradation), rather than failing the load.
