...
real bottleneck (and later to validate BVH node-visibility masks). Two committed
variants:

| Map | Lights | Stresses |
|-----|--------|----------|
| `stress-warren.map`     | none           | geometry/BVH walk + portal traversal only |
| `stress-warren-lit.map` | 157 dynamic    | + per-frame forward light loop, light culling, and shadow pools |
