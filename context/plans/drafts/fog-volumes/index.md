...
**`scripting/registry.rs`:** add `ComponentKind::FogVolume = 5` and `ComponentValue::FogVolume { density: f32, color: [f32; 3], scatter: f32, falloff: f32 }`. Add to the `VARIANTS` const array. Note: `EntityRegistry::new` initialises component storage as `[Vec::new(); N]` where N is the kind count — extend this array to accommodate the new `FogVolume` kind (currently `[Vec::new(); 5]`, becomes `[Vec::new(); 6]`).

**`scripting/conv.rs`:** extend `component_kind_from_name` with `"FogVolume" → ComponentKind::FogVolume` (matching PascalCase convention of existing variants). Note: adding a new kind requires updating five locations in this file — `component_kind_name`, `component_kind_from_name`, `FromJs`/`IntoJs`, `FromLua`/`IntoLua`, and the `setComponent` field block-list (lines ~361/441). The `worldQuery({ component: "fog_volume" })` filter uses snake_case — this is a separate lookup from the kind name. Extend `FromJs`/`IntoJs` and `FromLua`/`IntoLua` for `ComponentValue::FogVolume`. `setComponent` accepts `density`, `color`, `scatter`, `falloff`; AABB fields are silently ignored if present. `getComponent` returns all four.
