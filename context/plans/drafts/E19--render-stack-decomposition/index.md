...
- **`postretro-lighting*`** (cpu-only, `E19--lighting-cpu`) — `lighting::{mod,influence,spec_buffer}` (light packing). `cone_frustum` does **not** live here — it homes in `postretro-render-data` (it delegated to `compute_cull`, a GPU module, so lighting would have caught a `lighting → renderer` cycle). Light-domain script wiring descends here behind an optional `script-ffi` feature (off by default) per §12; cross-domain world-query/shared type registration stays near the scripting aggregation layer.
...
- **`script-ffi` / handler placement.** Script-primitive *wiring* descends into its subsystem crate behind an optional `script-ffi` feature (off by default, `script-ffi = ["dep:rquickjs","dep:mlua", ...]` per `scripting.md §12`) only when it is subsystem-owned. The marshalling substrate stays in `scripting-core`; the registrar is invoked from `Session::build`. `postretro-lighting` moves light-domain wiring only: `register_light_entity_primitives`, `register_sequenced_light_primitives`, `collect_light_handles`, `handles_to_json`, and the light animation validation/apply path. `WorldQueryComponent`, `WorldQueryFilter`, `Entity`, `EmitterEntity`, and non-light query aggregation stay near `scripting/primitives/mod.rs` / `entity_world_primitives.rs`. This is the precedent the Epic 16 combat crate mirrors.
...
1. **`lighting/script_primitives.rs` placement** — split the current mixed file. Light-domain scripting wiring descends into `postretro-lighting` behind an optional `script-ffi` feature; cross-domain world-query/shared type registration stays near `scripting/primitives/mod.rs` / `entity_world_primitives.rs`; the marshalling substrate stays in `scripting-core`; the registrar is still invoked from `Session::build`. Principle: `scripting.md §12` handler-placement spirit applies to subsystem-owned wiring, not shared aggregation. This is the precedent the Epic 16 combat crate mirrors. (`E19--lighting-cpu`.)
