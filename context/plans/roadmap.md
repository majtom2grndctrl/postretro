...
Plans ship in this sequence. The render foundation and combat tracks converge at the AI behavior plan; logical combat work (enemy HP/death, navigation) can proceed in parallel with the render track, but the integrated outcome needs both.

**Render foundation** (net-new per-entity mesh path):

- [ ] **Model pipeline spike** — prove the full risky path end-to-end with one real glTF: a skinned mesh, animated, SH-lit (binds the shared `sh_sample.wgsl` group), casting a CSM shadow, culled through a portal. Throwaway code; the deliverable is locked runtime contracts — GPU vertex layout (rigid + skinned), instance-indexed bone-palette layout, instance-friendly mesh-pass shape — plus two answered questions: does runtime glTF loading feel good enough to skip a mesh bake, and does per-frame sampling for many agents want an `ozz`-style baked pose buffer. Front-loaded because everything downstream rides on these decisions; durable outcomes migrate to `context/lib/` (`rendering_pipeline.md`, `build_pipeline.md`, `entity_model.md`) when the first render plan ships.
- [ ] **glTF model loading** — runtime loader: read glTF (mesh, skin weights, skeleton, inverse-bind matrices, clips) via the `gltf` crate into engine-side structs and GPU buffers in the spike's locked layout. Material textures resolve to their `.prm` equivalents through the existing texture pipeline (external PNG reference → `blake3` cache). `extras` metadata is read here and carried onto the entity. Renderer consumes handles, never raw glTF. The biggest single chunk. Shape ≈ the runtime-loader half of *Baked Texture Mips*, without the bake stage.
- [ ] **Mesh render pass + `MeshComponent`** — new render pipeline drawing per-entity meshes at the interpolated transform, SH-lit, portal/frustum-culled (camera-leaf lookup locates the entity cell). Skinned-capable and instance-friendly: per-instance transform plus a palette index into a shared bone-matrix buffer, renders in bind pose for now. `MeshComponent` carries a model handle; classname wiring spawns mesh-bearing entities from a map. Pass designed reusable for the depth/shadow path (`depth_prepass.wgsl` precedent). Shape ≈ *Emitter Entity* (component + render integration + classname routing), minus its reuse of the billboard pass — this pass is genuinely new.
...
- [ ] **Skinned animation runtime** — per-frame clip sampling and pose blending → bone-matrix palette; a shallow animation state surface (idle / locomotion / attack / death with crossfade). Engine owns sampling and blending — `ozz-animation-rs` kernels are a candidate (the spike decides) — while the state machine stays small and script-authored, not a visual editor or imported graph. Distant or off-screen agents sample at a reduced rate (animation time-slicing) to carry waves cheaply. Shape ≈ *Animated SH Volumes* (runtime + shader, one multi-task plan). Depends on the loaded clips and the mesh-pass palette binding.
...
- [ ] **Enemy entity + damage surface** — minimal enemy primitive: spawnable, carries HP, consumes the weapon's `DamagePayload`, dies at zero HP. This logical surface stands alone on the Milestone 6 entity model and damage events — no rendering dependency. Its hittable volumes are **dynamic**, though: bone-parented proxy capsules posed each frame from the skeleton (so hit zones depend on the render track's posed palette) and raycast separately from the static collision world — net-new, since the weapon hitscans only static geometry today. Hit-zone identity comes from glTF `extras` tags (`head`, `limb`); per-archetype damage multipliers live in the descriptor script. Model ships the spatial tag, script ships the balance — mirroring map `_tags` → entity behavior. Gives the shipped weapon a live target; no navigation.
- [ ] **Navigation foundation** — pathfinding against the Milestone 7 trimesh collider. The heavy, uncertain plan, isolated. Smallest workable primitive: move toward a target without clipping. Richer queries (line-of-sight, patrol paths) deferred. Depends on the enemy entity surface.
- [ ] **Enemy AI behavior** — simple state machine (idle → alert → attack → death), authored in the SDK as a reference behavior. Drives navigation (move toward player), attack (emit a damage hit at the player), and animation state (select the clip per logical state). The convergence point: depends on the enemy entity surface, the navigation foundation, and the skinned animation runtime. Behavioral time-slicing (distant agents think less often) is named for waves but stays shallow. A foundation to refine, not a stub.

**Testable outcome:** a skinned-model enemy spawns from a map, walks toward the player without clipping playing its locomotion clip, switches to an attack clip and deals damage in range, takes hitscan weapon damage, and plays a death clip then despawns at zero HP — lit by baked SH (optionally casting a CSM shadow), with placeholder SFX selling fire/impact/attack/death.
