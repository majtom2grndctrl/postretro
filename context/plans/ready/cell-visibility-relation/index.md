...
nearest coupled partners — a hard `O(N·K)` count bound the distance cap alone does not give, because
distance bounds path *length*, not pair *count* (a spatially compact but densely-connected region
keeps ~N² pairs within any distance cap). The right magnitudes are measurable only when real maps
...
- [ ] AC11 — Two structural caps bound the graded side-table to a hard `O(N·K)` entry count.
  ...
  deterministic given their pinned values. On a small fixture left effectively uncapped (distance cap
...
cap` → omitted (`perceivable` stays true, graded `None`). Pin `K` so the side-table holds `<= N·K`
entries regardless of density: each cell keeps its `K` nearest coupled partners by `distance`, and a
pair is stored if *either* endpoint keeps it (union — preserving symmetry). Add a bake stage `cell_visibility_bake` (name the stage function so Task 2 can locate it) after
