...
| CellVisibility | 46 | Reserved (not yet built) for the baked Cell→Cell coupling relation (reachability gate + graded distance/aperture over the portal graph) — `plans/ready/cell-visibility-relation`. Optional; absent → conservative all-perceivable fallback. Id 14 (`LeafPvs`) is a retired hole; do not reuse |
