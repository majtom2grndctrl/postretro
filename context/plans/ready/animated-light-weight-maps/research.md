...
- Limits Spec 2 should improve on:
  ...
  - Discrete 10 Hz, nearest-sample. Spec 2 uses non-linear interpolation (prerequisite, `animated-curve-eval` (`context/plans/in-progress/animated-curve-eval/`)) for smooth flicker/pulse/strobe.
...
**Recommendation for `animated-curve-eval`'s evaluator:** Catmull-Rom. Keyframe-only authoring matches the existing `Vec<f32>` brightness layout. Overshoot on strobe is acceptable for the retro aesthetic (chunky pulses are on-brand). No tangent authoring surface required.
