...
- [ ] Existing v2 PRLs load and render with no re-bake. No new section, no
      version change.
- [ ] A map with no animated lights renders identically to before (the new
      atlas binds a zero fallback; the animated correction is a no-op when
      `lm_anim` is zero).
...
compose-side storage binding (the reserved binding 8 slot), and expose a forward
view plus a zero fallback for the empty-map path, mirroring the existing
irradiance atlas and its `dummy_view`. In `compose_main`, for each contributing
texel light, decode `direction_oct_packed` to a unit vector and accumulate it
weighted by that light's current radiance contribution (the same `c * b *
weight` magnitude that drives the irradiance accum). Normalize the sum and store
it; if the fused length is below an epsilon (opposing lights cancel, or no
...
directions must not be linearly interpolated). Bind the compose output view, and
bind a 1×1 zero texture in the no-animated-weight-maps path (same fallback the
...
Sample it at `lightmap_uv`, derive the bumped/mesh NdotL ratio against the fused
animated direction, and build a `scale_anim` with the same epsilon guard and 4.0
cap as the static `scale`, gated off when `lm_anim` is negligible (its direction
is then unreliable). Combine as `static_direct = lm_irr * scale + lm_anim *
scale_anim`. Update the now-stale "lm_anim is not corrected" comment.
...
- **Direction encoding in the new atlas.** Store the fused **normalized vec3
  ...
  `.xyz` and re-normalizes. Reserve `.a` if a coverage/validity flag proves
  useful, else leave 1.0. This differs from the static atlas (oct in `Rgba8Unorm`
  rg) — acceptable because the animated atlas is compute-generated, not baked.
...
- **Radiance weighting.** Reuse the per-light `c * b * entry.weight` already
  computed for `accum`; weight the decoded direction by its luminance (or by the
  scalar magnitude) so the brightest-this-frame light dominates the fused
  direction, consistent with the irradiance.
