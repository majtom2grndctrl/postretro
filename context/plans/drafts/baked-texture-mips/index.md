...
- [ ] `prl-build` writes one `.prm` file per referenced texture into `content/<mod>/.prl-cache/tex/`, named `<blake3-hex>.prm`. Repeated builds with unchanged source PNGs skip mip-bake and file rewrite — the writer still re-hashes source PNGs and reads the existing `.prm` header's `bundle_hash` to compare.
...
4. Otherwise build mip chains per slot in linear `f32`, encode per *Wire format* below, write atomically — tempfile name `<hex>.prm.tmp.<pid>` in the same `.prl-cache/tex/` directory as the final file (per-process suffix avoids collisions if `prl-build` is ever invoked concurrently), then `std::fs::rename` for cross-platform atomic replacement. Write or rename errors surface as build failures, not warnings.
...
`PrmReadError` variants: `BadMagic { found: [u8; 4] }`, `UnsupportedVersion { version: u8 }`, `StageVersionMismatch { expected: u8, found: u8 }`, `ReservedSlotBitsSet { mask: u8 }`, `DimensionTooLarge { slot: u8, width: u16, height: u16 }`, `LevelCountMismatch { slot: u8, expected: u8, found: u8 }`, `PayloadBytesMismatch { slot: u8, expected: u32, found: u32 }`, `TotalBodyBytesMismatch { expected: u32, found: u32 }`, `Truncated`, `Io(std::io::Error)`. `Io(std::io::Error)` is reserved for the loader layer that wraps `std::fs::read` before parsing; the byte-slice parser itself never produces it.

Level load never fails on a `.prm` parse error. The runtime logs a `warn!` naming the texture and the failure reason. File-level errors (bad magic, version mismatch, stage_version mismatch, total_body_bytes mismatch, truncated header) fail the entire `.prm`; the runtime substitutes per-slot placeholders for all three slots and continues. Per-slot errors (level_count or payload_bytes arithmetic mismatch on one slot) fall back to that slot's placeholder; other slots that parsed cleanly are used.
...
- Runtime loader (`crates/postretro/src/startup/worker.rs::run_worker`): replace the texture-decode block at lines 73–88 (the `texture_root` assignment is at line 75) with `mod_cache_root = content_root.join(".prl-cache").join("tex")`. Pass `&world.texture_names` (`Vec<String>`) and `mod_cache_root` to the new `load_textures` signature (now `crates/postretro/src/render/loaded_texture.rs::load_textures`). Drop the `Vec<Option<String>>` wrap at lines 80–84 — the runtime previously synthesized `None` for missing PNGs; with the `.prm` upload path every entry corresponds to a present `String`. `TextureNamesSection` itself is unchanged.
- `.gitignore`: already covers `.prl-cache/` (matches at any depth); no change needed. Confirm at promotion.

## Open questions

- **Cache directory deletion as a recovery step.** Users hitting `warn!` logs naming a texture inside `content/<mod>/` should `rm -rf content/<mod>/.prl-cache/` (the per-mod sidecar cache — *not* the workspace-rooted `.prl-cache/` governed by `--cache-dir`) and rebuild. Document at promotion time in `build_pipeline.md`.
