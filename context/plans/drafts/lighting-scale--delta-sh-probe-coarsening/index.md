...
> **Build strategy:** two tiers (see Tasks). The CPU/compiler/wire substrate (Groundwork G1–G4) is built **directly and tested in-container** before `/orchestrate`, resolving every review-cluster decision into tested fact and producing the golden CPU reference; only the GPU thin-slice + fan-out (Orchestrated B1–B4) reaches `/orchestrate`. Groundwork in progress: **G1 done** (reconstruction math relocated to `postretro_level_format::sh_reconstruct`); **G2 done** (per-cell `cell_levels` wire encoding pinned on all three delta sections, versions bumped, round-trip + L2 + rejection tests green); **G3 partial** — slice α (pure classifier `level-compiler/src/sh_coarsen.rs`: two-phase gate + protection + fixpoint smoothing, 15 tests) and slice ε (render-cpu intra-brick reconstruction golden `reconstruct_delta_probe_tile` + kept-rank offsets, 14 tests) done and reviewed; **remaining: G3-γ** (thread levels into `delta_sections` compaction, synthesize L2 mean, cap-once → AC 1/2, P1/P2/P12) and **G3-δ** (`--sh-coarsen` flag, composed-tile→`BrickClass` provider, protection brush→AABB parse), then **G4**.
...
> **G3 slice status (this session).** α = pure classifier (`sh_coarsen.rs::classify_levels(bricks: &[BrickClass], affinity_dims, protect_aabbs: &[[f32;6]], params) -> Vec<u8>`), reviewed. ε = render-cpu golden (`sh_compose.rs::reconstruct_delta_probe_tile`), reviewed. Reviewer note reconciled: the map-p95 index uses **truncation** (`floor`, matching `relerr_opmap.py`'s `int(...)`), not `round`; and the darkness path is a deliberate **bypass** (spec AC5/P8), which agrees with the operating-point clamp on physical data (a sub-floor brick has near-zero probes ⇒ near-zero error). **Carry-forward the following into G3-γ/δ (contracts α/ε bake in):**
