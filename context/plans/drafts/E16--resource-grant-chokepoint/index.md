...
- *A string-addressed `grant("player.ammo", 20)`* — the `combat-events.md` §5 sketch. Distinct from the typed selector above and weaker than it: the string cannot carry the ammo pool key, and it names `player.ammoReserve` as a grant target when that name is a read-only owner-private *projection* computed from the reserve, not a writable slot. The string API teaches a false model of where resources live.
