...
2. **Bind group.** Add entries to group 4 (bindings 0–2: `lightmap_irradiance`, `lightmap_direction`, `lightmap_sampler`). These bindings are already declared in `forward.wgsl`; this task wires real textures in place of any stub. See the full cross-plan group layout in `context/plans/ready/lighting-spot-shadows/index.md` Task B step 3.
