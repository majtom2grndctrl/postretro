...
| # | Stage | Notes |
|---|-------|-------|
| 1 | Init logging and boot clock, collect raw args, resolve selected mod/content root + optional map path | Minimal pre-window work. Net role is NOT parsed here — it defers past first pixels. `--mod <name>` / `--mod=<name>` is the shipped single-mod boot handle: it names a mod, never a path, and selects `content/<name>`. A value holding `/`, `\`, or `:`, a `.`/`..` or leading-`-` value, and a `--mod` with no value are boot errors, as is the retired `--content-root`. A bare map argument is the raw-path dev bypass: with `--mod` a relative map path resolves inside `content/<name>` (`--mod dev maps/x.prl` loads `content/dev/maps/x.prl`), and one that still starts with `content/` (after any leading `./`; case-insensitively on Windows and macOS) is a boot error with a hint; an absolute path stays as given. Without `--mod` the map path is read from the working directory and the content root is derived from it. |
...
The boot splash is a **renderer-owned** path — a direct boot splash pass that clears the swapchain and draws the decoded logo as a textured quad. It does NOT use the UI system, UI JSON, glyphon, taffy, mod themes, the UI image registry, or `UiReadSnapshot`. The splash advances one frame per redraw, deferring all CPU-heavy work behind visible pixels. After successful command submission, the renderer returns a `PresentHandle`; only frames with a handle advance stage marks and scheduling. Transient or skipped acquire paths return no handle and re-request redraw without advancing.
...
**Mesh-sweep order.** The single mesh model sweep (model upload + clip-index resolve) runs *after* the data-archetype sweep (stage 11). Its upload set unions live mesh entities, every movement descriptor, weapon presentation models, resolved spawner archetypes, and client-suppressed enemies. Runtime materialization never uploads models. Newly accepted or spawned meshes resolve clips and socket bindings against the install-time tables before presentation. The sweep runs *before* the `levelLoad` fire (stage 13), because a `setAnimationState` reaction in `levelLoad` requires resolved clip indices.
...
| Renderer bloom profile (`ModManifest.render.bloom`) | Engine-global. Survives level load/unload and full-renderer rebuild; re-applied from the manifest by `run_deferred_mod_init` on resume, after the renderer is recreated. A committed staged reload replaces it (no start script restores the default); failed, rejected, or stale reloads leave it alone. |
...
Hot reload (debug only) stages entity descriptors, store declarations, the map catalog, and mod-global reaction/crossing/trigger-pool definitions off-thread, then reconciles them on the main thread. Compatible store schemas preserve live values; incompatible changes reject the staged result and preserve the previous descriptors/catalog/globals. Mesh blocks and weapon presentation model paths remain at their installed values until the next level load; ordinary weapon tuning still refreshes live. Complete deletion of a presentation-bearing descriptor is deferred on the same installed-snapshot boundary, even when no live holder is currently observable: network materialization and runtime spawners may still address it during the level. Deletion of a descriptor with no mesh or weapon presentation model takes effect immediately. A successful staged commit atomically replaces the remaining committed snapshots, then recomposes active sets for current level tags without re-rolling the live level. Stale or failed staged results leave committed globals and active sets unchanged. Removed store declarations never clear committed stores.
...
- **Mod discovery / multiple mod roots.** A scan of `content/*/` mod roots plus per-mod load-order resolution. Today one active mod root is selected explicitly with singular `--mod <name>`, by the CLI map path, or by the default dev root.
