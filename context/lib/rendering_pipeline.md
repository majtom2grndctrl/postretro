...
- The DDA face normal replaces the shading normal; the normal map still applies on TOP faces. The displaced position feeds dynamic light direction and attenuation. Depth-derived AO darkens the **SH indirect term only** (bit 8 of `LightTermMask`), which is legitimate because probes at ~1 m spacing know nothing of the receiver's own geometry (§4). Self-shadowing runs a second, shorter DDA toward **dynamic lights only**, capped to a small per-fragment budget; the lightmap keeps static-onto-static occlusion, and competing with it would risk the no-double-counting invariant.
