...
- `sdk/lib/ui/theme.{ts,luau}` — pure theme authoring helper (`defineTheme`) that preserves flat token maps while adding token accessors for editor completion and setup-time validation.
