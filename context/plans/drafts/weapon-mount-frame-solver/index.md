...
- [ ] When barrel/up axes are NOT declared, the tool runs geometric detection,
  ...
  and either refuses to emit a trusted euler or emits it flagged as unverified —
  it never presents a geometry-only guess as authoritative.
- [ ] A geometrically ambiguous weapon (near-symmetric long axis, or barrel and
  stock of similar cross-section) is reported as low-confidence in assist mode
  rather than silently resolved. The declared-axis path is unaffected by geometry.
...
- [ ] Solving at a non-reference `(clip, time)` (e.g. the limitator `reloading`
  clip) is reportable, and the tool surfaces that a rigid bake correct at the
  reference pose is not correct at wrist-reorienting poses (does not silently claim
  a single euler satisfies both).
- [ ] `socket_dump.rs` builds and runs against the shared solve, producing the
  same socket-frame and verify numbers it did before extraction (no drift).
- [ ] The shared solve reuses `postretro_model::gltf_loader::load_model` and the
  existing world-pose sampler — not a reimplementation — so the socket frame it
  solves against is identical to the one the engine mounts at.
...
a confidence/ambiguity signal (e.g. the two end radii and the axis-length ratio,
so a near-symmetric weapon is detectable); (b) resolution of a named socket
joint's world matrix at a given `(clip, time)`, reusing `load_model` and
`sample_clip_looped_world_modified` (the exact sampler
`attachments.rs::sample_modified_world_pose` uses), so there is no frame drift
from the runtime mount; (c) the glTF-space corrective delta `D = S^T · G^T`
...
socket-frame and verify numbers must not change. `gltf_loader.rs` is large (~4400
...
`print_help()` entry. It takes the skeleton model path, the socket name (default
`hand_r`), the reference clip (default `idle_aiming`) and time (default 0), the
weapon glTF path, and the authoring-intent declaration of the weapon-local barrel
and up axes (see Boundary inventory for the exact argument surface and axis
convention). It loads both models via `postretro_model::load_model`, calls the
...
line (preserving any `--grip`/`--scale`/`--socket` the author passes through for
convenience). It supports compose-with-current: given the weapon's current
`--rotate-euler` (the bake already applied), emit the TOTAL euler to re-bake from
the raw source, composed as `D_blender · R_current` before decomposition — the
same composition `socket_dump.rs` does with its args 6-8. The Blender-adapter math
...
corrective in its vertices), mounts it at the resolved socket frame via the Task 1
verify metrics, prints the declared barrel/up axes it validated against (so a
mismatch between the solve run's axes and the check run's axes is visible — see
Open questions), then prints `barrel·+Z`, `barrel·+Y`, `up·+Y`, and exits non-zero
when any is outside tolerance (`barrel·+Z ≥ 0.999`, `|barrel·+Y| ≤ 0.02`, `up·+Y ≥
0.999` as defaults, overridable), naming the failed metric. Check mode uses the
DECLARED barrel/up axes to know what "forward" was supposed to be, so a
mis-identified barrel cannot pass as correct (the false-positive class from the
brief). Also implement the assist/trust boundary shared with Task 2: when the
author omits declared axes, run geometric detection, print the detected
`[side, up, barrel]` with its confidence signal, and mark any emitted euler (solve
mode) or any pass (check mode) as UNVERIFIED — the tool must never present a
geometry-only result as authoritative. A low-confidence geometric result (from the
end-radii/axis-length signal in Task 1) prints a distinct warning. This task owns
the tolerance defaults, the exit-code contract, and the labelling; Task 2 owns the
solve/emit surface it plugs into.
...
`prop_to_gltf.py` → declare barrel/up axes and run `solve-weapon-mount` to get the
euler → re-bake with `--rotate-euler` → `solve-weapon-mount --check` to confirm in
the engine frame. State the reference-pose caveat (a rigid bake is exact at the
solve pose; wrist-reorienting poses need a skinned weapon) and the assist-vs-declared
distinction. Do NOT edit `context/lib/` here — that capture is a promotion step
...
| Declared barrel axis | unit `Vec3` in weapon-local (glTF) frame | CLI arg, weapon-local frame (glTF-frame components of the raw model as loaded) | n/a |
| Declared up axis | unit `Vec3` in weapon-local (glTF) frame | CLI arg, weapon-local frame | n/a |
| Engine forward / up targets | `+Z` forward, `+Y` up (mount target) | same | n/a |
| Socket reference | `SocketBinding::SkinnedJoint`, sampled at `(clip, time)` | `--socket NAME` (default `hand_r`), `--clip` (default `idle_aiming`), `--time` (default 0) | n/a |
...
|---|---|---|---|
| Solver's socket frame == the frame the engine mounts at (no drift) | Task 1 (reuse `load_model` + `sample_clip_looped_world_modified`) | Any reimplementation of load or sampling in the tool would drift it | AC "reuses loader/sampler", AC "socket_dump numbers unchanged" |
...
| Corrective is exact only at the solve `(clip, time)` | inherent (socket frame is pose-dependent) | Check mode at a different pose; Task 4 doc | AC on non-reference-pose reporting |
...
cargo run -p xtask -- solve-weapon-mount \
  ...
  --socket hand_r --clip idle_aiming --time 0 \
...
- **Declared axes: CLI args vs glTF `extras`.** The barrel/up axes are intrinsic to
  ...
  `extras` channel (the project's existing idiom for per-node authored intent —
  sockets, hit-zones, pose-masks) would give solve and check one source of truth and
  make the weapon self-describing. Deferred to keep the MVP arg-only, but this
  solve/check drift class is the reason to revisit `extras` before the CLI surface
  hardens. Owner call; if kept arg-only, Task 3's check should at least echo the
  axes it validated against so a mismatch is visible.
