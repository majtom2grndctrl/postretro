...
| capture-install | capture harness installs a level | moves payloads from its own world like `install_level_payload` | payloads taken; capture's world holds headers only; capture renders identically; every warmup and sample frame reads the installed atlas; capture never reinstalls or reloads |
| seam-outer-halftexel | lightmap `u` in `[0, 0.5/W)` and `(1−0.5/W, 1]` for each group, groups differ at the seam | chart gutter (`CHART_PADDING_TEXELS`) keeps baked UVs off these columns, so only driven UVs reach them — e.g. the helper run against a hand-built BC5 texture, as `sdf_light_select_test.rs` does for its helper; clamp before mapping | group 0 at `u` = 1 reads its own last column; group 1 at `u` = 0 its own first; equal to today's clamp-to-edge |
