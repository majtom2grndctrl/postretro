...
**OctahedralShVolume (id 34):** sibling replacement for legacy `ShVolume` (id 20), carrying base indirect irradiance. `SH_VOLUME_VERSION` is section-internal; current version 11 rejects stale sections. Probe metadata carries validity, density level, and hierarchy scale. Scale 0 is one 4×4×4 affinity brick; each higher wire scale doubles the aligned brick edge. Production selection currently stops at scale 1; compiler measurement overrides exercise scales 0–3. L0 stores valid probes in local x-fastest order and is always scale 0. L1 stores the node's eight corners in corner order; invalid corners are zero tiles. L2 stores one synthesized mean over valid node probes. Empty nodes store no tiles. Partial edge bricks stay L0. Node origins alone advance the stored-tile prefix; every member resolves that origin slot. Payload slots remain x-fastest and are tagged BC6H or `Rgba16Float`. Runtime derives a pointer-free indirection word from validated metadata and composes and samples the same stored atlas. See rendering_pipeline.md §4 "Adaptive base-probe spacing".
