...
- The brain re-aims the destination at the player's *raw* position every tick (`crates/postretro/src/scripting/systems/ai.rs:499-501`; the `SteeringIntent::Chase` arm; `set_destination(registry, outcome.id, player_pos)` is at 501), and `goal_velocity` re-issues full `move_speed` straight at the current waypoint each tick (`agent_steering.rs:461-493`).
...
- Nothing in `tick` diffs the resolved position against the prior position — it writes `t.position = result.position` (`agent_steering.rs:389`) but never measures progress (`agent_steering.rs:226-396`). `AgentComponent` carries no `last_position` / `stuck_ticks` / progress field (`crates/entities/src/components/agent.rs:34-95`).
- Replans fire only on destination drift (`REPLAN_DEST_THRESHOLD` 0.5 m, `agent_steering.rs:65`) or staleness (~30 ticks), via `admit_replans` (`agent_steering.rs:416-453`). `blocked` stays **false** because the path is perfectly routable — the capsule simply can't physically traverse it. A fresh route still ends in a waypoint the wedged capsule must *move* to reach, so re-pathing alone does not unwedge it.
- Compounding: the navmesh erosion is coarse (Chebyshev/cell-quantized, `navmesh_bake.rs:399-441`; funnel string-pulls over portal segments, `nav/path.rs:244-309`), so a sub-cell sliver near pillar corners still lets the capsule make contact; and the separation pass can push neighbors *deeper* into a wedge in waves (`agent_steering.rs:502-544`).
...
- **Test fixture requirement:** AC1a, AC1b, and AC3 require a concave-corner (pillar / interior-corner) trimesh fixture. The only existing steering test fixture is `LWall` (`agent_steering/tests.rs` ~55-190), a convex corner the agent successfully routes around — the opposite geometry and therefore unsuitable. The new fixture must be built the same single-source way as `LWall`: one wall description feeds BOTH the `CollisionWorld` trimesh AND a hand-built navmesh, with a navmesh route that stays valid through the wedge (so `agent.blocked` stays `false`). The fixture must be demonstrated to drive goal-projected progress to ~0 under the harness before stuck-detection is asserted; otherwise the test is vacuous.
