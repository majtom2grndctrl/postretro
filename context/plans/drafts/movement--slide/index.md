...
- [ ] Slope acceleration (D8): on a downhill slope, total horizontal speed during a slide increases per `slopeAssist`; on flat ground the slope term is zero (slide bleeds only by `slideDrag`); on an uphill it bleeds faster. `slopeAssist = 0` adds no slope term — the slide bleeds by `slideDrag` alone (modulo the substrate's own slope-following / ground-stick re-projection, which `slopeAssist` does not govern).
...
- [ ] Time guard: on a sustained downhill steep enough that the slope term outruns `slideDrag`, the slide cannot persist past `SLIDE_MAX_MS` (the backstop forces exit).
