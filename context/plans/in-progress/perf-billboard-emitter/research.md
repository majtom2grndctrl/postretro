...
3. **CPU sim/collect.** Four registry passes per frame — three over the `ParticleState` column (live-count tally, sim snapshot, collect) plus a `BillboardEmitter` snapshot — and per-particle allocations.
...
## Area D — CPU sim/collect (Slice 4)

Four full-registry walks via `iter_with_kind` (`registry.rs:546-563`; storage `[Vec<Option<ComponentValue>>; COUNT]` columns `:428`; `get_component`/`set_component` re-`validate` each call `:671-689`):
...
- **GOTCHA — sim gating.** Visibility is computed in the Render stage (`main.rs:1086`), AFTER the fixed-tick loop (frame order Input→Game→Audio→Render). Sim-stage culling would need a one-frame-stale visible set. Scope Slice 3 to RENDER-stage collector culling only; note sim gating as a follow-up.
