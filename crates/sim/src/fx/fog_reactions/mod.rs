// Fog reaction primitives: tag-targeted and sequenced mutation paths for
// FogVolumeComponent.
// See: context/lib/scripting.md §10.2
...
pub fn register_fog_reaction_primitives(registry: &mut ReactionPrimitiveRegistry) {
    registry.register("setFogDensity", |reg, targets, args| {
        let parsed: set_fog_density::SetFogDensityArgs = serde_json::from_value(args.clone())
            .map_err(|e| ReactionError::InvalidArgument {
                reason: format!("setFogDensity: failed to deserialize args: {e}"),
            })?;
        set_fog_density::dispatch(reg, targets, &parsed)
    });
    registry.register("setFogGlow", |reg, targets, args| {
        let parsed: set_fog_glow::SetFogGlowArgs =
        ...
        set_fog_glow::dispatch(reg, targets, &parsed)
    });
    registry.register("setFogEdgeSoftness", |reg, targets, args| {
        let parsed: set_fog_edge_softness::SetFogEdgeSoftnessArgs =
            serde_json::from_value(args.clone()).map_err(|e| ReactionError::InvalidArgument {
                reason: format!("setFogEdgeSoftness: failed to deserialize args: {e}"),
            })?;
        set_fog_edge_softness::dispatch(reg, targets, &parsed)
    });
    registry.register("setFogFalloff", |reg, targets, args| {
        let parsed: set_fog_falloff::SetFogFalloffArgs = serde_json::from_value(args.clone())
            .map_err(|e| ReactionError::InvalidArgument {
                reason: format!("setFogFalloff: failed to deserialize args: {e}"),
            })?;
        set_fog_falloff::dispatch(reg, targets, &parsed)
    });
    registry.register("setFogParams", |reg, targets, args| {
        let parsed: set_fog_params::SetFogParamsArgs = serde_json::from_value(args.clone())
            .map_err(|e| ReactionError::InvalidArgument {
                reason: format!("setFogParams: failed to deserialize args: {e}"),
            })?;
        set_fog_params::dispatch(reg, targets, &parsed)
    });
    registry.register("setFogAnimation", |reg, targets, args| {
        let parsed: set_fog_animation::SetFogAnimationArgs =
            serde_json::from_value(args.clone()).map_err(|e| ReactionError::InvalidArgument {
                reason: format!("setFogAnimation: failed to deserialize args: {e}"),
            })?;
        set_fog_animation::dispatch(reg, targets, &parsed)
    });
}
...
/// at run time.
pub fn register_sequenced_fog_primitives(
    ...
    registry.register("setFogDensity", move |id, args| {
        ...
        set_fog_density::dispatch(&mut reg, &[id], &parsed).map_err(reaction_to_sequence_error)
    });
    ...
    registry.register("setFogGlow", move |id, args| {
        ...
        set_fog_glow::dispatch(&mut reg, &[id], &parsed).map_err(reaction_to_sequence_error)
    });
    ...
    registry.register("setFogEdgeSoftness", move |id, args| {
        ...
        set_fog_edge_softness::dispatch(&mut reg, &[id], &parsed)
    ...
    registry.register("setFogFalloff", move |id, args| {
        ...
        set_fog_falloff::dispatch(&mut reg, &[id], &parsed).map_err(reaction_to_sequence_error)
    });
    ...
    registry.register("setFogParams", move |id, args| {
        ...
        set_fog_params::dispatch(&mut reg, &[id], &parsed).map_err(reaction_to_sequence_error)
    });
    ...
    registry.register("setFogAnimation", move |id, args| {
        ...
        set_fog_animation::dispatch(&mut reg, &[id], &parsed).map_err(reaction_to_sequence_error)
    });
}
