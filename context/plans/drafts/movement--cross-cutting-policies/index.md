...
Names reuse `movement.md` §2's closed vocabulary. The deferred wall rules (`projectOntoWallPlane`, `reflect`) extend the `horizontal` set with `movement--wall-run` (D8). The carry step runs at the transition edge against the outgoing state's resolved velocity: the `horizontal` transform applies to `component.velocity`; the `boost` transform reads the outgoing state's live boost vector before the new state is written.
