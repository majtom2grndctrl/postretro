...
**Dynamic direct.** Dynamic lights run a diffuse-only per-fragment loop with an influence-volume early-out; no surface, including movers, receives dynamic-light specular. Dynamic spot lights cast shadow maps (depth texture 2D-array pool, comparison sampler). The fixed 96-slot spot pool uses player-selectable 512² / 768² / 1024² layers (low / medium / high, default high); changing the tier updates renderer boot state and rebuilds the pool plus spot caches only at full-init or the next level install. Dynamic point lights cast cube-array shadows (6-face depth pool) at the existing 512² resolution and fixed slot count. Uncached pool slots render cone-culled static world geometry plus entity occluders; cached slots copy cached world depth into the live pool before adding current entity depth. Sun/directional lights cast no dynamic shadows. Light sources: FGD entities (`light`, `light_spot`, `light_sun`) and gameplay effects. Clustered forward+ binning deferred until profiling shows the flat loop bottlenecks. A renderer-owned dynamic world-depth cache reuses static world depth when a dynamic light's source identity and full projection are unchanged: three spot layers at the selected resolution and four atomic 6-face 512² cube units cover the campaign's scripted room-local peaks. Stable winners retain their cache layer across transient pool-slot moves; overflow deterministically falls back to the ordinary pool without evicting a warm winner. On a cold key the cache clears and draws static world depth. Every cached frame copies that depth into the assigned live pool layer before an entity pass loads it and adds current entity depth; a warm key skips the world cull and raster pass. World, skinned, and kinematic receivers sample the resulting minimum-depth pool through the existing PCF path, equivalent to two nearest depth comparisons per tap. Cache textures are copy sources with no fragment bindings, keeping the complete forward inventory at 16 sampled textures with cube support (15 without). Each full-resolution cache is allocated only for its light type; absent types need no placeholder textures. Planning uses fixed-size storage, and level reload invalidates all keys.
...
**Adaptive base-probe spacing.** PRL v11 irradiance data and its v4 direct-SH companion use one stored-atlas geometry for at-rest and composed data. Bake-time classification starts from 4×4×4 probe bricks, then may merge aligned 2×2×2 brick groups. Each emitted node stores its valid L0 probes, eight L1 node corners, or one L2 node mean. Protected, partial, delta-bearing, or over-error regions stay finer; base data is never coarser than a present grid-matched id 27, id 41, or id 45 delta entry. Compose writes each node slot once through the aligned origin workgroup; every loader-accepted scaled node therefore retains a valid probe in that origin brick. One load-derived word carries validity, level, node scale, and stored location to every SH consumer. The shared sampler reconstructs L1 across the complete node edge and keeps the whole-cell path at no more than eight stored-tile reads. World, entity, billboard, fog, and diagnostics therefore see the same field without a node table or another binding. Probe-depth visibility remains dense per probe. Laptop shared-memory iGPUs are the binding smoothness floor, below §10's desktop performance floor.

**Animated direct SH for dynamic receivers.** A script-animated baked light adds its static-occlusion, unit-radiance direct transport through `AnimatedDirectShDeltaVolumes` (PRL section 45) during the pre-frame direct-atlas compose. Brightness and RGB curves scale that delta once through the shared animation descriptor, so kinematic movers and skinned meshes receive the same pulse/color term without receiver-specific wiring or a dynamic-light entry. When a shadow-relevant mover or mesh enters the light's influence, the animated baked tier can promote it to the shared pool: its receiver term crossfades from `(1 − w) ×` the section-45 delta to `w ×` the runtime shadowed term for true self-shadowing. The runtime cone is frozen at the authored rest direction, matching the bake; author a live cone sweep across moving receivers as `light_dynamic_spot` instead.

**Billboard direct scatter.** Valid `BillboardDirectScatterVolume` (PRL section 47) replaces a billboard's directional direct-SH sample with normal-free RGB scatter. The billboard vertex shader samples group 3 binding 17 only. `FrameUniforms.has_scatter` at byte 112 is a load-fixed mode: zero selects dummy/legacy, one selects an immutable static base, and two selects a composed animated texture; both real modes remain nonzero for availability checks. A static base is visible only under `BAKED_DIRECT_STATIC`. A composed texture is visible when either baked-direct bit is set because its compose pass independently removes the static base and animated deltas. Animated maps compose section 48's dense deltas using the section-45 descriptor mapping, shared animation time, and the direct-term mask before the billboard draw. Maps whose scatter comes only from animated `static_light_map` entries use a zero-RGB section-47 base as section 48's grid/validity anchor. The scatter path has neither direct SH nor static-light-map specular; static SDF lights retain their existing spec-light path because section 47 does not bake them. Its runtime direct loop uses `light_count`, excluding both the raw animated-baked forward tail and appended promoted records rather than applying a direct-SH handoff. Missing, invalid, policy-oversized, or device-limit-incompatible scatter selects the exact legacy path: binding 17 is dummy, binding 15 supplies direct SH, static specular remains active, and the runtime loop uses `total_light_count`, retaining the selected-static promoted suffix while still skipping the raw animated-baked tail. Section 48's dense delta, CSR-offset, CSR-light, and descriptor-index buffers must each fit both the active device's storage-binding and single-buffer limits; one failure disables scatter whole for that level.

| Direct receiver | Static baked | Animated baked | Dynamic tier |
|---|---|---|---|
| Static world | lightmap | animated lightmap | — |
| Kinematic mover | direct-SH base | id 45 composed delta; promoted animated tier crossfades to pool self-shadow at rest direction | runtime direct loop |
...
**World specular shadowmask.** Compiler-selected non-SDF static world specular is multiplied by its baked `ShadowmaskAtlas` channel; absent, rejected, or dropped shadowmask data is fully lit, and this world-only signal remains independent of pool-shadow promotion and its crossfade. Decided, not yet built: with the atlas as BC5 groups side by side per layer (`build_pipeline.md` §PRL), one sampling path reads both groups at the fragment's own layer and returns the four masks in one vector, so the per-light select and both consumers — world specular and the promoted-union subtraction — are layout-blind. Each group samples as its own clamp-to-edge texture; bilinear never blends groups. Samples stay hoisted outside every light loop, costing at most one extra sample per consumer per fragment regardless of light count. A too-wide or misaligned atlas resolves to the all-visible placeholder, which is two texels wide so each group reads a real texel.
...
**Promoted animated lights.** A script-animated baked light promotes into the same shadow pool as dynamic and static-promoted lights when a shadow-relevant receiver intersects its influence and the light is portal-reachable. It mirrors static promotion on the *additive* side: a mover or mesh receives `(1 − w) × section-45 animated delta + w × runtime shadowed term`; the runtime cone is frozen at the authored rest direction so it represents the same illumination as the baked delta and no receiver sums the light twice. Only moving receivers gain the runtime term; world surfaces remain `lm_anim` and billboards keep their baked path. The fixed position and frozen cone let the world depth cache exactly as a static promoted light's. Eligibility is runtime-only: no separate compiler selection list exists, but promotion requires section-45 direct-delta affinity membership; sparse descriptor-only animated-baked rows are unpromotable. Eligible lights gate on brightness over the crossfade horizon (a forward-lookahead max of the curve), not instantaneous strobe value, so a strobe neither thrashes the pool nor holds a slot through a long dark trough. Animated candidates contend in the one `MAX_PROMOTED_SPOT`/`MAX_PROMOTED_CUBE` pool through the shared `slot_score` with dynamic and static-promoted candidates; brightness only controls membership in the candidate gate, and the ranker has no intensity term.
...
**Surface Depth (texel-space parallax).** Cobblestone-style surfaces read as having real depth: the specular slot is a two-channel surface map (R specular, G **depth below the surface**, see `resource_management.md` §4.6), and the fragment marches that depth field along the view ray before shading. The carve is INWARD only — the displaced surface never exceeds its real plane, so there is no silhouette artifact and collision is correct with zero work (the player walks on the stone tops, which IS the real plane).

The field is piecewise-constant per texel, and its lattice is the same texel grid `sample_post_retro` snaps albedo to, so stone side faces land exactly on albedo texel edges. Marching a grid of boxes is an exact 2D DDA (Amanatides-Woo), not a fixed-step POM: per texel `T` the solid starts at `D(T)`, and within `T` the ray either entered already inside the solid (SIDE wall hit, normal = the crossed axis negated), meets the top before leaving (TOP hit, normal = geometric), or steps on. The output is a marched UV, a hit depth, and one of five exact face normals.

- The authored depth's UNIT is a compile-time switch, `SURFACE_DEPTH_TEXEL_MODE`, and **texels is the shipped default**. In texel mode the authored value is a count of albedo texels, converted per fragment from that fragment's own texel rate, so the carve is a fixed depth in the lattice `sample_post_retro` already snaps to and the march's worst case stops depending on texture resolution or brush scale. Meters — a world distance, so one material keeps one physical depth however a mapper scaled the face — is the rejected trade, kept available behind the switch because brush UV scale is unconstrained in TrenchBroom and a mapper may reasonably want that invariant back. Either way the shader works in meters internally, solving world-units-per-UV-unit per fragment from `dpdx(world_position) / dpdx(uv)` and takes the march axes from that same Jacobian, so the axes and the scale cannot disagree.
- Parameters are prefix-driven like `shininess` and `emissive_strength` (`postretro-render-data::material::Material::surface_depth`) and ride in the per-material uniform's second 16-byte row, which was already allocated and already zeroed. No new binding, no new sampled texture (the forward pass is at exactly 16/16), and no change to the 128-byte group-0 `Uniforms` ABI. A `has_depth` flag is set at bind-group build time from the loaded specular slot's format, so a material without an `_h.png` sibling skips the march instead of walking an all-zero field.
- The DDA face normal replaces the shading normal; the normal map still applies on TOP faces. It replaces **only** the shading normal. The geometric normal keeps the backface test and the shadow-map receiver bias (see the never-list below). The SH lookup's **normal offset** also keeps the normal-mapped normal: that offset lifts the probe read off the surface to stop bleed through thin walls, and a side-wall normal points *along* the surface, so biasing by it would slide the lookup sideways across the floor instead of lifting it. The SH sampler therefore takes shading, offset, and geometric normals as three separate inputs; a caller with no carve passes the same normal for shading and offset and is unchanged. The displaced position feeds dynamic light direction and attenuation. Depth-derived AO darkens the **SH indirect term only** (bit 8 of `LightTermMask`), which is legitimate because probes at ~1 m spacing know nothing of the receiver's own geometry (§4); it is scaled by the same distance/LOD fade the carve is, so occlusion reaches its flat value continuously rather than stepping at the fade boundary. Self-shadowing runs a second, shorter DDA toward **dynamic lights only**, capped to a small per-fragment budget; the lightmap keeps static-onto-static occlusion, and competing with it would risk the no-double-counting invariant.
- **Never** writes `@builtin(frag_depth)` (the pass runs `depth_compare: Equal` with writes off — a fragment that wrote depth would fail its own test), **never** offsets `lightmap_uv` (charts carry only `CHART_PADDING_TEXELS = 2` of gutter), **never** feeds its face normal to shadow-map receiver bias (that path takes the geometric normal on purpose), and **never** calls a derivative inside the march (gradients are hoisted to the top of `fs_main`). Height is read with `textureLoad` at an explicit mip — no sampler, and the exact quantized values survive.
- The DDA's base mip is a **parameter**, never a hardcoded `0`, and the fade-to-flat LOD is measured in texels of that resident level, so a future streaming pass that drops top mips flattens a surface gracefully instead of popping. The fade is also a straight perf win: at range the texels go sub-pixel and the parallax is invisible.
- **Player on/off switch (D5).** `PlayerOptions::surface_depth_quality` (`player_options.md` §4) is **off/on** — two states, not a quality ladder — and applies by **rewriting the per-material uniform buffer** with `queue.write_buffer` — never by rebuilding bind groups (gameplay allocates nothing, `resource_management.md` §8.2) and never by growing the group-0 `Uniforms` ABI. `Off` packs the historical all-zero second row, so the render is byte-identical to the pre-feature engine at zero cost. `On` is the per-prefix values from `Material::surface_depth()` verbatim, with the full self-shadow budget; it is the default. There is deliberately **no middle state**: the retired `Low` tier only capped `max_steps` and scaled `fade_distance_meters` and never touched `depth_meters`, so it looked identical to the full effect except at grazing angles, where the capped march resolved short and read as a *shallower* carve — a cost lever priced in artifacts rather than a visual tier. Cost is therefore an all-or-nothing choice: someone on a weak GPU turns the march off entirely. The self-shadow budget still rides in the packed march word rather than a WGSL `const`: there is no shader-variant system (§8), so anything the switch must turn off has to reach the shader as data. The derivation is GPU-free and unit-tested in `postretro-render-cpu::{surface_depth, material_plan}`; the renderer only performs the write.
- The march lives in ONE shared snippet (`shaders/surface_depth.wgsl`, §8) concatenated into both the forward and mover pipelines. Movers are in scope; glTF/skinned models are not (they never bind the world material bundle). GPU-free reference and unit tests: `postretro-render-cpu::surface_depth`.

**Surface Depth: never observed on a GPU.** The feature was built without an adapter, so every claim above rests on unit tests over the GPU-free reference and on reading the shader. The data path is proven; the *picture* is not. None of the following is a known defect — each is an open question with a place to look. Delete an entry once someone has actually looked.

- **AA band width at carved edges — remedy applied, look here first anyway.** `sample_post_retro` used to compute its antialias band from `fwidth` of the UV it was handed, which under this feature is the **marched** UV — discontinuous across a quad, so the band measured the parallax step rather than the pixel footprint and widened into a multi-texel blur along every carved edge. It now takes the band width from the **pre-march** `ddx`/`ddy` it already receives for `textureSampleGrad` mip selection, which is the same quantity the un-marched path always computed. The function no longer calls a derivative at all. This was derived, not seen: confirm on a GPU that stone side faces snap to the albedo texel lattice, because that alignment is the aesthetic claim of the whole feature.
- **Side-face SH response.** Side walls take an exact axis normal perpendicular to the floor's, which swings their probe lookup hard. Unknown whether that reads as a pleasing contact darkening at the base of each stone or as an artifact ring around every one.
- **Side faces lose their baked direct term entirely — an open art decision, not a known defect.** The bumped-Lambert correction that makes the static lightmap respond to detail normals is `n_dot_l_bump / n_dot_l_mesh`. For a floor lit from above, a side-wall normal lies *in* the floor plane, so the numerator is ~0 while the denominator is ~1 and the whole baked direct contribution is scaled to nothing: mortar side faces receive only SH indirect. The correction caps its **upside** at 4.0 and has no matching floor on the downside, so the effect is absolute rather than graded. In a baked-light-dominant level — this engine's norm — that reads as a near-black outline around every stone. It may be exactly right (carved mortar genuinely is occluded from a top-down bake) or it may be too strong. Decide by looking, then either add a floor to the correction or write the reasoning down here and delete this entry.
- **The half-texel side-hit UV bias** is reasoned from where a side hit lands relative to the entered texel's centre line. It has never been seen.
- **Depth is authored in meters, but the unit is a switch.** `SURFACE_DEPTH_TEXEL_MODE` (`crates/render-data/src/material.rs`, mirrored in `surface_depth.wgsl` and pinned by `shader_constants_match_the_cpu_reference`) selects whether `depth_meters` is a world distance or a count of albedo texels; each unit has its own authoring table and its own cap, and flipping the constant selects both. Meters keeps a material's physical depth constant however a mapper scaled the face. Texels keep the carve at a fixed depth in the lattice the retro look is built on, and make the march's worst case resolution- and scale-independent — horizontal travel is `N * tan(theta)` texels with the texel rate cancelling out, so a high-resolution texture on a small brush can no longer exhaust the step budget. Texels is now the default: on a GPU the meters table read too shallow, and a direct conversion of it read too deep because the shipped stone sits well below the ~512 texels/m the conversion assumed. Concrete's 6 texels is what the since-retired `Low` tier was effectively producing and what looked best — that tier never reduced depth, it capped the budget at 8 steps, so past ~39 degrees off normal the march resolved short at roughly `8 / tan(theta)` texels. Authoring that depth directly gets the look at every angle rather than only where the budget ran out; the observation that a budget cap reads as a depth change, not as a cheaper version of the same picture, is also why D5 collapsed to off/on. Still eyeballed on one map, not measured.
- **Every tuning dial is a guess.** Per-prefix carve depths, the AO strength, the LOD fade window, and the per-prefix step caps and fade distances were chosen to be plausible and bounded, not measured against a frame.
- **No cost measurement exists.** No `POSTRETRO_GPU_TIMING` run (§12) has ever included this march. Per-fragment cost at the §10 perf floor is unknown, and the step caps are budgets nobody has priced. D5's two states are therefore the whole cost story: the only measured-free claim the engine makes is that `Off` costs nothing, because it uploads the pre-feature bytes.
...
Billboard instances come from `BillboardEmitterComponent` particles packed by `ParticleRenderCollector` each frame. The collector walks `ParticleState` entities in the entity registry, buckets them by `SpriteVisual.collection`, and hands the packed byte slices to `SmokePass::record_draws`. Bind group 6 carries a single shared sprite instance storage buffer sized to the frame's total live sprites; each collection draws from its own region via a `has_dynamic_offset` bind group (per-collection start offsets are padded to the 256-byte storage dynamic-offset alignment, the 32-byte per-instance stride unchanged within a region). The buffer grows on demand when a frame's padded total exceeds capacity. One collection still issues one draw call; there is no per-collection sprite cap.

**Collection-id keying.** A sprite *collection* is keyed by a **collection id** — the pair `(asset reference, draw contract)` — not by the asset reference alone. Distinct draw contracts over one asset (differing loop period, emissive, or specular) become distinct collections rather than colliding at registration. GPU texture arrays and views are cached by asset reference. Each collection id retains its own `SpriteDrawParams` uniform, group-1 bind group, region, and draw call, so the one-draw-call-per-collection rule and per-region layout above hold unchanged. Each producer derives the render-collect key locally from its matching raw contract: projectile bodies agree across peers because their shared descriptor supplies that contract, while emitters and fixed impacts use local fixed contracts. The key is never replicated or baked.
...
Promotion stills may additionally use capture-only `force_promotion` entries
(`tag`, `weight` in `0.0..=1.0`) for tagged section-45 animated lights. The
harness first makes the normal receiver/pool decision, then pins that assigned
row's crossfade `w`; it neither grants a slot nor changes ranking or cache
...
| 2 | **Runtime direct lighting + shadow receipt** — mesh-specific layout over the same underlying GPU buffers forward's group-2/group-5 shadow resources use, omitting forward's SDF-factor and scene-depth entries the mesh must not sample. b0 runtime light records (compact dynamic-tier prefix, raw animated-baked tail, selected-static suffix); b1 influence volumes; b2 scripted-animation descriptors; b3 anim samples; b4 params uniform (total light count / time / `LightTermMask` / ambient floor / dynamic-tier and descriptor-bearing-prefix counts); b5 spot shadow depth 2D-array; b6 comparison sampler; b7 light-space matrices uniform; b8 conditional cube-array depth; b9 promoted-depth cache spot layers; b10 conditional promoted-depth cache cube faces. |
...
- **Metallic/roughness PBR** — no energy-conserving BRDF, no metallic or roughness inputs, no image-based specular. The authored material vocabulary is a fixed, closed inventory: diffuse, specular intensity, tangent-space normal, emissive, and height — the last riding in the specular slot's second channel rather than in a slot of its own (`resource_management.md` §4). Widening that inventory is a scoped decision each time, not a step toward PBR.
