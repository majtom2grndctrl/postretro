...
pub(crate) fn named_reaction_from_lua(value: LuaValue) -> Result<NamedReaction, DescriptorError> {
...
pub(crate) fn progress_descriptor_from_lua(
    table: &Table,
...
pub(crate) fn crossing_descriptor_from_lua(
    value: LuaValue,
...
pub(crate) fn primitive_descriptor_from_lua(
    table: &Table,
...
pub(crate) fn sequence_steps_from_lua(arr: &Table) -> Result<Vec<SequenceStep>, DescriptorError> {
    let len = arr.raw_len();
...
pub(crate) fn get_required_u32_lua(
    table: &Table,
    field: &'static str,
