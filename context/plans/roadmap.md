...
- [ ] **Phase 7 — Scale + dedicated-server readiness.** Validate 16 players within a host-upstream bandwidth budget (priority-accumulator; interest management via portal/PVS if needed); prove the headless server entry point (dedicated-server split). This phase's loaded/jittery, many-remote conditions are also the home for the two deferred Phase 3 remote-interpolation refinements below — #2 in particular scales in severity with player count, and both are invisible on clean LAN co-op. Fold them in here as one tightly-scoped follow-up spec, **unless** a Phase 5/6 combat-under-latency playtest surfaces either sooner (a stuttering/briefly-frozen remote *enemy* reads worse than a teammate) and justifies pulling it forward:
