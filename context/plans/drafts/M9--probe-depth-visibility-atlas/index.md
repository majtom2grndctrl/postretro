...
- **Far sentinel value.** A sky-miss ray needs a finite "open" distance for the moments. Must stay f16-representable and large enough that an open probe reads as fully visible under spec #3's Chebyshev comparison. Strong lean: derive the sentinel from `cell_size` — a small multiple of the cell diagonal (e.g. 2–4×). The Chebyshev interpolant only ever compares a probe's stored depth against probe-to-fragment distances, and those distances are bounded by the spacing of the 8 surrounding probes — never by the whole map. A cell-relative sentinel reads as "fully open" at exactly the scale the interpolant operates in, scales gracefully across the wide range of map sizes modders author, and is trivially f16-representable. Spec #5's coarse open-area probe spacing strengthens this: the sentinel then tracks local probe density automatically. Confirm against spec #3 when that spec is written — the exact multiple is a judgment call for the interpolant author. A fixed global meters constant is the fallback if spec #3's Chebyshev comparison requires a map-scale reference.
