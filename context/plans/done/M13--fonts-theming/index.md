...
- **Token aliasing** (semantic tokens defined as references to primitive tokens, e.g. `"critical": "{red.500}"`, so remapping a primitive cascades through its semantics) — deferred until a design-system-scale consumer exists. Additive later: theme entries widen from literal values to the same value-or-reference untagged-union trick the descriptor fields use, no wire break.
...
- [ ] A token color resolves to the active theme's RGBA in the produced draw list; an unknown color token draws opaque magenta (CPU draw-list assertion). The warn-once-per-build behavior is a code-review/manual gate — the harness has no log capture, matching existing fallback-path practice.
- [ ] A spacing token on `gap`/`padding` resolves into layout: child rects sit at the theme-defined spacing (CPU layout assertion), and an unknown spacing token lays out as `0.0` (warn is a review/manual gate).
- [ ] A `text` node with `"font": "mono"` measures and shapes with the monospace face: for content chosen with provably divergent advances across the two faces (mixed narrow/wide glyphs, e.g. `"lllmmm"`), the measured width differs from `body` by more than the harness epsilon — not a bare `!=`. An unknown font token falls back to `body` (warn is a review/manual gate).
- [ ] A `ThemeDescriptor` round-trips through serde JSON, and an override containing only some token names resolves those to the override values and every unnamed token to the engine default (merge is per-token, not per-category). A key beyond the required set (e.g. `cyan.500`) round-trips, merges, and resolves from a widget reference exactly like a required one.
- [ ] Installing a theme override while a retained gameplay tree is alive rebuilds the tree on the next frame: the new token values appear without any descriptor change (theme-generation gate). Asserted at the `UiPass` level (or via a `#[cfg(test)]` accessor), where the generation gate lives — it is not observable from a `UiTree`-level test.
...
- [ ] No QuickJS/Luau bridge code is added; the theme is reachable only through Rust (engine default, engine-side setter, tests). Review/grep gate, not a runnable test.

## Tasks

### Task 1: Theme model + wire format
New `render/ui/theme.rs`: `UiTheme` with the three category maps, the engine default theme carrying the required token names — colors `critical` / `warning` / `ok` / `panel.default` (a literal flat key, dot and all), fonts `body` / `mono`, spacing `xs` / `s` / `m` / `l` — the serde `ThemeDescriptor` (camelCase wire: `colors` / `fonts` / `spacing`, each an optional map defaulting empty), and the per-token override merge. Default `fonts.body` is `"Inter"` (the embedded face's family); `fonts.mono` ships as a placeholder string Task 3 reconciles to the chosen mono face's real family name. Token lookup helpers return `Option` so resolution sites own the fallback-and-warn. Round-trip + merge tests. Pure data — no rendering, no taffy. Declare the module in `render/ui/mod.rs`.
...
In `render/ui/text.rs`. First sub-step: commit the mono TTF + license file under `content/base/fonts/` — the `include_bytes!` embed cannot compile without it, and the asset lands in D's worktree (the concurrent TW worktree never needs it). Then: register both faces in `build_font_system`/`UiTextRenderer::new`; add `family: String` to `UiText` as a new `UiText::new` parameter (live call sites: `tree::collect_node`'s `UiText::new` push, the splash/demo paths, the text.rs tests); `shape_text` selects `Family::Name(&t.family)` per line; `measure_run` gains a `family: &str` parameter (callers: `tree::measure_node`, text tests). Phasing: Task 3 passes the existing body-family constant (`UI_FONT_FAMILY`) at every new call site, so it compiles and behaves identically standalone; Task 4 swaps those constants for the token-resolved family carried on `NodeContext::Text`. This task also **updates `theme.rs`'s default `fonts.mono` placeholder to the chosen face's real family name** (from its TTF `name` table) — leaving the placeholder would silently fall back to `body` in Task 4 and defeat the mono-divergence AC — and asserts the embedded faces register exactly those family names, mirroring the Inter magic/family build-tests for the new face.

### Task 4: Resolution threading + theme generation
Thread the theme through the build path: `UiTree::from_descriptor(&AnchoredTree, &UiTheme)` passes the theme down the whole free-function build chain — `build_node` → `build_stack` / `build_grid` — resolving every `ColorValue`/`SpacingValue`/`font` token with the three fallback-and-warn behaviors. Spacing resolves to `f32` *before* `container_base_style`, whose resolved-scalar signature stays unchanged; colors and the family resolve into `NodeContext` (the family replacing Task 3's interim constant). `Renderer` owns `ui_theme` + generation and the engine-side setter; `RetainedGameplayTree` records its build generation; `UiPass::layout_tree` / `layout_gameplay_tree` gain the theme (plus the generation, for the gameplay gate) — callers: `record_splash_ui` and the gameplay record block in `render/mod.rs`, plus the gate tests — and the gameplay rebuild gate ORs in the generation change. The `from_descriptor` signature change breaks every direct test constructor — the `tree.rs` inline tests, `gameplay_ui_gate_test`, `demo_ui_gate_test`, and `splash_layout_test` all call `from_descriptor`/`build_draw_data*` — so this task keeps them all compiling by passing the default theme (Task 5 enriches them with token cases). When editing the `NodeContext::Text`/`Panel` literals in `build_node`, follow the concurrency note's rule: list fields explicitly, no `..` in struct literals. Depends on Tasks 1–3.

### Task 5: Demo-on-tokens + CPU gate
Switch `demo::build_demo_descriptor`'s text color to a color token and the swatch label's font to `mono`. Extend the CPU harness (`splash_layout_test` / `demo_ui_gate_test` patterns) to cover the AC set: resolution, fallbacks, mono measurement, merge, and the theme-change rebuild. The theme-generation rebuild is **not** reachable from a `UiTree`-level test — the gate lives on `UiPass::layout_gameplay_tree` — so assert it at the `UiPass` level (or through a `#[cfg(test)]` generation/rebuild accessor on the pass), not by extending the tree-level harness. Splash output stays byte-identical (literal colors resolve to themselves).
