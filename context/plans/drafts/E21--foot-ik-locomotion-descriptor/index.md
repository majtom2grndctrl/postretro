...
### In scope
- A foot-IK `PoseModifier` variant: a two-bone (hip-knee-ankle) analytic solver per leg, operating on masked local-TRS joints, consuming per-foot ground-probe results from `PoseInputs`. Wgpu-free and collision-free.
- Game-logic ground probes: the fixed tick computes each IK entity's animated foot world positions from the unmodified world pose, casts short downward rays against the collision world, and writes the results into the entity's `PoseInputs`.
...
### Task 5: Foot-IK two-bone solver modifier
Add a foot-IK `PoseModifier` variant to the stack (the prerequisite plan's open enum), operating on masked local-TRS leg joints. Per leg (hip → knee → ankle/foot), given the leg-chain mask and one `FootProbe` result from `PoseInputs`, run a planar two-bone analytic solve so the ankle reaches the probed ground-contact target: forward-compute the current hip and ankle world positions from the chain's local transforms, solve the knee angle from the two segment lengths and the hip→target distance (clamped when the target is out of reach so the leg never hyperextends), and write back the hip and knee local rotations; orient the foot toward the probed ground normal within an angular limit. A probe miss leaves that leg on its clip pose. The solver reads only `PoseInputs` and the local buffer — no collision, no wgpu. Add `crates/model` unit tests: flat-ground plant, sloped-ground plant with normal alignment, out-of-reach clamp, and miss fallback.

### Task 6: Game-logic ground probes feeding `PoseInputs`
Extend the tick→collector plumbing (from the prerequisite plan) with per-foot ground probes. Add a foot-probe array to `PoseInputs` (each entry: contact height, ground normal, hit flag). In `simulate_tick` (`crates/postretro/src/sim/mod.rs`), for each IK-enabled animated entity, compute its animated (pre-IK) foot world positions by sampling the unmodified world pose (`postretro_model::anim::sample_blended_world` / `sample_clip_looped_world` — already collision-free) at the entity's current anim params, transform each foot joint model→world via the entity transform, and cast a short downward ray with `collision::cast_ray` (or `cast_ray_combined` when movers are present) capturing time-of-impact and surface normal; write the results into the entity's `MeshComponent.pose_inputs`. This stays in the tick (the collector borrows the registry immutably) and reuses the walkable-normal convention (`COS_WALKABLE`) already used by movement ground-stick. Foot joints come from the leg-chain / foot-target tags (Task 7). Add a determinism-harness test (`sim/determinism_tests.rs`) asserting repeatable probe outputs.

### Task 7: Leg-chain and foot-target tagging at load
Extend the loader's pose-mask reading (from the prerequisite plan) with leg-chain and foot-target tags: per-node `extras` mask names identifying each leg's hip→knee→ankle chain and its foot/ankle target joint, reindexed through the same topo remap as the other masks. Build the foot-IK stack entry (one per leg) from these masks when the archetype's sync mode includes IK planting, and surface the per-leg foot joint index on `LoadedModel` so the tick's ground probe (Task 6) knows which joints to project. Mask names follow the Boundary inventory. Unknown/malformed tags warn, never fail.
...
## Rough sketch
- `crates/model`: `PoseModifier::FootIk { .. }` variant on the prerequisite plan's enum; a two-bone solver helper; `FootProbe { contact_height: f32, normal: Vec3, hit: bool }` added to `PoseInputs`; per-clip `travel_speed: Option<f32>` on the loaded clip metadata; per-leg foot joint index + leg masks on `LoadedModel`.
...
| leg chain (glTF `extras`) | leg mask via `read_pose_masks` | node `extras` `poseMask: "legL"` / `"legR"` | topo joint set |
| foot target (glTF `extras`) | per-leg foot joint | node `extras` `poseMask: "footL"` / `"footR"` | single joint |

Leg/foot masks extend the prerequisite plan's `poseMask` `extras` mechanism; they are glTF-authoring only.

## Open questions
- Number of legs: the solver and `PoseInputs.feet` array assume bipeds (two legs). Quadrupeds would need four probe slots; sizing the array generically vs. fixing it at two is a decision for the implementer — biped-fixed keeps the POD small and matches the shipped enemy set.
- `travelSpeed` measurement for clips whose root motion is not purely forward (strafe/turn clips): this plan measures XZ displacement magnitude. Directional locomotion is a non-goal, so a turn-in-place clip with near-zero net displacement should carry a descriptor override rather than rely on the derived value.
