...
| `lightmap_scale_region` | brush | Compiler-only per-surface lightmap density override; excluded from world geometry, collision, and runtime entities. A chart whose first face vertex lies in the region AABB uses `global_density / _lightmap_scale`; overlapping regions resolve last-defined wins in map entity order. | `_lightmap_scale` (finite positive float, default 1.0; below 1 is coarser, above 1 is finer) |
| `sh_protect_volume` | brush | Forces intersecting SH base-storage bricks and coarsened delta-section bricks to L0; excluded from static world geometry, collision, and runtime entities | `dilation` (world/engine-meter AABB expansion; default 0; negative rejected) |
| `streaming_seam_volume` | brush | Compiler-only portal cut and preferred far-side SH warm-up; never blocks a door or changes visibility | Exactly one convex brush spanning the intended portal plane; must match at least one portal with positive-area intersection and interior on both sides |
| `stream_resident_volume` | brush | Compiler-only pin of SH clusters intersecting its region, including their baked owner closure | Exactly one convex brush; must overlap at least one runtime cell by positive AABB volume |
| `stream_priority_region` | brush | Compiler-only rank for optional SH prefetch and seam warm-up under pressure | Exactly one convex brush; `_stream_priority` integer 0–3 (blank defaults to 0); must overlap at least one runtime cell |
...
- **`lightmap_scale_region`** — compiler resolves the brush hull to an ordered world-space AABB plus bounding planes and retains it only through lightmap chart planning. The first chart vertex (`p0`) decides AABB membership; source entity order provides deterministic last-defined-wins precedence. It emits no PRL payload and never enters the static world brush set.
- **`sh_protect_volume`** — compiler resolves the brush hull to a world/engine-space AABB, expands all faces by `dilation` meters, then forces every intersecting 4×4×4 SH base-storage brick and coarsened delta-section brick to L0 before seam smoothing. It emits no runtime entity or PRL section.
- **SH streaming hint brushes** — `streaming_seam_volume`, `stream_resident_volume`, and `stream_priority_region` are peeled from world geometry before BSP. After portal/cell construction, seams resolve against convex portal polygons and split the canonical cluster partition; resident and priority regions resolve against runtime cell AABBs. Only id-49 metadata and the resulting id-50 cluster packing change. They emit no runtime `MapEntity` or non-SH payload.
...
9. **Octahedral irradiance volume and light-selection bakes.** Bakes static-light indirect irradiance into octahedral atlas tiles and isotropic Chebyshev depth moments. When animated lights are present, also bakes the sparse indirect-only delta tile companion for runtime composition. It also completes direct-SH delta and entity-shadow selection before atlas preparation.
10. **Atlas preparation.** UV-unwraps world geometry and packs charts into the lightmap atlas. Per-layer atlas dimensions are bounded (cap 8192² per layer, which requires matching device `max_texture_dimension_2d` support — checked at renderer init); default density is 0.04 m/texel, and a per-map `_lightmap_density` worldspawn KVP opts a map into finer density. Charts that overflow one layer's area spill into additional array layers (up to a fixed layer cap); a single chart too large for one layer hard-fails the build.
11. **Fused lightmap and shadowmask bake.** Ray-casts per-texel irradiance and dominant incoming light direction from all static lights against the global BVH. Static `static_light_map` shadows are baked as **soft area-light visibility** (stratified shadow-ray sampling of each emitter, multiplied into irradiance), not a hard 1-texel gate; an authored `_light_size`/`_angular_diameter` of `0` short-circuits back to a single hard ray. A per-light warning fires when an emitter is too small to soften at the atlas density (sub-texel penumbra). Skipped when the map has no static lights.
...
section-presence decisions, an uncached metadata pass resolves authored streaming hints
against finalized portal polygons and runtime cells, partitions cells, and addresses only
...
- **Foreground stage.** Concurrent lifecycle stages can advance together, but the TUI highlights
  exactly one foreground stage. The fused lighting bake keeps ShadowmaskAtlas live in the background
  while Lightmap Bake owns foreground focus.
...
| DirectShVolume | 35 | When the map has static baked lights; metadata-derived stored static-direct layer-aware octahedral irradiance for movers/skinned meshes and legacy billboard fallback; BC6H at rest; no depth moments (read from id 34); same id 34 node set, slot order, geometry, format, and payload contract as OctahedralShVolume; section-internal `DIRECT_SH_VOLUME_VERSION` |
...
| ShadowmaskAtlas | 42 | When usable `EntityShadowLights` are emitted; per-selected-light baked world-visibility masks, one mask slot per selected light, with `0xFF` entries for globally dropped masks. BC5 `.rg`: four mask slots as two groups side by side per layer; omitted when lightmap layers are too wide to double |
| KinematicGeometry | 43 | Version 6 (v7 with mover sound keys decided, not yet built) when the map has at least one `kinematic_mover`; origin-relative mover vertices/indices/face metadata, static spin/blocking authoring, closed-mover portal associations, carried dynamic-light links, and `kinematic_waypoint` records |
...
| ClusterDirectory | 49 | Always in current compiler output; deterministic cell clusters, SH-family ownership/halo metadata, and authored seam/pin/priority hints; container-entry and internal version 2 |
...
v2 directory preserves the old 40-byte header and existing tables; its final header
words count appended sorted seam portal IDs and 16-byte cluster hint records. Seam cuts
keep marked portal endpoints in different clusters, even across alternate portal paths.
Pins and max-on-overlap priority are per-cluster hints; they never reuse the existing
...
**Delta tile layout (ids 27, 41, 45):** Each octahedral texel stores three little-endian f16 RGB halves. Probe validity comes from the section mask and kept set; delta texels carry no alpha. Disk payload and renderer storage-buffer layout are identical. Base atlases ids 34/35 keep their independent format, including alpha where their sampler contract needs it. Billboard scatter id 48 also keeps its independent four-half sample format.
...
**Animated billboard-scatter cap:** Id 48 has an independent 64 MiB maximum encoded-section size because every retained CSR entry expands to a dense 64-probe `Rgba16Float` block even when id 45's compact payload is empty. Compiler predicts this size from finalized id 45 before scatter cache lookup or bake and withholds ids 47/48 together on overage; pack repeats the guard. Loader validates container bounds, then soft-drops an over-cap id 48 before decode and preserves id 45. Renderer independently checks every id-48 storage buffer against the active device's storage-binding and single-buffer limits; failure binds the dummy scatter texture and selects legacy billboard lighting. The cap is policy only and adds no wire field or version change. Valid empty id-45/id-48 CSR pairs remain present so compose can base-seed.

**Lightmap (id 22):** versioned section carrying the baked irradiance and dominant-direction atlases. Irradiance is BC6H (`Bc6hRgbUfloat`) at rest by default — ~8× smaller than the uncompressed `Rgba16Float` debug path, hardware-decoded and -filterable; current static direction is `Rg8Unorm` octahedral on a nearest sampler (octahedral lerp ≠ slerp, so it is never compressed or linearly filtered). The loader still accepts the legacy `Rgba8Unorm` direction tag, whose unused `ba` channels were padding. Both atlases are `texture_2d_array`: charts that overflow one 8192²-capped layer spill into additional layers rather than failing the bake. BC6H is lossy, so the lightmap stage is exempt from the byte-identical determinism invariant (correctness is round-trip within tolerance; the cache keys on inputs regardless).

Wire layout (format version 2, all little-endian; source of truth `crates/level-format/src/lightmap.rs`):

- **Header (48 bytes):** `u32 version` (= 2; pre-v2 sections — whose first u32 was `width` — are rejected at parse as `InvalidData`), `u32 layer_count` (shared by both atlases, ≥ 1), then the decoupled irradiance and direction descriptors. Irradiance: `u32 irr_width`, `u32 irr_height` (per-layer, pow2 ≥ 4), `f32 irr_texel_density` (m/texel, informational), `u32 irr_format` (0 = Rgba16Float, 1 = Bc6hRgbUfloat), `u32 irr_total_bytes` (all irradiance layers combined). Direction: `u32 dir_width`, `u32 dir_height` (per-layer, pow2 ≥ 1), `f32 dir_texel_density` (informational; may differ from irradiance), `u32 dir_format` (0 = legacy Rgba8Unorm octahedral, 1 = current Rg8Unorm octahedral), `u32 dir_total_bytes` (all direction layers combined). Irradiance and direction dimensions are independent; the static bake keeps full-resolution irradiance but reduces direction after compositing at `--direction-texel-scale` (default 2), so `dir_width = irr_width / scale`, `dir_height = irr_height / scale`, and `dir_texel_density = irr_texel_density × scale` without a format-version bump.
...
- **Direction blob (`dir_total_bytes`):** layer-major. Each layer is `dir_width × dir_height × 2` bytes for current Rg8Unorm output (or ×4 for accepted legacy Rgba8Unorm), row-major.
...
**ChunkLightList (id 23):** per-chunk index lists into the static (baked-tier) spec-light set — dynamic lights are excluded, riding the runtime shadow-map path. The per-chunk cap is 256. Contained point and spot lights have hard first-tier precedence. Other overflow candidates rank by model-aware closest-AABB influence, then compacted static-light slot. This prioritizes strong lights across chunk faces, but neighboring chunks may still retain different subsets at an influence threshold. Admission is **err lit**: the list must carry every static light that can shade a receiver surface inside the chunk. The renderer's forward specular pass iterates only a chunk's list with no fallback, so a dropped reachable light leaves a chunk-plane gap in the specular highlight — the bake's admission test is the renderer's correctness boundary here.

**OctahedralShVolume (id 34):** sibling replacement for legacy `ShVolume` (id 20), carrying base indirect irradiance. `SH_VOLUME_VERSION` is section-internal; current version 11 rejects stale sections. Probe metadata carries binary validity, density level, and hierarchy scale. Scale 0 is one 4×4×4 affinity brick; each higher wire scale doubles the aligned brick edge. Production selection currently stops at scale 1; compiler measurement overrides exercise scales 0–3. L0 stores valid probes in local x-fastest order and is always scale 0. L1 stores the node's eight corners in corner order; invalid corners are zero tiles. L2 stores one synthesized mean over valid node probes. Empty scale-0 bricks store no tiles. Partial edge bricks stay L0. Every scaled node retains at least one valid probe in its aligned origin brick, so the runtime's elected origin workgroup can write its stored slots; the loader rejects a section that violates this invariant. Node origins alone advance the stored-tile prefix; every member resolves that origin slot. Payload slots remain x-fastest and are tagged BC6H or `Rgba16Float`. Runtime derives a pointer-free indirection word from validated metadata and composes and samples the same stored atlas. See rendering_pipeline.md §4 "Adaptive base-probe spacing".

**DirectShVolume (id 35):** baked static-direct octahedral irradiance for entities and billboards. `DIRECT_SH_VOLUME_VERSION` is section-internal; current version 4 rejects stale sections. It carries no probe metadata or depth moments: id 34 supplies validity, density levels, and node scales. Its stored-atlas geometry and node-major payload slots must exactly match id 34's stored set; loader cross-checks geometry and payload length before use. The irradiance blob is layer-major, matching id 34: all BC6H block bytes or uncompressed `Rgba16Float` texels for layer 0, then layer 1, etc. Stored BC6H at rest. Emitted only when the map has static baked lights. Runtime: sampled by skinned-mesh and billboard shaders, gated by `has_direct`; forward and fog pipelines bind but do not sample it.

**DirectShDeltaVolumes (id 41):** required sparse CSR companion for `EntityShadowLights`. Runtime promotion is all-or-nothing: every selection index in `EntityShadowLights` must appear in `affinity_lights` at least once. Missing, malformed, or partial direct deltas clear both sections at load so a selected light never runs as both direct SH and runtime light. Layout mirrors `DeltaShVolumes` affinity cells (`affinity_factor = 4`, `affinity_dims = ceil(DirectShVolume.grid_dimensions / 4)`, CSR `affinity_offsets`, and flat `affinity_lights`) but carries no animation descriptor mapping. Each affinity cell has its own coarsening level; each CSR entry stores tiles only for that cell's kept probes, by kept rank. L2 stores one synthesized valid-brick mean tile. Production bakes coarsen id 41 by default under the runtime-safe direct envelope; worldspawn `_sh_coarsen "0"` short-circuits classification and preserves uniform L0. Ids 27 and 45 remain uniform L0 until their animation/script amplitudes have a bounded runtime contract. `affinity_lights` entries are selection indices: zero-based positions in the `EntityShadowLights` list, not `AlphaLights` indices. Deltas use the same direct bake math as `DirectShVolume` before BC6H compression and are clipped to each selected light's reach. Compiler-side conservative dropping may omit a bounded-negligible entry through the existing CSR absence encoding, but always retains coverage for every selection index — including a selection whose baked direct-SH contribution is entirely zero (e.g. a spot whose cone reaches no valid probe). The baked delta is the far-LOD crossfade term, independent of the light's runtime near-tier shadow-pool term, so a zero entry means "baked contribution is zero here, use the runtime term," never "drop the light"; any reach- or contribution-based cull must preserve this canonical entry, or promotion clears both sections at load. Planning and baking share the same affinity-reach policy and assert exact CSR identity at their handoff, so the working-set gate cannot budget a different id-41 shape from the one that is materialized.

**ShadowmaskAtlas (id 42):** optional sibling for `EntityShadowLights`, used by entity→world shadows from promoted static lights. Header is little-endian `u32` format tag (BC5 side by side is the only value), `width`, `height`, `layer_count`, `selected_light_count`, followed by one slot byte per selected light (`0..3` = mask slot, `0xFF` = no mask/dropped). Slot bytes pad to 4-byte alignment. Payload is layer-major BC5 `.rg`: per layer, one `2 × width` by `height` plane of 4×4 blocks, group 0 (slots 0/1) in the left half and group 1 (slots 2/3) in the right; decoded `255` = fully visible. Width and height are nonzero multiples of 4. Dimensions and layer count match the Lightmap irradiance atlas; header `width` is the lightmap width, and the tag implies the doubled texture width. Parsing rejects an unknown tag, a length that disagrees with the tag's arithmetic, misaligned or zero dimensions, and an out-of-range slot; a payload in the retired untagged raw layout is rejected as a named format mismatch. Compiler consumes `EntityShadowLights` order; it never reselects from `AlphaLights`. `_bake_only 1` lights have no entries because they are absent from `AlphaLights` and the selected set. Channel assignment graph-colors selected lights that affect the same texel. A deterministic bounded exact search preserves the established channel/drop result on ordinary inputs. If exact search proves the overlap graph is not 4-colorable, masks drop globally from lowest intensity upward. If pathological overlap exhausts the exact-search budget, a deterministic conservative fallback processes the remaining masks from highest to lowest retention priority and drops a lower-priority mask when retained neighbors already occupy all four slots. Each light owns one slot, global across every atlas texel; its slot stays consistent everywhere it appears. Coloring therefore runs once, deterministically, after all graph work joins. The composite is **non-additive** — per-texel overlap graph plus graph-coloring, unlike the Lightmap's additive layer sum. Overlap membership is **unshadowed**: the shared exact chart walk applies the geometric predicate on light parameters, texel world position, and surface normal before any visibility ray is cast. A conservative authoritative light-influence-AABB × chart-world-AABB prune bounds that walk; the exact predicate remains final authority. Occlusion changes the stored value, never membership, so channel assignment is stable under occlusion edits. Making coverage shadow-aware would be a format-level decision, not a bake tweak. The selected-light graph uses an `n × n` one-byte atomic adjacency matrix, not materialized per-(light, texel) membership. The graph's node set is the selected-light list itself because slot bytes are written by selection index; skipped work never drops a node. After coloring, the compiler allocates exactly one raw RGBA8 fill (one byte per slot) and fills it with atlas layer outermost, then encodes it to BC5 and drops the fill before the section is cached or returned; encode scratch stays within a few raw layers. Whole-section cache storage streams the peak-overlap count, the section header, and the encoded payload into the atomic cache entry; it never materializes a second contiguous section payload. The uncached path streams selected lights in batches of at most four; the cached path streams one partition at a time. Both write serially into the fill and drop each batch or partition immediately. Partitions for globally dropped lights are still populated and cached. The graph never reads partition cache entries: it reruns on a section-memo miss, while a whole-section hit skips graph and fill work.

**ShadowmaskAtlas at rest.** BC5 `.rg` behind the in-payload format tag, at half the raw bytes; raw RGBA is retired, with no second decode path. Four mask slots per texel: slot `s` addresses group `s / 2`, channel `s % 2`. The two groups sit **side by side** in each layer, so the layer count stays the lightmap's and spends none of the array-layer budget. Masks mix hard 0/255 texels with penumbra, so the encoder picks BC4's six-value mode (explicit 0 and 255) per block when it lowers error; normal-map BC5 keeps min/max mode. A pre-tag payload loads fully lit with a named format-mismatch warning; fixtures re-bake. The encode runs in the bake before the whole-section memo, so warm output stays byte-identical to uncached — the BC6H lossy exemption does not extend here. A level whose lightmap layers cannot double within the pinned 8192 texture width omits id 42 with a named warning, before any memo probe or fill. A misaligned atlas fails the build in every profile rather than emitting a truncated payload. `--verbose` reports the bake's peak per-texel mask overlap, the input to raising mask capacity; nothing acts on it.

**KinematicGeometry (id 43):** optional section for deterministic brush movers. Version 5 carries mover identity/path, local collision/render geometry, static spin authoring, block/crush policy, optional auto-close override, named transition events, and per-mover portal associations for movers closed at their docked pose. Version 6 appends per-mover carried dynamic-light links (`AlphaLights` index plus local offset). Both additions are presentation-only: portal associations drive camera portal occlusion, while carried links resolve to local runtime entities; neither enters `CellVisibility`, non-camera coupling, or multiplayer content parity. Versions 1–5 remain loadable; v1–v5 decode with no carried-light links. Decided, not yet built: version 7 appends four optional per-mover sound keys (open, close, blocked, crush) with the `*_event` fields' optional-string encoding. They are presentation-only and outside the static-content hash, and v1–v6 decode with no sounds. The auto-close override has an explicit presence marker: absence inherits the mod default, including through a blank KVP; present zero disables it. Version 3 has its historical scalar rule: zero means absent/inherit and non-zero means authored. Version 2 defaults blocking fields and carries spin authoring; version 1 also defaults spin to zero and `carry_yaw = false`. Loader rejects non-zero degree-domain spin values that become zero in runtime radians, so pure-rotator admission and ramp semantics cannot change across the conversion boundary. Waypoint records carry `name`, `next`, and origin. Runtime hashes only deterministic prediction and collision inputs into the multiplayer static-content gate; host-only block policy and timers remain outside it. Mover brushes are not present in static `Geometry`, `Bvh`, static collision, lightmaps, SDF, portals, or navmesh.
...
**DeltaShVolumes (id 27):** sparse CSR companion for animated-light indirect deltas. Its affinity-cell structure uses `affinity_factor = 4`, `affinity_dims = ceil(base_dims / 4)`, CSR `affinity_offsets`, and flat `affinity_lights`. Each affinity cell has its own coarsening level; each CSR entry stores octahedral delta tiles only for that cell's kept probes, by kept rank. L2 stores one synthesized valid-brick mean tile. The delta bake stores indirect-only unit-radiance transport; animated direct lighting lives in `lm_anim`, and runtime descriptors apply authored color and intensity exactly once to both compose paths. Its reach is the conservative falloff cube for every light, including spotlights, because indirect bounce is not confined to the authored cone. `DELTA_SH_VOLUMES_VERSION` is section-internal and stale sections are rejected. Compiler-side conservative dropping may omit an entry through the existing CSR absence encoding only when its composed output is within the fixed error budget; script-mutable animated entries remain present. The pre-base-SH planner and bake share this reach policy and assert exact CSR identity.

**AnimatedDirectShDeltaVolumes (id 45):** sparse CSR companion for animated direct SH. Each affinity cell has its own coarsening level; each CSR entry stores tiles only for that cell's kept probes, by kept rank. L2 stores one synthesized valid-brick mean tile. Immutable spotlights use the same conservative cone-aware direct reach as ids 35 and 41. Script-mutable descriptor slots deliberately retain cube reach because later curve replacement can make an at-rest zero record nonzero; the existing zero-drop policy keeps those records present. An all-empty result is omitted rather than emitted as an empty section. Script membership is applied before planning, and the planner and bake assert exact CSR identity.

**Direct affinity reach.** Direct-SH consumers (ids 35, 41, and immutable id-45 entries) first intersect the falloff reach with a conservative spotlight-cone bound. The test encloses an affinity-cell AABB in a sphere and evaluates it in the same `f32` coordinate domain as the bake, with outward angular tolerance. It may retain a false positive, but it must never reject a cell containing a contributing probe. Directional and point lights keep their prior reach, and any degenerate or non-finite light/cell state conservatively keeps the cell. Reach changes affect every cache consumer of the resulting direct CSR, including static billboard direct scatter; their independently owned stage epochs must advance together.
...
**Participating stages.** Lightmap bake and SH volume bake, plus the ShadowmaskAtlas memo, the three SH delta bakes, CellVisibility, ChunkLightList, static and animated billboard direct-scatter, animated-light weight-map, SDF-atlas, and navmesh stages. Parse, BSP, portals, geometry, and BVH run uncached — they are fast enough that caching yields no measurable speedup.

**Cache grain.** Several channels are cached below whole-stage grain, so editing one light refreshes only the affected entries:

- **Lightmap — per-light, per-atlas-layer partitions.** Each static light's contribution for one atlas array layer is a separate `"lightmap_layer"` entry, keyed on that light's params, its influence-bounded geometry slice, density/sample-count, atlas layout, and target atlas layer. A partition header names the target layer; its ordered 8-byte records hold only a reached texel index and raw soft visibility. Unreached texels are absent; zero and NaN visibility remain present. The payload is also the ShadowmaskAtlas raw-mask source. `LAYER_FORMAT_VERSION` covers the partition payload. The warm path folds partitions one atlas layer at a time in global light order. It analytically reconstructs irradiance and weighted direction from the unshadowed Lambert term and visibility, derives coverage from record presence, normalizes once per covered texel, then dilates/encodes/appends that layer. This reproduces the uncached reference output byte-for-byte before BC6H. Exact in both warm and cold builds.
- **Lightmap — composited section (second level).** A `"lightmap_section"` entry memoizes the encoded `Lightmap` (id 22) section itself, keyed on the prepared atlas layout, ordered per-light layer fingerprints, and encode parameters (texel density, irradiance format, direction texel scale). The direct atlas-layout input keeps the empty-fold all-Sdf fallback dimension-sensitive. Decoded hits are also checked against current atlas and encode metadata before use. A no-edit rebuild hits it and skips reading the layers, compositing, and BC6H-encoding entirely — the per-light layers are the recompose fallback when any light, geometry, or atlas input changes. Pure memoization of an already-exact pipeline output, so it cannot perturb byte-identity; warm-only, like the layers.
- **ShadowmaskAtlas — selected-light section (second level).** A `"shadowmask_atlas"` entry memoizes the encoded `ShadowmaskAtlas` (id 42) section, preceded by the miss's peak per-texel overlap count so a hit reports it without rebuilding the graph; an entry without the count is a miss, and a wide-layer omission writes no entry. Key includes selected `EntityShadowLights` order, selected `"lightmap_layer"` fingerprints, atlas dimensions, and layer count. A no-edit rebuild hits it and skips graph construction, coloring, streaming fill, quantization, and section encoding. The shadowmask memo, analytic graph, and deterministic channel assignment resolve before the lightmap memo and before the shared per-chart walk. On a miss, graph construction reruns from geometry without reading partitions. The walk offers each live partition to the ordered lightmap fold and, when selected, the shadowmask fill; it performs no layer-cache read after the walk ends and no second visibility trace. Both section hits read no layers. A lightmap hit plus shadowmask miss reads each selected partition at most once per atlas layer; a one-light edit re-bakes only that light's partitions while unaffected partitions are read once. Warm-only, like the lightmap section memo. Cold `--no-cache`/`--release` recomputes through the same fused walk.
...
- **SH delta bakes — per CSR entry.** The three delta bakes (indirect, direct, animated-direct) cache each `(affinity cell, light)` sub-block, so editing one light re-bakes only its entries. Unlike the approximate warm base-SH volume, delta caching is **exact** — each entry bakes a single light in isolation, seeded off the light's global index, so a bounded light set never changes a kept entry's payload. Byte-identical warm and cold, through the full downstream post-bake pipeline (drop, coarsening, valid-probe compaction, payload cap).
- **CellVisibility, ChunkLightList — whole-section.** Memoized per section, not per element. CellVisibility keys on BSP/portal structure and no lights, so a light-only edit hits. ChunkLightList keys on geometry plus the static light set, so a dynamic-light-only edit hits.
...
**Budget observability.** Hit, miss, and eviction lines are info-level, and the compiler's default filter is `warn`, so a cache thrashing against its budget presents as an unexplained slow build. Each cached build tracks a deduplicated live set of entries it successfully read or wrote. It emits exactly one end-of-build warning when that total exceeds `--cache-max-size`, naming the total and budget. Repeated touches and write-then-read paths count once per entry. `--no-cache` and `--release` emit no cache-budget warning.
...
| `--sh-delta-working-set-max-size <SIZE>` | Pre-base-SH host-RAM budget for the cumulative dense ids 27, 41, and 45 working set (default 16 GiB). The production planner applies data-script membership, static-light selection, and each transport's current reach policy before any SH payload is materialized; plan/bake CSR identity is asserted for all three sections. A zero-byte budget is a cheap real-map projection that intentionally refuses before base-SH rays. The gate charges 3× cumulative dense bytes normally: 2× for dense-plus-compaction residency and one share reserved for co-resident base-volume copies. Coarsened `--sh-analyze` charges 4× because it retains another dense delta clone. Values use the same byte syntax as `--cache-max-size`; finite values above `u64` are rejected. Compiler-only. |
| `--no-cache` | Disable the cache entirely — neither read nor write, no directory created (no prune either) |
| `--release` | Produce a shippable map: the exact ship path (exact fused direct-lightmap/shadowmask bake + exact whole-volume SH). Intent-named ship mode; implies `--no-cache` (passing both is fine and identical). The interactive default is a warm build — ship only `--release` artifacts. |
...
| `--direction-texel-scale <N>` | Per-axis static dominant-direction-atlas reduction (default 2). Must be a positive power of two no larger than 64; `1` preserves full-resolution direction bytes. It folds only into the warm composited-section key, never per-light layer keys or chart placement. |
