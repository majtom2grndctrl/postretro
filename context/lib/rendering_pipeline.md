...
Design intent, not yet built (`plans/ready/projectile-weapon-enhancements`): a runtime dynamic light may attach to a **moving** gameplay entity (e.g. a projectile) and track that body's render pose each frame — the raw tick `Transform` for a sprite body, the interpolated pose for a model body — rather than a fixed spawn origin; and `LightAnimation` gains a **radius** channel (CPU-evaluated in the light bridge, packed into the light's range + influence, no shader change) so a light can grow/shrink. Client-materialized runtime lights require a client-side `absorb_dynamic_lights` enrollment call (absorb is host-only today).
...
| Billboard | direct-SH base | id 45 composed delta | runtime direct loop (per vertex) |
...
Design intent, not yet built (`plans/ready/projectile-weapon-enhancements`): a per-collection additive HDR **emissive** term (`sprite.rgb × emissive`, rides the free `SpriteDrawParams.params.w`) makes a billboard read full-bright and bloom — deliberately **unconditional**, not gated by `LightTermMask` (emissive is self-only, outside the light-term set; see §per-term-mask, bit 7 stays unwired). The existing per-frame **flipbook** (`frame_idx` from packed age) extends to non-particle sprite bodies (e.g. projectiles) by feeding an advancing age, gated on an authored cadence so a no-cadence body stays byte-identical. Sprite filtering is unchanged: billboards already sample through `sample_post_retro` (the same crisp texel-snap as forward geometry), on a linear sampler, single-mip.
