...
- [ ] `ModMapEntry.tags` is optional in TypeScript and Luau. Missing, `null`, or `nil` tags normalize to `[]` in Rust before commit; malformed present tag arrays degrade like other malformed catalog-entry fields.
...
- [ ] On a catalog-id load, the resolved entry (with normalized `tags`) is stored on the in-flight load; a CPU test reads its `tags` back from that field, confirming they are available before the data script runs.
