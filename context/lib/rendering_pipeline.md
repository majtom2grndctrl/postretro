...
- **AA band width at carved edges — look here first.** `sample_post_retro` computes its own antialias band from `fwidth` of the UV it is handed, and it is now handed the **marched** UV, whose screen-space derivative jumps at every stone edge. Where it spikes, the band widens and reconstruction stops snapping to texel centres — so stone side faces may read softer than intended. That matters more than it sounds: the aesthetic claim of this whole feature is that carved edges land on the albedo texel lattice. The remedy has a known shape — hand the band width the **pre-march** derivatives, exactly as the same function already does for `textureSampleGrad` mip selection, instead of taking `fwidth` of the warped UV.
