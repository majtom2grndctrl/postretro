...
**OctahedralShVolume (id 34):** sibling replacement for legacy `ShVolume` (id 20), carrying base indirect irradiance. `SH_VOLUME_VERSION` is section-internal; current version 10 rejects stale sections. Each full 4×4×4 affinity brick has one `density_level`: L0 stores valid probes in local x-fastest order, L1 stores all eight corners in corner order (invalid corners are zero tiles), and L2 stores one synthesized mean over valid probes. A brick with no valid probes stores no tiles; partial edge bricks are L0. One metadata-derived stored-atlas geometry replaces the v9 dense and compact geometry split. Payload slots are brick-major in x-fastest affinity order, tagged BC6H or `Rgba16Float`; the runtime composes and samples that stored atlas. See rendering_pipeline.md §4 "Variable base-probe density".

**DirectShVolume (id 35):** baked static-direct octahedral irradiance for entities and billboards. `DIRECT_SH_VOLUME_VERSION` is section-internal; current version 3 rejects stale sections. It carries no probe metadata or depth moments: id 34 supplies validity and density levels. Its stored-atlas geometry and brick-major payload slots must exactly match id 34's stored set; loader cross-checks geometry and payload length before use. The irradiance blob is layer-major, matching id 34: all BC6H block bytes or uncompressed `Rgba16Float` texels for layer 0, then layer 1, etc. Stored BC6H at rest. Emitted only when the map has static baked lights. Runtime: sampled by skinned-mesh and billboard shaders, gated by `has_direct`; forward and fog pipelines bind but do not sample it.
