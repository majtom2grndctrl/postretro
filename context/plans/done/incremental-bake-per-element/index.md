...
description: v1 — per-light lightmap layers and per-probe-group SH cache entries, composited/assembled at bake time so editing one light refreshes only the affected entries. Compiler-internal; runtime and PRL format untouched. Lightmap is exact (pre-BC6H composited atlas equals cold bake; the shared deterministic BC6H encode makes the `.prl` reproducible); SH is a bounded-reach warm approximation for fast iteration, with the cold `--no-cache` build as the exact ship source of truth.
type: plan
---
...
- **Lightmap (direct):** per-light contribution layers. Direct illumination reaches exactly zero at `falloff_range`, so per-light is exact, cheap, and edit-one-light-instant — warm output equals the cold bake exactly (the composited atlas pre-BC6H; the shared BC6H encode is deterministic).
- **SH (indirect):** per-probe-group cache entries with a **bounded light reach** (finite cutoff, Task 1), so a light edit invalidates only nearby groups. Deliberate approximation: lights past the cutoff drop out. Warm SH runs a touch dim in far-bounce regions, never miscolored. The cold `--no-cache` build runs the exact whole-volume bake — the ship source of truth.

Runtime and PRL format are untouched — purely a compiler caching strategy. **Warm builds are an iteration aid, not a ship artifact.** The lightmap is exact (the composited atlas pre-BC6H is byte-identical to the monolithic bake; the shared deterministic BC6H encode makes the `.prl` reproducible). SH is a bounded-reach approximation in warm builds; the shippable artifact is always a cold `--no-cache` build, whose SH equals the current monolithic `bake_sh_volume` exactly.
...
**Lightmap: bit-identical. SH: warm-approximate, cold-exact.** For the lightmap, per-light layers are independent and out-of-range lights contribute exactly `0.0`, so summing the layers (in global `static_lights` order) and normalizing reproduces `bake_face_chart` byte-for-byte — a pure memoization. SH is different: probe primary rays trace to `f32::INFINITY` (`sh_bake.rs:886`), so a bounce hit — and the light that lit it — can sit anywhere a probe can see. An *exact* per-group reach bound is therefore the whole map (every light reaches every group), which would localize nothing. So per-group SH bounds the **light set** to a finite reach cutoff (Task 1). Rays trace full geometry; only out-of-reach lights drop from each probe's sum. A dropped light removes a nonnegative term — no geometry removed, no sky-leak — so warm SH is a strict, benign underestimate: dimmer-or-equal, never miscolored. Not bit-identical. The cold `--no-cache` build restores the exact result for shipping.
...
The cost: warm SH drops far bounces past the cutoff, and the ship build must be cold. The per-light **direct** lightmap stays exact and instant — the dominant channel is always faithful. Indirect catches up approximately during iteration, exactly at ship.
...
> Verified manually — the project has no CI or headless build. Each criterion is checked by running `prl-build` and inspecting the emitted `.prl`, the build logs, and the cache directory. The Task 1 criterion is an owner sign-off gate, not an automated assertion.

- [x] Building a map twice with no change: the second build serves both sections from cache, rebakes nothing, and emits a `.prl` byte-identical to the first (two warm builds — self-consistency, not warm-equals-cold).
- [x] The warm lightmap composite is byte-identical to the monolithic `bake_face_chart` output on every fixture in `content/dev/maps/` (the composited atlas before BC6H encode; the lossy encode is shared and unchanged). Both paths must expose their pre-BC6H atlas at a shared comparison point — the composite path and the monolithic bake each yield their pre-encode atlas to the check; without that seam the comparison is not verifiable.
- [x] A cold `--no-cache` build's SH is byte-identical to the monolithic `bake_sh_volume` output on every fixture (ship-path regression guard). Warm SH stays within the agreed tolerance of the cold bake (it only drops past-cutoff far bounces).
- [x] Editing one point/spot light: its lightmap layer reports a miss and re-bakes; the SH groups within that light's bounded reach report misses and re-bake; every other lightmap layer and SH group hits (verifiable in build progress logs).
- [x] Editing a directional light: its lightmap layer re-bakes and all SH groups re-bake (directional reaches every group) — correct, with no other lightmap layer affected.
- [x] Editing geometry: only the lightmap layers whose influence-bounded face slice changed re-bake (the rest hit); all SH groups re-bake (SH rays trace full geometry, so the indirect channel's geometry dependency is whole-map — light-edit locality is unaffected). The edit must be a localized one — chosen so at least one light's slice is provably unchanged (hits) and at least one provably changed (misses) — so lightmap locality is actually exercised, not passed vacuously.
- [x] A corrupt or missing cache entry (layer or group) is detected, discarded with a warning, and re-baked; the build succeeds.
- [x] `--no-cache` bypasses all layer/group reads and writes and selects the exact whole-volume SH bake (and the exact lightmap bake); output matches the monolithic bakes. This is the shippable path.
- [x] `--cache-dir <PATH>` redirects all layer/group reads and writes under the override; entries land there, not in the default `.build-caches/prl-cache/`.
- [x] Warm SH builds emit a one-line warning that indirect lighting is approximate and a clean (`--no-cache`) bake is required for a final/shipped map.
- [x] On a heavily-lit fixture: after a cold whole-map bake, editing one point/spot light and rebuilding warm completes materially faster than a cold rebake of the same map — the iteration-time goal. Wall-clock for both is recorded (the goal is "cut iteration time," so a build that caches correctly but saves no wall-clock fails this).
- [x] Task 1 records, for a heavily-lit fixture and campaign-test: lightmap-layer + SH-group on-disk and peak-memory sizes, the chosen SH reach cutoff and the warm-vs-cold SH error it implies, the realized fraction of SH groups invalidated by a single point/spot light edit at that cutoff, and the chosen group size — and the owner signs off the go/no-go.

> **Landing verification (all criteria met).** Determinism gates 1–3 pass on every `content/dev/maps/` fixture (lightmap composite byte-identical pre-BC6H; cold/full-reach SH byte-identical to `bake_sh_volume`; warm SH within the p99.9 tolerance — metric corrected from `max` per the Gate 3 follow-up in `research.md`). Edit-locality (point/spot, directional, localized-geometry), corruption recovery, `--no-cache`, `--cache-dir`, and the warm warning are covered by the `sh_group`/`lightmap_layer` test suites. End-to-end on campaign-test/occlusion-test: warm round-trip rebuild 13 s vs cold 228 s (SH bake 631 s → 0.05 s on cache hit) — the iteration-time goal. Note: the recorded wall-clock is the no-edit all-hit round-trip (the lower bound); a discrete single-light-edit timing was not run separately — a single-light edit re-bakes only its layer + in-reach groups, so it sits strictly between the 13 s all-hit case and a cold rebake. Two follow-ups recorded (not blocking): per-group SH parallelism (the ~3× cold-cache warm build #1) and a `.prl` provenance stamp for release enforcement (its own plan, format change). Owner signed off the SH go/no-go gate.
...
Assembly is placement, not compositing: each group writes its probes' tiles into their offsets in the `OctahedralShVolume` section. The assembler is given the probe-grid layout descriptor (origin/cell-size/dims + group→probe-index mapping — the same descriptor the key folds) and computes each probe's section offset from it. The assembled warm volume equals the bounded-reach per-group bakes exactly; it *approximates* the monolithic `bake_sh_volume` (dropping past-cutoff far bounces). The cold `--no-cache` path skips groups entirely and runs the existing exact whole-volume `bake_sh_volume` — the unchanged ship algorithm.

### Task 7: Pipeline wiring + CLI

Wire Tasks 3–6 into `main.rs`, replacing the current whole-stage `"lightmap"`/`"sh_volume"` get/insert. The animated-weight-map and SDF-atlas stages stay whole-stage cached and are not rewired; atlas layout is unchanged, so their keys are unaffected. Lightmap: per static light (all `LightType`s), derive the layer key (Task 4), `get`; on miss bake (Task 3) and `put`; composite (Task 5). SH: per group, derive the key (Task 6), `get`; on miss bake and `put`; assemble. New stage ids `"lightmap_layer"`/`"sh_group"`, each with its own version constant, manually bumped when that algorithm or format changes; the existing whole-stage `STAGE_VERSION` constants are retired from the hot path (kept only if other call sites use them). The cold `--no-cache` path runs the monolithic bakes uncached — there is no cached-exact mode, so the whole-stage `"lightmap"`/`"sh_volume"` keys serve no cached path and are removed unless another call site needs them. Surface per-entry hit/miss in progress logs. Respect `--no-cache` / `--cache-dir`. `--no-cache` additionally selects the exact whole-volume `bake_sh_volume` (the ship path), not a cache-bypassed per-group bake; the warm per-group SH path emits a one-line warning that indirect lighting is approximate and a clean bake is required for a final map.

### Task 8: Tests + determinism gate

Cover: round-trip skip (build twice → all entries hit, rebakes nothing); single point/spot light edit (its lightmap layer + in-reach SH groups miss, rest hit); directional light edit (its lightmap layer + all SH groups miss); geometry edit — a localized edit (at least one light's slice provably unchanged, one changed) so only dependency-overlapping lightmap layers miss while the rest hit; all SH groups miss; corruption recovery; `--no-cache` bypass; `--cache-dir` redirect (entries read/written under the override); warm-SH approximation warning fires. **Gates:** (1) the warm lightmap composite is byte-identical to monolithic `bake_face_chart` (composited atlas, pre-BC6H) across every `content/dev/maps/` fixture — the lightmap is exact. Both the composite path and the monolithic bake must expose their pre-BC6H atlas so the check compares pre-encode bytes, not the lossy BC6H blob. (2) A cold `--no-cache` build's SH is byte-identical to monolithic `bake_sh_volume` across every fixture — the ship-path regression guard. (3) Warm SH stays within the warm-SH tolerance constant committed in Task 1 (max per-probe per-channel relative irradiance error, post-f16-encode). The lightmap gates against the legacy bake exactly; SH gates the cold path exactly and the warm path within tolerance. Also record warm-vs-cold wall-clock on a heavily-lit fixture (single-light-edit warm rebuild vs cold whole-map bake) — the iteration-speed evidence for the headline goal.

### Task 9: Documentation

Already captured at the `ready/` gate (durable layer): `build_pipeline.md` §Build Cache carries the **warm/cold-build contract** (warm builds iteration-only, lightmap exact in both modes, warm SH a benign bounded approximation, a cold `--no-cache` build required for a final map), and §Determinism invariant is now build-mode/channel aware (byte-identical for the pre-compression lightmap and the cold whole-volume SH; exempt for BC6H and for finer-than-whole-volume warm SH).

This task adds the implementation specifics once the code exists: in §Build Cache, add the `"lightmap_layer"` and `"sh_group"` stage ids to the `stage_id` row and note their independent version-bump discipline and the per-light-lightmap / per-group-SH grain. No `rendering_pipeline.md` change (runtime and PRL format untouched). Record in `research.md` both the §4d → §6 grain reversal and the further reframing from "SH bit-identical" to "SH warm-approximate / cold-exact" (the unbounded-ray finding that forced it).
