...
generated by ericw-tools during the build step.
```
...
```
BSP loader entry: src/<subsystem>/loader.rs
...
baked into the vertex buffer during BSP load.
```
...
| | Before | After |
|---|---|---|
| **In scope** | "Error handling TBD" | "Missing BSPX lump falls back to white lightmap, logs warning at load time" |
