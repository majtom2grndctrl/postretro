...
use crate::ir::{BakedIr, CURRENT_IR_VERSION, IrNode, IrType, bind};
use crate::movement::MovementScope;

use super::super::DescriptorError;

pub fn validate_at(value: f32) -> Result<f32, DescriptorError> {
...
pub fn validate_primitive_name(name: String) -> Result<String, DescriptorError> {
    if name.is_empty() {
...
pub fn ir_node_from_json(value: serde_json::Value, field: &str) -> Result<IrNode, DescriptorError> {
...
pub fn ir_type_label(ty: IrType) -> &'static str {
    match ty {
...
pub fn validate_positive_finite(value: f32, field: &str) -> Result<f32, DescriptorError> {
...
pub fn validate_non_negative_finite(value: f32, field: &str) -> Result<f32, DescriptorError> {
    if !value.is_finite() || value < 0.0 {
        return Err(DescriptorError::InvalidShape {
            reason: format!("`{field}` must be a finite value >= 0.0, got {value}"),
        });
...
pub fn validate_in_range_finite(
    value: f32,
    min: f32,
    max: f32,
    field: &str,
...
/// most `max`.
pub fn validate_in_range_finite_exclusive_min(
    value: f32,
    min: f32,
    max: f32,
    field: &str,
...
pub fn validate_interactive_name(
    kind: &str,
    has_label: bool,
...
pub fn validate_image_name(has_label: bool, decorative: bool) -> Result<(), DescriptorError> {
...
pub fn is_catalog_path_relative_to_content_root(path: &str) -> bool {
...
pub fn validate_finite_array3(value: [f32; 3], field: &str) -> Result<[f32; 3], DescriptorError> {
