...
pub fn movement_descriptor_from_lua(
    table: &Table,
...
pub fn forgiveness_params_from_lua(table: &Table) -> Result<ForgivenessParams, DescriptorError> {
...
pub fn dash_params_from_lua(table: &Table) -> Result<DashParams, DescriptorError> {
...
/// [`read_dash_bool_js`].
pub fn read_dash_bool_lua(table: &Table, field: &'static str) -> Result<BoolOrIr, DescriptorError> {
...
pub fn crouch_params_from_lua(table: &Table, radius: f32) -> Result<CrouchParams, DescriptorError> {
    let half_height = validate_positive_finite(
...
pub fn get_required_bool_lua(table: &Table, field: &'static str) -> Result<bool, DescriptorError> {
    if !table.contains_key(field).map_err(lua_err)? {
        return Err(DescriptorError::MissingField { field });
    }
    let raw: LuaValue = table.get(field).map_err(lua_err)?;
...
pub fn get_required_string_lua(
    table: &Table,
    field: &'static str,
...
pub fn get_required_f32_lua(table: &Table, field: &'static str) -> Result<f32, DescriptorError> {
    if !table.contains_key(field).map_err(lua_err)? {
        return Err(DescriptorError::MissingField { field });
    }
    let raw: LuaValue = table.get(field).map_err(lua_err)?;
