...
**D2 — Resize anchoring: feet planted, head/eye drops (FPS norm).** The pawn `Transform.position` is the capsule center; shrinking `half_height` with the feet fixed means the center moves DOWN by the half-height delta so the lowest point (`position.y - (half_height + radius)`) stays put. The intent calls the Task 2 resize helper, which writes the reduced `half_height` and returns the center delta; the dispatch applies that delta to `position`. `eye_height` drops correspondingly. Standing up reverses it (center rises) only after the stand-up probe clears (D7).
...
**D7 — Stand-up blocked: ceiling probe by sweeping the CROUCHED capsule up; if blocked stay crouched, re-check each tick.** When crouch-intent releases (D-dependency), the intent attempts to stand: collision exposes only `cast_capsule` (a sweep), no overlap query, so it `cast_capsule`s the CROUCHED-size capsule straight UP by `2 × (standing_half_height − crouched_half_height)` (the head-rise delta with feet planted). A hit within that distance ⇒ headroom blocked ⇒ remain `Crouching`, keep the crouched capsule, retry next tick. Clear ⇒ snap the collision capsule back to standing (raising the center, D2) and transition `Crouching`→`Normal`. The probe runs every tick crouch-intent is inactive, so the player auto-stands the first clear tick.
...
The implementation-precision items surfaced by review are now resolved and folded into Decisions / Tasks:

- Stand-up probe construction (was #1) → D7 / Task 2b: collision exposes only `cast_capsule`, so sweep the CROUCHED capsule up by `2 × (standing_half_height − crouched_half_height)`.
- Position-center mutation owner (was #2) → D2 / Task 2a: the resize helper returns the center delta; the dispatch applies it to `position`.
- Smoothed-eye storage (was #3) → Task 3 / D3: the intent writes the smoothed value into `capsule.eye_height` each tick; the follow read at main.rs:1013 already reads this field.
