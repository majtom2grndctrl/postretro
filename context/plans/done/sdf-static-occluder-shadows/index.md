...
| `casts_entity_shadows` | Per-light authoring flag (FGD `_cast_entity_shadows`) gating shadow-map-pool eligibility for dynamic entities (enemies / moving meshes). |
...
- **Light model:** a per-light `casts_entity_shadows` authoring flag (FGD `_cast_entity_shadows`) gating shadow-map-pool eligibility; `is_dynamic` reframed as an internal/seam-only field (geometry-moving lights only, no v1 authoring surface) so intensity-only animation no longer forces the runtime-dynamic path; updated `rank_lights` eligibility. (Task 1b.)
...
- **Scripting→animated-baked bridge:** a declarative light property (e.g. `_animated`) meaning "static geometry, intensity arrives at runtime; reserve a baked weight map." The compiler bakes a weight map for the light; at runtime the script-driven brightness/color curve routes into the section-indexed animated-compose descriptor buffer. This is a **new runtime write path + index mapping** (runtime light → baked section slot) — only the descriptor stride/format is shared with `scripted_light_descriptors`, not the buffer or index space (Task 2c). No compile-time auto-detection magic — modder-friendly favors an explicit flag. Includes re-tagging **all** current `_dynamic` campaign-test lights to static — all 17 are arena brightness-pulse lights, so all move onto this animated-baked path (which re-routes their script curve off the `is_dynamic`-gated path). (Task 2c.)
...
- [ ] A light authored with `_cast_entity_shadows 1` parses with `casts_entity_shadows == true` and feeds the `rank_lights` candidate set; one with `_cast_entity_shadows` absent or `0` parses `false` and is excluded from the set — dynamic-occluder shadows are strictly opt-in (default off). Asserted on the parsed `MapLight` flag (the candidate set is sourced from this flag, not the empty `is_dynamic`-filtered `level_lights` — Task 1b). [T1b]
...
- [ ] The animated-light weight map carries a per-light per-texel incoming direction (the bake retains what `light_contribution_and_direction` computes rather than discarding it). Bumping the animated-lightmap stage version invalidates the prior cache entry: first build after the bump is a miss and rebakes; the second is a hit. [T2b]
- [ ] No campaign-test light parses with `is_dynamic == true` after reclassification: every previously-`_dynamic` light is now static (all animated-baked — the 17 arena lights). Asserted on parsed PRL. [T2c]
- [ ] For the reclassified arena sweep lights, the compiled output carries an animated-baked weight map (not a `_dynamic` runtime entry), and the script-driven intensity curve still drives a phased brightness sweep equivalent to the script's — now via the animated-baked compose path, *not* the `is_dynamic`-gated forward/bridge path the lights left. Asserted on parsed PRL + a runtime curve-eval test that the now-static (`is_dynamic == false`) light's curve reaches the compose descriptor. [T2c]
- [ ] `setLightAnimation` accepts brightness-only animation on a now-static (`is_dynamic == false`) "script-driven intensity" light without error (the validation gate admits the new light class). Asserted in a scripting-primitive test. [T1b, T2c]
- [ ] A legacy `.prl` whose `MapLight` records predate `casts_entity_shadows` deserializes with `casts_entity_shadows == false` and retains its stored `is_dynamic`. [T1b]
...
| T1b | `_cast_entity_shadows` pool gating; `is_dynamic` = geometry-motion; `setLightAnimation` admits the static script-driven class |
...
| T2c | all-lights-static reclassification; arena weight-map + script-curve re-routed off `is_dynamic`; `setLightAnimation` static-class accept (with T1b); arena animated-baked shadow (visual) |
...
### Task 1b: Light model — geometry-vs-intensity split + entity-shadow opt-in (level-compiler/runtime)
...
- **Retire `_dynamic` as an authoring key; `is_dynamic` is internal/seam-only.** `is_dynamic` becomes set by **geometry motion** (position/aim animation), not by intensity (brightness/color) animation — but it has **no v1 authoring surface**: nothing moves yet, so it is simply always `false` for authored v1 content. It stays as the named code/architecture seam for the geometry-moving light class, to be given a proper FGD shape when geometry-moving lights land. No `_dynamic` boolean is exposed in v1: no light moves yet, and geometry-moving lights will likely need a different FGD entity / KVP set entirely, so pre-committing to a `_dynamic` flag now would be the wrong shape. Today `is_dynamic` is parsed from FGD `_dynamic` (`crates/level-compiler/src/format/quake_map.rs` ~:245) and the runtime forward loop iterates only `is_dynamic` lights (`filter_dynamic_lights`, `crates/postretro/src/render/mod.rs:3262`); the `_dynamic` key is removed outright (we're pre-release and Task 2c re-tags every map light). Intensity-only lights move *off* the runtime-dynamic path (onto the animated-baked path, Task 2c).

  **Reconcile with `bake_only` and curve validation — `is_dynamic` is not a clean single axis today.** In source, `is_dynamic == true` also means "runtime direct path only, **no bake contribution**" (`quake_map.rs` ~:243–244); a separate `_bake_only` flag (`quake_map.rs:231`) means "bakes but no runtime presence"; and `color_curve` is rejected unless the light is `_dynamic` or `_bake_only` (`ColorCurveOnBakedLight`, `quake_map.rs:325`, error text ~:60). Reframing `is_dynamic` to "geometry moves" silently changes what bakes: a former `_dynamic` light that was *excluded* from the bake would now bake. The reframing must state explicitly: (a) whether reclassified intensity-pulse lights now contribute to the static/SH bake (they should — animated-baked lights live on the bake); (b) how the new geometry-axis `is_dynamic` composes with `bake_only` (geometry-moving + bake-only is now a meaningful combination); (c) how the `color_curve` validation re-resolves now that `_dynamic` is retired as an authoring key — the `_animated` (script-driven intensity) class becomes the thing that admits an animated curve, in place of `_dynamic`. This reconciliation is part of Task 1b, not Task 2c.
- **Add a per-light `casts_entity_shadows` flag** (FGD `_cast_entity_shadows`) gating shadow-map-pool eligibility. `cast_shadows` currently defaults hardcoded `true` for all lights (`quake_map.rs:407`); the new flag is the *dynamic-entity* (enemy / moving-mesh) opt-in, defaulting to `false` — a static light casts a dynamic-occluder (enemy) shadow only when explicitly tagged `_cast_entity_shadows 1`. (This is a deliberate behavior change: the post-reclassification light set is all static, so the old `is_dynamic`-gated pool population no longer applies; eligibility is now strictly opt-in.) Update `rank_lights` in `crates/postretro/src/lighting/spot_shadow.rs` (`spot_shadow.rs:239`, currently gates on `is_dynamic && light_type == Spot`) to gate on the new flag. **This is more than swapping the gate.** `rank_lights` ranks the `lights` slice the caller hands it, which today is `level_lights` — the output of `filter_dynamic_lights` (`render/mod.rs:3270`, `filter(|l| l.is_dynamic)`). After Task 2c re-tags every light `is_dynamic == false`, `level_lights` is **empty**, so changing only the gate inside `rank_lights` leaves the pool silently empty. The shadow-pool candidate set must therefore be **decoupled from the `is_dynamic`-filtered `level_lights`** and sourced from `casts_entity_shadows`-tagged lights instead. **v1 consequence:** with every light static and `casts_entity_shadows` defaulting `false`, the pool is **empty until an author opts a light in** (real enemies land in M10) — the expected v1 state, not a regression. Note that `rank_lights` also consumes an `eligible_lights: &[bool]` slice the caller fills with visibility and brightness-suppression results (`render/mod.rs:2579–2603`, `BRIGHTNESS_SUPPRESSION_THRESHOLD`); `casts_entity_shadows` is an additional per-light pre-filter layered on top of that existing eligibility slice — it does not replace it.
...
Compose side: `animated_lightmap_compose.wgsl`. Runtime resource: `crates/postretro/src/render/animated_lightmap.rs` allocates the direction atlas. Each light's per-texel incoming direction — baked into the weight map (see below) — is the input to the radiance weighting.

**Per-light direction source — baked.** The compose pass needs each contributing light's per-texel incoming direction to weight it. Bake the per-light per-texel direction into the weight map (`crates/level-compiler/src/animated_light_chunks.rs` / `animated_light_weight_maps.rs`): `light_contribution_and_direction` (`animated_light_weight_maps.rs:198`) already computes the per-light per-texel direction and currently discards it — baking just retains it. A static-geometry light's per-texel incoming direction is constant (the light does not move), so it is bakeable; only the per-frame fusion weights (the lights' current radiances) are dynamic. Computing the direction in-compose was rejected as higher-risk — it would require adding both texel world position and per-light position inputs to the compose bind group, which the current compose shader has neither of.
...
- **Runtime intensity binding — a new runtime write path.** Route the scripted brightness/color curve into the animated-compose descriptor. This is real work, not wiring two existing halves together. Only the descriptor **stride/format** (`ANIMATION_DESCRIPTOR_SIZE = 48`: period/phase/brightness/color/direction curves over a shared `anim_samples` buffer) is shared with the forward pass's `scripted_light_descriptors` — **not** the buffer, the index space, or the upload path. The two descriptor buffers differ on every other axis:

  | | Animated-compose descriptors | `scripted_light_descriptors` |
  |---|---|---|
  | Index space | section slot (baked animated-light) | map-light index |
  | Bind group | group 1 binding 4 (compose) | group 3 (forward) |
  | Write path | compile-time only, from the baked section (`sh_volume.rs` populates from `sec.animation_descriptors`); `set_active` flips one descriptor field (a `u32`) at runtime | runtime upload (`upload_bridge_descriptors`) |

  **The slot mapping — compiler-emitted index side-table.** Lights have no unique per-light names: they carry shared `_tags` (ten lights all tagged `arena_1_light`) and are individuated only by runtime `EntityId`, so there is no per-light identity to stamp. Key the mapping on the internal map-light index instead — invisible to authors. The compiler emits a `map-light-index → animated-light section slot` table (the inverse of the envelope-slot assignment it already performs; non-animated / former-`is_dynamic` lights get a SENTINEL = no slot). At load, when the runtime creates a light entity from PRL light index `i`, it caches `slot_for_map_light[i]` on the `LightComponent` — resolution happens **once at load**, not per animation call.

  **The runtime write — extends a dormant stub.** `setLightAnimation` already stores the curve on `LightComponent.animation`; the bridge routes it through the cached slot into the compose descriptor buffer. The only runtime write into that buffer today is `Renderer::set_animated_light_active(slot, active)` (`render/mod.rs:2345`) — a half-scaffolded stub with **zero callers** that flips one `u32` descriptor field. The bridge generalizes it into a full descriptor write: when `setLightAnimation` fires, the renderer writes the whole curve *descriptor* (`ANIMATION_DESCRIPTOR_SIZE = 48`: period/phase/brightness/color/direction curves) into the buffer once; the compose shader evaluates it per frame against the time uniform — exactly how the baked animated lights already work. **Not** per-frame CPU sampling. Only the descriptor stride/format is shared with `scripted_light_descriptors` — not the buffer, the index space, or the upload path.
- **Full reclassification — re-routes the script path off the `is_dynamic` gate.** Re-tag **every** `_dynamic` light in `content/dev/maps/campaign-test.map` to static — no light in the engine moves yet, so all of them are misclassified. All 17 are arena-tagged and animate **brightness only**: 10 `arena_1_light` wide-cone spots (`light_spot`, intensity 250) and 7 `arena_wave_2` point lights (`light`, intensity 5), all driven by `content/dev/scripts/arena-lights.ts` (`setLightAnimation` with `color: null, direction: null`), geometry fully static. Every one moves onto the animated-baked path — there are no fixed-intensity or non-arena `_dynamic` lights to triage. Re-tagging means dropping the now-retired `_dynamic` key and adding `_animated` (script-driven intensity). **Preserve the scripted-brightness stress test** — keep the script driving the sweep; do **not** convert them to map-keyed `_animation` (the scripting is part of what this exercises). Pre-release with content we own, this is a straight re-tag — no migration tooling.

  **Reclassification breaks the current scripted-animation path; re-routing is mandatory, not free.** The arena sweep runs today *because* its lights are `is_dynamic == true`, and the whole scripted path is `is_dynamic`-gated: `filter_dynamic_lights` (`render/mod.rs`) builds `level_lights` via `filter(|l| l.is_dynamic)`; `upload_bridge_descriptors` (`render/mod.rs`) sizes and indexes the scripted descriptor buffer on `level_lights.len()`; `setLightAnimation` validates against `is_dynamic` (`scripting/primitives/light.rs`, via `LightComponent.is_dynamic`). Reclassifying an arena light to `is_dynamic == false` drops it out of `level_lights`, so it loses its scripted-descriptor slot and the sweep **silently dies**. (The shadow-map pool's candidate set is likewise decoupled from `level_lights` — resolved in Task 1b — so it does not silently empty for the same reason.) So reclassification *requires* re-routing these lights' script-driven curve off the `is_dynamic` forward/bridge path onto the new animated-baked compose path — the same new runtime write path + section-slot mapping from the Runtime-intensity-binding bullet above. It is not enough to flip the flag and "keep the script driving the sweep." Also revisit the `setLightAnimation` `is_dynamic` validation gate for the new "static-geometry, script-driven-intensity" light class: brightness-only animation must be permitted on these now-static lights. (The arena script passes `color: null`, so it does not trip the color guard today — but the gate's `is_dynamic` semantics change under this class.)

**Bridge mechanism — settled.** The declarative property (`_animated`, reserving a baked weight map) and the compile-marker↔runtime-intensity-binding handshake are both pinned: the compiler-emitted `map-light-index → slot` side-table, the load-time cache on `LightComponent`, and the descriptor write extending `set_animated_light_active` (all in the Runtime-intensity-binding bullet above).
...
Accumulate occlusion with the closest-passing-distance penumbra estimate. Early-out open regions via the baked DDGI `E[d]` moment sampled at the pixel. Write factors into one `Rgba8Unorm` half-res target — v1 needs exactly 2 channels: **R = static-lightmap aggregate factor, G = animated-baked aggregate factor**, with B/A reserved for future moving-light factors. Expose march-step cap, open-space skip threshold, and penumbra `k` as uniform fields (Task 7). Add a GPU-timing pair.

### Task 5: Forward shadow-factor integration (postretro)

In the forward fragment shader, multiply by the upsampled shadow factor: the **static-lightmap term** (gated on the unshadowed-mode flag from Task 2a/3 — skip the multiply for a shadowed/legacy lightmap to avoid double shadows) and the **animated-baked term** (its animated-baked aggregate factor). These are the only two terms in v1 — the geometry-moving per-light multiply lands with the seam in Task 4, when the first moving light exists. Do **not** apply the factor to shadow-map (enemy) results — those already carry their dynamic-occluder shadow. Upsample inline with a depth-aware 2×2 bilateral filter, applied per-channel (R for the static-lightmap term, G for the animated-baked term; no extra full-res target). The `fog_composite.wgsl` bilateral upsample was reverted in `f50314d` for perf — **re-derive** the filter, don't reuse it (`research.md`). Sample the shadow-factor buffer via a new binding in the shadows bind group (group 5); update the shared group-5 BGL and every pipeline using it together with the new binding's `visibility`. Note the ownership coupling: the group-5 BGL is built by `SpotShadowPool::bind_group_layout()` (`spot_shadow.rs:101`); the renderer (`render/mod.rs:1067–1068`) merely *calls* it and passes the result into `SpotShadowPool::new()`, which builds and owns the group-5 *bind group* from its own resources. So adding a binding to the BGL is a `spot_shadow.rs` edit, not a renderer-side one. The SDF shadow-factor target is created later in the Task 4 shadow pass, outside the pool — so the pool's bind-group construction must then include the new target. This requires either rebuilding `SpotShadowPool`'s bind group after the shadow target exists, or moving group-5 bind-group construction out of `SpotShadowPool` to the renderer; pick one during implementation. Gate application on the `SdfShadowMode` uniform (off → no factor applied).

### Task 6: Debug shadow-mode toggle (postretro)

Add an `SdfShadowMode` enum (SDF on / off / visualize) mirroring the `LightingIsolation` pattern (`ALL_VARIANTS`, `label()`) in `render/mod.rs`; add a field to `FrameUniforms` and the forward `Uniforms` (keep the uniform 16-byte aligned). Add a dropdown in `draw_diagnostics_panel()` (`crates/postretro/src/render/debug_ui/mod.rs:114`), panel-only (no keyboard chord), matching `LightingIsolation`. Visualize mode outputs the shadow factor (or march-step heatmap) instead of shaded color.

### Task 7: Debug quality sliders (postretro, dev-tools)

Add sliders to `draw_diagnostics_panel()` (`crates/postretro/src/render/debug_ui/mod.rs:114`) in a new collapsing section. SDF knobs (pure uniform scalars, per-frame write, no rebuild): max march steps, open-space skip threshold, penumbra/cone softness `k` — write through to the shadow-pass uniform via renderer setters. Fog knobs: `step_size` (uniform scalar — add a renderer setter that updates `FogParams.step_size` and re-uploads per frame; none exists today) and `fog_pixel_scale` (resolution knob — drive `Renderer::set_fog_pixel_scale`, which rebuilds the scatter target and bind group; a resource rebuild on change, not a per-frame uniform write). Seed slider state from live renderer values on first draw (`debug_ui/mod.rs:125–126`), matching the ambient-floor/indirect-scale pattern.
...
**Phase 2 (concurrent):** Task 2, Task 2a, Task 2b, Task 3 — independent once Task 1 exists. Task 2a touches only `lightmap_bake.rs`; Task 2b touches the compose shader + animated-lightmap runtime and the weight-map bake (to retain/bake per-light directions); Task 2 and Task 3 share no files with either.
...
**Per-light shadow flags.** `MapLight` (`crates/postretro/src/prl.rs`) gains a `casts_entity_shadows: bool` and the reframed `is_dynamic` semantics (Task 1b). `casts_entity_shadows` parses from FGD `_cast_entity_shadows`; `is_dynamic` is internal/seam-only with no v1 authoring key (always `false` for authored v1 content). Legacy PRLs: a missing `casts_entity_shadows` reads as `false` (dynamic-occluder shadows are opt-in); `is_dynamic` retains its stored value. Pin the field layout during implementation.
...
**Animated-light slot table.** The animated-light section gains a `map-light-index → animated-light slot` table (one `u32` per map light; SENTINEL = `u32::MAX` for non-animated lights), little-endian, mirroring existing section conventions (Task 2c). Pin exact section placement (which animated-light sub-section, field order) during implementation.

**`prl-build` CLI surface.** Two new flags: one to emit the SDF atlas section (Task 2), one to bake unshadowed irradiance (Task 2a). The animated-baked dominant direction is fused per-frame in the compose pass, but its per-light direction source is baked: the animated-lightmap bake retains and bakes each light's per-texel incoming direction into the weight map (Task 2b), so the existing animated-lightmap stage version bumps and its cache key invalidates that stage. On the branch the campaign-test build sets both new flags. Each flag is part of its stage's cache key, so flipping either invalidates only that stage. Default (no flags) reproduces `main`'s output byte-for-byte.
