...
**D7 — `slideDrag` is a constant linear boost decay (no Normal-friction fold), required > 0.** Unlike `dashDrag`'s dual path, `slideDrag` always linearly decelerates the boost (world-units/sec²). It is required positive (`validate_positive_finite`): a frictionless flat slide would never decay to the exit band, leaving only the time backstop to end it — a degenerate config the engine forbids at the source rather than papering over with the cap. Slide wants its own low, designed friction distinct from `Normal`'s contextual decay, so the base layer is left un-frictioned during the slide; on a sustained downhill the slope term (D8) can still outrun the drag, which is the `SLIDE_MAX_MS` backstop's job (D9).
...
- [ ] Boost decays: on flat ground, total horizontal speed bleeds at the constant linear `slideDrag` rate and the slide exits when it reaches the crouch tier (`slideDrag` is required > 0, so a flat slide always decays out).
...
- [ ] Descriptor parsers reject invalid `slide` fields symmetrically in JS and Luau (each path names the offending field; wording/granularity differ per path as `dash`/`crouch` do): `minSpeed` and `slideDrag` reject missing/non-finite/non-positive (zero rejected); `slopeAssist`, `entryBoost`, `minDurationMs`, and `steerRate` reject missing/non-finite/negative (zero allowed).
...
- [ ] The `carry.rs` doc-comment no longer names slide as the first `Zero`/`Scale` consumer (re-homed to the environment-probe states); slide is recorded as the first gameplay-meaningful `KEEP_ALL` consumer. (Source-inspection check, not a unit test.)
...
Add a forwarded floor-normal field to `SubstrateResult` (e.g. `floor_normal: Option<Vec3>`), populated in `integrate_collision` from the walkable contact it already computes — the slide-loop floor branch (~line 366), the swept ground-stick down-cast (~line 516), AND the ray-fallback down-cast (~line 540); record the last walkable normal from whichever fired (the ray fallback only carries a 2D-ish normal, but it fires on wall-pinned ticks that rarely coincide with a slide, so floor-vs-swept is the common path). Add a `last_floor_normal: Option<Vec3>` field to `PlayerMovementComponent`, written by `tick` from the `SubstrateResult` each tick so the NEXT tick's intent reads last-tick contact (the D8 forward-not-sideways rule). Initialize it `None` in `from_descriptor`. No existing-state behavior change — this only records and forwards data no current state reads. Regression-gated. (Wall-normal forwarding stays out — wall-run's job.)

### Task 3: `SlideParams` data surface — struct, parse/validate, SDK emit, component field
Add a `SlideParams` struct (`data_descriptors.rs`) with six literal fields: `min_speed` (`minSpeed`, `validate_positive_finite`), `slide_drag` (`slideDrag`, `validate_positive_finite`), `slope_assist` (`slopeAssist`, `validate_non_negative_finite`), `steer_rate` (`steerRate`, `validate_non_negative_finite`), `entry_boost` (`entryBoost`, `validate_non_negative_finite`), `min_duration_ms` (`minDurationMs`, `validate_non_negative_finite`). Parse `slide` as an OPTIONAL sub-object on `PlayerMovementDescriptor` with the present-then-all-required `contains_key`/null-guard discipline `dash`/`crouch` use, via `slide_params_from_js`/`slide_params_from_lua` mirroring `crouch_params_from_*`. Materialize `slide: Option<SlideParams>` onto `PlayerMovementComponent` in `from_descriptor` (clone, mirrors `crouch`); when `slide.is_some() && crouch.is_none()`, warn once and store `slide = None` (D3). Emit the SDK type: `register_type("SlideParams").field(...)` in `primitives/mod.rs` alongside `DashParams`/`CrouchParams`; add an optional `slide?: SlideParams` field to the `PlayerMovementDescriptor` chain (mirror the optional `crouch?` field); add `"SlideParams" => "SlideParams".to_string()` to BOTH type-name maps in `typedef.rs`; update `EXPECTED_TS`/`EXPECTED_LUAU` (and `_WITH_DOCS` — reword the `PlayerMovementDescriptor` `.doc()` to name `slide` as another optional sub-object); regenerate the committed `.d.ts`/`.d.luau` and confirm the drift test passes. Adding `slide: Option<SlideParams>` to `PlayerMovementDescriptor` breaks every `PlayerMovementDescriptor { .. }` construction site — **~13 sites across 8 files** (the compiler flags each; let it drive the list). Two are PRODUCTION parsers that get REAL wiring (bind a `slide` local via `slide_params_from_js`/`slide_params_from_lua` and include it), NOT `slide: None`: the literals inside `movement_descriptor_from_js` (`data_descriptors.rs` ~1599) and `movement_descriptor_from_lua` (~2797). Every other site is a test/builder literal that takes `slide: None`, exactly as the `crouch` field did: `refresh_plan.rs` (~857), `systems/ui_proxy.rs` (~171), `systems/health.rs` (~137), `runtime.rs` (~3767), `components/player_movement.rs` (`minimal_descriptor`), `movement/scope.rs` (`minimal_descriptor`), and the FIVE helpers in `movement/mod.rs` (`canonical_descriptor`, `double_jump_descriptor`, `dash_descriptor`, `forgiveness_descriptor`, `crouch_descriptor`). Missing any site fails to compile; setting either parser site to `None` instead of wiring it silently drops wire-`slide`. Pure data-surface work; see Boundary inventory.

### Task 4: `Sliding` state + intent + transitions
Add `MovementState::Sliding { elapsed_ms: f32, boost: Vec3, eye_current: f32 }` (`player_movement.rs`); dispatch it in `dispatch_state_intent`; add a `Sliding` arm to `outgoing_boost` returning the live `boost`. Add `try_enter_slide` (mirroring `try_enter_dash`) and wire it into `normal_intent`'s crouch branch: when `crouch_intent` is active, grounded, `slide` + `crouch` both present, and horizontal speed ≥ `slide.min_speed`, enter `Sliding`; otherwise fall through to `Normal` → `Crouching`. `try_enter_slide` seeds `boost = horizontal velocity + entry_boost·slide_dir` and sets `component.velocity.xz = boost.xz` (D2), resizes the capsule to the crouched dimensions via `resize_capsule` (Feet anchor) applying the center delta to `position`, seeds `eye_current` at the standing eye, then RESTORES `capsule.eye_height` to that pre-resize standing value (because `resize_capsule` snapped it to the crouched target — mirror `normal_intent`'s crouch branch so the eye smooths instead of popping), and returns `Transition { Sliding{..}, KEEP_ALL }`. Add `sliding_intent` in `states/sliding.rs` (mirror `dash_intent`): gravity (airborne); **slope accel**: with `n = component.last_floor_normal` (skip when `None`/flat), compute along-plane gravity `a = gravity_vec − (gravity_vec·n)·n`, take its horizontal (XZ) component, add `slope_assist · a_xz · dt` to the boost (D8 — boost stays horizontal); reconcile the tracked `boost` against realized velocity (the dash clamp); **direction-steer** rotating the boost toward input at `steer_rate` deg/s (D10); constant-linear `slide_drag` decay of the boost (D7); recombine into `component.velocity`; eye smoothing toward the crouched eye via the crouch exponential-approach using `CrouchParams.transition_rate`, written to `capsule.eye_height`; accumulate `elapsed_ms`. Exit order: a jump edge (D6) and the ledge/airborne handoff (D5 — detect via the one-tick-stale `!component.is_grounded`, like dash) are ALWAYS checked first; then, only when `elapsed_ms ≥ min_duration_ms` (D9), the crouch-release stand-up and the speed-decay exit (total horizontal speed ≤ `ground.speed.crouch`); `elapsed_ms ≥ SLIDE_MAX_MS` always forces exit. The stand path uses `standup_clearance_probe` + `stand_up_resize` (mirroring crouch D10): clear → `Normal`, blocked → `Crouching`. Exits are all `KEEP_ALL`; a `Crouching` exit constructs `Crouching { eye_current }` from the slide's live `eye_current` so the eye stays continuous across the handoff. Add the `SLIDE_MAX_MS` engine const (the generous backstop, D7/D9) near `DASH_MAX_MS`. Update the `carry.rs` doc-comments per D4 (re-home the `Zero`/`Scale` "first consumer" prediction to wall-run / the environment-probe states, and KEEP the `#[allow(dead_code)]` on `Zero`/`Scale`/`Drop` — no production consumer remains after re-homing). Test fixture: the slope and time-guard ACs need a NEW sloped-floor `TriMesh` world helper (a walkable tilted plane, `n.y ∈ (cos_walkable, 1)`) plus a settle helper — no existing world helper builds a non-flat floor; add it with the slide tests. No `apply_carry`/`tick` carry changes — the seam already routes `KEEP_ALL`. Borrow note: `dispatch_state_intent` already moves the active state's data out of the component (`std::mem::replace`) before calling the intent, so `sliding_intent` reads `component.last_floor_normal` and writes `component.velocity`/`capsule` with no aliasing against its in-place `boost`/`eye_current`/`elapsed_ms` — exactly as `dash_intent` reads `ground`/`fall` today.
...
Units: `minSpeed` world-units/sec (finite > 0), `slideDrag` world-units/sec² (finite > 0), `entryBoost` world-units/sec (finite ≥ 0), `slopeAssist` unitless (≥ 0), `steerRate` degrees/sec (≥ 0), `minDurationMs` ms (≥ 0). Crouched capsule dimensions come from `CrouchParams` (D3). The `?` marks the whole sub-object optional: absent ⇒ slide disabled; present ⇒ all six fields required.

## Open questions
- **`SLIDE_MAX_MS` value (feel, not architectural).** The generous safety backstop (seconds, not dash's 200 ms) bounding a sustained-downhill slide. Pick the value in-engine so it never cuts a legitimate descent.
