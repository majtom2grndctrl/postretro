...
**Per channel, by its nature.** The lightmap's per-light contribution reaches *exactly* zero beyond `falloff_range` (hard cutoff, `lightmap_bake.rs:849-869`), so a layer has bounded support and per-light layers sum back to the monolithic result bit-for-bit. SH is indirect, expensive, and bounce-coupled — decomposing it per light would force either re-casting each probe's primary rays per light (a cold-build regression) or culling probes by a bounding box (which drops genuine far-field bounces, because SH falloff is keyed on the *bounce hit point*, not the probe — `sh_bake.rs:524-597`). Chunking SH spatially instead keeps the "cast once, sum all in-range lights, project" structure intact per probe, so each group is the monolithic bake restricted to a probe subset.
...
- **Lightmap.** `bake_face_chart` (`lightmap_bake.rs:695`) loops `for light in static_lights` (`:734`), accumulating `irr += contribution * v` (`:758`) and `weighted_dir += to_light * lum` (`:765`), then storing `direction[idx]` from `weighted_dir.normalize()` (normalize `:774`, store `:779`). Atlas from `prepare_atlas` (`:172`; empty-light branch `:186` skips UV writes — call once with the full static set). `LightmapInputs`/`LightmapConfig` (`:119`/`:130`; `lightmap_density :131`, `area_sample_count :135`).
...
- **Geometry / BVH.** `BvhPrimitive` is one-per-face (`bvh_build.rs:32-44`) with content-derived `sort_key` field (`:41`, populated by `primitive_sort_key(material_bucket_id, cell_id, index_offset)` `:96-103`). `Bvh::build` permutes the slice in place (`:156`). A face's triangles are `index_offset..index_offset+index_count` (from the parallel `GeometryResult.face_index_ranges[face_idx]`, `FaceIndexRange` `geometry.rs:19-23`) into `GeometryResult.geometry.indices`/`.vertices`.
- **Cache.** `cache::CacheKey::new(stage_id, stage_version, input_hash)` (`cache.rs:27`); `StageCache::{get,put}` (`:62`/`:114`); `get` validates length+hash and returns a miss on failure. Existing keys `"lightmap"`/`"sh_volume"` (`main.rs:308`/`:436`) hash `postcard::to_allocvec(inputs)` + blake3 (`:299-308`); default dir `<workspace>/.build-caches/prl-cache/` (`:186`). Section ids `Lightmap = 22`, `OctahedralShVolume = 34` (`level-format/src/lib.rs:114,191`).
...
- **Bake:** run the existing per-probe algorithm (`bake_probe_rgb_with_moments` → `pack_octahedral_irradiance_tile`) over the group's probe indices with the reaching-light set; store the group's octahedral tiles + depth moments + validity. `get`/`put` against `StageCache`.
...
**Phase 2 (concurrent):** the lightmap chain (Task 2 → Tasks 3, 4 → Task 5) and the SH track (Task 6) are independent subsystems and proceed in parallel; both reuse the Task 4 geometry-slice definition.
