...
- Tile dimension N (stored in the section header so resolution can change via re-bake, not a format break) and border width (border = 1). Default N to the DDGI 6×6-incl-border (4×4 interior) reference; the Open Questions tuning only changes the default, not the format.
