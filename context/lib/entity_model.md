...
**Dynamic accuracy.** Per-shot spread is a dynamic axis, not a fixed cone. Descriptor-tuned bloom, movement, and optional vertical bias compose the effective hitscan half-angle through one shared host/client calculation. Authored tuning replicates; bloom accumulator and idle timer remain live per-instance state across hot reload. Projectiles converge on the crosshair and never sample the cone. Each machine exposes its predicted neutral-degree result through local-only `player.spread` HUD state; the reticle maps it to a spread ring in script.

**Splash / area effects.** Splash is a `splash` block on `WeaponDescriptor` — a peer of the projectile block, not nested under it — so it composes onto whatever resolution locates a detonation point (projectile impact first; hit point, aim point, or caster position later), never a separate area-effect descriptor kind. It damages every live damageable entity in a blast radius with authored linear distance falloff, each once through the damage chokepoint; falloff is pre-scaled into the amount-only `DamagePayload`, not a payload field. Host-authoritative — resolves host-side, reaches clients as replicated Health results, adds no replicable component kind; the explosion is a presentation-channel event. Not built yet.
...
| 10 | Projectile launch finalization | Closes spawn grace for projectiles launched after the flight stage without moving them. They begin flight on the next fixed tick. |
| 11 | Death sweep | Processes entities whose health reached zero after same-tick damage |
...
**Radial entity overlap.** A one-to-many spatial query, sibling to the nearest-entity raycast: given a world center and radius it returns every live damageable entity whose broad-phase volume intersects the sphere, each with its nearest-point distance. Caller-agnostic — no weapon, projectile, or splash knowledge — so splash, a future melee shape-sweep, and area damage-over-time share one implementation. It reuses the raycast broad phase (authored AABB or derived model bound) and the shared liveness gate; an optional static-world occlusion argument drops wall-hidden targets, and a caller-supplied exclusion predicate removes the owner and non-damageable entities. First member of a non-ray query family; the melee sweep is its narrow-phase sibling. Not built yet.
...
- **Peers are candidates; the engine may rank retaliation over distance.** The offered set includes brain-bearing enemies, not only player pawns, so an enemy can perceive a peer that damaged it. Ranking stays the engine's — candidacy is still boolean and narrowing — but the engine adds a **retaliation** term: a candidate whose recent accumulated damage exceeds an authored per-pair or per-archetype **tolerance** is preferred over the nearest hostile, tuned by authored scalars, never an authored ranking expression. The engine admits such an over-tolerance attacker to candidacy even when sentiment leaves the pair non-hostile — its own widening, not authored candidacy's — but never prices the think stride off it; the stride stays the nearest sentiment-hostile distance. A retaliation-acquired target is engine-held against distance hysteresis until it is lost or a stronger over-tolerance provoker unseats it; standing down otherwise stays an authored guard.
- **Hostility is mutable per-entity state, not a fixed enemy/friend archetype.** The engine floor narrows fresh acquisition by it — a candidate is offered only when the evaluating enemy is hostile toward it — while dropping a target that turns friendly stays authored, per the split above. Hostility resolves from authored content: factions are named manifest data, and a directional per-pair **sentiment** scalar decides it — negative hostile, zero neutral, positive allied — seeded so unauthored content reproduces the prior faction-inequality rule (different faction hostile, same faction neutral) with zero authoring. The durable authored contract is the target-hostility fact a guard reads, not the numeric faction leaf beneath it: that leaf is an index into the sentiment table. Sentiment is moving from frozen content to runtime-mutable state (not yet built): the authored scalar becomes a baseline, and the AI reads a *live* value that authored reactions and an impact effect move and an author-tunable decay eases back toward baseline — so hostility can change mid-session. The live value is host-authoritative and replicated as state, not content (`networking.md`).
