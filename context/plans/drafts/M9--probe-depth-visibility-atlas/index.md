...
- Baker: `bake_probe_indirect_rgb` (or a thin wrapper beside it) in `crates/level-compiler/src/sh_bake.rs` returns the SH coeffs plus `(sum_d, sum_d2)`; `bake_sh_volume` divides by `RAYS_PER_PROBE` and stamps the moments into each `ShProbe`. Reuse `closest_hit`'s `Hit.distance`; sky miss → a `FAR_SENTINEL` const. `STAGE_VERSION` 1 → 2.
...
- **Far sentinel value.** A sky-miss ray needs a finite "open" distance for the moments. Candidates: a fixed large constant (e.g. a multiple of the grid diagonal) or the world AABB diagonal. Must stay f16-representable and large enough that an open probe reads as fully visible under Chebyshev. Pick a concrete constant during Task 2; record it. Leaning toward a fixed large meters constant for determinism independent of map size, but a grid-diagonal-relative value is the alternative if open-probe variance needs to scale with the map.
...
- **Does the existing determinism test exercise varied distances?** `sh_volume_bake_produces_byte_identical_output_on_repeated_runs` uses a single floor triangle; confirm its probes produce non-uniform hit distances (and thus exercise the moment accumulation), or add geometry that does. Verify during Task 3.
