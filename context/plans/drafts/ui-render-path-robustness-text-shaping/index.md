...
This is forward-looking hardening + coverage + design-for-cost. It does **not** re-fix the landed clobber.
...
- [ ] The UI encode entry point consumes a single composition value spanning all stack layers; there is no public/`pub(crate)` per-layer encode that a caller can invoke in a loop. Removing the composition type or calling encode per-layer fails to compile or trips the debug guard.
- [ ] A debug-build guard fires if glyphon `prepare` is reached more than once between frame boundaries on the shared `UiTextRenderer`. Release builds carry no guard cost.
...
- [ ] All existing UI tests (`multi_batch_test`, `splash_golden_test`, the `tree.rs` readout-aliasing regression, theme/demo/gameplay gates) still pass unchanged in behavior.
- [ ] A headless test renders two stacked layers — a lower layer whose bound text readout shows string S0 and an upper layer showing a different string S1 — into an offscreen target, reads it back, and asserts the lower layer's text region renders S0's glyph coverage, not S1's. The test self-skips (prints a skip line, returns) when no GPU adapter is present and never fails CI for adapter absence.
- [ ] The golden's assertion discriminates the bug: a simulated single-`prepare`-per-layer clobber (or the pre-fix code path) would fail it; the composed path passes.
- [ ] A design note (in this plan's `research.md` or a `## Rough sketch` subsection) specifies the per-node shaped-buffer cache: where buffers live, the invalidation triggers (content / theme / device-scale / viewport change), the node-identity cache key, per-stack-layer namespacing, and the measurement threshold that warrants landing it. No premature cache rewrite lands.
...
Add a debug guard against a second glyphon `prepare` per frame on the shared `UiTextRenderer`: a frame-scoped flag (set on `prepare`, cleared at a per-frame reset point the renderer already has) feeding a `debug_assert!`. Per development_guide §3.6, gate any guard-only helper with `#[cfg(debug_assertions)]` so it compiles out in release. Plumbing: the reset call-site is the renderer's per-frame UI entry; name it in the sketch. The splash path (single composition already) must satisfy the same guard.
...
- **A.** New `pub(crate)` composition type in `render/ui/mod.rs` (e.g. `UiComposition<'a>`) owning the ordered batches + concatenated `Vec<UiText>` in bottom→top painter order; its constructor takes the per-layer `UiDrawData` slice (+ the white bind group, image registry resolver) and performs the fold currently inline in `render_frame_indirect` (`mod.rs` ~5419–5515). `UiPass::encode` becomes `encode(&mut self, …, composition: &UiComposition)`; the internal quad/text loop is unchanged. Frame-`prepare`-guard: a `bool`/counter on `UiTextRenderer` set in `prepare_text`, asserted `< 2`, reset at the renderer's per-frame UI entry; gate the helper with `#[cfg(debug_assertions)]`. Splash path (`record_splash_ui`) builds a single-layer `UiComposition` so it shares the boundary and guard.
...
- **prepare-guard reset site.** Confirm the exact per-frame renderer entry point that should clear the `prepare` guard, so a multi-frame run doesn't trip it spuriously (the guard must reset every frame, not only on the gameplay path — the splash path prepares too).
