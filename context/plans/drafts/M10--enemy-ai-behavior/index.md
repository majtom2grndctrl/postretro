...
- **`components.ai` descriptor block** — authored on the entity archetype: detection range, attack range, attack damage, attack cooldown, move speed, leash (lose-target) range, optional death-despawn delay, and the mapping of logical states (idle / alert / attack / death) to the archetype's declared animation-state names. Rust descriptor struct + parser in a **new module**, wired into `EntityTypeDescriptor` as one field with one parse dispatch — keeping the 9579-line `data_descriptors.rs` delta to a single hook, not an extension of its body. QuickJS/Luau parse twins: same validation, same warn-and-degrade.
- **AI FSM tick system** — engine-owned, new module + a thin `run_ai_tick` wrapper in `main.rs`, hooked **after** the player movement tick and **before** plan 1's agent steering tick (so a destination set this tick is steered this tick). Per enemy: resolve the player pawn + position; evaluate transitions (idle↔alert by detection/leash range, alert→attack by attack range, any→death at zero HP); drive plan 1 (set the agent destination to the player while chasing, clear it when idle); on an attack tick (in range + cooldown elapsed) apply damage to the player via the Rust damage chokepoint and emit an attack event; select the animation state per logical state via the engine switch path. Distant / off-screen enemies evaluate on a think stride (shallow time-slicing for waves), aligned with the animation resample precedent.
...
- [ ] The `ai` descriptor parses identically under QuickJS and Luau: valid tuning materializes a brain; out-of-range or non-finite fields warn-and-degrade to defaults on both runtimes (no abort on either); a logical-state mapping naming an undeclared animation state warns and that mapping degrades (runnable parse-twin tests, the health/mesh descriptor precedent).
...
- [ ] Distant enemies evaluate the FSM on a reduced think stride while near enemies evaluate every tick; a state-changing condition is not missed because of striding (runnable unit test on the think-stride gate).
...
New engine-internal brain component (new `ComponentKind`): logical state, attack-cooldown timer, think-stride counter, resolved tuning. New descriptor struct + parser module for the `ai` block (detection/attack/leash ranges, attack damage, attack-cooldown ms, move speed, death-despawn ms, logical-state→animation-state map). Wire into `EntityTypeDescriptor` with one field + one parse dispatch (no expansion of `data_descriptors.rs` body). Materialize the brain — and an agent component (plan 1), capsule from the navmesh agent params, move speed from the descriptor — on spawn in `data_archetype.rs`. Validation: numeric ranges, finite-ness, and logical-state targets must name animation states the archetype's `mesh` block declares; warn-and-degrade on violation, twinned across both runtimes.

### Task 2: AI FSM tick system

Engine-owned FSM tick (new module + `App::run_ai_tick` wrapper in `main.rs`, hooked after player movement and before the agent steering tick). Per enemy on its think tick: locate the player pawn (the `PlayerMovement`-bearing entity) and its position; evaluate transitions; drive the steering API (set/clear destination); on attack apply damage to the player via the Rust chokepoint (`apply_damage`) + emit the attack event; request the mapped animation state via the engine switch path (`switch_animation_state`). Think-stride time-slicing by player distance. Depends on plan 1's steering API and Task 1.
...
- Damage the player: `scripting::components::health::apply_damage(registry, player_id, &DamagePayload { amount })` — the same chokepoint the weapon uses (`scripting.md` §10.5). Zone multipliers are a damage-*site* concern; the enemy's contact attack has no hit zone, so it passes a flat amount.
...
- Player lookup: the player pawn is the first `PlayerMovement`-bearing entity (`scripting/systems/ui_proxy.rs` already finds the pawn-with-health — reuse the lookup shape).
...
`states` maps the four logical states to animation-state names, e.g. `{ idle, alert, attack, death }` → declared `mesh` state names. The component / descriptor block name `ai` is a working name — resolvable during review against the engine-closed component vocabulary (`entity_model.md` §1).
