...
Airborne jump branch fires when `!is_grounded && jump_pressed && air_jumps_remaining > 0 && velocity.y <= air.jump_ceiling`, using `air.jump_velocity`. `air_jumps_remaining` resets to `air.jumps` on any tick with floor contact. So a descriptor with `air.jumps >= 1` yields a working double-jump today. Task 5 is consolidation + tests, not new mechanics.
...
- `PlayerMovementDescriptor` / `GroundParams { speed: SpeedParams { walk, run }, accel, step_height, max_slope }` / `AirParams { forward_steer, accel, max_control_speed, bunny_hop, jumps, jump_velocity, jump_ceiling }` / `FallParams { terminal_velocity }` in `scripting/data_descriptors.rs`. JS parser `movement_descriptor_from_js`, Luau parser `movement_descriptor_from_lua` — symmetric validation via `validate_non_negative_finite` etc.
