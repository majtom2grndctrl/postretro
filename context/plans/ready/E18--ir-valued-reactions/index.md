...
- **Retire `state-delta-reactions`.** The draft was already removed in a prior commit; document `increment`/`decrement`/`clamp-to-range` as the `runtime`-builder pattern (the write path already validates and clamps to a slot's declared range).
...
- [ ] (grep gate, not a runtime test) No `incrementState`/`decrementState` symbol ships in `sdk/` or `crates/`, and the `E18--state-delta-reactions` draft is absent — both already hold today; this gate keeps them holding.

## Tasks

### Task 1: IR-valued `setState` — bind against `StoreScope`, evaluate at the write point

Widen the `setState` value from a frozen literal to a literal-or-IR, mirroring movement's dash-field pattern. **SDK:** `updateState<T>(ref, value)` (`sdk/lib/ui/reactions.ts:309`) accepts `value: T | RuntimeValue`; the emitted descriptor carries the `RuntimeValue` node (or the bare literal) under `args.value`. **Deserialize:** in the `setState` arg parse, branch on whether `value` is an IR node object vs a bare literal, reusing `ir_node_from_json` (`crates/foundation/src/data_descriptors/validate/foundation.rs:55`) and the `NumberOrIr`/`validate_dash_expr` shape (both defined in the **foundation** crate — `NumberOrIr` at `crates/foundation/src/data_descriptors/types/movement.rs:163`, `validate_dash_expr` at `crates/foundation/src/data_descriptors/validate/foundation.rs:29`; the scripting-core readers `read_dash_number_js` (`js/movement.rs:406`, returns `NumberOrIr`) and `read_dash_bool_js` (`:446`, returns `BoolOrIr`) only consume them). Arg-parse produces a type-agnostic `IrNode`; number-vs-boolean is settled by `StoreScope` type projection at bind against the target slot's declared type — the parse does not need the slot type. **Bind:** wrap the parsed `IrNode` as `BakedIr { version: CURRENT_IR_VERSION, output: Some(slot), root }` (the epoch is stamped here at wrap-time; the inline `args.value` node carries no version of its own) and bind against `StoreScope::script(ctx)` (`crates/scripting-core/src/ir/scopes.rs:62`) at level install — type projection and the readonly deny (`scopes.rs:104`) do the validation the current readonly gate did. **System path — mind the crate wall.** `SystemReactionCommand::SetState { slot, value: serde_json::Value }` lives in the **entities** crate (`crates/entities/src/reactions/system_commands.rs:44`), which cannot name `BoundProgram<StoreScope>` (`StoreScope` is scripting-core) — so the command keeps carrying the raw node (`value: serde_json::Value` already holds the IR wire shape); the *bound* `BoundProgram<StoreScope>` is held in a postretro-side install-time side map keyed to the reaction/command, exactly the split Task 2 uses for `Watcher`. **Build the install-time bind pass (new):** the shipped `setState` handler (`crates/postretro/src/scripting/systems/system_reactions.rs:207-216`) only `serde_json::from_value`s args per-fire at dispatch — there is NO install-time pass over system reactions today. Add one: when system reactions are installed, bind each `SetState` IR value once against `StoreScope::script(ctx)` (this is what surfaces the readonly/non-projectable rejection at level install per AC-3), store the `BoundProgram` in the side map, and at drain (`:212`→dispatch) `eval_and_write` (`crates/foundation/src/ir/eval.rs:41`) the bound program against a script-capability `StoreScope` over the live slot table, in enqueue order. **In-tick path:** extend `BoundTriggerCommand::StoreSlot { slot, value: SlotValue }` (`crates/postretro/src/trigger_bindings.rs:113`) — `BoundTriggerCommand` is in the postretro crate, so it *can* carry the `BoundProgram<StoreScope>` directly — evaluated in-tick via `eval_and_write` against a `StoreScope` over the tick slot table; this path already binds at install, so only the value branch is new. A literal value keeps the shipped fast path in both surfaces. Plumbing: the install-time binds need `ScriptCtx` (already threaded to trigger install; the new system-reaction install pass must be handed the same `ScriptCtx`); the `BoundProgram` is stored once (side map for the system path, on the command for the in-tick path) so no per-fire re-bind occurs.

### Task 2: IR-predicate crossings

Generalize the crossing watcher from an `above`/`below` literal to a `Bool` IR predicate. **Mind the crate boundary:** the descriptor/condition types live in the **entities** crate (`CrossingCondition` / `CrossingDescriptor` at `crates/entities/src/data_descriptors/types/reactions.rs:69` / `:86`), which depends only on `foundation`; `StoreScope` is a **scripting-core** type (`BoundProgram` itself lives in foundation, but the instantiation `BoundProgram<StoreScope>` cannot be named from entities because `StoreScope` is scripting-core). So the raw predicate (`IrNode`, a foundation type) is carried on the descriptor/condition, and the *bound* `BoundProgram<StoreScope>` lives only on the private `Watcher` (scripting-core) — mirroring how `Watcher` already holds derived `previous`/`max` state the descriptor does not. **SDK:** add a predicate overload `onStateCrossing(predicate: RuntimeValue, fire: (NamedReactionDescriptor | string)[]): CrossingDescriptor` beside the shipped threshold form `onStateCrossing(ref, condition, fire)` (`sdk/lib/ui/reactions.ts:79`), emitting a `CrossingDescriptor` predicate variant. The shipped descriptor is `{ slot, max?, fire, levels? } & ({above}|{below})` (`max` = the shipped optional fire-count cap; see `reactions.ts:16`); the predicate variant carries a `predicate` node in place of `slot`/`above`/`below`. **Pin the wire discriminant:** the deserializer distinguishes the two forms by the presence of `predicate` (IR form) vs `slot` (threshold form) — no threshold descriptor carries a `predicate` and vice versa. The shipped threshold form stays. **Entities-crate edit:** add an `Ir(IrNode)` variant to `CrossingCondition` (`reactions.rs:69`) carrying the raw predicate node, and drop the `Copy` from its `#[derive(Debug, Clone, Copy, PartialEq)]` (`reactions.rs:68`) since `IrNode` is not `Copy` — convert the by-value copy at `state_crossings.rs:110` to a `.clone()`; `IrNode` is `PartialEq`, so the descriptor/`Watcher` `PartialEq` derives still hold. **scripting-core edit:** extend the private `Watcher` in `crates/scripting-core/src/state_crossings.rs` (edge test at `:119`) to hold the bound `BoundProgram<StoreScope>` for the `Ir` case. **Escape the mandatory-`slot` path:** `CrossingDescriptor.slot: String` is non-optional (`reactions.rs:87`) and `initialize` (`:56`) unconditionally runs the `slot_is_number(slot_table, &crossing.slot)` guard and `continue`s on failure — a predicate descriptor has no meaningful slot, so it would be skipped before its `Ir` arm runs. Make `slot` optional (`Option<String>`, `None` for the predicate form) or relocate the `slot_is_number` guard inside the threshold arm; `Watcher.slot` (`:21`) likewise carries no slot for a predicate (make it `Option`, or store the predicate's source-slot names for diagnostics only). At `initialize` (`:54`), for the `Ir` arm bind the condition's `IrNode` against `StoreScope::script(ctx)` — type-check it to `Bool`, warn+skip if it does not (mirroring the non-Number-slot skip). Plumbing: `initialize` (`(&mut self, &DataRegistry, &SlotTable)` today) must additionally receive `ScriptCtx` to construct `StoreScope::script(ctx)`; the real caller is `rebuild_reaction_subscribers` (`crates/postretro/src/scripting/lifecycle.rs:901`), which already holds `script_ctx` (`:1115`) — thread it from there; the compiler flags the one other (test) call site at `ui_proxy.rs:583`. The bound program is stored on the `Watcher` so no per-frame re-bind occurs. Each frame in `detect` (`:79`), `eval_value` the predicate against the slot table; a watcher fires on `false`→`true` and re-arms on `true`→`false`, reusing the existing `previous`-endpoint lifecycle. The predicate reads slots by name through `StoreScope`, so it needs no single `slot` field.

### Task 3: SDK typedefs, parity, and `state-delta-reactions` retirement

Land the author surface. First widen the **Rust-authored typedef surface** — the `.d.ts`/`.d.luau` are emitted from the typedef module at `crates/postretro/src/scripting/typedef/` (not from the `sdk/lib/*.ts` builders), guarded by the drift test at `crates/postretro/src/scripting/typedef/tests/committed.rs` — for the widened `setState`/`updateState` value type and the new `onStateCrossing` predicate overload; then regenerate `sdk/types/postretro.d.ts` / `.d.luau` via `gen-script-types` and update the committed snapshot. Ensure the Luau `updateState`/`onStateCrossing` builders and the `runtime` Luau twin canonicalize to byte-identical IR (the `wrap`-rule parity already asserted for `runtime`). Add a short "counters and derived state" subsection to `docs/scripting-reference.md` (beside its existing "Runtime values" §`:191` / "Where runtime values are accepted" §`:243`) showing `updateState(ref, runtime.add(runtime.read(slot), 1))` for increment, `runtime.sub(...)` for decrement, and `runtime.clamp(...)` (or the slot's declared range) for bounding — so the retired state-delta reactions' use cases have a documented home. (The `E18--state-delta-reactions` draft was already removed in a prior commit; nothing to delete — just confirm its absence.)

### Task 4: Tests and fixtures

Prove both surfaces and the accumulation contract. Unit/e2e: same-frame double-increment reaches +2 — drive each surface at its own pump: the system-command surface via the app-side reaction drain, the in-tick surface by firing a trigger `on_fire` within one `simulate_tick` (the two drive points Task 1 builds; the predicate/reject tests can template off the existing `state_crossings.rs` test module and the `scopes.rs` bind harness). A derived clamped write lands the evaluated value; a readonly-target IR rejects at bind; a literal `setState` is unchanged; a two-slot AND predicate fires only when both hold and re-arms correctly; a non-Bool predicate is rejected. Parity: a TS fixture and a Luau fixture author an identical increment-and-predicate flow and produce identical IR. Determinism: extend the headless harness with an IR-write and an IR-predicate tick sequence in the green-and-stays-green gate.
