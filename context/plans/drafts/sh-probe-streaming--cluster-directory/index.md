...
- [ ] **AC3 — Address completeness:** every present SH-family section has one resource descriptor; absent sections have none. Ranges cover required probe/node reconstruction support and all entry-bearing affinity cells, including empty-validity zero-contribution entries and partial edge bricks. The same logical directory addresses BC6H and uncompressed versions of identical grid/metadata/CSR inputs without byte-offset dependence.
...
- [ ] **AC7 — Inert output:** section 49 is never read by rendering, visibility, gameplay, content-parity hashing, or frame scheduling. With identical loaded lighting payloads, directory-present and directory-absent files allocate identical SH resources and render identical same-adapter frames. Manual windowed verification also covers animated billboard lighting, excluded by the static capture harness.
