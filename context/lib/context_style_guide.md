...
**After:**
```
Renderer traverses portal graph, draws visible faces with baked lightmaps
from prl-build.
```
...
reverb parameters by looking up which runtime cell the listener is
currently in and checking whether that cell falls within an
env_reverb_zone entity.
```

**After:**
```
Audio (kira): reverb parameters resolved per runtime cell via
...
**Precedents are candidates, not obligations.** Ground the precedent's input domain, rejection behavior, precision, lifetime, and consumers against the required invariant. Mandate reuse only when those semantics match. Otherwise state the property to preserve and cite the precedent to investigate.
...
**Before:**
```
The map loader needs to handle the case where the PRL navmesh
section is missing because not all maps will be compiled with
navigation data, and in that case the AI system should avoid trying
to run pathfinding.
```

**After:**
```
NavMesh section is optional — not all maps support navigation.
Loader signals absence via an `Option`. AI disables pathfinding for
that level. Missing section is not an error.
```

The "after" is longer. That's fine — the optional-section contract and its degradation behavior earn the extra sentences. Brevity means no wasted words, not fewest words.
