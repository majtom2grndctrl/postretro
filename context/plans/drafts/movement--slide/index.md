...
- [ ] Direction-steer (D10): with `steerControl > 0`, sustained input perpendicular to the slide rotates the velocity toward the input direction over time (magnitude preserved, relative to the post-reconcile/post-decay boost); with `steerControl = 0` the slide direction is committed (input does not rotate it).
