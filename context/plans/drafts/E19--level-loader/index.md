...
- Update the importers to the crate path: ~32 files in `crates/postretro/src/` reference `crate::prl`/`prl::`, including ~10 inline `crate::prl::Foo` paths with no `use` statement — search `rg 'crate::prl(_loader)?\b'`, not just `use crate::prl`. Other crates match only in doc-comments; there are no real external importers.
- Sink the bare `LightInfluence` struct (`{ center, radius }`, today in `lighting/influence.rs`) down into `postretro-render-data`, and re-point `LevelWorld.light_influences` (`prl.rs:296`) plus its construction in `prl_loader.rs` to it. It is a bounding-sphere cull volume in the `Aabb`/`cone_frustum` family — epic Decision 10 — so the loader depends *down* on it, not *up* on `lighting`; the GPU `pack_influence` packer stays in `lighting`. (Coordinates with `E19--render-data`/`E19--lighting-cpu`.)
