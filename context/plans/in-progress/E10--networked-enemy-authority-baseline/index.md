...
- Animation-state replication. The first remote enemy may render in descriptor default animation while its transform moves. `E10--enemy-locomotion-animation` owns the follow-up if remote locomotion needs velocity-derived animation or a focused animation-state payload.
...
Bump `SNAPSHOT_VERSION`. Keep the existing `RawEntityRecord.has_entity_class` + `entity_class` fields, but change validation semantics: descriptor class is valid on non-despawn entity records that carry a finite `Transform`. Add a registry-blind `WireTransform` finiteness check in `crates/net/src/wire.rs` and use it when validating `Transform` payloads and `entity_class` records. Movement-authority metadata remains valid only with `PlayerMovementState`. Despawn records still reject every metadata flag and non-empty class string. Update raw-to-typed validation tests, drift guards, malformed-envelope tests, and snapshot encode/decode round trips.
...
| Remote enemy presentation | descriptor `mesh` block -> client `MeshComponent` | descriptor class names only; descriptor lookup is engine-side and mesh data stays shared local content | existing descriptor data, no shape change | existing descriptor data, no shape change | n/a |
