...
- [ ] Placement holds: `DamagePayload`/`NavAgentParams`/`ModMapEntry` and the foundation-clean descriptors (movement/weapon/health/ai/light) resolve from `postretro-foundation`; the reaction/crossing descriptors, `MeshDescriptor`, `EntityTypeDescriptor`, `apply_damage`/`attach_agent`, and `EntityId` resolve from `postretro-entities`; `weapon`/`nav` define neither sunk POD; `MenuCamera`/`Frontend` stay in `postretro`.
