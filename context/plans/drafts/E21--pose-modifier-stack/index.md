...
- State-gated aiming. Enemies aim the torso whenever a target is held (else the velocity heading); gating aim to specific logical states (e.g. only in Attack) is a small follow-on that belongs with the locomotion descriptor in the sibling plan, not here.
