...
| `kinematic_mover` | brush | Runtime-moving brush payload, excluded from static BSP/BVH/lightmap/collision | `name`, `path` (first waypoint name), `move_mode` (`once`/`ping_pong`), `speed`, `wait_ms`, `start_on_spawn`, `spin_axis` (local `x y z`), `spin_speed` (deg/s), `spin_accel` (deg/s²), `carry_yaw` (default false), `_tags` |
...
- **`kinematic_mover`** — compiler normalizes a finite local `spin_axis`, validates finite `spin_speed` and finite non-negative `spin_accel`, then packs the static rotation authoring with the mover. A non-zero spin requires a non-zero axis and may use one waypoint as a pure rotator; zero-spin movers still require at least two waypoints.
...
| KinematicGeometry | 43 | Version 2 when the map has at least one `kinematic_mover`; origin-relative mover vertices/indices/face metadata, static spin authoring, and `kinematic_waypoint` records |
...
**KinematicGeometry (id 43):** optional section for deterministic brush movers. Version 2 carries `mover_id`, `name`, `_tags`, origin, first waypoint `path`, speed, endpoint wait, move mode, start flag, local-space vertex/index/face metadata, and static spin axis, initial speed (deg/s), acceleration (deg/s²), and `carry_yaw`. Version-1 sections remain loadable and default those rotation fields to zero spin, zero acceleration, and `carry_yaw = false`. Waypoint records carry `name`, `next`, and origin. Runtime hashes the decoded kinematic and collision inputs into the multiplayer static-content gate; peers with different mover sections cannot pass app handshake. Mover brushes are not present in static `Geometry`, `Bvh`, static collision, lightmaps, SDF, portals, or navmesh.
