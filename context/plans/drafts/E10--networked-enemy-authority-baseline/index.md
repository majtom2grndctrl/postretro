...
Extend level install so a connected client skips descriptor map placements that would create authoritative AI enemies. Use the same AI-enemy predicate as host registration, or add a test invariant that every descriptor `ai` materialization produces live `Brain` + `Agent` components. Static props, FX, lights, and other non-AI descriptor placements keep materializing locally as today. Add tests proving connected clients skip AI descriptors while single-player and host installs keep them.
