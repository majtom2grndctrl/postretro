...
| crossing predicate | `CrossingCondition::Ir(BoundProgram<StoreScope>)` beside `Below`/`Above` | `CrossingDescriptor` predicate variant, discriminated by presence of `predicate` (vs `slot`), carrying a `RuntimeValue` node | `onStateCrossing(predicate, fire[])` (predicate overload) | same |
