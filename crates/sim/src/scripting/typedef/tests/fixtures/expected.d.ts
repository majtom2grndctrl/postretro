...
declare module "postretro" {
  ...
  /** Direct-hit velocity change independent of damage. Omit the block for no direct knockback. */
  export type KnockbackDescriptor = {
    /** Added speed in metres/sec. Must be finite in 0..=1000; 0 disables push. */
    speed: number;
    /** Blend hit travel direction toward world up before normalization. Defaults to 0; finite in 0..=1. A zero or cancelled direction falls back to up. */
    upwardBias?: number;
  };

  /** Radial push using the blast radius and occlusion, independently of damage and selfDamage. */
  export type SplashKnockbackDescriptor = {
    /** Added speed at the blast center in metres/sec. Must be finite in 0..=1000. */
    speed: number;
    /** Blend outward direction toward world up before normalization. Defaults to 0; finite in 0..=1. A coincident or cancelled direction falls back to up. */
    upwardBias?: number;
    /** Fraction of push speed at the blast edge, with linear falloff. Defaults to 0; finite in 0..=1. Independent of damage minFraction. */
    minFraction?: number;
    /** Owner-only push multiplier. Defaults to 1; finite in 0..=10. Set 0 to disable self push, independently of selfDamage. */
    selfScale?: number;
  };

  /** Player or enemy response to hit impulses. All fields are optional; omission uses the defaults. */
  export type KnockbackResponse = {
    /** Received impulse multiplier. Defaults to 1; finite in 0..=10. Set 0 for immunity. */
    scale?: number;
    /** Grounded decay rate in inverse seconds. Each tick multiplies protected impulse by max(0, 1 - groundDrag * dt). Defaults to 8; finite in 0..=1000. */
    groundDrag?: number;
    /** Airborne decay rate in inverse seconds, using the same rule as groundDrag. Defaults to 0; finite in 0..=1000. Gravity still acts normally. */
    airDrag?: number;
    /** Fraction of normal steering while knockback remains. Defaults to 1; finite in 0..=1. Abilities remain available. */
    control?: number;
  };
  ...
  export type SplashDescriptor = {
    ...
    minFraction?: number;
    /** Whether the firing pawn can be damaged by its own blast. Defaults to true; does not suppress knockback. */
    selfDamage?: boolean;
    /** Optional radial push independent of damage. Omit for no blast push. */
    knockback?: SplashKnockbackDescriptor;
  };
  ...
  /** A dynamic point light that travels with the projectile body. It lights nearby surfaces only, casts no entity shadows, and never changes damage or hit detection. */
  export type ProjectileLight = {
    /** The light's colour as exactly three finite numbers `[r, g, b]`, in linear space with each channel in `[0, 1]`. It is multiplied by `intensity` to produce the radiance cast on nearby surfaces. */
    color: readonly [number, number, number];
    /** Linear multiplier on `color` that sets how bright the cast light is. Use a finite number of 0 or greater. Under the default `Linear` falloff, the near-field radiance on a surface is roughly `colorChannel * intensity` (bounded), and a surface blooms once that result exceeds the bloom threshold of 1.0 — so keep `intensity` at about 1.0 or below for a bright glow that does not bloom. */
    intensity: number;
    /** The distance in metres over which the light attenuates to zero. Use a finite number greater than 0. */
    falloffRange: number;
    /** How the light attenuates with distance. Optional; defaults to `Linear`. `Linear` — bounded attenuation in `[0, 1]`, the default and least likely to bloom. `InverseDistance` — `1/d` falloff. `InverseSquared` — physically-accurate `1/d²` falloff; unbounded near the source so it blooms easily, use with care on fast or close projectiles. */
    falloffModel?: FalloffKind;
  };

  /** A transient point light spawned at a projectile impact. It always fades over `fadeMs`, casts no entity shadows, and never changes damage or hit detection. */
  export type ProjectileImpactLight = {
    /** The flash colour as exactly three finite numbers `[r, g, b]`, in linear space with each channel in `[0, 1]`. It is multiplied by `intensity` to produce the radiance cast on nearby surfaces. */
    color: readonly [number, number, number];
    /** Linear multiplier on `color` that sets how bright the flash is. Use a finite number of 0 or greater. A surface blooms once the resulting near-field radiance exceeds the bloom threshold of 1.0, so keep `intensity` at about 1.0 or below for a bright flash that does not bloom. */
    intensity: number;
    /** The starting distance in metres over which the flash attenuates to zero. Use a finite number greater than 0. */
    radius: number;
    /** Optional final distance in metres over which the flash attenuates to zero. When present it must be at least `radius`, and the reached distance expands from `radius` to `peakRadius` while the flash fades. Omit to hold `radius` for the whole flash. */
    peakRadius?: number;
  ...
  export type ActionVerb = {
    /** Name of the root graph attack this activity fires. */
    attack: string;
  };
  ...
  export type BehaviorGraphDescriptor = {
    ...
    /** Optional enemy impulse response. Omit for default scale 1, groundDrag 8, airDrag 0, control 1. */
    knockback?: KnockbackResponse;
  ...
  export type PlayerMovementDescriptor = {
    ...
    fall: FallParams;
    /** Optional player impulse response. Omit for default scale 1, groundDrag 8, airDrag 0, control 1. */
    knockback?: KnockbackResponse;
  ...
  export type ImpulseParams = {
    /** Default critical-spring settle rate in 1/sec. Must be finite in [0.1, 240]; larger values settle sooner without rebound. */
    tension: number;
    /** Absolute per-channel ceiling on the summed presented offset. Every field must be finite in [0, 180]. */
    max: ImpulseChannels;
    /** Sparse state-keyed entry/exit displacement definitions. */
    states: ImpulseStates;
  };

  /** Signed camera displacement in degrees. Entry and exit channels must be finite in [-180, 180]; `ImpulseParams.max` uses the same shape with a [0, 180] range. */
  ...
  /** Sparse impulse rows for the closed movement state vocabulary. Only JavaScript own properties and Luau direct table entries define rows. Unknown or non-string entries among those are rejected; prototype and `__index` rows are ignored. */
  ...
  export type ImpulseStateParams = {
    /** Optional state-specific settle rate in 1/sec. Must be finite in [0.1, 240] when present. */
    tension?: number;
