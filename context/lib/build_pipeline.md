...
**ShadowmaskAtlas (id 42):** optional sibling for `EntityShadowLights`, used by entity→world shadows from promoted static lights. Header is little-endian `u32 width`, `height`, `layer_count`, `selected_light_count`, followed by one channel byte per selected light (`0..3` = RGBA channel, `0xFF` = no mask/dropped). Channel bytes pad to 4-byte alignment. Payload is `layer_count × width × height × 4` bytes of layer-major `Rgba8Unorm` visibility data (`255` = fully visible). Dimensions and layer count match the Lightmap irradiance atlas. Compiler consumes `EntityShadowLights` order; it never reselects from `AlphaLights`. `_bake_only 1` lights have no entries because they are absent from `AlphaLights` and the selected set. Channel assignment graph-colors selected lights that affect the same texel. A deterministic bounded exact search preserves the established channel/drop result on ordinary inputs. If exact search proves the overlap graph is not 4-colorable, masks drop globally from lowest intensity upward. If pathological overlap exhausts the exact-search budget, a deterministic conservative fallback processes the remaining masks from highest to lowest retention priority and drops a lower-priority mask when retained neighbors already occupy all four channels. Each light owns one channel, global across every atlas texel; its channel stays consistent everywhere it appears. Coloring must therefore see a light's whole overlap set before assigning. The composite is **non-additive** — per-texel overlap graph plus graph-coloring, unlike the Lightmap's additive layer sum. The shadowmask site owns the per-light resident-memory bound; the lightmap composite cannot reach it. Overlap membership is **unshadowed**: a texel enters a light's set by a geometric predicate on light parameters, texel world position, and surface normal, decided before any visibility ray is cast. Occlusion changes the stored value, never the membership, so the overlap graph is derivable without baking and channel assignment is stable under occlusion edits. Making coverage shadow-aware would be a format-level decision, not a bake tweak. Per-tile *coloring* stays foreclosed without a per-texel channel store — the format a no-drop ShadowmaskAtlas would own — but per-tile graph *construction* is not foreclosed: adjacency may accumulate in any order provided coloring runs once over the completed graph. The graph's node set is the selected-light list itself, because channel bytes are written by selection index; work may be skipped, a node may never be dropped. *Not yet built:* `lighting-scale--shadowmask-cold-working-set` owns the analytic graph pass. Today the compiler derives the graph from a materialized per-(light, texel) record whose residency scales with light count times covered texels.
