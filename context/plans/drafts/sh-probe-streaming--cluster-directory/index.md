...
For each covered brick, choose the lowest covering cluster id as owner; all other covering clusters are halo. Expand coverage to include complete support of every id-34 adaptive node touched by a cluster, including its derived origin brick, then recompute owners from the final covering sets. Id 34 stores each probe's `node_scale`, not a node origin. The adaptive-node closure is: for each dense probe index already covered by a cluster, read that probe's `node_scale`; compute its affinity brick as `floor(probe_index / 4)`; derive `node_origin_brick = floor(brick / 2^scale) * 2^scale`; derive `node_origin_probe = node_origin_brick * 4`. For scale 0, use the clipped scale-0 brick support above, including partial edge bricks. For scale > 0, the current format accepts only a full aligned `4 * 2^scale` probe cube entirely inside the id-34 grid; if `node_origin_probe + [4 * 2^scale - 1]` exceeds any grid axis, the id-34 section is invalid before directory validation. The scale>0 support AABB is therefore the section-49 half-cell-expanded box around that full aligned cube's outer centers. Add every affinity brick whose section-49 support AABB intersects the node support AABB, and repeat until no new bricks are added. This affects only resource ranges, never the cell partition. It preserves scale-0/1 production and accepted scale-2/3 measurement data without splitting stored nodes. Id 34/35 dense ranges contain every clipped probe index of covered bricks; id 47 follows the same dense grid coverage. Coalesce maximal consecutive indices per cluster/resource. Sparse companions address covered affinity bricks, including empty CSR rows; all entries in one `(section, affinity cell)` inherit the same owned/halo status. Id 48 inherits exactly id 45's coverage and ownership. Cross-resource owner agreement follows the common covering set; halo grants reconstruction access, never another accumulation right. Dense ranges claim no stored-slot ownership: Slice 3 resolves their adaptive-node representation.
...
- [ ] **AC3 — Address completeness:** every emitted/present SH-family section has one resource descriptor, including valid-empty sections; absent or withheld sections have none. Ranges cover required probe/node reconstruction support and all entry-bearing affinity cells, including empty-validity zero-contribution entries and partial scale-0 edge bricks. The same logical directory addresses BC6H and uncompressed versions of identical grid/metadata/CSR inputs without byte-offset dependence.
