...
**View feel is render-side, not tick-side.** First-person view feel — head bob, strafe tilt, ambient sway, state-transition impulse — reads the followed pawn's movement, writes nothing back. It runs at render rate. Its integrator state lives engine-side, never on the movement component or the per-tick interpolation state — tick state stays position-only. Bob's vertical and lateral oscillators have independent frequencies measured in cycles per metre travelled; setting lateral frequency to half the vertical frequency produces the traditional figure-eight gait. State-transition impulses use one critically damped spring per closed movement-state key. Their settle rates are limited to 0.1–240/sec and authored channel magnitudes to 180 degrees so every accepted descriptor stays numerically stable. A positional view-feel offset defines the effective render eye for the whole render stage: the view matrix, runtime cell lookup, portal-traversal apex, camera uniforms, and render diagnostics must all consume that same position. Entry: `view_feel.rs`.
...
A richer authored surface — response curves, a velocity expression like `boost = f(speed, charges, grounded)` — is reachable *without* an author function: the **Typed Command Buffer** (`scripting.md` §11) lets the author describe such behavior as a typed, engine-evaluated IR that crosses the FFI as data. Movement adopts it for dash tuning: dash descriptor value fields accept a literal or an engine-evaluated expression over a movement-local input namespace mirroring the trigger vocabulary above, with the evaluation moment (dash entry vs per-tick) engine-pinned per field. Wider adoption (a transition graph, response curves) stays open; the command buffer is the canonical mechanism for it. New predicates and carry-rules are still added deliberately, vetted against the flexibility band (§3) — never via a general escape hatch.
