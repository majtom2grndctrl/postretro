...
> Consumes the **Mod State Store** (`../mod-state-store/`, scripting-foundation prereq). Grounding: `research.md` (esp. §9 anchors, §11 decisions).
...
- [ ] The engine-owned `player.health` / `player.ammo` slots are exposed to the bind path as a typed, ranged, readonly schema — the published M10 contract.
