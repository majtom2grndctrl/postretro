...
- Shadow casting. The pass is built depth-reusable so a later skinned-depth variant can render NPCs into the existing 12-light real-time shadow pool (`lighting/spot_shadow.rs`) — the *Dynamic mesh shadow casting* task. That pool is stable and independent of the in-flight static-lighting (SDF) rewrite; shadows are deferred from the slice for scope, not because they are blocked. No shadow renders this slice.
...
New pass in `render/`: upload the Task 2 mesh into GPU buffers in the locked vertex layout; allocate the shared bone-palette buffer; build the pipeline binding camera uniforms and the material bind group (via existing `.prm`→`LoadedTexture` path, pre-baked texture). Flat-lit — no lighting bind group this slice; leave the slot additive. Cull one instance: `find_leaf(entity_position)` → test membership in the current visible-cell set → draw or skip. Draw at the per-instance model matrix. Depth-reusable pipeline shape — a skinned-depth variant (position + bone palette, no material) must later feed the 12-light spot-shadow pool (`lighting/spot_shadow.rs`); no shadow target this slice. Wire construction + per-frame `record_draw` into the renderer; keep edits to the large `render/mod.rs` / `main.rs` render loop minimal (construct + call only).
...
- Pass pattern: `SmokePass::new()/record_draw()` (`render/smoke.rs`); depth shape from `depth_prepass` (`render/mod.rs:1703`); shadow consumer for the later skinned-depth variant: spot-shadow pool (`lighting/spot_shadow.rs`).
...
- **Lighting deferred to the rewrite; shadow casting uses the existing pool.** Two distinct systems, deferred for different reasons. (1) How an NPC is *lit* — indirect SH + direct light — rides the dynamic-entity lighting interface, which is mid-rewrite (static lights / static-geometry SDF shadows) and unsettled. The slice renders flat-lit; the broadening *Mesh render pass* adds lighting against the settled interface, additively. (2) How an NPC *casts* a shadow — render into the existing 12-light real-time pixmap pool (`lighting/spot_shadow.rs`) — is stable and independent of that rewrite; the *Dynamic mesh shadow casting* task adds a skinned-depth pipeline variant to the pool. The slice keeps its depth-only variant compatible with the pool and leaves an additive lighting bind slot — neither breaks the locked contracts. (Roadmap M5's "CSM sun shadows" line no longer reflects the tree, but that is the user's active lighting area.)
