...
- A camera-independent, normal-free direct-scatter field for billboards.
...
1. **Billboard direct light is isotropic scatter.** The renderer samples a
   normal-free RGB direct-scatter field at the sprite centre. It replaces the
   billboard's baked direct-SH surface response for `static_light_map` lights;
   it does not choose another fake surface normal.
...
5. **No double count.** When scatter data is usable, billboards do not sample
   `DirectShVolumeSection` and `static_light_map` entries do not add the old
   surface-like static-specular term. Static SDF entries keep their current
...
- [ ] `[unit]` The billboard shader's static-light-map direct path has no
      camera-facing `NdotL` or static-specular contribution, while its dynamic
      path retains range/cone rejection and has no Lambert cosine.
...
texture at the sprite centre when usable. Route static-light-map direct through
that result, retain the existing static-SDF behavior, and remove the replaced
static-light-map specular term. Make runtime dynamic direct isotropic by
removing only the Lambert cosine after existing influence/range/cone work.
Preserve the legacy shader branch when scatter is unavailable.
...
|---|---|---|---|
| Smoke direct color is independent of camera direction. | Task 3 bakes normal-free transport; Task 4 samples it without `N = V`. | Legacy fallback must remain explicit; no static-light-map specular or direct-SH double path. | AC 1, 5, 7 |
...
| No receiver double-counts a physical direct term. | Task 4 replaces billboard static-light-map direct SH with scatter. | Static promotion remains mesh-only; SDF and dynamic policies stay disjoint. | AC 5, 7 |
