...
- **Key files**: `scripting/components/health.rs` (new), `scripting/registry.rs`, `scripting/data_descriptors.rs` (both the JS `entity_descriptor_from_js` and Luau `entity_descriptor_from_lua` parser paths gain a `health` arm), `scripting/builtins/data_archetype.rs`, `scripting/refresh_plan.rs` (hot-reload component-refresh dispatch — `DescriptorComponentKind::Health` arm), `scripting/runtime.rs` (drives the hot-reload refresh via `plan_descriptor_refresh` / `apply_descriptor_refresh_plan`; the only refresh site with `ctx.slot_table` access, so the slot-range hot-reload re-set hook lives here), `weapon/mod.rs`, `scripting/reactions/apply_damage.rs` (new) + `reactions/mod.rs` (module decl) + `reactions/registry.rs`, `scripting/systems/ui_proxy.rs`, `scripting/slot_table.rs`, `scripting/typedef.rs`, `main.rs` (tick loop, level install), `content/dev/scripts/`.
