...
**Spawn and seeding.** Spawn instances from a `loadout` on a new inventory block of the player descriptor, replacing the top-level `defaultWeapon` string. `EntityTypeDescriptor` has no serde path — two hand-written adapters read it, one per runtime, and both must gain a reader; a helper in the runtime core also carries `default_weapon`. Two pawn spawn sites build the inventory in this task: the player-start path and the network-slot path for a host-simulated remote pawn. The third — the connected client's own-pawn materialization, which today spawns no weapon and attaches no reserve — is deferred to Task 5. Seed the ammo reserve **once per distinct ammo type**, taking the first occupied slot's authored reserve; the seeding helper credits additively, so a per-slot loop would double a shared pool. Loadout composition is resolved here and only here.

Weapons are a `weapon` block on `defineEntity`; there is no `defineWeapon` (`plans/done/M10--weapon-primitives/index.md:101`). Add `lowerMs` and `raiseMs` to that block, to the weapon component, its constructor, its effective-stats projection, and its descriptor-refresh path. Both are `u32` milliseconds like every sibling on the **component** timer path (`cooldown_remaining_ms`, `state_remaining_ms`), optional, defaulting to 0. (The descriptor's `cooldown_ms` and `reload_ms` are `f32`; `lower_ms` and `raise_ms` are deliberately `u32` on both descriptor and component — do not "correct" them to match the `f32` siblings.) No lower-bound rejection — a deliberate divergence from the `>= 1` rule the ammo numerics use, because a zero-duration equip is a legal authored choice.
...
The **switch declaration** is a client→host message on the reliable path carrying the target slot. It is not a per-tick input field: it is a discrete event, so it needs none of the edge-gating, gap-hold neutralization, or catch-up-drop handling the fire and use edges require. No wire-version or app-protocol bump is required: there are no deployed peers, every run is a fresh build, and the tuning payload's epoch bump (Task 5) is the compatibility gate that rejects a stale client. Note that the `firing_slot` field added to `InputCommand` does change bitcode layout; this is safe only because there are no deployed peers — the same justification covers this layout change. The host validates it — the slot is occupied, and the reload-interrupt rule permits it — then repoints its own `Inventory`. On refusal the host tells the client, which returns to its prior active slot and clears any equip state it had started. A declaration naming the already-active slot is a no-op.
...
**This task assigns the client's inventory creation.** The client's first payload for its pawn builds its `Inventory` and spawns one instance per occupied slot from the archetype names; later payloads merge into the live inventory rather than re-materializing the pawn. The two messages — pawn baseline and tuning payload — arrive independently, and the ordering is not guaranteed. **Pawn baseline first** (the common case): the client spawns weapon instances using descriptor defaults for fire values and equip durations, so the player can switch and shoot immediately. When the tuning payload arrives, it merges host-authoritative values into the live inventory without re-materializing — the brief window on descriptor defaults is invisible in co-op PvE. **Tuning payload first**: the client queues the payload and applies it when the pawn materializes, spawning instances with host-authoritative values from the start. The install path clears tuning **before** decoding, deliberately, so a bad replacement cannot leave stale state live; preserve that and state its consequence, that the `Inventory` survives the clear and an in-flight switch completes on durations already latched into its components.

Resolve the payload from the pawn's live `Inventory`, not the authored descriptor array. The send is change-detected but reachable only from slot lifecycle and post-install; add one trigger, a descriptor refresh that changes any weapon's tuning (the production call site is `crates/scripting-core/src/refresh_plan.rs`, which calls `WeaponComponent::refresh_from_descriptor`), so a hot-reloaded equip duration reaches both roles. Loadout composition is not re-sent, because composition is resolved at spawn. Keep the canonical-JSON control-channel transport, the per-client change detection, and the cleared view-feel field. Update the committed fixture.
