...
| 4 | `DataRegistry` → `runtime::ModMapEntry` → VMs (`data_registry.rs:13`; `runtime/types.rs:19-20` imports luau/quickjs) | POD dragged through a VM-coupled module | Sink `ModMapEntry` (+ `MenuCamera`/`Frontend` if needed) POD into the substrate. |
| 5 | `data_descriptors/validate.rs:135` → `movement::MovementScope` (binds dash IR at declaration) | behavior (validation) | Resolved by the movement cluster living in the substrate, which descriptor validation can reach. |
...
- POD descriptor **types** (the structs in `data_descriptors/types/*.rs`): movement params + `NumberOrIr`/`BoolOrIr`/`DashParams`, `WeaponDescriptor`/`FireMode`/`ResolutionMode`, `MeshDescriptor`/`AnimationState`/`InterruptPolicy`, `HealthDescriptor`/`HitboxDescriptor`, `AiDescriptor`/`AiStateNames`, `LightDescriptor`, `EntityTypeDescriptor`, `NamedReaction`/`CrossingDescriptor`/`PrimitiveDescriptor`/`ProgressDescriptor`/`SequenceStep`, POD manifest types `ModThemeTokens`/`ModFontAssets`. Plus pure validators (`error.rs` `DescriptorError`; numeric/crossing/IR validators in `validate.rs`).
- POD value types: `Vec3Lit`/`EulerDegrees` (relocated out of `conv.rs`).
- Sunk subsystem PODs: `DamagePayload`, `NavAgentParams`, `ModMapEntry` (+ `MenuCamera`/`Frontend`).
...
- `ctx.rs` (`ScriptCtx`), `slot_table.rs` + `engine_state_catalog.rs`, `provenance.rs`, `error.rs` (`ScriptError`), `reactions/system_commands.rs` (`SystemCommandQueue` + command enum), and a **slimmed `DataRegistry`** storing only POD descriptor types + `ModMapEntry`.
...
Each floor crate owns the FFI marshalling impls (`FromJs`/`IntoJs`/`FromLua`/`IntoLua`) for the types it defines, behind an optional `script-ffi` feature that pulls `rquickjs`/`mlua`. Only the runtime crate enables the feature. Substrate: FFI for `Vec3Lit`/`EulerDegrees` + descriptor types. Data-model: FFI for `EntityId`/`Transform`/`ComponentKind`/`ComponentValue` + component types. Default builds of both crates have no VM deps — that is the firewall. Precedent: `crates/level-format`'s optional `serde`/`gltf-resolve` features.
