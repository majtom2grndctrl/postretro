...
**Planned — collection id keying (not yet built).** A sprite *collection* is keyed by a **collection id** — the pair `(asset reference, draw contract)` — not by the asset reference alone. Distinct draw contracts over one asset (differing loop period, emissive, or specular) become distinct collections rather than colliding at registration. GPU texture upload dedups by asset reference: one `texture_2d_array` per asset, shared across its collection ids, so collection count does not multiply VRAM. Each collection id keeps its own `SpriteDrawParams` uniform, group-1 bind group, region, and draw call — the one-draw-call-per-collection rule and the per-region layout above hold unchanged. The render-collect key on each particle is its collection id, derived per peer from the shared descriptor, never replicated or baked, so host and client agree without a wire field.
