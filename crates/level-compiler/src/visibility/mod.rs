// Visibility encoding: BSP node/leaf sections and exterior leaf culling.
...
/// Build BSP leaf and portal sections for the PRL file.
///
...
/// Encode BSP leaves into the PRL leaves section.
///
...
pub fn log_stats(result: &VisibilityResult, portal_count: usize) {
    log::info!(
        "Visibility: {} empty leaves, {} portals",
...
mod tests {
    ...
    fn encode_vis_for_test(tree: &BspTree) -> VisibilityResult {
        let exterior = HashSet::new();
        encode_vis(tree, &exterior)
    }
    ...
    fn empty_tree_produces_empty_vis() {
        ...
        let result = encode_vis_for_test(&tree);
        assert_eq!(result.empty_leaf_count, 0);
        assert!(result.leaves_section.leaves.is_empty());
    }

    #[test]
    fn all_solid_leaves_produce_no_empty_leaf_count() {
    ...
    #[test]
    fn single_empty_leaf_encoded_correctly() {
