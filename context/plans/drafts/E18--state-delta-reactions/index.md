...
- `incrementState` / `decrementState` reactions: read a numeric slot's current value, apply a signed delta, validate-and-clamp to the slot's declared range, write it back. `incrementState` applies `+by`; `decrementState` applies `-by`; `by` is a positive magnitude, default 1; a negative or non-finite `by` is rejected. Args `{slot: string, by?: number}`.
...
- [ ] **Bound path, positive:** a trigger `on_fire = increment` raises the counter within the same sim tick it fires (observable in a headless `simulate_tick` test with no app loop).
