...
- A **locomotion-intent signal** derived from the agent's actual horizontal (XZ) speed, read through the existing steering read surface (`agent_steering::path_state(registry, id)` → `AgentPathState::velocity`). The landed facing block already reads this at ai.rs ~535 (gated on Alert|Attack); the locomotion-intent signal should REUSE that `path_state`/`velocity` read — compute `moving` once and share it with facing. (The Chase arm's `path_state` call consumes only `blocked`, not velocity.) Compared against a small speed epsilon.
...
- [ ] Reference descriptor tuning updated in both `entities.ts` and the `.luau` twin with parity preserved; no typedef shape change (raw number only); drift test between `entities.ts` and the `.luau` twin stays green.

## Rough sketch

- **Locomotion intent.** In `run_ai_tick`'s apply pass, reuse the `path_state(registry, id).velocity` XZ magnitude already read by the landed facing block (ai.rs ~535). REUSE `FACING_MOVE_SPEED_EPSILON: f32 = 0.05` (ai.rs:79) — promote it to a shared `MOVE_SPEED_EPSILON` (0.05 m/s) so facing and locomotion animation agree by construction on what "moving" means. Do NOT introduce a second `LOCOMOTION_SPEED_EPSILON`; a mismatched threshold would cause facing and animation to disagree. `moving = xz_speed > MOVE_SPEED_EPSILON`. Note the AI tick runs before the steering tick (`run_ai_tick` precedes `run_agent_tick`), so `velocity` reflects last tick's movement — a one-tick lag, imperceptible for animation.
...
- **Trigger + latch.** Add a small field to `BrainComponent` (e.g. `locomotion_moving: bool`, `#[serde(default)]`, seeded `false` at `from_descriptor`) recording the last-applied locomotion intent. EVERY animation request — both the existing `state_changed` branch AND the new locomotion-intent branch — writes `brain.locomotion_moving = moving` as part of the same apply step. Unify the two into ONE switch site: `if state_changed || moving != brain.locomotion_moving { resolve name via the helper; switch_animation_state(...); brain.locomotion_moving = moving; }`. This single call must share the existing warn-once `warned` set and `anim:{name}` key, and write the latch even when the switch returns `AlreadyInState`. This prevents a redundant re-switch on the tick after an Idle→Alert(moving) entry.
