...
- [ ] The reference enemy, migrated to a single-entry `attacks` map with `action: { attack: "..." }`, holds its transition and damage cadence exactly: same distance at which its swing connects, same damage per swing, same cooldown interval, same attack-clip replay — asserted as concrete numeric checks, not shape-parsing alone. The reference-enemy trace fixture (`trace_reference_fixture`/`BrainTrace`, `ai_tests.rs`), which already runs the shipped archetype through a scripted approach and records per-tick `player_hp`/state/animation, is the vehicle: extend it to pin the connect distance, per-swing damage, and cooldown interval, rather than resting on suite-green alone. The pose-fixture enemy's migration holds the same parity. The AI test suite passes once its fixtures are migrated to the map shape — semantic preservation, not a byte-identical descriptor.
- [ ] On a flat fixture with the two-attack reference enemy: at a distance within only the hitscan attack's reach the player takes that attack's damage once per that attack's cooldown with the hosting state's animation active; within the melee attack's reach the melee attack's damage and animation apply instead — routing driven entirely by the authored per-state distance guards.
...
Extend the attack seam in `crates/postretro/src/scripting/systems/ai/mod.rs` — the cooldown/`action_for_state` gate in the compute pass and the apply-pass `apply_damage_with_context` + `ENEMY_ATTACK_EVENT`/`ENEMY_ATTACK_SOURCE_ID` + `restart_animation_clip`. Resolve the current state's named attack against Task 2's per-attack tuning table, gate on that attack's own cooldown-map entry and its `maxRange`, and fire: a `Contact` entry keeps the `apply_damage_with_context` contact-damage path (unchanged `DamageContext`) and fires no ray; a `weapon` entry synthesizes an origin from the firing enemy's posed weapon-socket world position via `posed_socket_world` (`crates/postretro/src/scripting/systems/hit_zones.rs` — the same posed-palette path hit-zones already sample), falling back to the firing enemy's hitbox center when no socket resolves, and a direction toward the target's hitbox center, then resolves through the weapon ray path. No new authored eye-height field. Confirm at this task that the AI fire stage actually has what `posed_socket_world` needs — a `HitZoneStore` handle and the firing entity's current animation time — since `ai/mod.rs` does not plumb either today; wire them in if absent. `resolve_nearest_hit` (`crates/postretro/src/weapon/mod.rs`) already takes no `WeaponComponent`, so calling it from the AI fire path is a visibility change (it is a private `fn` today), not a decoupling; add an ignore-shooter parameter to `nearest_entity_hit` (which `resolve_nearest_hit` calls) so the ray excludes the firing enemy. `nearest_entity_hit`'s only other callers are `weapon/mod.rs`'s player fire path (production) and its own test module plus the `impact_policy.rs` test module (both `#[cfg(test)]`) — update all of them for the new parameter. Route the weapon hit through the same zone-multiplier scaling and `apply_damage_with_context` the player path uses. A fired attack re-arms only its own cooldown-map entry (Invariant: independent cooldowns). Feed `@brain.attackCooldownMs` from the current state's attack's remaining cooldown at the scope refresh (`BrainFacts.attack_cooldown_ms`), reading 0 when the current state fires no attack — the fact's single-number shape is unchanged; only which attack's timer it reports generalizes across states. The refresh runs before the tick's transition selects the next state, so the fed value is the pre-transition (current) state's attack; the fire gate that follows reads the post-transition (`next_index`) state's attack — on a switch tick these name different attacks by design (see Ordering pins). In `crates/postretro/src/scripting/systems/ai/combat_slots.rs`, resolve each engaged agent's standoff from Task 2's per-attack tuning table, keyed by the firing state's named attack, instead of the graph-wide `engagement_radius()` (D2).

### Task 4: Reference archetype, overlay, fixture verification

In `sdk/behaviors/reference/entities.{ts,luau}`, migrate the reference enemy and the pose-fixture enemy from the singular `attack` block + `action: "attack"` to a single-entry `attacks` map + `action: { attack: "<name>" }`, holding each one's cadence (Task 2/3 preserve the contact path; this is a shape change, not a tuning change). Give the reference enemy a second attack: a hitscan zap weapon descriptor, a second `attacks` entry referencing it, an attack-firing zap state with distance-guard routing to and from the melee state, and the zap clip/state added to its `mesh.animations`. Migrate every AI test fixture to the new shape (`ai/mod.rs` tests, `brain_scope.rs`, `crates/entities/src/components/brain.rs` `authored_graph()`, and the `behavior.rs` descriptor tests). Extend the agent-diagnostics overlay state label with the firing state's attack name. On the movement-feel fixture, verify the two-reach behavior (melee reach vs. hitscan reach) and the occlusion AC, and confirm co-op clients show the distinct attack animation states via the replicated state name (no wire change expected).

Cover the three newest ACs with fixtures, referencing the Ordering pins rows for their exact sequencing rather than restating it: AC5 — an enemy in a long-reach attack-firing state parks at that attack's engagement radius, and a non-attack/attackless graph parks at the graph-level default. AC6 — a fixture graph reads `@brain.attackCooldownMs` across an attack-firing state and a non-attack state, asserting both the reported value and the switch-tick semantics (pre-transition fact, post-transition fire). AC9 — an enemy in an attack-firing state with the target outside every attack's `maxRange` ceiling and no true transition guard holds in place and faces the target. Add a direct self-exclusion assertion for AC7 (a firer whose own hitbox sits on the ray never registers as the hit), and confirm the pose-fixture enemy's migration holds the same cadence and overlay-label parity as the reference enemy (AC2).
...
| An authored or weapon-inherited cooldown resolves to 0 or otherwise not `> 0` | Validated finite and `> 0` at parse time for authored values (Task 1), at spawn time for the weapon-inherited override path (Task 2) | Validation fails rather than producing a same-tick or every-tick refire |
...
- **Muzzle-tip offset.** The weapon-entry hitscan origin is the posed weapon socket's raw world position, not an offset muzzle tip. A precise muzzle-tip offset is future work owned by **Epic 16 › projectile** (`roadmap.md`) — a visible projectile's travel origin is its first consumer. **Owner:** Epic 16 › projectile.
