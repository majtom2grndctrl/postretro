...
- [ ] Per-frame CPU cost to sample one skeleton's clip into the palette is logged and projected to wave scale (use the roadmap's wave size as N; if unspecified, project at N=200 and state the assumption in `findings.md`). Confirms or refutes "no `ozz`-style baked pose buffer." Note in `findings.md` that this is the CPU side only; the GPU-skinning / palette-upload cost at N instances is the actual wave risk and is the many-instance task's measurement, not this slice's.
...
| Contract | Constraint |
|---|---|
| Skinned vertex attributes | position; UV; octahedral normal **and** a packed tangent (`u16`×2: tangent component + bitangent sign in the high bit), mirroring `WorldVertex`'s `normal_oct` / `tangent_packed`; joint indices ×4; weights ×4. Tangent is included now — a known-imminent lighting consumer may want it, and a touched-once vertex pays 4 bytes cheaply versus re-baking every model after a layout break. |
...
| Component | `ComponentKind::Mesh` / `ComponentValue::Mesh(MeshComponent)`. `ComponentValue` uses `tag = "kind"` + `rename_all = "snake_case"`, so the new `Mesh` variant serializes as `{"kind": "mesh"}` — no per-variant serde attribute. |
...
Parse one glTF via the `gltf` crate into the Task 1 structs: mesh geometry + skinning attributes (positions, normals, tangents, UVs, joint indices, weights), the joint hierarchy + inverse-bind matrices, and one animation clip's keyframes. The one hardcoded model must reference an external PNG; embedded-texture glTF is out of scope this slice. Resolve the material's texture to a **pre-staged** 32-byte `blake3` cache key. The hardcoded model's key is supplied at the spawn seam (a constant alongside the model path) and passed to the same `.prm`-open path `load_textures` uses — it is **not** carried in the PRL `TextureCacheKeysSection` (section 32), which covers only map textures. The loader does **not** hash the PNG at runtime. The loader returns an error value on malformed/unsupported input (no panic); the caller handles absence. No wgpu. Narrow: one model, one clip; multi-mesh/multi-clip generality is the broadening task.

### Task 3: Mesh render pass (GPU)
New pass in `render/`: upload the Task 2 mesh into GPU buffers in the committed vertex layout; allocate the shared bone-palette buffer; build the pipeline binding camera uniforms and the material bind group (via existing `.prm`→`LoadedTexture` path, pre-baked texture). Flat-lit — the slice's pass binds only group 0 (camera) and group 1 (material); the lighting group index is part of the provisional lighting contract, chosen by the broadening task (per `rendering_pipeline.md` §9 group allocation). Cull one instance: `find_leaf(entity_position)` → test membership in the current visible-cell set → draw or skip. Membership is `cells.contains(&(find_leaf(pos) as u32))` — cell ids equal leaf indices in the current compiler (`visibility.rs:462`); on `VisibleCells::DrawAll` the instance always draws. Constraint: the cull must read the frame's `VisibleCells::Culled` set before it is reclaimed into scratch (`main.rs:1132` moves and clears the Vec) — record the mesh draw in the same window as the world indirect draw, ahead of that reclaim. Draw the one instance with a **direct draw call** at the per-instance model matrix — do not contort the path to pre-fit the wave-scale indirect layout (that abstraction is validated at scale by the many-instance task, never at N=1). Prefer a pipeline structure where a position-only depth-only variant would be a natural later add (design note, not a contract); build no depth variant and couple to nothing in the spot-shadow pool. Wire construction + per-frame `record_draw` into the renderer; keep edits to the large `render/mod.rs` / `main.rs` render loop minimal (construct + call only).
...
A render-frame **mesh collector** runs in the render-frame collection sub-stage of the redraw handler, walks `iter_with_kind(ComponentKind::Mesh)`, read-only-borrows each entity's `Transform`, and supplies the per-instance matrix + model handle to the pass. It is a render-phase collector, **not** a game-logic-tick bridge — game logic never touches the renderer (`Renderer owns GPU` invariant). Mirror the particle render-frame collector (`systems/particle_render.rs`), not a tick-stage bridge. Spawn exactly one entity carrying a `MeshComponent` through a single named seam that hardcodes the model path/handle — the chokepoint a future classname handler resolves; if the load returned an error, this seam logs a `warn!` naming the model path and skips the spawn (satisfying the degrade AC's warning clause). Small `main.rs` render-stage edit to run the collector, mirroring the particle render-collector call site.
...
- Material: `load_textures` / `LoadedTexture` (`render/loaded_texture.rs:295`, `:26`) → `build_material_bind_group` (`render/mod.rs:434`). `load_textures` takes a `&TextureCacheKeysSection` and resolves a baked key to a `.prm` — it does not hash PNGs (keys are blake3 of source PNG bytes, baked at build time).
...
- Render-frame collector pattern: `systems/particle_render.rs` runs as the render-frame collection sub-stage of the redraw handler (`main.rs:1006`, just before `render_frame_indirect`); it packs draw data and never touches wgpu (game-layer-owned, so the wgpu boundary stays in the pass). `systems/emitter_bridge.rs` runs in the game-logic sub-stage (`main.rs:922`, after the script tick) and mutates the registry — same redraw pass, different ordered sub-stages. The mesh collector mirrors `particle_render`, not `emitter_bridge`.
...
**Material for the slice:** the model's texture is resolved through the existing `.prm`→`LoadedTexture` GPU binding using a **pre-baked** `.prm` (produced offline by the existing baker for the one model). Its 32-byte `blake3` key is staged as a constant at the spawn seam and handed to the same `.prm`-open path `load_textures` uses — not via the PRL `TextureCacheKeysSection`, which only indexes map textures. No runtime hashing — the baked-over-computed invariant holds. Build-time automation of model-PNG baking, and any runtime PNG → key path, are deferred to the broadening *glTF mesh loading* task.
