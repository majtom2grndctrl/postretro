...
- [ ] A touch trigger with `on_fire` naming a composed reaction executes its consequential steps within the same sim tick it fires (observable in a headless `simulate_tick` test with no app loop: mover starts, damage applies, slot value changes on the same tick).
...
- [ ] With `--features dev-tools`, the diagnostics window shows a Triggers tab listing each trigger's name, tags, armed/latched state, rearm countdown, occupancy, and bound event names; without the feature, no trigger UI code is compiled in.
...
- `BoundTriggerCommand` lives beside the trigger system in `postretro` (it references `MoverCommand`, `SlotValue`, tags — all entities-crate types; no VM coupling).
...
- Occupancy representation: `HashMap<EntityId, HashSet<PlayerId>>` (or equivalent) replacing the boolean pair map; enter edge = insertion, leave edge = removal, count = set len. `PlayerId` is already `Hash + Eq + Copy`.
...
Empty string = "no event" on every boundary; no `Option`/null encoding.
