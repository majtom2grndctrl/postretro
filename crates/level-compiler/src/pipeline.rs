...
// `StageId`/`StageDescriptor` moved to `crate::stage` so lightweight consumers
// (`reporter`, `tui`) and the library target can depend on the stage vocabulary
// without pulling in this orchestration module. Re-exported so existing
// `crate::pipeline::StageId` / `StageDescriptor` paths keep resolving.
pub use crate::stage::{StageDescriptor, StageId};
