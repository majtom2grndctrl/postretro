...
**KinematicGeometry (id 43):** optional section for deterministic brush movers. Version 2 carries `mover_id`, `name`, `_tags`, origin, first waypoint `path`, speed, endpoint wait, move mode, start flag, local-space vertex/index/face metadata, and static spin axis, initial speed (deg/s), acceleration (deg/s²), and `carry_yaw`. Version-1 sections remain loadable and default those rotation fields to zero spin, zero acceleration, and `carry_yaw = false`. Loader rejects non-zero degree-domain spin values that become zero in runtime radians, so pure-rotator admission and ramp semantics cannot change across the conversion boundary. Waypoint records carry `name`, `next`, and origin. Runtime hashes the decoded kinematic and collision inputs into the multiplayer static-content gate; peers with different mover sections cannot pass app handshake. Mover brushes are not present in static `Geometry`, `Bvh`, static collision, lightmaps, SDF, portals, or navmesh.
