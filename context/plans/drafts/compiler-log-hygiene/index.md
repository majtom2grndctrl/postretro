...
> - **`lighting-scale--sh-delta-footprint-instrumentation`.** Shares the `pack.rs` section-size logging surface. This spec downgrades the per-section byte breakdown to `debug!`; that spec adds an indirect `DeltaShVolumes` size line and an "SH-total vs non-SH" aggregate. Coordination pinned (matching that spec's Dependencies note): the id-27 `DeltaShVolumes` size line is part of the breakdown and follows it to `debug!` in either landing order; only the "SH-total vs non-SH" aggregate stays `info!` and this downgrade must exempt it. That spec's direct-delta bake summary lives at the `pipeline.rs` call site, outside this spec's `pack.rs` sweep, and also stays `info!`.
