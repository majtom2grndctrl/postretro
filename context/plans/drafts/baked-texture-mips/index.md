...
- Delete the PNG-fixture-driven tests from the pre-rename `crates/postretro/src/texture.rs`: the five `build_name_map_*` tests (which follow `build_name_to_path_map` into the compiler), `load_textures_loads_matching_pngs`, `load_textures_case_insensitive_match`, `load_textures_missing_produces_checkerboard`, `load_textures_none_entry_produces_checkerboard`, `load_textures_corrupt_png_produces_checkerboard` (line 444), `load_textures_empty_names_produces_empty_set` (line 483), the `None` slot in `load_textures_preserves_index_order`, and the eleven sibling-probe tests at lines 528–718. 23 of 27 tests in the file go. The four `checkerboard_*` tests at lines 290–328 move with the placeholder logic to `crates/postretro/src/render/loaded_texture.rs`. Replacement: `.prm`-fixture tests built via a `#[cfg(test)]` helper module in `postretro-level-format::prm` that emits in-memory `.prm` byte blobs.
...
- Migrate the workspace stage cache: change `cache.rs`'s hard-coded path from `<workspace>/.prl-cache/` to `<workspace>/.build-caches/prl-cache/`. Update `.gitignore` from `.prl-cache/` to `.build-caches/`. `--cache-dir` semantics unchanged (still overrides the stage cache root only).
...
- The compiler's `cache.rs` `StageCache` mechanism is unused for textures — the on-disk `.prm` file (content-addressed filename) IS the texture cache. Existence + parse-success at `<cache>/<hex>.prm` is the cache hit signal; the `bundle_hash` field in the header is the input-fingerprint check for the all-slots-unchanged path. However, `cache.rs` IS touched: its hard-coded path `<workspace>/.prl-cache/` migrates to `<workspace>/.build-caches/prl-cache/` so all build caches share one root.
