...
- **`.get()` is a bind-reference producer, never a synchronous read.** The "no live VM during gameplay" invariant (`ui.md` §6, `scripting.md` §11) forecloses a runtime value read — there is no VM to read from. Not a design choice; the architecture decides it.
- **Cross-reference validation (`textEntryTarget`/`initialFocus` node ids) is load-time, not factory-time.** A factory is a pure *local* builder (emitter precedent) and cannot see the whole tree; Rust owns the retained tree and validates referential integrity at the bridge, emitting a named diagnostic and degrading (`ui.md` §5). Factories do shape-only validation. Rejected: whole-tree validation in the factory (it lacks the sibling context).
- **TS and Luau surfaces are equivalence-by-parity, not token-identical.** The dual-runtime parity invariant (M6/M7 contract tests) requires the same names, arg order, and tree shape across runtimes; the binding mechanism follows the shipped SDK convention (TS flat exports installed as globals; Luau a returned module table, the `UiReactionsSdk` pattern). The parity test pins it. Not a design degree of freedom.

## Open questions

None. The candidates above were resolved against existing project invariants (no-live-VM, Rust-owns-the-tree / degrade-not-abort, dual-runtime parity) and recorded as Decisions.
