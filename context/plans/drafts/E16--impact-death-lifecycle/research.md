...
- AI-tick 0-HP handler: `crates/postretro/src/scripting/systems/ai.rs:601-633` forces `LogicalState::Death`, clears target; despawn timer `death_despawn_remaining_ms` seeded `:622`, despawn in pass 4 `:907-914`. **Resurrect already exists:** HP restored above 0 while in Death → recover to Idle, clear countdown, `ai.rs:648-651`.
...
- Storage write exists internally: `HealthComponent.current` public, written via `registry.set_component`; `refresh_from_descriptor` clamps on hot-reload (`health.rs:275-277`). `setHealth` = new absolute chokepoint mirroring `apply_damage`; resurrect must reset `death_handled = false` (the `ai.rs:648-651` recovery keys off it).
