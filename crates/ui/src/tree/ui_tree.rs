...
pub struct UiTree {
    ...
    /// build. The fresh/test path (`build_draw_data`) never reads or fills it —
...
impl UiTree {
    ...
    pub fn from_descriptor(tree: &AnchoredTree, theme: &UiTheme) -> Self {
    ...
    #[cfg(any(test, feature = "test-fixtures"))]
    pub fn draw_rebuild_count(&self) -> u32 {
    ...
    /// metrics. Only the CPU `FontSystem` is threaded in, and the tree holds no
    /// GPU or renderer-owned font state of its own.
    ///
