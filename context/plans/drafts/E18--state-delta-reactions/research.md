...
- SDK `updateState` (which emits the `setState` primitive) lives in `sdk/lib/ui/reactions.{ts,luau}` (`updateState` `reactions.ts:309` / `.luau:263`), returning `PrimitiveReactionDescriptor`; not in the `SequenceStep` union. Consumers import these UI reaction builders from `"postretro/ui"` (the root `"postretro"` barrel deliberately excludes UI symbols), verified in `content/dev/scripts/typed-handles-fixture.ts`. The reaction-body primitive list is hand-authored in `crates/scripting-core/src/typedef/templates/ui_sdk_module.d.ts:176-208`; `gen_script_types` (`crates/postretro/src/bin/gen_script_types.rs`) does not auto-emit names.
