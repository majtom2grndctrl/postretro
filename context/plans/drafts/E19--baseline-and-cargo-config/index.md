...
- [ ] The `dev-fast` profile decision is recorded (adopted or rejected) with the dependency-`opt-level`/debug-info comparison numbers and the before/after `campaign-test` frame-time that drove it.
